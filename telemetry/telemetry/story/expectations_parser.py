  ...
  def __init__(self, path=None, raw=None):
    ...
    if path:
      if not os.path.exists(path):
        raise ParseError('Path to expectation file must be valid.')
    ...
    for line in raw_data.splitlines():
      ...
      if line.startswith(self.TAG_TOKEN):
        ...
        self._expectations.append(
            self._ParseExpectationLine(line, self._tags, count))
      count += 1

  @classmethod
  def _ParseExpectationLine(cls, line, tags, count):
    match = cls.MATCHER.match(line)
    if not match:
      raise ParseError(
          'Expectation has invalid syntax on line %d: %s' % (count, line))
