...
def _GetExpectations(bench, path=None, data=None):
  return bench.InitializeExpectations(path=path, data=data)


def _IsBenchmarkEnabled(bench, possible_browser, expectations_file):
  b = bench()
  expectations = _GetExpectations(b, path=expectations_file)
  return (
...
class List(command_line.OptparseCommand):
  ...
  def ProcessCommandLineArgs(cls, parser, args, environment):
    ...
    cls._expetation_file = environment.expectation_file
...
class Run(command_line.OptparseCommand):
  ...
  def Run(self, args):
    b = self._benchmark()
    e = _GetExpectations(b, path=self._expectations_path)
    return min(255, b.Run(args, benchmark_expectations=e))
