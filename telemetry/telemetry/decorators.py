...
def Info(emails=None, component=None, documentation_link=None):
  ...
  def _Info(func):
    ...
    if documentation_link:
      assert 'documentation_link' not in info_dict, (
          'document link can only be set once')
      info_dict['documentation_link'] = documentation_link
  ...
  if documentation_link:
    assert isinstance(documentation_link, str), (
        'Documentation link must be a str')
    assert (documentation_link.startswith('http://') or
            documentation_link.startswith('https://')), ('Documentation url is '
                                                         'malformed')
  return _Info
...
def GetDocumentationLink(test):
  info_attr_name = InfoAttributeName(test)
  benchmark_info = getattr(test, info_attr_name, {})
  if 'documentation_link' in benchmark_info:
    return benchmark_info['documentation_link']
  return None
