...
def _MatchTestNameToTestFilter(test_filter, test_name):
  return any(fnmatch.fnmatch(test_name, t)
             if t.endswith('*') else t == test_name
             for t in test_filter.split('::'))
...
def LoadTestCasesToBeRun(
  ...
  test_cases = []
  noop_func = lambda x, y: True
  if test_filter_str:
    test_filter_matcher_func = _MatchTestNameToTestFilter
  else:
    test_filter_matcher_func = noop_func
  if filter_tests_after_sharding:
    test_filter_matcher = noop_func
    post_test_filter_matcher = test_filter_matcher_func
  else:
    test_filter_matcher = test_filter_matcher_func
    post_test_filter_matcher = noop_func
...
def _CreateTestArgParsers():
  ...
  parser.add_argument(
      ...
      'globs, to run just that subset of tests')
  parser.add_argument(
