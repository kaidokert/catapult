...
class RunTestsUnitTest(unittest.TestCase):
  ...
  def _RunTest(
    ...
    try:
      ...
      assert ret == expected_return_code, ('%d != %d' %
  ...
  def _RunTestsWithExpectationFile(
  ...
  def testSkipOnlyWhenTestMatchesTestFilterWithBrowser(self):
    try:
      # if browser can be found then run the below test
      # we need to make sure both classifier functions in run_tests.py
      # are behaving correctly
      test_name = 'unit_tests_test.AnotherFailingTest.test_fail'
      _, _, actual_skips = self._RunTest(
          [], [], [test_name], test_name=test_name, no_browser=False,
  ...
  def testTestFailsAllRetryOnFailureRetriesAndIsNotARegression(self):
    self._RunTestsWithExpectationFile(
