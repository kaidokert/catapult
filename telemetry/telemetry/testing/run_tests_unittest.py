...
class RunTestsUnitTest(unittest.TestCase):
  ...
  def baseTest(self,
    ...
    try:
      passed_args = [test_name, '--write-full-results-to=%s' % temp_file_name]
  ...
  def testSkipTestNoExpectations(self):
    self.baseTest(
        [],
        [],
  ...
  def _RunUnitWithExpectationFile(self, full_test_name, expectation,
    ...
    try:
      passed_args = [full_test_name, ('--write-full-results-to=%s'
                                      % results.name)] + ['--tag=%s' % tag
                                                          for tag in
                                                          test_tags.split()]
      ret = unittest_runner.Run(config, passed_args=passed_args + extra_args)
  ...
  @decorators.Disabled('chromeos')  # crbug.com/696553
  def testSkipTestWithExpectationsNonSkipExpectation(self):
    self._RunUnitWithExpectationFile('unit_tests_test.SkipTest.test_skip',
                                     'Pass Crash Failure',
                                     expected_exit_code=1)
    result = (self._test_result['tests']['unit_tests_test']
              ['SkipTest']['test_skip'])
    self.assertEqual(result['actual'], 'SKIP')
    self.assertEqual(result['expected'], 'CRASH FAIL PASS')
    self.assertIn('is_unexpected', result)
    self.assertIn('is_regression', result)
