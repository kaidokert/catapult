...
class BrowserTestRunnerTest(unittest.TestCase):
  ...
  def _RunBrowserTest(self, modulename, classname,
    ...
    config = project_config.ProjectConfig(
        ...
        benchmark_dirs=[
            os.path.join(util.GetTelemetryDir(), 'examples', 'browser_tests')],
        default_chrome_root=os.path.join(util.GetTelemetryDir(),
                                         '..', '..', '..')
    )
    try:
      ...
      args = ['%s' % classname,
              ...
              '--browser=%s' % options.browser_options.browser_type]
      try:
        if options.device:
          args.extend('--device=%s' % options.device)
      finally:
        print options
      browser_test_runner.Run(config, args)
  ...
  def testCreateBrowserExpectedFail(self):
    test_result = self._RunBrowserTest('create_browser_test',
                                       'CreateBrowserTest',
                                       'PassTest', 'Failure')
    test_result = (test_result['tests']['browser_tests']
                   ['create_browser_test']['CreateBrowserTest']['PassTest'])
    assert test_result['expected'] == 'FAIL'
    assert test_result['actual'] == 'PASS'
    assert 'is_unexpected' in test_result
    assert not 'is_regression' in test_result
