...
class BrowserTestRunnerTest(unittest.TestCase):
  ...
  def _RunBrowserTest(self, modulename, classname,
                      test_name, expectation, test_tags='foo', extra_args=None):
    extra_args = [] if not extra_args else extra_args
    ...
    try:
      browser_test_runner.Run(config,
                              ['%s' % classname,
                               ...
                               '--test-filter=.*%s.*' % test_name] + extra_args)
  ...
  def testSkipWithExpectationsFileWithSkipExpectation(self):
    ...
    assert test_result['expected'] == 'SKIP'
    assert test_result['actual'] == 'SKIP'
  ...
  def testSkipTestCmdArgsWithExpectationsFile(self):
    ...
    test_result = (test_result['tests']['browser_tests']
                   ['skiptests_test']['SkipTestExpectationFiles']['PassTest'])
    assert test_result['expected'] == 'CRASH FAIL'
    assert test_result['actual'] == 'SKIP'
    assert not 'is_unexpected' in test_result
    assert not 'is_regression' in test_result
  ...
  def testSkipTestWithExpectationsFile(self):
    ...
    assert test_result['expected'] == 'FAIL CRASH PASS'
    assert test_result['actual'] == 'SKIP'
    assert not 'is_unexpected' in test_result
    assert not 'is_regression' in test_result
