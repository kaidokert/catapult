...
class MockHasExpectationsFiles(
  ...
  def ExpectationsFiles(cls):
    return [cls._expectations_file.name]


class BrowserTestRunnerTest(unittest.TestCase):
  def setUp(self):
    self._test_result = {}
    if not MockHasExpectationsFiles._expectations_file:
      MockHasExpectationsFiles._expectations_file = (
          tempfile.NamedTemporaryFile(delete=False))

  def tearDown(self):
    MockHasExpectationsFiles._expectations_file.close()
  ...
  def testMacExamplePlatformTagsGenerated(self):
    ...
    tags = set(MockHasExpectationsFiles.GenerateTags(options, possible_browser))
