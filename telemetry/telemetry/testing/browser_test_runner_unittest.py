...
class BrowserTestRunnerTest(unittest.TestCase):
  ...
  def _testBrowserTest(self, modulename, classname,
    ...
    config = project_config.ProjectConfig(
        ...
        benchmark_dirs=[
            os.path.join(util.GetTelemetryDir(), 'examples', 'browser_tests')],
        default_chrome_root=os.path.normpath(os.path.join(telemetry_dir,
                                                          '..', '..', '..'))
    )
    try:
      browser_test_runner.Run(
  ...
  def testTagGenerationExpectedPass(self):
    test_result = self._testBrowserTest('generate_tags_test',
  ...
  def testCreateBrowserExpectedFail(self):
    test_result = self._testBrowserTest('create_browser_test',
                                        'CreateBrowserTest',
                                        'PassTest', 'Failure')
    test_result = (test_result['tests']['browser_tests']
