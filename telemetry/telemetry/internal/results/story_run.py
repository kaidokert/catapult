...
def _FormatTimeStampFileSafe(epoch):
...
class Artifact(object):
  def __init__(self, local_path, legacy_path, content_type=None):
    """
    Args:
      local_path: an absolute local path to an artifact file.
      legacy_path: path to an artifact file relative to the output dir.
          Must use only '/' as a separator on all platforms.
          Used in json_3_output_formatter.
...
class StoryRun(object):
  def __init__(self, story, output_dir=None):
    ...
    if output_dir is None:
      ...
      run_dir = '%s_%s_%s' % (
          ...
          _FormatTimeStampFileSafe(self._start_time),
          random.randint(1, 1e5),
      )
      self._artifacts_dir = os.path.join(self._output_dir, 'artifacts', run_dir)
  ...
  def AddArtifact(self, name, artifact_path):
    ...
    legacy_path = os.path.relpath(local_path, self._output_dir).replace(
        os.sep, '/')

    self._artifacts[name] = Artifact(local_path, legacy_path)

  def IterArtifacts(self, subdir=None):
    """Iterate over all artifacts in a given sub-directory.

    Returns an iterator over Artifact objects.
    """
