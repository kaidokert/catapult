...
class PageTestResults(object):
  def __init__(self, output_formatters=None, progress_reporter=None,
    ...
    # |_interruption| is None if the benchmark has not been interrupted. Otherwise
    # it is a string explaining the reason for the interruption. Interruptions
    # occur for unrecoverable exceptions.
  ...
  def MarkBenchmarkInterrupted(self, reason):
    logging.fatal(reason)
    self._interruption = self._interruption or reason

  def BenchmarkWasInterrupted(self):
    return bool(self._interruption)

  def BenchmarkInterruptionReason(self):
