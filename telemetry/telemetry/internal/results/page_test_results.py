...
class PageTestResults(object):
  ...
  def AddMeasurement(self, name, unit, samples):
    """Record a measurement of the current story run.

    Measurements are numeric values directly obtained by a benchmark while
    a story is being run (e.g. by running some JavaScript on the page or
    ...
    preserving the existing behavior. When no clients remain creating legacy
