...
class TelemetryInfo(object):
  ...
  def _UpdateDiagnostics(self):
    ...
    def SetDiagnosticsValue(name, value):
      name_type = reserved_infos.GetTypeForName(name)
      diag_class = all_diagnostics.GetDiagnosticClassForName(name_type)
      self.diagnostics[name] = diag_class(value)

    if self.benchmark_name:
      SetDiagnosticsValue(reserved_infos.BENCHMARKS.name, [self.benchmark_name])
    if self.benchmark_start_us:
      SetDiagnosticsValue(reserved_infos.BENCHMARK_START.name,
                          self.benchmark_start_us / 1e3)
    if self.benchmark_descriptions:
      SetDiagnosticsValue(reserved_infos.BENCHMARK_DESCRIPTIONS.name,
                          [self.benchmark_descriptions])
    if self.label:
      SetDiagnosticsValue(reserved_infos.LABELS.name, [self.label])
    if self.had_failures:
      SetDiagnosticsValue(reserved_infos.HAD_FAILURES.name, [self.had_failures])
    if self._story_name:
      SetDiagnosticsValue(reserved_infos.STORIES.name, [self._story_name])
    if self.story_tags_list:
      SetDiagnosticsValue(reserved_infos.STORY_TAGS.name,
                          [self.story_tags_list])
    if self.storyset_repeat_counter:
      SetDiagnosticsValue(reserved_infos.STORYSET_REPEATS.name,
                          [self.storyset_repeat_counter])
    if self.trace_start_us:
      SetDiagnosticsValue(reserved_infos.TRACE_START.name,
                          self.trace_start_us / 1e3)
    if self.trace_url:
      SetDiagnosticsValue(reserved_infos.TRACE_URLS.name, [self.trace_url])
