  ...
  def testSnappingPageWithImage(self, mockGet):
    ...
    try:
      ...
      shutil.copyfile(os.path.join(path.GetUnittestDataDir(),
                                   src_html_filename),
                      dest_html_path)
      self.platform.SetHTTPServerDirectories(test_dir)
      url = self.platform.http_server.UrlOf(dest_html_path)
      outfile = StringIO.StringIO()
      ...
      snap_page_util._SnapPageToFile(
          self.finder_options, url, interactive=False, verbose=True,
      ...
      with open(os.path.join(test_dir, 'image', '0-target.png')) as f:
