...
class PosixPlatformBackend(desktop_platform_backend.DesktopPlatformBackend):
  ...
  def LaunchApplication(self,
    ...
    def IsElevated():
      ...
      rc, stdout, stderr = self.RunCommandWithRC(['/usr/bin/sudo', '-nv'])
      stdout += stderr
    ...
    def IsSetUID(path):
      ...
      stdout, _ = self.RunCommand(
          ['find', dirname, '-perm', '/4000', '-name', basename])
    ...
    if elevate_privilege and not IsSetUID(application):
      if self.has_interface and not self.interface.local:
        ...
        if not _CanRunElevatedWithSudo(
            application, platform_backend=self) and not IsElevated():
          if not sys.stdout.isatty():
            ...
            # sudo. Fail with a helpful error message. For more information,
            # see: https://code.google.com/p/chromium/issues/detail?id=426720
            text = (
          ...
          # Else, there is a tty that can be used for a useful interactive
          # prompt.
