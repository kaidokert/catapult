...
class PerfettoTracingAgent(tracing_agent.TracingAgent):
  ...
  def StartAgentTracing(self, config, timeout):
    ...
    start_perfetto = (
        'cat %s | %s --config - --txt --out %s >/dev/null 2>&1 &' % (
            self._trace_config_temp_file.name,
            ANDROID_PERFETTO,
            self._trace_output_temp_file.name,
        )
    )
    self._device.KillAll(ANDROID_PERFETTO, exact=True, quiet=True)
    with self._PersistentShell() as pshell:
      pshell.RunCommand(start_perfetto, close=True)
    logging.info('Started perfetto system tracing.')
    return True

  def StopAgentTracing(self):
    self._device.KillAll(
        ANDROID_PERFETTO, exact=True, signum=device_signal.SIGINT)
