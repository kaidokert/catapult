...
class TelemetryTracingAgent(tracing_agent.TracingAgent):
  ...
  def StartAgentTracing(self, config, timeout, write_proto=False):
    del config  # Unused.
    del timeout  # Unused.
    ...
    trace_event.trace_enable(os.fdopen(fd, 'wb'), write_proto)

    assert self.is_tracing, 'Failed to start Telemetry tracing'
    self._use_proto = write_proto
    return True
  ...
  def CollectAgentTraceData(self, trace_data_builder, timeout=None):
    ...
    try:
      with open(self._trace_log) as f:
        if self._use_proto:
          # FIXME: implement reading protos
          data = f.read()
        else:
