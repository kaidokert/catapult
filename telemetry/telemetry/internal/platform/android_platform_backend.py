...
class AndroidPlatformBackend(
  ...
  def __init__(self, device, require_root):
    ...
    self._cpu_temperature = cpu_temperature.CpuTemperature(self._device)
  ...
  def WaitForCpuTemperature(self, temp):
    ...
    if self._cpu_temperature.IsSupported():
      self._cpu_temperature.LetCpuCoolToTemperature(temp)
    else:
      logging.warn('Either the current device is not supported or the '
                   'specified temperature zones do not exist.')
