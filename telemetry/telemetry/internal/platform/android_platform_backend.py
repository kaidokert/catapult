...
class AndroidPlatformBackend(
  ...
  def PushProfile(self, package, new_profile_dir):
    ...
    for root, dirs, files in os.walk(new_profile_dir):
      posix_root = root.replace(os.sep, posixpath.sep)
      rel_root = posixpath.relpath(posix_root, new_profile_dir)
      device_root = posixpath.join(profile_dir, rel_root)

      filtered_files = [f for f in files if f != "lib"]
      device_paths.extend(
          posixpath.join(device_root, n) for n in filtered_files + dirs)
