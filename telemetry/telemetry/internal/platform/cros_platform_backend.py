...
class CrosPlatformBackend(
  ...
  def GetOSVersionName(self):
    os_release = self._ReadReleaseFile('/etc/lsb-release')
    if os_release and 'CHROMEOS_RELEASE_NAME' in os_release:
      return os_release.get('CHROMEOS_RELEASE_NAME')

    raise NotImplementedError('Missing CrOS name in lsb-release')

  def GetOSVersionDetailString(self):
    os_release = self._ReadReleaseFile('/etc/lsb-release')
    if os_release and 'CHROMEOS_RELEASE_VERSION' in os_release:
      return os_release.get('CHROMEOS_RELEASE_VERSION')
