...
class _SkipSystemCheckFuchsiaDeviceTest(unittest.TestCase):
  def setUp(self):
...
class FuchsiaSDKUsageTest(_SkipSystemCheckFuchsiaDeviceTest):
  def testSkipDownloadSDKIfExists(self):
    ...
    with mock.patch('os.path.exists', return_value=True) as path_mock:
      with mock.patch(_FUCHSIA_DEVICE_IMPORT_PATH +
                      '._DecompressFuchsiaSDK') as decompress_mock:
