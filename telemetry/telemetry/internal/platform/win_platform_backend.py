...
class WinPlatformBackend(desktop_platform_backend.DesktopPlatformBackend):
  ...
  def TakeScreenshot(self, file_path):
    ...
    logging.warning('BMP pixels: %s' % base64.b64encode(pixels))
    ...
    def FileIsDone(path):
      try:
        with open(path) as f:
          logging.warning('Opened file correctly: %s' % path)
          logging.warning('File contents: %s' % base64.b64encode(f.read()))
          logging.warning('Read file correctly.')
        return True
      except:
        logging.warning('Failed opening file, waiting 1 second before retry.')
        time.sleep(1)
        return FileIsDone(path)
    FileIsDone(file_path)
    return True
