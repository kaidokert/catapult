...
class MacPlatformBackend(posix_platform_backend.PosixPlatformBackend):
  ...
  def TakeScreenshot(self, file_path):
    ...
    # TODO: use the built-in timeout for subprocess after python 3 lands
    timeout_in_sec = 10
    try:
      sp = ps_util.RunSubProcWithTimeout(
          ['screencapture', file_path], timeout_in_sec)
      return sp.returncode
    except py_utils.TimeoutException:
      logging.warning(
          'Screencapture did not finish after $ds.' % timeout_in_sec)
    return None
