...
class GPUDevice(object):
  ...
  def __init__(self, vendor_id, device_id, vendor_string, device_string,
               driver_vendor=None, driver_version=None):
  ...
  def __str__(self):
    ...
    driver = 'DRIVER = '
    if self._driver_vendor:
      driver += self._driver_vendor
      if self._driver_version:
        driver += ' (%s)' % self._driver_version
    return '%s, %s, %s' % (vendor, device, driver)
