...
_UP_SEVEN_DIRS = [os.pardir] * 7
_CHROMIUM_SRC = os.path.abspath(os.path.join(__file__, *_UP_SEVEN_DIRS))
assert os.path.basename(_CHROMIUM_SRC) == 'src', ('Did not climb up to the '
            'chromium/src/ directory! Instead reaches %s. This will '
            'probably not work correctly!' % _CHROMIUM_SRC)
_BUILD_FUCHSIA = os.path.join(_CHROMIUM_SRC, 'build', 'fuchsia')
sys.path.append(_BUILD_FUCHSIA)
import boot_data


_OUT_DIR = os.path.abspath(os.path.join(_CHROMIUM_SRC, 'out', 'fuchsia'))
...
def _FindFuchsiaDevice():
  ...
  netaddr_path = os.path.join(_CHROMIUM_SRC, 'third_party', 'fuchsia-sdk',
                              'sdk', 'tools', 'netaddr')
