...
class LinuxPlatformBackend(
  ...
  def _ReadReleaseFile(self, file_path):
    if not self._li.FileExistsOnDevice(file_path):
      return None
  ...
  def FlushEntireSystemCache(self):
    rc, _, _ = self._li.RunCmdOnDeviceWithRC(['/sbin/sysctl', '-w', 'vm.drop_caches=3'])
  ...
  def _InstallIpfw(self):
    ...
    if not self._li.local:
      raise NotImplementedError
    ipfw_bin = binary_manager.FetchPath(
        'ipfw', self.GetArchName(), self.GetOSName())
    ipfw_mod = binary_manager.FetchPath(
