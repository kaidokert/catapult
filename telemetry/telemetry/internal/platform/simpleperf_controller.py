...
class SimpleperfController(object):
  ...
  def __init__(self, process_name, periods, frequency):
    ...
    self._device_has_results = False

  def _install_simpleperf(self):
  ...
  def _start_sampling(self):
    ...
    profile_cmd = [self._device_simpleperf_path, 'record', '-g',
  ...
  def _stop_sampling(self):
    ...
    proc.wait()
    self._device_has_results = True

  def DidStartBrowser(self, browser):
    assert self._browser is None
    self._browser = browser
    if self._periods and self._browser.supports_simpleperf:
      self._install_simpleperf()
