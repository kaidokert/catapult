...
class AndroidSimpleperfProfiler(Profiler):
  ...
  def _install_simpleperf_for_package(cls, device, package):
    ...
    if package_arch == 'armeabi-v7a':
      arch_dir = 'arm'
    elif package_arch == 'arm64-v8a':
      arch_dir = 'arm64'
    else:
      raise Exception('Unrecognized android app ABI: "%s"' % package_arch)
    host_path = os.path.join(util.GetChromiumSrcDir(),
                             'third_party',
                             'android_ndk',
                             'simpleperf',
                             'bin',
