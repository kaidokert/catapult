...
class NetworkControllerBackend(object):
  ...
  def Open(self, wpr_mode):
    ...
    """
    logging.warning('**** Open(%r) ****', wpr_mode)
    if self.is_open:
      use_live_traffic = wpr_mode == wpr_modes.WPR_OFF
      if self.use_live_traffic != use_live_traffic:
        self.Close()  # Will need to re-open the TsProxy server.
      else:
        # We may reuse the TsProxy server and fowarder
        if self._wpr_mode != wpr_mode:
          self.StopReplay()  # Will need to re-start the WPR server.
  ...
  def is_replay_active(self):
    return self._wpr_server is not None
  ...
  def StartReplay(self, archive_path, make_javascript_deterministic,
    ...
    """
    logging.warning('**** StartReplay(%r, %r, %r) ****',
