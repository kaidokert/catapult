...
HTML_SUFFIX = '.html'
STRIP_QUERY_PARAM_REGEX = re.compile('\?.*$')
...
def _FetchImages(verbose, image_dir, frame_number, external_images):
  ...
  for i in xrange(image_count):
    ...
    # Strip any query param and all subsequent characters.
    ...
    sys.stdout.flush()
    if verbose:
      print 'Fetching image [frame_number=%d, %d/%d, local_file=%s, ' +
          'url=%s].' % (frame_number, i, image_count, image_file, image_url)
    try:
...
def _GetLocalImageDirectory(snapshot_path):
  ...
  return os.path.abspath(image_dir_relpath[
      :image_dir_relpath.rfind('.html')])

def _SnapPageToFile(finder_options, url, interactive, verbose,
                    snapshot_path, snapshot_file, enable_browser_log):
  ...
  try:
    ...
    for i in xrange(len(external_images)):
      _FetchImages(verbose, image_dir, i, external_images[i])

  finally:
    browser.Close()


def SnapPage(finder_options, url, interactive, verbose, snapshot_path,
             enable_browser_log):
  """ Save the HTML snapshot of the page whose address is |url| to
  ...
  with codecs.open(snapshot_path, 'w', 'utf-8') as f:
    _SnapPageToFile(finder_options, url, interactive, verbose, snapshot_path, f,
        enable_browser_log)
