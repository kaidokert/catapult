...
def GetDevToolsBackEndIfReady(devtools_port, app_backend, browser_target=None):
  client = _DevToolsClientBackend(app_backend, browser_target)
  try:
    client.Connect(devtools_port)
...
class _DevToolsClientBackend(object):
  ...
  and destroying them.
  """
  def __init__(self, app_backend, browser_target):
  ...
  def Connect(self, devtools_port):
    try:
      self._Connect(devtools_port)
    except:
      self.Close()  # Close connections made if failed to connect to all.
      raise

  def _Connect(self, devtools_port):
  ...
  def Close(self):
    ...
    # interact with them before closing).
