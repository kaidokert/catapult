...
class _DevToolsStreamReader(object):
  def __init__(self, inspector_socket, stream_handle, trace_handle, callback):
    ...
    Args:
      ...
      callback: A callback function to call when trace reading is completed.
    """
  ...
  def Read(self):
    ...
    assert self._trace_handle is not None, 'all trace data has been read'
  ...
  def _NotificationHandler(self, res):
    if res.get('method') == 'Tracing.dataCollected':
      ...
      _DevToolsStreamReader(
          ...
          self._ReceivedAllTraceDataFromStream).Read()
