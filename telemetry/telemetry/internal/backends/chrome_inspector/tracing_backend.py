...
class _DevToolsStreamReader(object):
  ...
  def Read(self, stream_handle, trace_handle, callback):
    """Start reading data from the stream_handle and write to trace_handle.

    Args:
      stream_handle: A stream handle, as returned by Chrome tracing, from where
        to read the trace data.
      trace_handle: A Python file like object where to write the trace data.
      callback: A callback function to call when all trace data has been read.
    """
  ...
  def _GotChunkFromStream(self, response):
    ...
    # Convert the trace data that received as UTF32 to its native encoding of
...
class TracingBackend(object):
  ...
  def _NotificationHandler(self, res):
    if res.get('method') == 'Tracing.dataCollected':
      ...
      reader = _DevToolsStreamReader(self._inspector_websocket)
      reader.Read(stream_handle, trace_handle, self._ReceivedAllTracingData)

  def _ReceivedAllTracingData(self):
    self._has_received_all_tracing_data = True
