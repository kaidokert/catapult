...
class StartupArgsTest(unittest.TestCase):
  ...
  def testFlagsMerged(self):
    ...
    # Merging is done after sorting sets, so alphabetical order is expected.
    for arg in startup_args:
      if arg.startswith('--disable-features='):
        self.assertEquals(arg, '--disable-features=Feature1,Feature2,Feature3')
        disable_count += 1
      elif arg.startswith('--enable-features='):
        self.assertEquals(arg, '--enable-features=Feature4,Feature5,Feature6')
        enable_count += 1
      elif arg.startswith('--force-fieldtrials='):
        self.assertEquals(arg, '--force-fieldtrials=Group1/Exp1/Group2/Exp2/'
                          + 'Group3/Exp3/Group4/Exp4')
        force_fieldtrials_count += 1
      elif arg.startswith('--force-fieldtrial-params='):
        self.assertEquals(arg, '--force-fieldtrial-params=Group1.Exp1:id/abc,'
                          + 'Group2.Exp2:id/bcd,Group4.Exp4:id/cde')
