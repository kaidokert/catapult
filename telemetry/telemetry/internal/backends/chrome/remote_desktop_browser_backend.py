...
class RemoteDesktopBrowserBackend(chrome_browser_backend.ChromeBrowserBackend):
  """The backend for controlling a remotely-executed browser instance, on Linux,
  Mac or Windows.
  """
  def __init__(self, desktop_platform_backend, remote_platform_backend,
               browser_options,
    ...
    self._remote_backend = remote_platform_backend
    ...
    if self._flash_path and not self._interface.FileExists(self._flash_path):
  ...
  def processes(self):
    ...
    tmp = self._interface.RunCmdOnDevice('ps -aux | grep chrome')[0].strip()
  ...
  def supports_uploading_logs(self):
    return (self.browser_options.logs_cloud_bucket and self.log_file_path and
            self._interface.FileExistsOnDevice(self.log_file_path))

  def _GetDevToolsActivePortPath(self):
    port_path = self._interface.path.join(self.profile_directory, DEVTOOLS_ACTIVE_PORT_FILE)
    logging.debug('Port path: %s', port_path)
    return port_path

  def _GetDevToolsFileContents(self, file):
    logging.warning('Trying to get lines %s', file)
    if not self._interface.IsFile(file):
      logging.warning('File %s does not exist', file)
      raise EnvironmentError('DevTools file doest not exist yet')
    else:
      logging.warning('File does exist')

    logging.warning('Getting file contents')
    ...
    lines = self._interface.GetFileContents(file).strip()
    logging.info('Contents: %s', lines)
    if lines:
      lines = [line.rstrip() for line in lines.splitlines()]
    if not lines:
      logging.warning('File does not have lines')
      raise EnvironmentError('DevTools file empty')
    return lines

  def _FindDevToolsPortAndTarget(self):
    devtools_file_path = self._GetDevToolsActivePortPath()
    logging.info('Getting lines from %s', devtools_file_path)
    ...
    logging.warning('Contents: %s', lines)
    return devtools_port, browser_target

  def _FindUIDevtoolsPort(self):
    devtools_file_path = self._inteface.path.join(self.profile_directory,
  ...
  # TODO THIS IS NOT RIGHT = it's the SSH process.
  def GetPid(self):
    if self._proc:
