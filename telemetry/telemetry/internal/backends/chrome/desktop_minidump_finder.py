...
class DesktopMinidumpFinder(object):
  ...
  def _GetAllCrashpadMinidumps(self, minidump_dir):
    if not minidump_dir:
      self._explanation += ['No minidump directory provided. Likely attempted '
                            'to retrieve the Crashpad minidumps after the '
    ...
    try:
      ...
      if not crashpad_database_util:
        self._explanation += ['Unable to find crashpad_database_util. This is '
                              'likely due to running on a platform that does '
                              'not support Crashpad.']
        return None
    ...
    for report_line in report_output.splitlines():
      ...
      if current_indentation >= last_indentation:
        ...
        try:
          ...
          self._explanation += ['Expected to find keys "Path" and "Creation '
  ...
  def _GetAllMinidumpPaths(self, minidump_dir):
    ...
    if reports_list:
      ...
      self._explanation += ['No minidumps found via crashpad_database_util, '
                            'falling back to globbing for Breakpad minidumps.']
      dumps = self._GetBreakpadMinidumpPaths(minidump_dir)
      if dumps:
        self._explanation += ['Found Breakpad minidump via globbing.']
        for dump in dumps:
  ...
  def _GetBreakpadMinidumpPaths(self, minidump_dir):
    if not minidump_dir:
      self._explanation += ['Attempted to fetch Breakpad minidump paths '
  ...
  def _GetMostRecentMinidump(self, minidump_dir):
    ...
    if not most_recent_dump:
      crashpad_dump = False
      self._explanation += ['No minidump found via crashpad_database_util, '
                            'falling back to globbing for Breakpad minidump.']
      dumps = self._GetBreakpadMinidumpPaths(minidump_dir)
      if dumps:
