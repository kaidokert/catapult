...
class ChromeBrowserBackend(browser_backend.BrowserBackend):
  ...
  def Close(self):
    ...
    if self._devtools_client:
      ...
      # happen on mac if START_BROWSER_IN_FOREGROUND, because chrome is then
      # started using `open` command and telemetry only kills the 'open'
      # command, leaving Chrome still running.
      # Therefore force a clean shutdown on mac.
      if ("ENSURE_CLEAN_CHROME_SHUTDOWN" in os.environ or \
          ("START_BROWSER_IN_FOREGROUND" in os.environ and \
