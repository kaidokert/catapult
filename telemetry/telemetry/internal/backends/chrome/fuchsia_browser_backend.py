...
class FuchsiaBrowserBackend(chrome_browser_backend.ChromeBrowserBackend):
  ...
  def GetStandardOutput(self):
    if self._browser_log_proc:
      ...
      # to capture messages emitted but not yet processed.
      try:
        (stdout_data, _) = self._browser_log_proc.communicate(timeout=5)
        logging.error('captured %d bytes of log data', len(stdout_data))
      except subprocess.TimeoutExpired as te:
        logging.error('captured %d bytes of log data in 5s', len(te.stdout))
        stdout_data = te.stdout

      if isinstance(stdout_data, bytes):
        stdout_data = stdout_data.decode()
      self._browser_log += stdout_data
