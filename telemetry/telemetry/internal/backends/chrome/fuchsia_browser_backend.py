...
class FuchsiaBrowserBackend(chrome_browser_backend.ChromeBrowserBackend):
  ...
  def _CloseOnDeviceBrowsers(self):
    if self.browser_type == WEB_ENGINE_SHELL:
      ...
      if self.browser_type == CAST_STREAMING_SHELL:
  ...
  def GetStandardOutput(self):
    # Return an already-cached value since this function may be called more than
    # once while handling a single failure.
    if self._browser_log:
      return self._browser_log

    assert self._tmp_output_file, 'GetStandardOutput called after Close'
