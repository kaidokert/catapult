...
class FuchsiaBrowserBackend(chrome_browser_backend.ChromeBrowserBackend):
  ...
  def Start(self, startup_args):
    output_root = os.path.join(self._output_dir, 'gen', 'fuchsia_web', 'shell')

    try:
      if self.browser_type == WEB_ENGINE_SHELL:
        self._StartWebEngineShell(startup_args)
        browser_id_files = [
            os.path.join(output_root, 'web_engine_shell', 'ids.txt'),
            os.path.join(output_root, 'web_engine', 'ids.txt'),
        ]
      elif self.browser_type == CAST_STREAMING_SHELL:
        self._StartCastStreamingShell(startup_args)
        browser_id_files = [
            os.path.join(output_root, 'cast_streaming_shell', 'ids.txt'),
            os.path.join(output_root, 'web_engine', 'ids.txt'),
        ]
      else:
