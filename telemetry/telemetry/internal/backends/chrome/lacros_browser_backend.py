...
class LaCrOSBrowserBackend(chrome_browser_backend.ChromeBrowserBackend):
  ...
  def _RunCommandAndPrint(self, cmd):
  ...
  def _QuoteArgs(self, startup_args, arg_name):
    arg_str = '--' + arg_name + '='
  ...
  def _RmArgs(self, startup_args, arg_name):
  ...
  def _LaunchLacrosChromeHelper(self, startup_args):
    print("Start lacros-chrome")

    # reformat disable features flag
    self._QuoteArgs(startup_args, 'enable-features')
    self._QuoteArgs(startup_args, 'disable-features')
    self._QuoteArgs(startup_args, 'force-fieldtrials')
    self._QuoteArgs(startup_args, 'force-fieldtrial-params')
    self._QuoteArgs(startup_args, 'proxy-bypass-list')
   
    self._RmArgs(startup_args, 'enable-features')
    self._RmArgs(startup_args, 'disable-features')

    for i in range(40):
      time.sleep(1)
      self._RunCommandAndPrint(['EGL_PLATFORM=surfaceless', 'XDG_RUNTIME_DIR=/run/chrome', 'python', '/mojo_connection_lacros_launcher.py', '-s', '/tmp/lacros.sock', './../usr/local/lacros-chrome/chrome', '--ozone-platform=wayland', '--user-data-dir=/usr/local/lacros-chrome/user_data', '--enable-gpu-rasterization', '--enable-oop-rasterization', '--lang=en-US', '--breakpad-dump-location=/usr/local/lacros-chrome/', '--no-sandbox'] + startup_args)
  ...
  @exc_util.BestEffort
  def Close(self):
    super(TODODefineThis_LaCrOSBrowserBackend, self).Close()
  ...
  def EnterOverviewMode(self, timeout):
    self._devtools_client.window_manager_backend.EnterOverviewMode(timeout)

  def ExitOverviewMode(self, timeout):
    self._devtools_client.window_manager_backend.ExitOverviewMode(timeout)
