...
class PossibleCrOSBrowser(possible_browser.PossibleBrowser):
  ...
  def Create(self):
    ...
    if self._app_type == 'lacros-chrome':
      ...
      pw = os.environ.get('TMPDIR').join('profraw')
      pw_file = pw.join('default-%2m.profraw')

      print('Setting LLVM_PROFILE_FILE to : %s' % str(pw_file))


      self._DEFAULT_CHROME_ENV.append('LLVM_PROFILE_FILE=%s' % str(pw_file))
      print('lacros-chrome env vars to chrome: %r' % self._DEFAULT_CHROME_ENV)
