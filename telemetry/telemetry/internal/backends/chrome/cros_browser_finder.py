...
class PossibleCrOSBrowser(possible_browser.PossibleBrowser):
  ...
  def SetUpEnvironment(self, browser_options):
    ...
    self._existing_minidump_dir = tempfile.mkdtemp()
    cri.RunCmdOnDevice(
  ...
  def _SetChromeEnvironment(self, env=None):
    ...
    if not cri.root_is_writable:
      ...
      return
    self._existing_env_filepath = tempfile.NamedTemporaryFile(delete=False).name
    cri.RunCmdOnDevice(
  ...
  def _RestoreChromeEnvironment(self):
    ...
    self._platform_backend.cri.RmRF(self._existing_env_filepath)
