...
class CrOSBrowserBackend(chrome_browser_backend.ChromeBrowserBackend):
  CROS_MINIDUMP_DIR = '/tmp/telemetry_minidumps'
  ...
  def GetRecentMinidumpPathWithTimeout(self, timeout_s, oldest_ts):
    ...
    # While unlikely, the device/emulator's clock might be off, so calculate an
    # offset that can be added to the host time to get the corresponding device
    # time. The pulled dumps have their mtime set to their mtime on the device,
    # so oldest_ts can be directly compared to os.path.getmtime().
    stdout, _ = self._cri.RunCmdOnDevice(['date', '+%s'])
    host_time = time.time()
    time_offset = int(int(stdout.strip()) - host_time)
    oldest_ts += time_offset
  ...
  def _PullDumps(self):
    ...
    device/emulator.
    """
    stdout, _ = self._cri.RunCmdOnDevice(['ls', '-1', self.CROS_MINIDUMP_DIR])
    device_dumps = stdout.splitlines()
    for d in device_dumps:
      local_path = os.path.join(self._tmp_minidump_dir, d)
      if not os.path.exists(local_path):
        device_path = posixpath.join(self.CROS_MINIDUMP_DIR, d)
        self._cri.GetFile(device_path, local_path)
        # Set the local version's modification time to the device's.
        stdout, _ = self._cri.RunCmdOnDevice(
            ['ls', '--time-style', '+%s', '-l', device_path])
        stdout = stdout.strip()
        # We expect whitespace-separated fields in this order: mode, links,
        # owner, group, size, mtime, filename.
        mtime = int(stdout.split()[5])
        os.utime(local_path, (mtime, mtime))

  def _SymbolizeMinidump(self, minidump_path):
    # TODO: Upload to GS like DesktopBrowserBackend?
  ...
  def _GetStackFromMinidump(self, minidump):
    ...
    if platform.system() != 'Linux':
      logging.warning('Cannot get stack traces unless running on a Linux host.')
      return None
    ...
    os_name = 'linux'
    # TODO: Figure out if we need to map from Python's architecture name to
    # Telemetry's architecture names. Works on x86_64.
