...
class PossibleAndroidWebViewBrowser(PossibleAndroidBrowser):
  """A launchable android WebView browser instance."""
  def __init__(self, browser_type, finder_options, android_platform,
               backend_settings, local_apk=None):
    super(PossibleAndroidWebViewBrowser, self).__init__(
        browser_type, finder_options, android_platform, backend_settings, local_apk, 'android_webview')
...
def _FindAllPossibleBrowsers(finder_options, android_platform):
  ...
  for settings in ANDROID_BACKEND_SETTINGS:
    if finder_options.IsBrowserTypeRelevant(settings.browser_type):
      ...
      webview_types = [
          android_browser_backend_settings.ANDROID_WEBVIEW.browser_type,
          android_browser_backend_settings.ANDROID_WEBVIEW_BUNDLE.browser_type,
          android_browser_backend_settings.ANDROID_WEBVIEW_GOOGLE.browser_type,
          android_browser_backend_settings.ANDROID_WEBVIEW_GOOGLE_BUNDLE.browser_type,
          android_browser_backend_settings.ANDROID_WEBVIEW_INSTRUMENTATION.browser_type
      ]

      if settings.browser_type in webview_types:
        p_browser = PossibleAndroidWebViewBrowser(
