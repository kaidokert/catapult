...
def _FindReferenceLocalApk(android_platform, is_bundle=False):
  """Find the local apl path for reference browser type.

  None will be returned if such apk does not exist.
  """
...
def _GetReferenceAndroidBrowser(android_platform, finder_options):
  reference_build = _FindReferenceLocalApk(android_platform)
  if reference_build:
  ...
  for settings in ANDROID_BACKEND_SETTINGS:
    if finder_options.IsBrowserTypeRelevant(settings.browser_type):
      ...
      if finder_options.IsBrowserTypeReference():
        local_apk = _FindReferenceLocalApk(android_platform,
                                           finder_options.IsBrowserTypeBundle())
