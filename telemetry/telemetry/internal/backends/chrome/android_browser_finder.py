...
class PossibleAndroidBrowser(possible_browser.PossibleBrowser):
  ...
  def UpdateExecutableIfNeeded(self):
    ...
    for apk in self._support_apk_list:
      logging.warninng('Installing %s on device if needed.', apk)
    ...
    if self._local_apk:
      logging.warninng('Installing %s on device if needed.', self._local_apk)
      ...
      if self._compile_apk:
        ...
        logging.warninng('Compiling %s.', package_name)
    ...
    if ((is_webview_apk or
      ...
      package_name = apk_helper.GetPackageName(self._local_apk)
      logging.warninng('Setting %s as WebView implementation.', package_name)
