...
def _GetLocalReferenceApk(android_platform, is_bundle=False):
  """Get the local apk path for reference browser type.
...
def _GetReferenceAndroidBrowser(android_platform, finder_options):
  reference_build = _GetLocalReferenceApk(android_platform)
  if reference_build:
  ...
  for settings in ANDROID_BACKEND_SETTINGS:
    if finder_options.IsBrowserTypeRelevant(settings.browser_type):
      ...
      if finder_options.IsBrowserTypeReference():
        local_apk = _GetLocalReferenceApk(
