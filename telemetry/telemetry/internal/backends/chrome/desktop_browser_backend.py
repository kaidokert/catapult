...
class DesktopBrowserBackend(chrome_browser_backend.ChromeBrowserBackend):
  ...
  def _CheckOutput(self, cmd, env=None):
    ...
    if rc != 0:
      raise subprocess.CalledProcessError(
          returncode=rc, cmd=cmd, output=stdout, stderr=stderr)
    return stdout

  def _Execute(self, cmd, env=None):
    proc = self._StartProc(
  ...
  def supports_uploading_logs(self):
    return (self.browser_options.logs_cloud_bucket and self.log_file_path
            and self.IsFile(self.log_file_path))
  ...
  def Start(self, startup_args):
    ...
    if self.is_logging_enabled:
      sys.stderr.write('Chrome log file will be saved in %s\n' %
                       self.log_file_path)
