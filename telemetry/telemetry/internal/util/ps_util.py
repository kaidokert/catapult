...
def RunSubProcWithTimeout(args, timeout, process_name):
  ...
  try:
    ...
    logging.warning(
        ('Process %s failed to finished after %ds. Will terminate it.' %
...
def _TerminateOrKillProcess(process, process_name):
  ...
  if not done:
    logging.warning(
        'Failed to terminate/kill the process %s after 20 seconds. \n%s'
        % (process_name, _GetProcessDescription(process)))
  return
