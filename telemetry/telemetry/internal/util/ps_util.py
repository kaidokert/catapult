...
def RunSubProcWithTimeout(args, timeout):
  ...
  try:
    ...
    _TryTerminateOrKillProcess(sp)
    raise
  return sp


def _TryTerminateOrKillProcess(process):
  p = threading.Thread(target=_TerminateOrKillProcess, args=(process,))
  p.start()

def _TerminateOrKillProcess(process):
  done = True
  try:
    process.terminate()
    py_utils.WaitFor(
  ...
  if not done:
    logging.warning('Failed to terminate/kill the process: %s.' %
                    _GetProcessDescription(process))
  return
