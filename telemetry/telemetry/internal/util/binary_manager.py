...
def ReinstallHelperIfNeeded(
  ...
  Args:
    ...
    device: (device_utils.DeviceUtils) a device to install the helper when
        os_name == 'android'
  Raises:
    Exception: When the binary could not be fetched or cannot be pushed to the
        device.
  """
  device_serial = '' if device is None else device.serial
  if (device_serial, install_path) in _installed_helpers:
    return
  arch = None
  if os_name == 'android':
    arch = device.GetABI()
  ...
  _installed_helpers.add((device_serial, install_path))
