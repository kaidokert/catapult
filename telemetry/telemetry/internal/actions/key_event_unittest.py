...
class KeyPressActionTest(tab_test_case.TabTestCase):
  ...
  def testModifierKey(self):
    ...
    control = key_event.EncodeModifiers(control=True)
    # <Ctrl-A> select everything inside input box.
    self._PressKey('a', modifiers=control)
    self._PressKey('X')
    ...
    if platform != 'mac':
      ...
      # <Ctrl-A> is "go to the beginning of the text box" in OSX.
      self._tab.WaitForJavaScriptCondition(
          'document.querySelector("textarea").value === "XHello, World!"',
          timeout=1)
