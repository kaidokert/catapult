...
def RunBenchmark(benchmark, finder_options):
  ...
  if possible_browser:
    ...
    can_run_on_platform = benchmark._CanRunOnPlatform(
        platform_module.GetHostPlatform(), finder_options)

  # For now, test expectation only applicable for the cases the testing target
  # involves the browser.
  if possible_browser:
