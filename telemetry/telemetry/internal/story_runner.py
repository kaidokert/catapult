  ...
  try:
    ...
    for storyset_repeat_counter in xrange(pageset_repeat):
      for story in stories:
        ...
        if results.BenchmarkWasInterrupted():
          results.Skip(results.BenchmarkInterruptionReason(), is_expected=False)
          results.DidRunPage(story)
        ...
        try:
          ...
          try:
            if (not results.BenchmarkWasInterrupted() and
                interruption and state and state.platform):
    ...
    if not results.BenchmarkWasInterrupted():
...
def RunBenchmark(benchmark, finder_options):
  ...
  with results_options.CreateResults(
    ...
    try:
      ...
      logging.fatal(
          'Benchmark execution interrupted: %s(%s)' % (type(e), e))
      exception_formatter.PrintFormattedException()
      return_code = 2
