...
def AddCommandLineArgs(parser):
  ...
  # overridden by passed in commandline arguments. See https://goo.gl/JZ2yZQ.
  group.add_option('--pageset-repeat', default=1, type='int',
                   help='Number of times to repeat the entire pageset. ')
  group.add_option('--smoke-test-mode', action='store_true',
                   help='Treat the benchmark run as a functional test run.'
                   'Benchmark authors who use iterations should treat this '
                   'flag as a request to reduce iterations to 1. (An '
                   'iteration is a repeat of the benchmark without restarting '
                   'Chrome. It must be supported in benchmark-specific code.)')
