...
class BrowserFinderOptions(optparse.Values):
  ...
  def CreateParser(self, *args, **kwargs):
    ...
    def ParseArgs(args=None):
      ...
      # multiple profiling processes at the same time. It is not gauranteed that
      # the correct profiling process will be terminated at the end of a session
      # when multiple profiling processes are running at the same time.
      if (len(self.interval_profiling_periods) > 1
          and 'story_run' in self.interval_profiling_periods):
        raise ValueError('Cannot provide other periods along with the story_run'
                         ' period.')
