...
class BrowserFinderOptions(optparse.Values):
  ...
  def CreateParser(self, *args, **kwargs):
    ...
    group.add_option(
        ...
        type=str, default='', metavar='"--flag <options> ..."',
        help='Addtional arguments to pass to the CPU profiler. This is used '
        'only on ChromeOS. On ChromeOS, pass the perf command name followed by '
        'the options to pass to the perf command. Supported perf commands are '
        'record and stat. Eg: record -e cycles -c 4000000 -g . '
        'Note: "-a" flag is added to the perf command by default. Do not pass '
        'options that are incompatible with the system-wide profile collection.'
    )
    parser.add_option_group(group)
