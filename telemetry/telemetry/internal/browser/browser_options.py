...
class BrowserFinderOptions(optparse.Values):
  ...
  def CreateParser(self, *args, **kwargs):
    ...
    group.add_option(
        ...
        'May be specified multiple times except when the whole_story_run '
        'period is used; available choices are ["navigation", "interactions", '
        '"whole_story_run"]. Profile data will be written to '
    ...
    def ParseArgs(args=None):
      ...
      if (len(self.interval_profiling_periods) > 1
          and 'whole_story_run' in self.interval_profiling_periods):
        raise ValueError('Cannot provide other periods along with'
                         ' whole_story_run.')
