...
class BrowserOptions():
  ...
  def __init__(self):
    # optparse.OptionGroup used to transfer properties from the main
    # finder_options in UpdateFromParseResults.
    self._browser_options_group = None
  ...
  def AddCommandLineArgs(self, parser):
    ...
    parser.add_option_group(group)
    self._browser_options_group = group
  ...
  def UpdateFromParseResults(self, finder_options):
    ...
    for option in self._browser_options_group.option_list:
      name = option.dest
