...
class _ChromeOSController(_PlatformController):
  ...
  # _CheckAndKillProfilingProcess checks if the profiling process is alive. If
  # the profiling process is alive, profiling is terminated.
  def _CheckAndKillProfilingProcess(self, ssh_process):
    # Poll the SSH process to check if the connection is still alive. If it is
    # alive, the returncode should not be set. If the returncode is set, the
    # profiling process exited prematurely.
    ssh_process.poll()
    if ssh_process.returncode != None:
      raise Exception('Profiling process exited prematurely.')
    pidof_line = self._platform_backend.RunCommand(
        ['pidof', self.PERF_BINARY_PATH]).strip()
    pidof_lines = pidof_line.split()
    if not pidof_lines:
      raise Exception('Could not get pid of the running perf process.')
    # Kill the profiling process using its PID. Terminating the SSH connection
    # doesn't kill the profiling process.
  ...
  def SamplePeriod(self, period, action_runner, **_):
    ...
    try:
      ...
      try:
        self._CheckAndKillProfilingProcess(ssh_process)
      except Exception: # pylint: disable=broad-except
        self._platform_backend.RunCommand(['rm', '-f', out_file])
        if not has_existing_exception:
          raise
        # Print current exception and propagate existing exception.
        exception_formatter.PrintFormattedException(
            msg='Exception raised when cleaning profiling process: ')
  ...
  def GetResults(self, page_name, _, results):
    if not results.current_page_run.ok:
      for _, device_file in self._device_results:
        self._platform_backend.RunCommand(['rm', '-f', device_file])
      self._device_results = []
      return
