...
class SystemWideProfilingController(object):
  ...
  def SampleSession(self, story_name, action_runner):
    ...
    profiling_process.poll()
    if profiling_process.returncode is not None:
      raise Exception('Perf process is not alive and the return code is %s', profiling_process.returncode)
  ...
  def GetResults(self, story_name, results):
    ...
    with results.CreateArtifact(story_name, 'perf', prefix=prefix, suffix='.perf.data') as fh:
      local_file = fh.name
      fh.close()
