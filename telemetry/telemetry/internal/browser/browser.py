...
class Browser(app.App):
  ...
  def LogSymbolizedUnsymbolizedMinidumps(self, log_level):
    ...
    for unsymbolized_path in paths:
      ...
      # Store the symbolization attempt as an artifact, but if the artifact
      # implementation will end up logging instead, skip so that we don't
      # clutter stdout with duplicate information.
      if not artifact_logger.WillLogArtifacts():
        minidump_name = os.path.basename(unsymbolized_path)
        artifact_path = os.path.join('symbolize_attempts', minidump_name)
        logging.log(log_level, 'Saving symbolization attempt as artifact %s',
                    artifact_path)
        artifact_logger.CreateArtifact(
            'symbolize_attempts', artifact_path, sym[1])
      if sym[0]:
