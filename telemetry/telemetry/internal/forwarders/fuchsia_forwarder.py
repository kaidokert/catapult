...
class FuchsiaSshForwarder(forwarders.Forwarder):
  ...
  def __init__(self, target_id, local_port, remote_port, port_forward):
    ...
    # Store the original parameters for the port forwarding cancellation.
    ...
    if port_forward:
      ...
      remote_port = port_forward(target_id, local_port)
    else:
      from test_server import port_backward
      local_port = port_backward(target_id, remote_port, local_port)
  ...
  def Close(self):
    ...
    cancel_port_forwarding(self._target_id,
                           0 if self._port_forward else self._fuchsia_port,
