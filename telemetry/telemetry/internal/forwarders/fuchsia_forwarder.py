...
class FuchsiaForwarder(Forwarder):
  ...
  def __init__(self, forwarder, local, remote, reverse):
    super(FuchsiaForwarder, self).__init__()
    self._forwarder = forwarder

    if not reverse:
      assert local and not remote
      remote = self._forwarder.ForwardLocalPortAndGetRemote(local)
    else:
      assert remote and not local
      local = util.GetUnreservedAvailableLocalPort()

    self._forwarder.ForwardPort(local, remote, reverse)
...
class FuchsiaForwarderFactory(ForwarderFactory):
  ...
  def ForwardPort(self, local, remote, reverse):
    scheme = net_test_server.ForwardingScheme(local, remote, reverse)
    self._ssh_forwarder.Map([scheme])
