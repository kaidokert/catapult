...
class AndroidForwarderFactory(forwarders.ForwarderFactory):
  ...
  def Create(self, port_pair,  # pylint: disable=arguments-differ
    ...
    try:
      ...
      if logging.getEffectiveLevel() > logging.WARNING:
        logging.warning('Increase verbosity to see more debug information.')
        raise

      try:
        hex_remote_port = hex(port_pair.remote_port)
      except TypeError:
        hex_remote_port = None
      ...
      def ListDeviceTcpPortsInUse():
        ...
        proc_net_tcp_target = ':%s ' % hex_remote_port[2:]
      ...
      if port_pair.remote_port:
        if hex_remote_port:
          debug_infos.append(ListDeviceTcpPortsInUse)
        debug_infos.extend(ListRelevantLsofEntries)
