...
class LinuxBasedForwarderFactory(forwarders.ForwarderFactory):
  ...
  def __init__(self, interface):
    super().__init__()
    logging.info('INTERFACE: %s', str(interface))
    logging.info('INTERFACE LOCAL: %s', str(interface.local))
    logging.info('INTERFACE HOSTNAME: %s', str(interface.hostname))
  ...
  def Create(self, local_port, remote_port, reverse=False):
    ...
    if self._interface.local:
      logging.info('THIS IS LOCAL')
      logging.info('INTERFACE %s', str(self._interface))
      logging.info('INTERFACE hostname %s', str(self._interface.hostname))
      return do_nothing_forwarder.DoNothingForwarder(local_port, remote_port)
    logging.info('SHIT IS NOT LOCAL')
    return LinuxBasedSshForwarder(self._interface,
                            local_port,
                            remote_port,
...
class LinuxBasedSshForwarder(forwarders.Forwarder):
  ...
  def __init__(self, interface, local_port, remote_port, port_forward):
    super(LinuxBasedSshForwarder, self).__init__()
    self._interface = interface
    self._proc = None
    ...
    with tempfile.NamedTemporaryFile() as stderr_file:
      self._proc = subprocess.Popen(
          self._interface.FormSSHCommandLine(['-NT'], forwarding_args,
  ...
  def Close(self):
    if self._proc:
      self._proc.kill()
      self._proc = None
    super(LinuxBasedSshForwarder, self).Close()
