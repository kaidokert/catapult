...
class DummyPage(page_module.Page):
  def __init__(self, page_set, name, side_effect=None, serving_dir=None):
  ...
  def Run(self, _):
    if self._side_effect is not None:
      if isinstance(self._side_effect, Exception):
        raise self._side_effect  # pylint: disable=raising-bad-type
      else:
        self._side_effect()
...
class FakeBenchmark(benchmark.Benchmark):
  def __init__(self, stories=None, cloud_bucket=None):
    ...
    for story in stories:
      if isinstance(story, basestring):
        ...
        raise TypeError(story)
      self._story_specs.append(story)
  ...
  def page_set(self):
    ...
    for kwargs in self._story_specs:
      story_set.AddStory(DummyPage(story_set, **kwargs))
    return story_set
...
class RunBenchmarkTest(unittest.TestCase):
  ...
  def testRunBenchmark_DisabledWithExpectations(self):
    ...
    story_runner.RunBenchmark(fake_benchmark, options)
    results = self.ReadIntermediateResults()
    self.assertTrue(results['testResults'])  # Some tests ran.
  ...
  def testRunBenchmark_WithOwnerInfo(self):
    ...
    story_runner.RunBenchmark(fake_benchmark, options)
    results = self.ReadIntermediateResults()
    logging.warning(json.dumps(results, indent=2))
  ...
  def testRunBenchmark_WithOwnerInfo_NoUrl(self):
  ...
  def testRunBenchmarkReturnCodeDisabledStory(self):
    fake_benchmark = FakeBenchmark()
    fake_benchmark.SetExpectations('fake_benchmark/* [ Skip ]')
    options = self.GetFakeBrowserOptions()
  ...
  def testRunBenchmarkReturnCodeCaughtException(self):
    fake_benchmark = FakeBenchmark(stories=[{
        'name': 'story1', 'side_effect': exceptions.AppCrashException()}])
    options = self.GetFakeBrowserOptions()
    return_code = story_runner.RunBenchmark(fake_benchmark, options)
    self.assertEqual(return_code, 1)

  def testRunBenchmarkReturnCodeUnhandleableError(self):
    fake_benchmark = FakeBenchmark(stories=[{
        'name': 'story1', 'side_effect': MemoryError('Unexpected exception')}])
    options = self.GetFakeBrowserOptions()
    return_code = story_runner.RunBenchmark(fake_benchmark, options)
  ...
  def testDownloadMinimalServingDirs(self):
    fake_benchmark = FakeBenchmark(stories=[
        {'name': 'story_foo', 'serving_dir': '/files/foo'},
        {'name': 'story_bar', 'serving_dir': '/files/bar'},
    ], cloud_bucket=cloud_storage.PUBLIC_BUCKET)
    options = self.GetFakeBrowserOptions(overrides={'story_filter': 'foo'})
    with mock.patch(
      ...
      get_files.assert_called_once_with(
          '/files/foo', cloud_storage.PUBLIC_BUCKET)
