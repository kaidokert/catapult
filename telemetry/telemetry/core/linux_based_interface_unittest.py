...
class LinuxBasedInterfaceTest(unittest.TestCase):
  ...
  @mock.patch.object(linux_based_interface.LinuxBasedInterface, 'OpenConnection')
  @mock.patch.object(linux_based_interface, 'GetAllCmdOutput')
  @mock.patch.object(linux_based_interface.LinuxBasedInterface, '_FormSCPToRemote')
  def testPushFileLocal(self, mock_form_scp, mock_run_cmd, unused_mock):
    with self._GetLocalLBI() as lbi:
      ...
      self.assertRegex(' '.join(mock_run_cmd.call_args[0][0]), 'cp.*foo\sremote_foo')
  ...
  def testPushFileRemote(self, mock_abspath, mock_ssh_warnings,
                         mock_form_scp, mock_run_cmd, unused_mock):
    with self._GetLBI(remote='address') as lbi:
      self.assertFalse(lbi.local)
      mock_run_cmd.return_value = ('stdout do not care', 'stderr but will get filtered')
      ...
      mock_form_scp.assert_called_once_with('/some/abs/path/to/foo',
                                            'remote_foo',
                                            extra_scp_args=mock.ANY)
  ...
  def testPushFileRemote(self, mock_tempfile, mock_push_file, unused_mock):
    with self._GetLocalLBI() as lbi:
  ...
  @mock.patch.object(linux_based_interface.LinuxBasedInterface, 'OpenConnection')
  @mock.patch.object(linux_based_interface.LinuxBasedInterface, '_FormSCPFromRemote')
  @mock.patch.object(linux_based_interface, 'GetAllCmdOutput')
  @mock.patch.object(os.path, 'abspath')
  @mock.patch.object(shutil, 'copyfile')
  def testGetFileLocal(self, mock_copyfile, mock_abspath, mock_cmd, mock_scp, unused_mock):
    with self._GetLocalLBI() as lbi:
      self.assertTrue(lbi.local)
      ...
      mock_copyfile.assert_called_once_with('remote/file', '/local/file/abspath')
      ...
      mock_copyfile.assert_called_once_with('/path/in/other/dir', '/local/file/dir')
  ...
  @mock.patch.object(os.path, 'abspath')
  @mock.patch.object(shutil, 'copyfile')
  def testGetFileRemote(self, mock_copyfile, mock_abspath, mock_cmd, mock_scp, unused_mock):
    with self._GetLBI(remote='address') as lbi:
      self.assertFalse(lbi.local)
      mock_abspath.return_value = '/local/file/abspath'
      ...
      mock_scp.assert_called_once_with('remote/file',
                                       '/local/file/abspath',
                                       extra_scp_args=[])
      ...
      self.assertRaises(OSError,
                        lbi.GetFile, 'does it matter', 'at all')

      mock_cmd.reset_mock()
      mock_scp.reset_mock()

      # Check that edge case with filename mismatch is dealt with.
      mock_cmd.side_effect = [('stderr incoming!', 'filename does not match request'),
                              ('stderr works!', '')]
      mock_scp.side_effect = ['first scp command'.split(' '), 'second scp command'.split(' ')]
      ...
      mock_scp.assert_has_calls([
          mock.call('remote/file', '/local/file/abspath',
                    extra_scp_args=[]),
          mock.call('remote/file', '/local/file/abspath',
                    extra_scp_args=['-T'])
      ])
  ...
  def testGetFileContents(self, mock_open, mock_named_temp, mock_getfile, unused_mock):
    with self._GetLBI(remote='address') as lbi:
      some_mock = mock.Mock()
      some_mock.name = 'foo'
      ...
      self.assertEqual(lbi.GetFileContents('path/to/some/file'),
                       'these are file contents')

      # Check that we got a temp file created
      mock_named_temp.assert_called_once_with(mode='w')
      mock_getfile.assert_called_once_with('path/to/some/file',
                                           'foo')
  ...
  def testListProcesses(self, mock_cmd, unused_mock):
    ps_output = """      1       0 /sbin/init splash                                                                                                                                                S
      2       0 [kthreadd]                                                                                                                                                S
      3       2 [rcu_gp]                                                                                                                                                I
      4       2 [rcu_par_gp]                                                                                                                                                I
      5       2 [netns]                                                                                                                                                I
...
"""
  ...
  @mock.patch.object(linux_based_interface.LinuxBasedInterface, 'OpenConnection')
  @mock.patch.object(linux_based_interface.LinuxBasedInterface, 'GetChromeProcess')
  def testGetChromePid(self, mock_get_proc, unused_mock):
    with self._GetLocalLBI() as lbi:
      mock_get_proc.return_value = {'pid': 42, 'path': '/path/to/chrome', 'args': ['some', 'args']}
  ...
  def testIsServiceRunning(self, mock_systemd, mock_cmd, unused_mock):
    with self._GetLocalLBI() as lbi:
      ...
      self.assertRaisesRegex(AssertionError, 'some stderr',
                        lbi.IsServiceRunning, 'foo-service')
      mock_cmd.assert_called_once_with(['systemctl', 'show', '-p', 'MainPID', 'foo-service'], quiet=mock.ANY)
...
""", 'ignored stderr')
  ...
  def testIsHTTPServerRunningOnPort(self, mock_cmd, unused_mock):
    with self._GetLocalLBI() as lbi:
      ...
      mock_cmd.reset_mock()
      mock_cmd.return_value = 'Connection somewhere not refused', 'Connection NOT refused'
  ...
  def testTakeScreenshotLocallyMakesDirectoryAndCopiesFile(
    ...
    with self._GetLocalLBI() as lbi:
      ...
      mock_run_cmd.assert_has_calls([
          ...
          mock.call(
              (lbi._SCREENSHOT_BINARY + ' /var/log/screenshots/foo && echo').split(' ') +
  ...
  def testTakeScreenshotRemoteMakesDirectoryAndCopiesFile(
    ...
    with self._GetLBI(remote='address') as lbi:
      ...
      mock_get_file.assert_called_once_with(
          '/var/log/screenshots/foo', '/path/to/foo')
  ...
  @mock.patch.object(os.path, 'exists')
  @mock.patch.object(os, 'makedirs')
  def testTakeScreenshotRemoteWithErrorLogsIt(
      self,
      mock_mkdirs,
      mock_exists,
      mock_get_file,
    ...
    with self._GetLBI(remote='address') as lbi:
      ...
      with self.assertLogs(level='ERROR') as log:
        ...
        self.assertIn('Unable to pull screenshot file /var/log/screenshots/path to /some/path', log.output[0])
  ...
  @mock.patch.object(linux_based_interface.LinuxBasedInterface, 'OpenConnection')
  @mock.patch.object(linux_based_interface.LinuxBasedInterface, 'GetFileContents')
...
"""
    with self._GetLocalLBI() as lbi:
      ...
      self.assertEqual(lbi.LsbReleaseValue('DISTRIB_DESCRIPTION', 'default'), '"Debian GNU/Linux buzzian"')
      mock_contents.assert_called_once_with('/etc/lsb-release')
      self.assertEqual(lbi.LsbReleaseValue('DISTRIB_CODENAME', 'default'), 'buzzian')
      self.assertEqual(lbi.LsbReleaseValue('DISTRIB_ID', 'default'), 'Debian')
      self.assertEqual(lbi.LsbReleaseValue('DISTRIB_RELEASE', 'default'), 'buzzian')
      self.assertEqual(lbi.LsbReleaseValue('ANOTHER_PROPERTY', 'default'), 'foobuntu')
      self.assertEqual(lbi.LsbReleaseValue('DNE_PROPERTY', 'default'), 'default')

  @mock.patch.object(linux_based_interface.LinuxBasedInterface, 'OpenConnection')
  @mock.patch.object(linux_based_interface.LinuxBasedInterface, 'CloseConnection')
