...
""", 'ignored stderr')
  ...
  @mock.patch.object(os.path, 'exists')
  @mock.patch.object(os, 'makedirs')
  def testTakeScreenshotRemoteWithErrorGetsLogged(
      self, unused_mkdirs, mock_exists, unused_get_file, mock_run_cmd,
      unused_mock):
    mock_exists.side_effect = [False, OSError]
    with self._GetLBI(remote='address') as lbi:
      self.assertFalse(lbi.local)
      mock_run_cmd.side_effect = [
          ('making a directory', ''),
      ...
      with self.assertLogs(level='ERROR') as log:
        ...
        self.assertIn(('Unable to pull screenshot file '
                       '/tmp/log/screenshots/path to /some/path'),
                      log.output[0])
