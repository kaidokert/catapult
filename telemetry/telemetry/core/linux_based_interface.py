...
CHROME_PROCESS_REGEX = [re.compile(r'^/opt/google/chrome/chrome '),
...
def StartCmd(args, cwd=None, quiet=False, stdout=None, stderr=None, env=None):
  return cmd_util.StartCmd(args, cwd, quiet, stdout, stderr, env=env)
...
class LinuxBasedInterface:
  ...
  _DEFAULT_SSH_COMMAND = ['ssh',
                          '-o ForwardX11=no',
                          '-o ForwardX11Trusted=no', '-n']
  ...
  def GetChromeProcess(self):
    # TODO: implement for Linux. This is likely from systemd or from something
    # else during out tests???
    raise NotImplementedError

  # TODO: This is ChromeOS specific, but can be made generic.
  ...
  def FormSSHCommandLine(self, args, extra_ssh_args=None, port_forward=False,
    ...
    if env:
      for k, v in env.items():
        logging.info('before: %s=%s', k, v)
        k = _sanitize(k)
        v = _sanitize(v)
       # v = shlex.quote(str(v))
       # k = shlex.quote(str(k)).replace('\n',' ').replace('\r', ' ')
        logging.info('after: %s=%s', k, v)
  ...
  def RunCmdOnDevice(self, args, cwd=None, quiet=False, connect_timeout=None,
                     port_forward=False, env=None):
    stdout, stderr = GetAllCmdOutput(
        self.FormSSHCommandLine(
            args, connect_timeout=connect_timeout, port_forward=port_forward, env=env),
        cwd=cwd,
        quiet=quiet)
  ...
  def RunCmdOnDeviceWithRC(self, args, cwd=None, quiet=False, connect_timeout=None):
    popen = self.StartCmdOnDevice(args, cwd=cwd, quiet=quiet, connect_timeout=connect_timeout)
    popen.wait()
  ...
  def TestFile(self, path, exists=False, is_file=False, is_dir=False):
    ...
    logging.debug("TestFile(<text>, %s, %s)->%s" % (path, condition, test_result))
    return test_result
  ...
  def MkdTemp(self, prefix=None):
    ...
    if prefix:
      cmd.append(prefix)
    rc, stdout, _ = self.RunCmdOnDeviceWithRC(cmd)
    assert rc == 0, 'Could not make a temporary directory'
    return stdout.strip()
  ...
  def GetFile(self, filename, destfile=None):
    ...
    if ('filename does not match request' in stderr and
      ...
      args = self._FormSCPFromRemote(filename, destfile, extra_scp_args=['-T'], user=self._REMOTE_USER)
      _, stderr = GetAllCmdOutput(args, quiet=True)
      stderr = self._RemoveSSHWarnings(stderr)
    if stderr != '':
      raise OSError('No such file or directory %s' % stderr)
  ...
  def PullDumps(self, host_dir):
    ...
    stdout, _ = self.RunCmdOnDevice(
        ['ls', '-1', cmd_helper.SingleQuote(
            self.ExpandPath(self.MINIDUMP_DIR))])
  ...
  def TakeScreenshot(self, file_path):
    ...
    stdout, stderr = self.RunCmdOnDevice([self._SCREENSHOT_BINARY,
                                          var_path,
                                          '&&',
                                          'echo',
