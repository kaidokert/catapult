...
from telemetry.core.util import GetUnreservedAvailableLocalPort
...
class FuchsiaInterface(object):
  ...
  def PutFile(self, source, dest):
    self._device.PutFile(self, source, dest)
  ...
  def ForwardLocalPortAndGetRemote(self, local_port):
    self.ssh_forwarder.Map([(0, local_port)])
    return self.ssh_forwarder.GetDevicePortForHostPort(local_port)

  def ForwardAvailablePort(self):
    local = GetUnreservedAvailableLocalPort()
