...
if os.path.exists(TEST_SCRIPTS_ROOT):
  ...
  assert not TEST_SCRIPTS_ROOT in sys.path
  sys.path.insert(0, TEST_SCRIPTS_ROOT)
  from common import get_ssh_address, \
                     run_continuous_ffx_command as ffx_run_continuous, \
                     run_ffx_command as ffx_run
  from compatible_utils import get_ssh_prefix


def _assert_fuchsia_package_available():
  assert TEST_SCRIPTS_ROOT in sys.path, \
...
def run_ffx_command(cmd, target_id=None, check=True, **kwargs):
  ...
  _assert_fuchsia_package_available()
