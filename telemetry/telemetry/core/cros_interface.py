...
class CrOSInterface(object):
  ...
  # ChromeOS normally writes minidumps to /var/log/chrome/'Crash Reports'/, but
  # in order to have each test run's crash dumps be separate from each other
  # without risking deletion of dumps that existed before any tests were run,
  # we dump to a different directory that gets cleared after each test.
  ...
  def PullDumps(self, host_dir):
    ...
    """
    stdout, _ = self.RunCmdOnDevice(['ls', '-1', self.CROS_MINIDUMP_DIR])
    device_dumps = stdout.splitlines()
    for d in device_dumps:
      host_path = os.path.join(host_dir, d)
