...
class _HeapMetric(object):
  ...
  def _Recurse(self, obj, parent_related_names, parent_breakdown):
    ...
    parent_related_names.Set(hist.short_name, hist.name)
    parent_breakdown.Set(
        hist.short_name, parent_breakdown.Get(hist.short_name) + size)
    breakdown.Set(
        'other', size - sum(subsize for subname, subsize in breakdown))
