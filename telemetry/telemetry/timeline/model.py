...
class TimelineModel(event_container.TimelineEventContainer):
  def __init__(self, trace_data=None, shift_world_to_zero=True):
    ...
    Args:
        trace_data: Either an in-memory RawTraceData or a TraceDataBuilder.
          If the later, then the raw data will be extracted and the builder
          cleaned up to free its resources.
  ...
  def _ImportTraces(self, trace_data, shift_world_to_zero=True):
    ...
    """
    if isinstance(trace_data, trace_data_module.TraceDataBuilder):
      trace_data = trace_data.AsRawTraceData(clean_up=True)
    elif not isinstance(trace_data, trace_data_module.RawTraceData):
