...
class SystemTraceConfigUnittest(unittest.TestCase):
  ...
  def testCallstackSamplingConfigWithCmdlines(self):
    config = system_trace_config.SystemTraceConfig()
    config.EnableCallstackSampling(
  ...
  def testCallstackSamplingConfigWithFrequency(self):
    config = system_trace_config.SystemTraceConfig()
    config.EnableCallstackSampling(
        target_cmdlines="", sampling_frequency_hz=1234)
    self.assertEqualIgnoringWhitespace(
        config.GetTextConfig(), """
      buffers: {
          size_kb: 200000
  ...
  def testCallstackSamplingConfigWithCmdlinesAndFrequency(self):
    config = system_trace_config.SystemTraceConfig()
    config.EnableCallstackSampling(
