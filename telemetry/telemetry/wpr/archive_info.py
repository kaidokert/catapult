...
class WprArchiveInfo(object):
  ...
  def _NextWprFileName(self, file_hash):
    ...
    # Match new archive names which include a shortened hash and older names
    # which only have a sequence number.
    pattern = re.compile(
        ('(?P<BASE>.*?)_(([a-f0-9]{{10}}))|(?P<NUMBER>[0-9]+).{extension}'
         .format(extension=extension)))
    for wpr_file in wpr_files:
      match = pattern.match(wpr_file)
      if not match:
    ...
    new_filename = '%s_%s.%s' % (base, file_hash[:10], extension)
