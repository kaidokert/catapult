...
class SharedAndroidStoryState(story_module.SharedState):
  ...
  def CloseBrowser(self):
    if self._browser is not None:
      try:
        # Flush tracing so that we retain a copy of the trace from this browser
        # before it's closed.
        if self.tracing_controller.is_tracing_running:
          self.tracing_controller.FlushTracing()
        # Close all tabs before closing the browser. Prevents a bug that would
        # cause future browser instances to hang when older tabs receive a
        # DevTools request.
