...
*   There are two major ways to collect data (often referred to as
    ...
    * _Ad hoc measurements:_  These are measurements that do not require traces
       - for example, when a metric is calculated directly in the test page in
        Javascript and we simply want to report this number. Currently
        PressBenchmark and associates PressStory subclasses are examples that
        use ad hoc measurements. (In reality, PressBenchmark uses
        _DualMetricMeasurement_ `StoreTest`, where you can have both ad hoc and
    ...
    *  _Timeline Based Measurements:_ These are measurements that require
       ...
       Metrics are computed based on that trace after the story finishes
       running. New metrics should generally be timeline based measurements:
       Computing metrics on the trace makes it possible to compute many
       different metrics from the same run easily, and the collected trace is
       useful for debugging metrics values.
