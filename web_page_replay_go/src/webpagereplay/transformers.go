...
// script source directive. If a header set does not have a CSP header or if the CSP header does
// not have a script-src directive, getCSPScriptSrcDirectiveFromHeaders returns an empty string.
...
func NewScriptInjectorFromFile(filename string, replacements map[string]string) (
...
// If nonce is an empty string, getScriptWithNonce returns the script block without attaching a
// nonce attribute.
// Some responses may specify a nonce inside their Content-Security-Policy, script-src directive.
// The script injector needs to set the injected script's nonce attribute to open execute
// permission for the injected script.
...
func (si *scriptInjector) Transform(_ *http.Request, resp *http.Response) {
	...
	transformResponseBody(resp, func(body []byte) []byte {
		...
		if idx == nil {
			log.Printf("ScriptInjector(%s): no start tags found, skip injecting script",
				resp.Request.URL)
			return body
		}
		n := idx[1]

		// If the response has a content-script-policy script src directive that specifies a nonce,
		...
		// scripts containing the nonce, WPR must add the nonce token to injected scripts. Please see
