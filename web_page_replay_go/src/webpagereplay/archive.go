...
func (a *Archive) ForEach(f func(url *url.URL, req *http.Request, resp *http.Response) error) error {
	for _, urlmap := range a.Requests {
		for urlString, requests := range urlmap {
			...
			for index, archivedRequest := range requests {
				...
				if err := f(fullURL, req, resp); err != nil {
					return err
				}
			}
		}
...
func (a *Archive) FindRequest(req *http.Request) (*http.Request, *http.Response, error) {
	...
	reqURL := req.URL
	assertCompleteURL(reqURL)

	var bestRatio float64
	if len(hostMap[reqURL.String()]) > 0 {
		...
		for _, archivedRequest := range hostMap[reqURL.String()] {
			curReq, curResp, err := archivedRequest.unmarshal(reqURL.Scheme)
			if err != nil {
	...
	if bestURL != "" {
		return findExactMatch(hostMap[bestURL], req.Method, reqURL.Scheme)
	}
...
func (a *Archive) Edit(edit func(req *http.Request, resp *http.Response) (*http.Request, *http.Response, error)) (*Archive, error) {
	clone := newArchive()
	err := a.ForEach(func(fullURL *url.URL, oldReq *http.Request, oldResp *http.Response) error {
...
func (a *Archive) Merge(other *Archive) error {
	...
	err := other.ForEach(func(fullURL *url.URL, req *http.Request, resp *http.Response) error {
...
func (a *Archive) Add(method string, urlString string) error {
	...
	// Print a warning if an existing request has been overwritten.
	if foundReq, _, notFoundErr := a.FindRequest(req); notFoundErr != ErrNotFound {
		if foundReq.URL.String() == url.String() {
			log.Printf("Potentially adding duplicate request: %v", url)
		}
	}
