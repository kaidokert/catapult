...
type Archive struct {
	...
	// Archive can serve responses in chronological order. If a client wants to reset the Archive
...
func (a *Archive) findBestMatchInArchivedRequestSet(incomingReq *http.Request, archivedReqs []*ArchivedRequest) (*http.Request, *http.Response, error) {
...
// Reset the archive to serve responses from the start.
...
// order to the start.
func (a *Archive) ResetReplayChronology() {
	a.CurrentSessionId++
}
