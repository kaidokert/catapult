...
func (r *ReplayCommand) Flags() []cli.Flag {
	return append(r.common.Flags(),
		...
		cli.BoolFlag{
			...
			Usage:       "When an incoming request matches multiple recorded responses, " +
			             "serve response in chronological sequence. I.e. wpr responds to " +
			             "the first request with the first recorded response, and the second " +
			             "request with the second recorded response.",
