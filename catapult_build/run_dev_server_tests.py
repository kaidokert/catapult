...
def RunTests(args, chrome_path):
  ...
  Returns 124 if tests exceed args.timeout_sec.
  """
  user_data_dir = None
  xvfb_process = None
  chrome_process = None
  ...
  try:
    ...
    timer = Timer(args.timeout_sec, KillServer)
    timer.start()
    ...
    if timed_out:
      print 'Tests did not finish before %d seconds' % args.timeout_sec
      return 124
    else:
    ...
    if xvfb_process:
      # xvfb does not exist on Windows, so we don't have to use KillProcess.
      xvfb_process.kill()
