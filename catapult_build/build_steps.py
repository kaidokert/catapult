...
def _BuildPythonCmd(checkout_path, script, args=None, venv=True):
  ...
  cmd = ['vpython' if venv else 'python', '-u']
  cmd.append(os.path.join(checkout_path, script))
  if args:
    cmd.extend(args)
  return cmd
...
def main(args=None):
  ...
  steps = [{
      ...
      'cmd': python_cmd(
          ...
          args=[args.api_path_checkout, ','.join(_STALE_FILE_TYPES)]),
  }]
  ...
  for test in _CATAPULT_TESTS:
    ...
    uses_app_engine_sdk = test.get('uses_app_engine_sdk', False)
    step['cmd'] = python_cmd(test['path'], venv=not uses_app_engine_sdk)
    if step['name'] == 'Systrace Tests':
      step['cmd'] += ['--device=' + args.platform]
    if test.get('additional_args'):
      step['cmd'] += test['additional_args']
    if uses_app_engine_sdk:
