...
class GetIfChangedTests(BaseFakeFsUnitTest):
  ...
  def testNoHashComputationNeededUponSecondCall(
    ...
    with open('%s.fetchts' % file_path, 'r') as f:
      raw = f.read().strip()
      last_fetch_ts = float(raw)

    hash_modified_ts = os.path.getmtime('%s.sha1' % file_path)

    self.assertTrue(os.path.exists('%s.sha1' % file_path))
    self.assertGreater(last_fetch_ts, hash_modified_ts)
