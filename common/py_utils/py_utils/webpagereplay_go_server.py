  ...
  """
    ...
    def _GetGoBinaryPath(self, replay_options):
        ...
        if USE_LOCAL_WPR in replay_options:
            # Init WPR dependencies (this will fail if it has already been done)
            subprocess.call(
                'go mod init github.com/catapult-project'.split(' '))
    ...
    * archive_path: A string of the path to the archive file.
    ...
    def _CleanUpTempLogFilePath(self, log_level):
        ...
        if logging.getLogger('').isEnabledFor(
                log_level) or USE_LOCAL_WPR in self._replay_options:
