...
def NamedTemporaryFile(mode='w+b', suffix='', prefix='tmp'):
  ...
  - The file is _not_ deleted you close the temporary file handle, so you can
    close it and then use the file name to re-open it again later.
...
def TemporaryFileName(prefix='tmp', suffix=''):
  """A context manager to just get a path to a file that does not exist.
