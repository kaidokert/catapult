...
class PSUtilCompatibilityTest(unittest.TestCase):
  ...
  @unittest.skipUnless(
      hasattr(psutil.Process, 'cpu_affinity'), 'cpu_affinity not available')
  def testCpuAffinity(self):
    """Test that, if supported, we can set and reset cpu affinity.

    Note: cpu_affinity is available in Linux, Windows, and FreeBSD only.
    """
    p = psutil.Process(self.current_pid)
