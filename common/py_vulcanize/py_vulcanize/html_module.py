  ...
  def Load(self, excluded_scripts):
    ...
    for script in self.scripts:
      if script.is_external:
        if excluded_scripts and any(re.match(pattern, script.src) for pattern in excluded_scripts):
          continue
...
def Parse(loader, module_name, module_dir_name, is_component, parser_results,
  ...
  for href in parser_results.scripts_external:
    if exclude_scripts and any(re.match(pattern, href) for pattern in exclude_scripts):
      continue
