...
from perfetto_proto_classes import (
    TracePacket, InternedData, EventCategory, LegacyEventName,
    ThreadDescriptor, TrackEvent, LegacyEvent, write_trace_packet
)
...
def _intern_category(category, trace_packet, tid):
  ...
  if category not in categories:
    ...
    if trace_packet.interned_data is None:
      trace_packet.interned_data = InternedData()
    trace_packet.interned_data.event_category = EventCategory()
...
def _intern_event_name(event_name, trace_packet, tid):
  ...
  if event_name not in event_names:
    ...
    if trace_packet.interned_data is None:
      trace_packet.interned_data = InternedData()
    trace_packet.interned_data.legacy_event_name = LegacyEventName()
...
def write_thread_descriptor_event(output, pid, tid, ts):
  ...
  thread_descriptor_packet = TracePacket()
  thread_descriptor_packet.trusted_packet_sequence_id = _get_sequence_id(tid)
  thread_descriptor_packet.thread_descriptor = ThreadDescriptor()
  ...
  write_trace_packet(output, thread_descriptor_packet)
...
def write_event(output, ph, category, name, ts, args, tid):
  ...
  write_trace_packet(output, packet)
