...
def _build_metadata_for_json():
  telemetry_metadata_for_json = {
      "benchmarkStart": _benchmark_metadata["benchmark_start_time_us"] / 1000.0,
      "traceStart": _benchmark_metadata["story_run_time_us"] / 1000.0,
      "benchmarks": [_benchmark_metadata["benchmark_name"]],
      "benchmarkDescriptions": [_benchmark_metadata["benchmark_description"]],
      "stories": [_benchmark_metadata["story_name"]],
      "storyTags": _benchmark_metadata["story_tags"],
      "storysetRepeats": [_benchmark_metadata["story_run_index"]],
  }
  if "label" in _benchmark_metadata:
    telemetry_metadata_for_json["labels"] = [_benchmark_metadata["label"]]
  if "had_failures" in _benchmark_metadata:
    telemetry_metadata_for_json["hadFailures"] = [
        _benchmark_metadata["had_failures"]]
...
def _write_footer():
  if _format in [JSON, PROTOBUF]:
    ...
    _log_file.write('],\n"metadata": ')
    metadata = _build_metadata_for_json() if _benchmark_metadata else {}
...
def trace_add_benchmark_metadata(
  ...
  if _format == PROTOBUF:
    ...
    _benchmark_metadata["benchmark_start_time_us"] = benchmark_start_time_us
    _benchmark_metadata["story_run_time_us"] = story_run_time_us
    _benchmark_metadata["benchmark_name"] = benchmark_name
    _benchmark_metadata["benchmark_description"] = benchmark_description
    _benchmark_metadata["story_name"] = story_name
    _benchmark_metadata["story_tags"] = story_tags
    _benchmark_metadata["story_run_index"] = story_run_index
    if label:
      _benchmark_metadata["label"] = label
    if had_failures:
      _benchmark_metadata["had_failures"] = had_failures
  elif _format == JSON:
