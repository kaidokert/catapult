...
def _build_metadata_for_json():
  telemetry_metadata_for_json = {
      "benchmarkStart": _telemetry_metadata["benchmark_start_time_us"] / 1000.0,
      "traceStart": _telemetry_metadata["story_run_time_us"] / 1000.0,
      "benchmarks": [_telemetry_metadata["benchmark_name"]],
      "benchmarkDescriptions": [_telemetry_metadata["benchmark_description"]],
      "stories": [_telemetry_metadata["story_name"]],
      "storyTags": _telemetry_metadata["story_tags"],
      "storysetRepeats": [_telemetry_metadata["story_run_index"]],
  }
  if "label" in _telemetry_metadata:
    telemetry_metadata_for_json["labels"] = [_telemetry_metadata["label"]]
  if "had_failures" in _telemetry_metadata:
    telemetry_metadata_for_json["hadFailures"] = [
        _telemetry_metadata["had_failures"]]
...
def _write_footer():
  if _format == PROTOBUF:
    ...
    _log_file.write('],\n"metadata": ')
    metadata = _build_metadata_for_json() if _telemetry_metadata else {}
    json.dump(metadata, _log_file)
    _log_file.write('}')
  else:
    raise TraceException("Unknown format: %s" % _format)
...
def trace_add_telemetry_metadata(
  ...
  global _telemetry_metadata
  if _format in [PROTOBUF, JSON_WITH_METADATA]:
    _telemetry_metadata["benchmark_start_time_us"] = benchmark_start_time_us
    _telemetry_metadata["story_run_time_us"] = story_run_time_us
    _telemetry_metadata["benchmark_name"] = benchmark_name
    _telemetry_metadata["benchmark_description"] = benchmark_description
    _telemetry_metadata["story_name"] = story_name
    _telemetry_metadata["story_tags"] = story_tags
    _telemetry_metadata["story_run_index"] = story_run_index
    if label:
      _telemetry_metadata["label"] = label
    if had_failures:
      _telemetry_metadata["had_failures"] = had_failures
  elif _format == JSON:
