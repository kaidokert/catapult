...
from application import buganizer_migration_utils as bm_utils
...
class BuganizerClient:
  ...
  def GetIssuesList(self, limit, age, status, labels, project='chromium'):
    ...
    components = bm_utils.FindBuganizerComponents(monorail_project_name=project)
    if not components:
      logging.warning(
    ...
    if labels:
      ...
      hotlists = bm_utils.FindBuganizerHotlists(label_list)
      if hotlists:
