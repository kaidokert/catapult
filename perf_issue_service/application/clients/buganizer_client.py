...
class BuganizerClient:
  ...
  def NewComment(self,
    ...
    # TODO: handle missing issue state
    ...
    hotlists = []
    if labels:
      hotlists = b_utils.FindBuganizerHotlists(labels)
    if cc or hotlists:
      ...
      if hotlists:
        ...
        to_add_hotlists = list(set(hotlists) - set(current_hotlists))
        if to_add_hotlists:
          add_list.append('hotlistIds')
          add_issue_state['hotlistIds'] = to_add_hotlists
        to_remove_hotlists = list(set(current_hotlists) - set(hotlists))
        if to_remove_hotlists:
          remove_list.append('hotlists')
          remove_issue_state['hotlists'] = to_remove_hotlists

    modify_request = {
       'issueComment': {'comment': comment}
    }
    if add_list:
      modify_request['addMask'] = ','.join(add_list)
      modify_request['add'] = add_issue_state
    if remove_list:
      modify_request['removeMask'] = ','.join(remove_list)
