...
class AlertGroup:
  ...
  def GetGroupsForAnomaly(
    ...
    ungrouped_name=DEFAULT_UNGROUPED_GROUP_NAME,
    ...
    for config in matched_configs:
      ...
      if not has_overlapped:
        if create_on_ungrouped:
          ...
          ungrouped = cls._GetUngroupedGroup(ungrouped_name)
          if ungrouped:
  ...
  def ProcessUngroupedAlerts(cls, group_name):
    ...
    logging.info('Loaded %s ungrouped alerts from %s. ID(%s)',
                  len(ungrouped_anomalies), group_name, ungrouped.key.id)

    parity_results = {}
    for anomaly in ungrouped_anomalies:
      group_type = datastore_client.AlertGroupType.test_suite
      anomaly_source = anomaly.get('source')
      if anomaly_source and anomaly_source == 'skia':
        group_type = datastore_client.AlertGroupType.test_suite_skia
