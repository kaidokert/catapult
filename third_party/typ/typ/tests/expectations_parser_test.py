...
"""
    ...
    def testConflictsUsingUserDefinedTagsOverlapFunction(self):
        ...
        tags_are_equivalent = lambda t1, t2: (
            t1 == t2 or t1 == map_child_tag_to_parent_tag.get(t2,t2) or
            t2 == map_child_tag_to_parent_tag.get(t1,t1))
        expectations = expectations_parser.TestExpectations()
        _, errors = expectations.parse_tagged_list(
            test_expectations, tags_are_equivalent=tags_are_equivalent)
        self.assertIn("Found conflicts for test a/b/c/d:", errors)
