...
class ArtifactResultsArtifactCreationTests(unittest.TestCase):
  def test_create_artifact_writes_to_disk(self):
    ...
    try:
      ...
      path = os.path.join(tempdir, ar.ARTIFACT_DIRNAME, 'test_name', '0',
  ...
  def test_create_artifact_duplicates_allowed(self):
    ...
    try:
      ...
      path1 = os.path.join(tempdir, ar.ARTIFACT_DIRNAME, 'test_name', '0',
                          'artifact_type', 'artifact_name.ext')
      path2 = os.path.join(tempdir, ar.ARTIFACT_DIRNAME, 'test_name', '0',
  ...
  def test_artifact_paths_are_relative(self):
    ...
    try:
      ...
      relpath = os.path.join(ar.ARTIFACT_DIRNAME, 'test_name', '0',
                             'artifact_type', 'artifact_name')
      self.assertEqual(artifacts['artifact_type'][0], relpath)
    finally:
      shutil.rmtree(tempdir)

  def test_iteration_changes_filepath(self):
    tempdir = tempfile.mkdtemp()
    try:
      ar = artifact_results.ArtifactResults(tempdir)
      ar.EnsureArtifactTypeDefined('artifact_type', 'mime_type')
      ar.IncrementIteration()
      with ar.CreateArtifact(
          'test_name', 'artifact_type', 'artifact_name') as f:
        f.write('contents')
      artifacts = ar.GetArtifactsForTest('test_name')
      invalid_path = os.path.join(tempdir, ar.ARTIFACT_DIRNAME, 'test_name',
                                  '0', 'artifact_type', 'artifact_name')
      valid_path = os.path.join(tempdir, ar.ARTIFACT_DIRNAME, 'test_name', '1',
                                'artifact_type', 'artifact_name')
  ...
  def test_previous_iteration_results_not_carried(self):
    tempdir = tempfile.mkdtemp()
    try:
      ar = artifact_results.ArtifactResults(tempdir)
      ar.EnsureArtifactTypeDefined('artifact_type', 'mime_type')
      with ar.CreateArtifact(
          'test_name', 'artifact_type', 'artifact_name') as f:
        pass
      first_artifacts = ar.GetArtifactsForTest('test_name')

      ar.IncrementIteration()
      ar.EnsureArtifactTypeDefined('another_type', 'mime_type')
      with ar.CreateArtifact('test_name', 'another_type', 'artifact_name') as f:
        pass
      second_artifacts = ar.GetArtifactsForTest('test_name')

      self.assertNotEqual(first_artifacts, second_artifacts)
      self.assertIn('artifact_type', first_artifacts)
      self.assertNotIn('another_type', first_artifacts)
      self.assertIn('another_type', second_artifacts)
      self.assertNotIn('artifact_type', second_artifacts)
    finally:
      shutil.rmtree(tempdir)
