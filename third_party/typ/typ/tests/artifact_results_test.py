...
class ArtifactResultsDataRetrievalTests(unittest.TestCase):
  def test_iter_artifacts(self):
    """Tests that IterArtifacts returns the expected data."""
    tempdir = tempfile.mkdtemp()
    try:
      ar = artifact_results.ArtifactResults(tempdir, 'test_name', 0)
      with ar.CreateArtifact(
      ...
      for a_type, file in ar.IterArtifacts():
        self.assertEqual(a_type, 'artifact_type')
        self.assertEqual(file,
            os.path.join('test_name', '0', 'artifact_type', 'artifact_name'))
    finally:
      shutil.rmtree(tempdir)

  def test_iter_artifact_types(self):
    """Tests that IterArtifactTypes returns the expected data."""
    tempdir = tempfile.mkdtemp()
    try:
      ar = artifact_results.ArtifactResults(tempdir, 'test_name', 0)
      with ar.CreateArtifact(
      ...
      for a_type, m_type in ar.IterArtifactTypes():
        self.assertEqual(a_type, 'artifact_type')
        self.assertEqual(m_type, 'mime_type')
    finally:
      shutil.rmtree(tempdir)
