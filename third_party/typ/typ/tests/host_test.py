...
class HostRecievesProcessOutput(unittest.TestCase):
    ...
    def test1(self):
        ...
        self.assertEqual(
            subprocess.call(
                ...
                'typ.tests.host_test.HostRecievesProcessOutput.test2',
                '-vv'], stdout=sys.stdout, stderr=sys.stderr),0)
        out, _ = host.restore_output()
        self.assertIn('  Started test2', out)
        self.assertIn('    Started test3', out)
    ...
    def test2(self):
        ...
        self.assertEqual(
            subprocess.call(
                [sys.executable, self.script_path,
                'typ.tests.host_test.HostRecievesProcessOutput.test3',
                '-vv'], stdout=sys.stdout, stderr=sys.stderr), 0)
        out, _ = host.restore_output()
        self.assertIn('  Started test3', out)
