...
class ArtifactsLinkCreationTests(unittest.TestCase):
  def test_create_link(self):
    ...
    self.assertEqual(ar.files, {'link': 'http://testsite.com'})
  ...
  def test_create_link_invalid_url(self):
    ar = artifacts.Artifacts(None, 'test_name', 'test_basename', 0)
    with self.assertRaises(ValueError):
      ar.CreateLink('link', 'http:/malformedurl.com')

  def test_create_link_gs(self):
    ar = artifacts.Artifacts(None, 'test_name', 'test_basename', 0)
    ar.CreateLink('link', 'gs://some-gs-link')
    self.assertEqual(ar.files, {'link': 'gs://some-gs-link'})

  def test_create_link_newlines(self):
    ar = artifacts.Artifacts(None, 'test_name', 'test_basename', 0)
    with self.assertRaises(ValueError):
      ar.CreateLink('link', 'http://some\nbadurl.com')
