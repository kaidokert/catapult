...
class ArtifactsArtifactCreationTests(unittest.TestCase):
  ...
  def test_create_artifact_writes_to_disk_iteration_1_no_test_dir(self):
    ...
    self.assertEqual(
        host.read_binary_file(
            host.join(output_dir, 'retry_1', 'stdout', 'text.txt')),
        b'contents')
  ...
  def test_create_artifact_writes_to_disk_iteration_1_test_dir(self):
    ...
    self.assertEqual(
        host.read_binary_file(
            host.join(output_dir, 'a.b.c', 'retry_1', 'stdout', 'text.txt')),
        b'contents')
  ...
  def test_force_overwriting_artifact_does_not_raise_error(self):
    ...
    self.assertEqual(
        host.read_binary_file(
            host.join(output_dir, 'a.b.c', 'initial', 'stdout', 'text.txt')),
        b'contents')
    ar.CreateArtifact('artifact_name', file_rel_path, b'overwritten contents',
  ...
  def test_create_artifact_writes_to_disk_initial_results_dir(self):
    ...
    ar.CreateArtifact('artifact_name', file_rel_path, b'contents')
    self.assertEqual(
        host.read_binary_file(host.join(output_dir, 'a.b.c', 'initial', 'stdout', 'text.txt')),
        b'contents')
  ...
  def test_file_manager_writes_file(self):
    ...
    ar.CreateArtifact('artifact_name', file_path, 'hello world')
    self.assertEqual(
        host.read_binary_file(file_path), 'hello world')
