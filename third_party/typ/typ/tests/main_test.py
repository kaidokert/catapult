...
class TestCli(test_case.MainTestCase):
    ...
    def test_implement_test_name_prefix_exclusion_in_isolate_arg(self):
        files = {'output_test.py': OUTPUT_TEST_PY}
        _, out, _, files = self.check(
            ['--test-name-prefix', 'output_test.FailTest.',
              '--isolate', 'test_out_err_fail', '-vvv', '-j', '3'],
            files=files, ret=1, err='')
        # isolated tests will always run last and the test queued output
        # will always come before the test finished output
        self.assertIn('[2/3] test_out_err_fail queued\n'
                      '[3/3] test_out_err_fail failed unexpectedly:', out)
