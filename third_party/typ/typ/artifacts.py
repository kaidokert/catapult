...
class Artifacts(object):
  def __init__(self, output_dir, test_name, test_basename, iteration):
    ...
    Note that this does not strictly adhere to the spec outlined in the
    documentation - the "artifact_types" field is omitted since it was deemed
    redundant.
  ...
  def CreateArtifact(self, artifact_name):
    ...
    Where possible, use one of the helper methods below instead. If a suitable
    one does not exist, consider adding it or filing a bug on https://crbug.com
    to have it added.

    Args:
      artifact_name: A string specifying the name for the artifact, such as
          "failure_screenshot.png" or "benchmark_log.tx". This should be unique
  ...
  def CreateLink(self, artifact_name, path):
    ...
    Args:
      artifact_name: A string specifying the name for the artifact, such as
          "triage_url".
      path: A string to be used for the artifact's path.
    """
    ...
    if len(path.splitlines()) > 1:
      raise ValueError('Given path %s contains newlines' % path)
