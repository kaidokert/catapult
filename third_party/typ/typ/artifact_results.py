...
class ArtifactResultsBase(object):
  ...
  def IncrementIteration(self):
    raise NotImplementedError(
        'IncrementIteration not implemented in subclass')
  ...
  def HasArtifactData(self):
    raise NotImplementedError(
        'HasArtifactData not implemented in subclass')
...
class ArtifactResults(ArtifactResultsBase):
  ...
  def CreateArtifact(self, test_name, artifact_type, artifact_name,
    ...
    if self._iteration not in self._artifact_map[test_name]:
      self._artifact_map[test_name][self._iteration] = {}
    if artifact_type not in self._artifact_map[test_name]:
      self._artifact_map[test_name][self._iteration][artifact_type] = []
    self._artifact_map[test_name][self._iteration][artifact_type].append(
  ...
  def GetArtifactsForTest(self, test_name):
    return self._artifact_map.get(test_name, {}).get(self._iteration, {})
  ...
  def _DuplicateArtifactExists(self, test_name, artifact_type, artifact_name):
    artifact_path = self._GenerateRelativeArtifactPath(
        test_name, artifact_type, artifact_name)
    artifacts = self._artifact_map.get(
        test_name, {}).get(self._iteration, {}).get(artifact_type, [])
    return artifact_path in artifacts

  def _GenerateFullArtifactPath(self, test_name, artifact_type,
                                artifact_name):
    return os.path.join(
        self._artifact_dir, test_name, str(self._iteration), artifact_type, artifact_name)
