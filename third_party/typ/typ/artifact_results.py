...
class ArtifactResults(object):
  ...
  def CreateArtifact(self, artifact_type, mime_type, artifact_name):
    ...
    Args:
      artifact_type: A string specifying the artifact type to add, such as
          "screenshot" or "log".
      mime_type: A string specifying the MIME type to correspond to the given
          |artifact_type|, such as "image/png" or "text/plain".
      artifact_name: A string specifying the name for the artifact, such as
          "failure_screenshot.png" or "benchmark_log.txt".
    ...
    existing_paths = self._artifact_map.setdefault(artifact_type, [])
  ...
  def HasArtifacts(self):
    return len(self._artifact_map) > 0
  ...
  def IterArtifactTypes(self):
    return self._artifact_type_map.iteritems()

  def CreateScreenshot(self, artifact_name):
    """Helper method for creating a screenshot artifact.

    Data written to this should be a PNG image.
  ...
  def CreateLog(self, artifact_name):
    """Helper method for creating a log artifact.

    Data written to this should be an arbitrary amount of plaintext.
  ...
  def CreateLink(self, artifact_name):
    """Helper method for creating a link artifact.

    Data written to this should be a single URL.
  ...
  def CreateTrace(self, artifact_name):
    """Helper method for creating a trace artifact.

    Data written to this should be JSON-formatted trace data.

    Args:
      artifact_name: A string specifying the name of the artifact, such as
          "perf_trace.json".
    """
  ...
  def _GenerateRelativeArtifactPath(self, artifact_type, artifact_name):
    return os.path.join(
        self._test_name, str(self._iteration), artifact_type, artifact_name)
