...
class ResultSinkReporter(object):
    ...
    def report_individual_test_result(
        ...
        artifacts[STDOUT_KEY] = {
            'contents': (base64.b64encode(result.out.encode('utf-8')).decode(
                             'utf-8') if result.out else '')
        }
        artifacts[STDERR_KEY] = {
            'contents': (base64.b64encode(result.err.encode('utf-8')).decode(
                             'utf-8') if result.err else '')
        }
