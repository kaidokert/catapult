...
class Runner(object):
    ...
    def _summarize(self, full_results):
        ...
        self.print_()
        if num_failures:
            self.update('All failed tests:\n', elide=False)
            failed_tests = sorted(
                list(json_results.failed_tests_names(full_results)))
            for t in failed_tests:
                self.print_('  %s' % t)
        if num_regressions:
            self.update('All regressed tests (unexpectedly failed):\n',
                        elide=False)
            regressed_tests = sorted(
                list(json_results.regressed_tests_names(full_results)))
