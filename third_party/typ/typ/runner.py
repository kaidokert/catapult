...
from typ.pool import PoolGroup
...
class Runner(object):
    ...
    def _run_tests(self, result_set, test_set, all_tests):
        ...
        if len(test_set.parallel_tests):
            ...
            jobs = 1
        use_global_pool = self.args.use_global_pool
        print('\n\n\nUsing global pool %s\n\n\n' % use_global_pool)

        child = _Child(self)
        pool_group = PoolGroup(h, jobs, _run_one_test, child, _setup_process,
                               _teardown_process, use_global_pool)
        global_pool = pool_group.make_global_pool()
        ...
        try:
            ...
            self.final_responses.extend(global_pool.join())
