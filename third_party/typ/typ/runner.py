...
class Runner(object):
    ...
    def _run_tests(self, result_set, test_set, all_tests):
        ...
        while retry_limit and tests_to_retry:
            ...
            for test_name in tests_to_retry:
                test_set.isolated_tests.append(TestInput(test_name))
    ...
    def default_classifier(self, test_set, test):
        if (self.args.all
            or not self.args.test_filter
            or self.does_test_match_filter(test)):
            if not self.args.all and self.does_test_match_any_skip_globs(test):
                test_set.add_test_to_skip(test, 'skipped by request')
            elif self.does_test_match_isolate_globs(test):
                test_set.add_test_to_run_isolated(test)
            else:
                test_set.add_test_to_run_in_parallel(test)

    def does_test_match_filter(self, test_case):
        _validate_test_starts_with_prefix(
    ...
    def does_test_match_positional_args(self, test_case):
        _validate_test_starts_with_prefix(
            self.args.test_name_prefix, test_case.id())
        return any(test in test_case.id()[len(self.args.test_name_prefix):]
                   for test in self.args.tests)

    def does_test_match_isolate_globs(self, test_case):
        _validate_test_starts_with_prefix(
            self.args.test_name_prefix, test_case.id())
        test_name = test_case.id()[len(self.args.test_name_prefix):]
        return any(fnmatch.fnmatch(test_name, glob)
    ...
    def does_test_match_any_skip_globs(self, test_case):
        _validate_test_starts_with_prefix(
            self.args.test_name_prefix, test_case.id())
        test_name = test_case.id()[len(self.args.test_name_prefix):]
        return any(fnmatch.fnmatch(test_name, glob) for glob in self.args.skip)
