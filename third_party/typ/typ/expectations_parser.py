...
class TestExpectations(object):
    ...
    def validate_condition_tags(self, tags, raise_ex_for_bad_tags=True,
                                check_file_tags=False, check_tags_in_file=False):
        # This function will have two modes. If the check_file_tags is set
        # to False, then the function will be used to validate if each tag in
        # the tags list is declared in a test expectations file. This validation
        # will make sure that the tags written in the test expectations files
        # match tags that are generated by the test runner. If check_file_tags
        # is set to True, then the function will validate the tags declared in
        # the expectations file. It will check if each tag in the file is in the
        # tags list which will be a reference set of tags. If the check_tags_in_file
        # argument is set to true then it will check if all tags in the tags
        # list are in the expectations file.
        ...
        if not check_file_tags:
            ...
            if unknown_tags:
                msg = (
                    'Tag%s in expectations file are unknown. Please make sure the '
                    'aforementioned tag%s are added to the reference set.'
                    % _pluralize_unknown(unknown_tags))
                if raise_ex_for_bad_tags:
                    raise ValueError(msg)
                else:
                    logging.warning(msg)
            if check_tags_in_file:
                missing_tags = sorted([t for t in tags if t not in tags_set])
                if missing_tags:
                    msg = (
                        'Tag%s are missing from the expectations file. Please make '
                        'sure the aforementioned tag%s are added to the expectations '
                        'file.' % _pluralize_unknown(missing_tags))
                    if raises_ex_for_bad_tags:
                        raise ValueError(msg)
                    else:
                        logging.warning(msg)
