...
class TaggedTestListParser(object):
    ...
    def _parse_expectation_line(self, lineno, line):
        ...
        tag_set_ids = set()
        tags_to_lower_case = [tag.lower() for tag in tags]
        for t in tags_to_lower_case:
        ...
        if len(tag_set_ids) != len(tags):
            ...
            for t in tags:
                tags_by_tag_set_id[self._tag_to_tag_set[t.lower()]].append(t)
        ...
        return Expectation(
            reason, test, tags_to_lower_case, results, lineno, retry_on_failure)
