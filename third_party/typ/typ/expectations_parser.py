...
class TestExpectations(object):
    ...
    def _validate_condition_tags(self, tags, validate_tags):
        ...
        tags_set = set(reduce(lambda x,y: list(x)+list(y), self.tag_sets or [[]]))
        unknown_tags = sorted([
            t for t in tags if tags_set and t not in tags_set])
        if unknown_tags:
            msg = (
                'Tag%s not declared in any expectations file. '
                'There may have been a typo in an expectations file. '
                'Please make sure the aforementioned tag%s declared in '
                'an expectations file.' %
                (_pluralize_unknown(unknown_tags),
                 's are' if len(unknown_tags) > 1 else ' is'))
            if validate_tags:
                raise Exception(msg)
            else:
                logging.warning(msg)
