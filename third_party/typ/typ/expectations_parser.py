...
class TestExpectations(object):
    ...
    def validate_condition_tags(self, tags, raise_ex_for_bad_tags=True,
                                check_file_tags_against_reference=False,
                                check_reference_tags_in_file=False):
        # This function will have two modes. If the check_file_tags_against_reference
        # arg is set to False, then the function will be used to validate if each
        # tag in the tags list is declared in a test expectations file. This
        # validation will make sure that the tags written in the test expectations
        # files match tags that are generated by the test runner. If
        # check_file_tags_against_reference is set to True, then the function will
        # validate the tags declared in the expectations file. It will check if
        # each tag in the file is in the tags list which will be a reference set
        # of tags. If the check_tags_in_file argument is set to true then it will
        # check if all tags in the tags list are in the expectations file.

        def _report_error(message_template, tags):
            def _pluralize_tags(tags_to_report):
                if len(tags_to_report) > 1:
                    return ('s %s ' % ', '.join(tags_to_report[:-1]) +
                            'and %s ' % tags_to_report[-1] + 'are',
                            's are')
                else:
                    return (' %s ' % tags_to_report[0] + 'is', ' is')
            msg = message_template % _pluralize_tags(tags)
        ...
        if self.tag_sets:
            tags_set = reduce(lambda x, y: x | y, self.tag_sets)
        else:
            tags_set = set()
        if check_file_tags_against_reference:
            unknown_tags = sorted([t for t in tags_set if t not in tags])
            if unknown_tags:
                _report_error(
                    'Tag%s in the expectations file but not in the reference set. '
                    'Please make sure the aforementioned tag%s added to the '
                    'reference set.', unknown_tags)
            if check_reference_tags_in_file:
                missing_tags = sorted([t for t in tags if t not in tags_set])
                if missing_tags:
                    _report_error(
                        'Tag%s in the reference set but not in the expectations file. '
                        'Please make sure the aforementioned tag%s are added to '
                        'the expectations file.', missing_tags)
        else:
            unknown_tags = sorted([t for t in tags if tags_set and t not in tags_set])
            if unknown_tags:
                _report_error(
                    ...
                    'the top of the expectations file.', unknown_tags)
