...
class TaggedTestListParser(object):
    ...
    def _parse_expectation_line(self, lineno, line):
        ...
        for i in range(len(test)-1):
            if test[i] == '*' and i > 0 and test[i-1] != '\\':
                raise ParseError(lineno,
...
class TestExpectations(object):
    ...
    def parse_tagged_list(self, raw_data, file_name='',
        ...
        for exp in parser.expectations:
            ...
            exp.test = filter(lambda c: c!='\\', exp.test)
            if is_glob:
    ...
    def check_for_broken_expectations(self, test_names):
        ...
        patterns_to_exps = self.glob_exps.copy()
        ...
        for pattern, exps in patterns_to_exps.items():
            ...
            for l in pattern:
                if l == '*':
                    ...
                    break
                if l not in _trie:
                    glob_broken_exps.extend(exps)
                    broken_exp = True
                    break
                _trie = _trie[l]
            if not broken_exp and not is_glob and '\0' not in _trie:
                glob_broken_exps.extend(exps)
        return broken_exps + glob_broken_exps
