...
class TaggedTestListParser(object):
    ...
    def _parse_raw_expectation_data(self, raw_data):
        ...
        while lineno <= num_lines:
            ...
            if line.startswith(self.TAG_TOKEN):
                ...
                self.tag_to_tag_set.update({tg:id(tag_set) for tg in tag_set})
    ...
    def _parse_expectation_line(self, lineno, line, tag_sets):
        ...
        tag_set_ids = {self.tag_to_tag_set[tag]
                       for tag in tags if tag in self.tag_to_tag_set}
        for tag in tags:
        ...
        if len(tag_set_ids) != len(tags):
            ...
            id_to_tags = {}
            for tag in tags:
                id_to_tags.setdefault(self.tag_to_tag_set[tag], []).append(tag)
            error_msg += '\n  - '.join(
