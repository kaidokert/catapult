...
class _ProcessPool(object):
    ...
    def send(self, msg, type=_MessageType.Request):
        req = (_MessageType.Request, msg)
        if self.stable_jobs:
            self.requests[self.next_request_index].put(req)
            self.next_request_index = (self.next_request_index + 1) % self.jobs
        else:
    ...
    def close(self):
        for _ in self.workers:
            self.send(None, type=_MessageType.Close)
        self.closed = True
