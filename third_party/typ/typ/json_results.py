...
def _results_for_test(test_name, results, artifact_type_map):
    ...
    for r in results.results:
        if r.name == test_name:
            ...
            # Handle artifacts
            if not r.artifact_results or not r.artifact_results.HasArtifacts():
                continue
            if 'artifacts' not in value:
                value['artifacts'] = {}
            for artifact_type, file in (
                    r.artifact_results.IterArtifacts()):
                value['artifacts'].setdefault(artifact_type, []).append(file)
            for artifact_type, mime_type in (
                    r.artifact_results.IterArtifactTypes()):
