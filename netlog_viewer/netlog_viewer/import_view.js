...
var ImportView = (function() {
  ...
  ImportView.prototype = {
    ...
    displayTrials_(trialsAtStart, trialsAtEnd) {
      ...
      // end of the capture, if available.
      $(ImportView.LOADED_INFO_ACTIVE_FIELD_TRIAL_GROUPS_ID).textContent =
          (trialsAtEnd) ? trialsAtEnd.join(' ')
                        : (trialsAtStart) ? trialsAtStart.join(' ') : '';
      ...
      // Calculate the Field Trial list's additions and removals.
      let trialsAtStartSet = new Set(trialsAtStart);
      let trialsAtEndSet = new Set(trialsAtEnd);
      let listAdded = trialsAtEnd.filter(item => !trialsAtStartSet.has(item));
      let listRemoved = trialsAtStart.filter(item => !trialsAtEndSet.has(item));
      ...
      $(ImportView.LOADED_INFO_REMOVED_FIELD_TRIAL_GROUPS_ID).textContent =
          listRemoved.join(' ');
      setNodeDisplay(row_removed, listRemoved.length > 0);
    },
