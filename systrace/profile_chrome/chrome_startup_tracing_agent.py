...
class ChromeStartupTracingAgent(tracing_agents.TracingAgent):
  ...
  def _SetupTracing(self):
    ...
    if self._webapk_package:
      ...
      logging.warning('Forces to stop the browser provided by --browser: %s. '
                      'Please make sure this browser matches the host browser '
                      'of the WebAPK %s.', self._package_info.package,
