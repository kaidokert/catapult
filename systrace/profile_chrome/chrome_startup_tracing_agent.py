...
class ChromeStartupTracingAgent(tracing_agents.TracingAgent):
  def __init__(self, device, package_info, external_app_package, cold, url,
               trace_time=None):
    tracing_agents.TracingAgent.__init__(self)
    self._device = device
    self._package_info = package_info
    self._external_app_package = external_app_package
    self._cold = cold
  ...
  def _SetupTracing(self):
    ...
    if self._external_app_package:
      self._device.ForceStop(self._external_app_package)
    if self._cold:
    ...
    elif not self._external_app_package:
      launch_intent = intent.Intent(
          package=self._package_info.package,
          activity=self._package_info.activity,
          data=self._url,
    ...
    # crbug.com/782425: start tracing for WebApks.
      ...
      launch_intent = intent.Intent(
          package=self._external_app_package,
...
class ChromeStartupConfig(tracing_agents.TracingConfig):
  def __init__(self, device, package_info, external_app_package, cold, url,
    ...
    self.external_app_package = external_app_package
    self.cold = cold
    self.url = url
...
def add_options(parser):
  ...
  options.add_option('--external-app-package', help='Specify the package name '
                     'of the app to launch the given URL. ')

  return options

def get_config(options):
  return ChromeStartupConfig(options.device, options.package_info,
                             options.external_app_package, options.cold,
