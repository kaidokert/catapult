...
def GenerateHTMLOutput(trace_results, output_file_name):
  ...
  # Open the file in binary mode to prevent python from changing the
  # line endings, then write the prefix.
  html_file = io.open(output_file_name, 'wb')
...
#   html_file = open(output_file_name, 'w')
  ...
  html_file.write(six.ensure_binary('<!-- BEGIN TRACE -->\n'))
  for result in trace_results:
    html_file.write(six.ensure_binary('  <script class="trace-data" type="application/text">\n'))
    html_file.write(six.ensure_binary(_ConvertToHtmlString(result.raw_data)))
    html_file.write(six.ensure_binary('  </script>\n'))
  html_file.write(six.ensure_binary('<!-- END TRACE -->\n'))
