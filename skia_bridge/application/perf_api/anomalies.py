...
def GetAnomalyData(anomaly_obj):
  ...
  if subscription_names:
    bug_components = subscription_names[0].get('bug_components', [''])
    bug_labels = subscription_names[0].get('bug_labels', [])
    bug_cc_emails = subscription_names[0].get('bug_cc_emails', [])
