...
INTERNAL_CLIENTS = ['perf-chrome-internal@skia-infra-corp.iam.gserviceaccount.com']
...
def AlertGroupDetailsPostHandler():
  try:
    is_authorized, client_email = auth_helper.AuthorizeBearerToken(request, ALLOWED_CLIENTS)
    ...
    if alert_group:
      ...
      if len(anomalies) > 0:
        ...
        logging.info('No anomalies exist for alert group or user does not have permission to view data.')
        return {}
    else:
