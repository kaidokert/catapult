...
def CreateRowGroups(rows):
  ...
  for row in rows:
    ...
    row_key = '%s->%s->%s->%s->%s' % (test_data['master_name'],
                                      test_data['bot_name'],
...
def UploadRowGroup(gcs_client, rows):
  ...
  gcs_client.UploadDataToBucket(filename, json.dumps(skia_data), bucket_name,
                                'application/json')
...
def _GetLinks(rows):
  ...
  row = rows[0]
  bot_list = sorted([r['a_bot_id'] for r in rows])
  bot_list_str = ', '.join(bot_list)
  links['Bot Id'] = bot_list_str
