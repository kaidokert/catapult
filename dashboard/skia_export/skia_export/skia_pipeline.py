...
def main(properties):
  ...
  non_repo_rows = Metrics.counter('main', 'non_repo_rows')
  ...
  def HasMasters(row):
    ...
    has_right_master = True
    if properties['masters']:
      has_right_master = test_parts[0] in properties['masters']
  ...
  master_filter = [('master_name', 'IN', properties['masters'])]
  test_metadata_dicts = (
      p
  ...
  def RowKey(row):
    return (row['master_name'], row['bot_name'], row['suite_name'],
            row['revision'], row['a_bot_id'], row['internal_only'], row['timestamp'])
