...
def main():
  ...
  def RowEntityToDict(entity):
    ...
    try:
      logging.getLogger().info(entity['timestamp'])
      d = {
  ...
  def RowKey(row):
    return (row['master_name'], row['bot_name'], row['suite_name'],
            row['r_commit_pos'], row['internal_only'], row['timestamp'])
  ...
  class WriteSkiaPerfBucket(beam.DoFn):
    ...
    def process(self, element):
      ...
      if key['internal_only']:
        ...
        public_writes.inc()
      test_path = '%s/%s/%s/%s.json' % (
          key['master'], key['bot'], key['benchmark'],
          str(key['commit_position']))
      filename = 'gs://%s/ingest/%s/%s' % (bucket_name, key['timestamp'], test_path)
