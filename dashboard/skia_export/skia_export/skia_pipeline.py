...
from skia_export.utils import FloatHack, PrintCounters
...
def main():
  ...
  def log_stuff(element):
    logging.getLogger().info('Data %s', element)
    return [element]
  ...
  def RowKey(row):
    return (row['master_name'], row['bot_name'], row['suite_name'],
            row['r_commit_pos'], row['internal_only'])
  ...
  class WriteSkiaPerfBucket(beam.DoFn):
    ...
    def process(self, element):
      ...
      if key['internal_only']:
        ...
        public_writes.inc()
      test_path = '%s/%s/%s/%s/%s.json' % (
          key['master'], key['bot'], key['benchmark'],
          str(key['commit_position']), time.time())
      filename = 'gs://%s/ingest/%s/%s' % (bucket_name, datetime.datetime.now()
                                         .strftime('%Y/%m/%d/%H'), test_path)
      logging.getLogger().info('filename', filename)
      with GcsIO().open(filename, mode='w', mime_type='application/json') as file:
