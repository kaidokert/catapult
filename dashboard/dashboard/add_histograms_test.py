...
class AddHistogramsTest(testing_common.TestCase):
  ...
  def testComputeRevision_UsesCommitPositionIfBothPresent(self):
    hs = _CreateHistogram(name='foo', commit_position=123456,
                          revision_timestamp=234567)
    rev = add_histograms.ComputeRevision(hs)
    self.assertEqual(rev, 123456)

  def testComputeRevision_UsesRevisionTimestampIfCommitPositionAbsent(self):
    hs = _CreateHistogram(name='foo', revision_timestamp=234567)
    rev = add_histograms.ComputeRevision(hs)
