...
tr.exportTo('cp', () => {
  ...
  function dummyBots(testSuites) {
    const bots = [];
    if (testSuites.filter(s => s.indexOf('v8') === 0)) {
    ...
    if (testSuites.filter(s => s.indexOf('system_health.common') === 0)) {
      bots.push.apply(bots, [
  ...
  function dummyStories(testSuites) {
    ...
    if (testSuites.filter(s => s.indexOf('system_health') === 0)) {
      stories.push.apply(stories, [
        'blank:about:blank',
