...
tr.exportTo('cp', () => {
  ...
  const SHERIFFS = [
    'Binary Size',
    'Blink Memory Mobile',
    'Chrome Perf Accessibility',
    'Chromium Perf AV',
    'Network Service',
    'Pica',
    'Tracing Perftests',
  ];
  const SHERIFF_OPTIONS = [
  ...
  function dummyAlertsSources() {
    ...
    for (const label of PERF_SHERIFFS) {
      sheriffOptions.push({
        label,
        value: label + ' Perf Sheriff',
      });
    }
    for (const label of SHERIFFS) {
      sheriffOptions.push({
        label,
        value: label + ' Sheriff',
      });
    }
    for (const label of SHERIFF_OPTIONS) {
      sheriffOptions.push({label, value: label});
    }
