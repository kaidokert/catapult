...
tr.exportTo('cp', () => {
  class ReportTemplate extends cp.ElementBase {
    ...
    async onSelectSource_(event) {
      if (this.source.selectedOptions.includes(ReportTemplate.CREATE)) {
        this.$.source.blur();
        for (let i = 0; i < 10; ++i) {
    ...
    canEdit_(table, userEmail) {
      return ReportTemplate.canEdit(table, userEmail);
    }
  ...
  ReportTemplate.placeholderTable = name => {
    return {
      ...PLACEHOLDER_TABLE,
      name,
    };
  ...
  ReportTemplate.DEFAULT_NAME = 'Chromium Performance Overview';
  ReportTemplate.CREATE = '[Create new report]';
  ...
  ReportTemplate.actions = {
    connected: statePath => async(dispatch, getState) => {
      ...
      if (state.minRevision === undefined ||
        ...
        ReportTemplate.actions.selectMilestone(
            statePath, state.milestone)(dispatch, getState);
      }
      await ReportTemplate.actions.loadSources(statePath)(dispatch, getState);
    ...
    authChange: statePath => async(dispatch, getState) => {
      ReportTemplate.actions.loadSources(statePath)(dispatch, getState);
    },

    selectMilestone: (statePath, milestone) => async(dispatch, getState) => {
      dispatch({
        type: ReportTemplate.reducers.selectMilestone.name,
        statePath,
        milestone,
      });
    },
    ...
    restoreState: (statePath, options) => async(dispatch, getState) => {
      dispatch({
        type: ReportTemplate.reducers.restoreState.name,
        statePath,
        options,
      });
    ...
    toggleEditing: (statePath, tableIndex) => async(dispatch, getState) => {
      ...
      if (table.canEdit !== true) {
        // TODO isLoading
        await ReportTemplate.actions.renderEditForm(
    ...
    loadReports: statePath => async(dispatch, getState) => {
      ...
      let testSuites = [];
      if (state.source.selectedOptions.includes(ReportTemplate.CREATE)) {
        testSuites = await cp.TeamFilter.get(rootState.teamName).testSuites(
            await new TestSuitesRequest({}).response);
      }
      dispatch({
        type: ReportTemplate.reducers.requestReports.name,
        statePath,
        testSuites,
      });
      ...
      for await (const {results, errors} of new cp.BatchIterator(readers)) {
        ...
        dispatch({
          type: ReportTemplate.reducers.receiveReports.name,
          statePath,
          reports: results,
          testSuites,
        });
        // ReportTemplate.actions.renderEditForms(statePath)(dispatch,getState);
        // ReportTemplate.actions.prefetchCharts(statePath)(dispatch, getState);
      }
    ...
    renderEditForms: statePath => async(dispatch, getState) => {
      ...
      await Promise.all(state.tables.map(async(table, tableIndex) => {
        await cp.idle();
        await ReportTemplate.actions.renderEditForm(statePath, tableIndex)(
            dispatch, getState);
      }));
    },
    ...
    templateAddRow: (statePath, tableIndex, rowIndex) =>
      async(dispatch, getState) => {
        dispatch({
          ...
          rowIndex,
          testSuites: await new TestSuitesRequest({}).response,
        });
  ...
  ReportTemplate.reducers = {
    ...
    receiveSourceOptions: (state, {reportNames}, rootState) => {
      ...
      if (window.IS_DEBUG || rootState.userEmail) {
        options.push(ReportTemplate.CREATE);
      }
    ...
    requestReports: (state, action, rootState) => {
      ...
      for (const name of selectedNames) {
        ...
        if (!tableNames.has(name)) {
          if (name === ReportTemplate.CREATE) {
            tables.push(ReportTemplate.newTemplate(
                rootState.userEmail, action.testSuites));
          } else {
            tables.push(ReportTemplate.placeholderTable(name));
          }
        }
      }
    ...
    receiveReports: (state, action, rootState) => {
      ...
      for (const report of action.reports) {
        ...
        const rows = report.report.rows.map(
            row => ReportTemplate.transformReportRow(
  ...
  ReportTemplate.newTemplate = (userEmail, testSuites) => {
    return {
      isEditing: true,
      name: '',
      owners: userEmail,
  ...
  ReportTemplate.newTemplateRow = ({testSuite, bot, testCase}) => {
    return {
      label: '',
      testSuite: {
        ...testSuite,
  ...
  ReportTemplate.newStateOptionsFromQueryParams = queryParams => {
    const options = {
  ...
  ReportTemplate.getSessionState = state => {
    return {
  ...
  ReportTemplate.getRouteParams = state => {
    const routeParams = new URLSearchParams();
    const selectedOptions = state.source.selectedOptions;
    if (state.containsDefaultSection &&
        selectedOptions.length === 1 &&
        selectedOptions[0] === ReportTemplate.DEFAULT_NAME) {
      return routeParams;
    }
    for (const option of selectedOptions) {
      if (option === ReportTemplate.CREATE) continue;
  ...
  ReportTemplate.transformReportRow = (
