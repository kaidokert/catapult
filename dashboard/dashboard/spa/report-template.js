...
tr.exportTo('cp', () => {
  class ReportTemplate extends cp.ElementBase {
    ...
    async onMatrixChange_(event) {
      await this.dispatch('descriptor', this.statePath, event.model.rowIndex);
    }
  ...
  ReportTemplate.actions = {
    descriptor: (statePath, rowIndex, matrix) => async(dispatch, getState) => {
      // matrix: {suites, measurements, bots, cases}
    },
    ...
    save: statePath => async(dispatch, getState) => {
      ...
      const table = Polymer.Path.get(rootState, statePath);
