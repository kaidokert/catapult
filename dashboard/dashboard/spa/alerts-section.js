...
tr.exportTo('cp', () => {
  ...
  class AlertsSection extends cp.ElementBase {
    ...
    static newBugSummary(alertGroups) {
      ...
      for (const alert of AlertsSection.getSelectedAlerts(alertGroups)) {
        if (!alert.improvement) {
          pctDeltaRange.addValue(Math.abs(alert.percentDeltaValue));
        }
    ...
    static async fileBug(alertKeys, summary, description, owner, cc) {
      const body = new FormData();
      body.set('finish', 'true');
      body.set('keys', alertKeys.join(','));
      ...
      await fetch('/file_bug', {method: 'POST', body});
    }
    ...
    static async fetchAlerts(options) {
      ...
      try {
        ...
        return cp.dummyAlerts(Boolean(options.improvements));
      }
    }
  ...
  AlertsSection.reducers = {
    fileNewBug: cp.ElementBase.statePathReducer((state, action) => {
      return {
        ...
        newBug: {
          isOpen: true,
          summary: AlertsSection.newBugSummary(state.alertGroups),
          description: '',
          owner: 'TODO',
          cc: '',
        },
