...
tr.exportTo('cp', () => {
  ...
  class AlertsSection extends cp.ElementBase {
    ...
    breakWords_(str) {
      return AlertsSection.breakWords(str);
    }

    static breakWords(str) {
  ...
  AlertsSection.transformBug = bug => {
    ...
    return {
      ...
      description: AlertsSection.breakWords(bug.summary),
      revisionRange,
    };
  };
  ...
  AlertsSection.fetchTimeseries = async (headers, alert, colorForAlert) => {
    ...
    try {
      ...
      timeseries.data = cp.dummyTimeseries(alert.improvement);
    }
    return timeseries;
  };
