...
tr.exportTo('cp', () => {
  ...
  class AlertsSection extends cp.Element {
    ...
    static layoutPreview(sectionId) {
      return async (dispatch, getState) => {
        ...
        const sectionState = getState().sections[sectionId];
  ...
  cp.sectionReducer('alerts-section.selectAlert', (state, action, section) => {
    ...
    if (!alertGroup.isExpanded && (action.alertIndex === 0)) {
      ...
      alerts = alerts.map(alert => cp.assign(alert, {
        isSelected,
      }));
    } else {
    ...
    const alertGroups = cp.assignInArray(section.alertGroups, action.alertGroupIndex, {
      alerts,
    });
    return {
  ...
  cp.sectionReducer('alerts-section.selectAllAlerts', (state, action, section) => {
    const select = !section.anyAlertsSelected;
    return {
      anyAlertsSelected: select,
      alertGroups: section.alertGroups.map(alertGroup => cp.assign(alertGroup, {
        alerts: alertGroup.alerts.map(alert => cp.assign(alert, {
          isSelected: select,
        })),
      })),
    };
  ...
  cp.sectionReducer('alerts-section.receiveAlerts', (state, action, section) => {
    return {
      isLoading: false,
  ...
  cp.sectionReducer('alerts-section.toggleGroupExpanded', (state, action, section) => {
    return {
      alertGroups: cp.assignInArray(section.alertGroups, action.alertGroupIndex, {
        isExpanded: !section.alertGroups[action.alertGroupIndex].isExpanded,
      }),
    };
  });

  cp.sectionReducer('alerts-section.focusSource', (state, action, section) => {
    return {
      source: cp.assign(section.source, {
        isFocused: action.isFocused,
      }),
    };
  });

  cp.sectionReducer('alerts-section.keydownSource', (state, action, section) => {
    return {
      source: cp.assign(section.source, {
  ...
  cp.sectionReducer('alerts-section.togglePreviewing', (state, action, section) => {
    return {
      isPreviewing: !section.isPreviewing,
    };
  });

  cp.sectionReducer('alerts-section.startLoadingAlerts', (state, action, section) => {
    return {
      isLoading: true,
      source: cp.assign(section.source, {
        isFocused: false,
  ...
  cp.sectionReducer('alerts-section.startLoadingPreview', (state, action, section) => {
    return {
      isLoading: true,
    };
  });

  cp.sectionReducer('alerts-section.layoutPreview', (state, action, section) => {
    return {
      isLoading: false,
