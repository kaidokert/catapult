...
tr.exportTo('cp', () => {
  ...
  class AlertsSection extends cp.ElementBase {
    ...
    toggleNonOverlappingRecentBugs_() {
      this.dispatch('toggleNonOverlappingRecentBugs', this.statePath);
    }
  ...
  AlertsSection.actions = {
    toggleNonOverlappingRecentBugs: statePath => async (dispatch, getState) => {
      // Changing the contents of a paper-dialog does not automatically resize
      // it. We must manually close it, modify the state, then re-open it in
      // order for the paper-dialog to resize.
      dispatch(cp.ElementBase.actions.updateObject(
          `${statePath}.existingBug`, {isOpen: false}));
      dispatch({
        type: AlertsSection.reducers.toggleNonOverlappingRecentBugs.typeName,
        statePath,
      });
    },
    ...
    fileExistingBug: statePath => async (dispatch, getState) => {
      ...
      let recentBugs = state.recentBugs;
      if (!state.existingBug.nonOverlappingRecentBugs) {
        recentBugs = recentBugs.filter(AlertsSection.isBugOverlapping(
            selectedAlerts));
      }
    ...
    submitNewBug: statePath => async (dispatch, getState) => {
      ...
      const alerts = AlertsSection.getSelectedAlerts(state.alertGroups);
      try {
        const bugId = await AlertsSection.fileBug({
          headers: rootState.authHeaders,
          alertKeys: alerts.map(a => a.key),
          ...state.newBug,
  ...
  AlertsSection.reducers = {
    toggleNonOverlappingRecentBugs: cp.ElementBase.statePathReducer(
        (state, action) => {
          const nonOverlappingRecentBugs =
            !state.existingBug.nonOverlappingRecentBugs;
          const selectedAlerts = AlertsSection.getSelectedAlerts(
              state.alertGroups);
          let recentBugs = state.recentBugs;
          if (!nonOverlappingRecentBugs) {
            recentBugs = recentBugs.filter(AlertsSection.isBugOverlapping(
                selectedAlerts));
          }
          return {
  ...
  AlertsSection.newState = options => {
    ...
    return {
      ...
      existingBug: {
        isOpen: false,
        nonOverlappingRecentBugs: false,
        recentBugs: [],
      },
      isLoading: false,
      isOwner: false,
  ...
  AlertsSection.isBugOverlapping = selectedAlerts => {
    ...
    return bug => bug.revisionRange.intersectsRangeInclusive(selectedRange);
  };

  AlertsSection.newBugSummary = alerts => {
  ...
  AlertsSection.newBugLabels = alerts => {
    let labels = new Set();
    labels.add('Pri-2');
  ...
  AlertsSection.newBugComponents = alerts => {
  ...
  AlertsSection.fileBug = async options => {
