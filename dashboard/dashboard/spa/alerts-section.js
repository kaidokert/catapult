...
tr.exportTo('cp', () => {
  ...
  class AlertsSection extends cp.ElementBase {
    ...
    async onTapRecentBugs_(event) {
      await this.dispatch('toggleRecentBugs', this.statePath);
    }
    ...
    async onCancelRecentBugs_(event) {
      await this.dispatch('toggleRecentBugs', this.statePath);
    }
    ...
    observeRecentBugs_() {
      this.dispatch('observeRecentBugs', this.statePath);
    }
  }
  ...
  AlertsSection.properties = {
    ...
    recentBugs: {
      type: Array,
      statePath: 'recentBugs',
      observer: 'observeRecentBugs_',
    },
  };
  ...
  AlertsSection.actions = {
    ...
    observeRecentBugs: statePath => async(dispatch, getState) => {
      dispatch({
        type: AlertsSection.reducers.receiveRecentBugs.typeName,
        statePath,
      });
    },
  };
  ...
  AlertsSection.reducers = {
    ...
    receiveRecentBugs: (state, action, rootState) => {
      return {
        ...state,
        bug: {
          ...state.bug,
          options: rootState.recentBugs.map(
              AlertsSection.transformRecentBugOption),
        }
      };
    },
  };

  AlertsSection.transformRecentBugOption = bug => {
    return {
