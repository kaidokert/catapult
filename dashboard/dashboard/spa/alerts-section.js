...
tr.exportTo('cp', () => {
  ...
  class AlertsSection extends cp.ElementBase {
    ...
    static clearAllFocused(sectionState) {
      return cp.ElementBase.updateObjectAtPath(sectionState, 'source', {
        isFocused: false,
      });
    }
  ...
  AlertsSection.actions = {
    toggleGroupExpanded: (statePath, alertGroupIndex) =>
      async (dispatch, getState) => {
        dispatch({
          ...
          statePath: statePath.concat(['alertGroups', alertGroupIndex]),
        });
      },
    ...
    layoutPreview: statePath => async (dispatch, getState) => {
      ...
      const state = getState();
      const section = cp.ElementBase.getStateAtPath(state, statePath);
  ...
  AlertsSection.reducers = {
    selectAlert: cp.ElementBase.updatingReducer((section, action) => {
      ...
      if (!alertGroup.isExpanded && (action.alertIndex === 0)) {
        ...
        alerts = cp.ElementBase.updateObjectAtPath(
            alerts, [action.alertIndex], {isSelected});
      }

      const alertGroups = cp.ElementBase.updateObjectAtPath(
    ...
    selectAllAlerts: cp.ElementBase.updatingReducer((section, action) => {
    ...
    receiveAlerts: cp.ElementBase.updatingReducer((section, action) => {
      return {
        isLoading: false,
        isPreviewing: true,
    ...
    togglePreviewing: cp.ElementBase.updatingReducer((section, action) => {
      return {isPreviewing: !section.isPreviewing};
    }),

    startLoadingAlerts: cp.ElementBase.updatingReducer((section, action) => {
      return {
        isLoading: true,
        source: {
          ...section.source,
    ...
    layoutPreview: cp.ElementBase.updatingReducer((section, action) => {
      return {
        isLoading: false,
