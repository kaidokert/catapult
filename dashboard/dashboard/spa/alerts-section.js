...
tr.exportTo('cp', () => {
  ...
  AlertsSection.reducers = {
    ...
    receiveAlerts: (state, action, rootState) => {
      ...
      const recentBugIds = new Set(recentBugs.map(bug => bug.id));
      for (const bug of state.existingBug.recentBugs) {
        if (!recentBugIds.has(bug.id)) {
          recentBugs.push(bug);
        }
      }
      ...
      state = {
        ...
        bug: {
          ...
          options: [...new Set([
            ...state.bug.options,
            ...recentBugs.map(bug => bug.id).filter(bug => bug.match(/^\d+$/)),
          ])],
        },
        isLoading: false,
        isOwner: false,
    ...
    onBugKeyup: (state, action, rootState) => {
      // TODO this is hacky
      const options = state.existingBug.recentBugs.map(bug => bug.id).filter(
          bug => bug.match(/^\d+$/));
      if (!options.includes(action.bugId)) {
        options.unshift(action.bugId);
      }
      return {
        ...state,
        bug: {
