...
tr.exportTo('cp', () => {
  ...
  class AlertsSection extends cp.Element {
    ...
    static clearAllFocused(sectionState) {
      return cp.Element.setStateAtPath(sectionState, 'source', {
        isFocused: false,
      });
    }
  ...
  AlertsSection.actions = {
    ...
    openCharts: (statePath, ctrlKey) => async (dispatch, getState) => {
      ...
      const section = cp.Element.getStateAtPath(state, action.statePath);
  ...
  AlertsSection.reducers = {
    ...
    focusSource: (state, action) => {
      ...
      return cp.Element.setStateAtPath(state, action.statePath + '.source', {
    ...
    startLoadingPreview: cp.statePathReducer((section, action) => {
