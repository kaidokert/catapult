...
tr.exportTo('cp', () => {
  ...
  AlertsSection.actions = {
    updateAlertColors: statePath => async (dispatch, getState) => {
      dispatch({
        reducer: AlertsSection.reducers.updateAlertColors,
        statePath,
      });
    },
    ...
    changeBugId: (statePath, bugId) => async (dispatch, getState) => {
      ...
      try {
        ...
        dispatch({
          reducer: AlertsSection.reducers.removeSelectedAlerts,
          statePath,
          bugId,
        });
    ...
    openCharts: (statePath, ctrlKey) => async (dispatch, getState) => {
      ...
      for (const alert of selectedAlerts) {
        dispatch({
          reducer: cp.ChromeperfApp.reducers.newSection,
    ...
    openNewBugDialog: statePath => async (dispatch, getState) => {
      ...
      if (!userEmail) return;
      dispatch({
        reducer: AlertsSection.reducers.openNewBugDialog,
        statePath,
        userEmail,
      });
    },
    ...
    openExistingBugDialog: statePath => async (dispatch, getState) => {
      ...
      if (!userEmail) return;
      dispatch({
        reducer: AlertsSection.reducers.openExistingBugDialog,
        statePath,
      });
    },
    ...
    loadAlerts: statePath => async (dispatch, getState) => {
      ...
      dispatch({
        reducer: AlertsSection.reducers.startLoadingAlerts,
        statePath,
      });
      ...
      dispatch({
        reducer: AlertsSection.reducers.receiveAlerts,
        statePath,
        alerts,
        recentBugs,
      });
