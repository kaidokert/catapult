  ...
  AlertsSection.actions = {
    ...
    selectSource: (statePath, selectedOptions) =>
      async (dispatch, getState) => {
        ...
        for (const source of selectedOptions) {
          let sheriff;
          if (source.startsWith('Sheriff:')) {
            sheriff = source.split(':')[1];
          }
          const body = new FormData();
          body.append('json', JSON.stringify({sheriff}));
          let signal;
          ...
          try {
            ...
            alerts.push.apply(alerts, jsonResponse.anomaly_list);
          } catch (err) {
            alerts.push.apply(alerts, cp.dummyAlerts());
          }
        }
        fetchMark.end();
    ...
    receiveAlerts: cp.ElementBase.statePathReducer((state, action) => {
      ...
      alertGroups = alertGroups.map((alerts, groupIndex) => {
        alerts = alerts.map(alert => {
          ...
          let unitName = 'unitlessNumber_smallerIsBetter';
          if (tr.b.Unit.byName[alert.units]) {
            ...
            if (info) {
              const suffix = tr.b.Unit.nameSuffixForImprovementDirection(
                  info.defaultImprovementDirection);
              unitName = info.name + suffix;
              deltaValue *= info.conversionFactor || 1;
            }
          }
          const baseUnit = tr.b.Unit.byName[unitName];
          const unitSuffix = tr.b.Unit.nameSuffixForImprovementDirection(
              baseUnit.improvementDirection);
          ...
          return {
            ...
            bot: `${alert.master}:${alert.bot}`,
            testSuite: alert.testsuite,
            measurement: testParts[0],
            story: testParts.slice(1).join('/'),
            deltaValue,
            deltaUnit: baseUnit.correspondingDeltaUnit,
            percentDeltaValue: deltaValue / alert.median_before_anomaly,
