...
tr.exportTo('cp', () => {
  ...
  AlertsSection.getQueryParams = state => {
    const queryParams = {};
    for (const source of sectionState.source.sources) {
      if (source.startsWith('Bug:')) {
        queryParams.bug = source.split(':')[1];
      } else if (source.startsWith('Releasing:')) {
        queryParams.releasing = source.split(':').slice(1).join(':');
      } else {
        queryParams.sheriff = source;
      }
    }
