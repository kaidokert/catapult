...
const PCT_UNIT = tr.b.Unit.byName.normalizedPercentage;
...
export function autotriage(alerts, triagedAlerts) {
  ...
  if (maxPctDelta < MIN_PCT_DELTA) {
    const explanation = html`
      The largest regression is smaller than ${PCT_UNIT.format(MIN_PCT_DELTA)}.
    `;
  ...
  const explanation = html`
    A ${PCT_UNIT.format(maxPctDelta)} regression is significant with no similar
    triaged alerts.
  `;
  return {bugId, explanation};
}
