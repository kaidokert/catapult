...
tr.exportTo('cp', () => {
  ...
  class MultiChart extends Polymer.GestureEventListeners(cp.ElementBase) {
    ...
    async onDotMouseOver_(event) {
      ...
      if (event.path[0].tagName !== 'IRON-ICON') {
        dotRect = await cp.measureElement(event.path[0]);
      } else {
        const datumIndex = event.model.parentModel.line.data.indexOf(
            event.model.datum);
        const circles = event.path[1].parentElement.querySelectorAll(
            `circle[line-index="${event.model.parentModel.lineIndex}"]`);
        const circle = circles[datumIndex];
        dotRect = await cp.measureElement(circle);
      }
  ...
  MultiChart.actions = {
    dotMouseOver: (statePath, chartRect, lineIndex, dotRect, isIcon) =>
  ...
  // The offset from the |dotRect| corner to the circle in each of the x and y
  // dimensions is versin(pi / 4) * dotRadius.
  if (!Math.versin) Math.versin = x => 1 - Math.cos(x);
  const VERSIN_45 = Math.versin(Math.PI / 4);
  ...
  MultiChart.reducers = {
    dotMouseOver: cp.ElementBase.statePathReducer((state, action) => {
      ...
      const todoMath = 8;
      const offset = (state.dotRadius * VERSIN_45) - (
          action.isIcon ? todoMath : 0);
      ...
      if (action.dotRect.y > chartMid.y) {
        bottom = Math.round(10 * (
          action.chartRect.bottom - action.dotRect.top - offset)) / 10;
        bottom += 'px';
      } else {
        top = Math.round(10 * (
          action.dotRect.bottom - action.chartRect.top - offset)) / 10;
        top += 'px';
      }
