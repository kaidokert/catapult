...
tr.exportTo('cp', () => {
  ...
  MultiChart.reducers = {
    dotMouseOver: cp.ElementBase.statePathReducer((state, action) => {
      ...
      const round = x => Math.round(10 * x) / 10;

      let top = '';
      let bottom = '';
      if (dotCenter.y > chartCenter.y) {
        bottom = round(action.chartRect.height - dotCenter.y + offset) + 'px';
      } else {
        top = round(dotCenter.y + offset) + 'px';
      }

      let left = '';
      let right = '';
      if (dotCenter.x > chartCenter.x) {
        right = round(action.chartRect.width - dotCenter.x + offset) + 'px';
      } else {
        left = round(dotCenter.x + offset) + 'px';
      }
