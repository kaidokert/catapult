...
import TimeseriesRace from './sw-utils/timeseries.js';
...
self.addEventListener('fetch', event => {
  ...
  if (event.request.url.startsWith(`${location.origin}/api/timeseries2`)) {
    const race = new TimeseriesRace(event.request);

    event.respondWith(makeFakeResponse());
    event.waitUntil(createListener(event.request.url, race));
  }
});
...
async function createListener(url, race) {
