...
tr.exportTo('cp', () => {
   ...
   * @property {!Array.<NormalizedSequence>} sequences
  ...
  class LineChart extends Polymer.GestureEventListeners(Polymer.Element) {
    ...
    path_(sequence) {
      return LineChart.path(sequence);
    }
    ...
    antiBrushes_(brushes) {
      return LineChart.antiBrushes(brushes);
    }
    ...
    onDotClick_(event) {
      this.dispatchEvent(new CustomEvent('dot-click', {
        detail: {
          ...
          sequence: event.model.parentModel.sequence,
          sequenceIndex: event.model.parentModel.sequenceIndex,
        },
      }));
    }
