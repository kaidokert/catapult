...
tr.exportTo('cp', () => {
  ...
  ChartTimeseries.fetch =
    async (fetchDescriptor, legacy, headers, signal, options, dispatch) => {
      ...
      const columns = ['revision', 'timestamp'];
      if (legacy) {
        testPath = ChartTimeseries.compileTestPath(fetchDescriptor, true);
        columns.push('value');
      } else {
        // TODO figure out what to do when the user asks for pct_99 when the
        // metric doesn't set that in summaryOptions
  ...
  ChartTimeseries.layout = (lineDescriptor, timeserieses) => {
    ...
    function mergeHistograms() {
      if (histIndices.length === 1) return hist;

      const merged = timeserieses[0][histIndices[0]].clone();
    ...
    function histX(hist) {
      // TODO avg chromiumCommitPositions
      return hist.revision;
    }

    function nextHist() {
      // Increment the histIndex whose hist has the smallest x coordinate.
      const hists = histIndices.map((histIndex, seriesIndex) =>
        timeserieses[seriesIndex][histIndex]);
      let seriesIndexWithMinX = -1;
      let minX = Infinity;
      for (let seriesIndex = 0; seriesIndex < timeserieses.length;
          ++seriesIndex) {
        const timeseries = timeserieses[seriesIndex];
        const histIndex = histIndices[seriesIndex];
        if (histIndex === (timeseries.length - 1)) continue;
        ...
        const x = histX(hist);
        if (x < minX) {
          seriesIndexWithMinX = seriesIndex;
          minX = x;
        }
      }
      if (seriesIndexWithMinX < 0) {
        // Done iterating over all timeserieses.
        return undefined;
      }
      ++histIndices[seriesIndexWithMinX];
