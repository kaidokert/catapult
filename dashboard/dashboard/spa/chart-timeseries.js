...
tr.exportTo('cp', () => {
  const LEVEL_OF_DETAIL = {
    ...
    // statistic and pointId/timestamp/commitPos.
    // Fetches /api/timeseries?annotations=false&startRev=0&endRev=inf
    X_Y: 'x,y',

    // chart-section's main chart can draw its lines using X_Y FastHistograms
    // while asynchronously fetching annotations (e.g.  alerts)
    // for a given pointId/timestamp/commitPos range for tooltips and icons.
    // If an extant request overlaps a new request, then the new request can
    // fetch the difference and await the extant request.
    // Fetches /api/timeseries?annotations=true&startRev&endRev
    ...
    // large range.
    // Fetches /api/histogram?testpath&pointId
    HISTOGRAM: 'histogram',
  };
  ...
  class Descriptor {
    constructor(options) {
      ...
      this.statistics = options.statistics || ['avg'];
    }
  ...
  // Supports X_Y and ANNOTATIONS levels of detail.
  class FastHistogram {
   ...
   *       requests: {
   *         ${hash(level of detail, revision range)}: {
   *           level of detail,
   *           revision range,
   *           abortController,
   *           promise,
   ...
   * pointId/timestamp/commitPos range logic.
   */
