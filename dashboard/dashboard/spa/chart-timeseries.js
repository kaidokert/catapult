  ...
  ChartTimeseries.fetch =
    async (fetchDescriptor, legacy, headers, signal, options, dispatch) => {
      ...
      if (legacy) {
        ...
        if (fetchDescriptor.statistic.startsWith('pct') ||
            fetchDescriptor.statistic.startsWith('ipr')) {
          columns.push('hist');
        } else {
  ...
  ChartTimeseries.layout = (lineDescriptor, timeserieses) => {
    ...
    function nextHist() {
      for (let seriesIndex = 0; seriesIndex < timeserieses.length;
        ...
        if ((lineDescriptor.maxRev &&
             (hist.revision <= lineDescriptor.maxRev)) ||
            (lineDescriptor.maxTimestamp &&
    ...
    while (hist = nextHist()) {
      lineData.push({
        hist,
        x: hist.revision,  // TODO max chromiumCommitPositions
        y: row.running[lineDescriptor.statistic],
        icon: getIcon(hist),
      });
    }
    return lineData;
