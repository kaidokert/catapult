...
tr.exportTo('cp', () => {
  ...
  ChartTimeseries.actions = {
    prefetch: (statePath, lineDescriptors) => async(dispatch, getState) => {
      await Promise.all(lineDescriptors.map(async lineDescriptor => {
        ...
        for await (const { errors } of batchIterator) {
          if (errors.length > 0) {
            throw errors;
          }
    ...
    measureYTicks: (statePath, lineDescriptor) => async(dispatch, getState) => {
  ...
  ChartTimeseries.reducers = {
    layout: (state, action, rootState) => {
      ...
      // Take each timeserieses
      for (const value of Object.values(action.timeseriesesByLine)) {
        const [lineDescriptor, ...timeseries] = value;
      ...
      // Do not iterate over lines past here
  ...
  ChartTimeseries.loadLines = (statePath, lineDescriptors) => async(
    ...
    // Collect a bunch of timeseries readers for every line
    ...
    for await (const { results, errors } of batchIterator) {
      ...
      dispatch({
        ...
        // Question(Sam): Why is this here? It's not being used in layout
        statePath,
      });
      ChartTimeseries.actions.measureYTicks(statePath, lineDescriptor)(dispatch,
          getState);
    }
  };
