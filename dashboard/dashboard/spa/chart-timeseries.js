...
tr.exportTo('cp', () => {
  ...
  ChartTimeseries.reducers = {
    layout: cp.ElementBase.statePathReducer((state, action) => {
      ...
      if (state.yAxis.generateTicks) {
        ...
        if (state.normalize) {
          for (const line of lines) {
            line.ticks = ChartTimeseries.generateYTicks(line.yRange, line.unit);
          }
          if (lines.length === 1) {
            ticks = lines[0].ticks;
          }
          ...
          for (const [unitName, range] of yRangeForUnitName) {
            ...
            const ticks = ChartTimeseries.generateYTicks(range, unit);
  ...
  ChartTimeseries.generateYTicks = (displayRange, unit) => {
    ...
    return ChartTimeseries.generateTicks(dataRange).map(y => {
      return {
        text: unit.format(y),
        yPct: round(1 - displayRange.normalize(y)) + '%',
      };
    });
  };
