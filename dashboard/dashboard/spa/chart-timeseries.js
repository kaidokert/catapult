      ...
      if (state.yAxis.generateTicks) {
        if (state.normalize) {
          for (const line of lines) {
            ...
            // to compute the ticks. TODO store both
            ...
            line.ticks = ChartTimeseries.generateTicks(trueRange).map(y => {
              return {
                text: line.unit.format(y),
                yPct: round(1 - line.yRange.normalize(y)) + '%',
              };
            });
          }
        } else {
          ...
          for (const [unitName, range] of yRangeForUnitName) {
            ...
            // Use the extended range to compute yPct, but the unextended range
            // to compute the ticks. TODO store both in yRangesForUnitName
            const trueRange = tr.b.math.Range.fromExplicitRange(
          ...
          let ticks = [];
          if (!state.normalize && (ticksForUnitName.size === 1)) {
