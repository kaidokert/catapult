...
tr.exportTo('cp', () => {
  ...
  ChartTimeseries.assignColors = lines => {
    ...
    for (const line of lines) {
      ...
      if (color) {
        line.color = color.lighten(0.15).toString();
      } else {
        line.color = 'white';
      }
    }
  ...
  ChartTimeseries.reducers = {
    layout: cp.ElementBase.statePathReducer((state, action) => {
      ...
      if (state.xAxis.generateTicks) {
        state = ChartTimeseries.generateXTicksReducer(state, xRange, rawXs);
      }

      if (state.yAxis.generateTicks) {
        state = ChartTimeseries.generateYTicksReducer(
  ...
  ChartTimeseries.generateXTicksReducer = (state, xRange, rawXs) => {
  ...
  ChartTimeseries.generateYTicksReducer = (
      state, yRangeForUnitName, yExtension) => {
