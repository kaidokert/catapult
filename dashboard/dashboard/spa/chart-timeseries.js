...
tr.exportTo('cp', () => {
  ...
  ChartTimeseries.reducers = {
    layout: (state, action, rootState) => {
      ...
      for (const value of Object.values(action.timeseriesesByLine)) {
        const [descriptor, ...timeserieses] = value;
        const data = ChartTimeseries.aggregateTimeserieses(
            descriptor, timeserieses, state.levelOfDetail, {
              minRevision: state.minRevision,
              maxRevision: state.maxRevision,
            });

        if (data.length === 0) continue;
        if (!timeserieses.length || !timeserieses[0].length) continue;
        ...
        const newLine = {descriptor, unit, data, strokeWidth: 1};
        const lineIndex = state.lines.findIndex(line =>
          ChartTimeseries.lineDescriptorEqual(
              line.descriptor, newLine.descriptor));
        if (lineIndex === -1) {
  ...
  ChartTimeseries.loadLines = statePath => async(dispatch, getState) => {
    ...
    const readers = [];
    const lineDescriptors = state.lineDescriptors.slice(
        0, ChartTimeseries.MAX_LINES);
    ...
    for await (const {results, errors} of new cp.BatchIterator(readers)) {
      ...
      const timeseriesesByLine = collateTimeseriesByLine(
          results, state.lineDescriptors);
  ...
  function getIcon(datum) {
    ...
    // The stale data icon cp:clock is set by
    // ChartCompound.reducers.updateStale.
    return {};
  }
