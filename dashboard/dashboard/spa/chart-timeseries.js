...
tr.exportTo('cp', () => {
  ...
  ChartTimeseries.actions = {
    ...
    dotMouseOver_: (statePath, line, datum) => async(dispatch, getState) => {
      ...
      rows.push({
        name: 'test suite',
        value: line.descriptor.testSuites.join('\n'),
      });
      rows.push({name: 'measurement', value: line.descriptor.measurement});
      rows.push({name: 'bot', value: line.descriptor.bots.join('\n')});
      rows.push({
        name: 'test case',
        value: line.descriptor.testCases.join('\n'),
      });
  ...
  ChartTimeseries.assignColors = lines => {
    ...
    for (const line of lines) {
      ...
      if (lines.length === (1 + testLines.length)) {
        // There's only a single ref build line, so make it grey for visual
        // simplicity.
  ...
  ChartTimeseries.reducers = {
    layout: (state, action, rootState) => {
      ...
      const lineData = ChartTimeseries.mergeTimeserieses(
      ...
      if (lineData.length === 0) return state;
