...
tr.exportTo('cp', () => {
  ...
  ChartTimeseries.reducers = {
    layout: cp.ElementBase.statePathReducer((state, action) => {
      ...
      const {xRange, yRangeForUnit} = cp.ChartBase.normalizeLinesInPlace(
          lines, state.normalize, state.zeroYAxis);
      state = {...state, lines};

      const round = x => roundDigits(x * 100, 1);

      let ticks = ChartTimeseries.generateTicks(xRange).map(x => {
        return {
          text: '' + x,
      ...
      const yTicksForUnit = new Map();
      for (const [unit, range] of yRangeForUnit) {
        const ticks = ChartTimeseries.generateTicks(range).map(y => {
          return {
            text: unit.format(y),
            yPct: round(1 - range.normalize(y)),
          };
        });
        yTicksForUnit.set(unit, ticks);
      }
      if (yTicksForUnit.size === 1) {
        ticks = [...yTicksForUnit.values()][0];
