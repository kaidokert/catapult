...
tr.exportTo('cp', () => {
  ...
  class ChartTimeseries extends cp.ElementBase {
    ...
    async onGetTooltip_(event) {
      this.dispatch('getTooltip', this.statePath,
          event.detail.nearestLine, event.detail.nearestPoint);
    }
  ...
  ChartTimeseries.actions = {
    ...
    getTooltip: (statePath, line, datum) => async(dispatch, getState) => {
      dispatch({
  ...
  ChartTimeseries.reducers = {
    ...
    getTooltip: (state, {line, datum}, rootState) => {
      if (!line || !datum) return {...state, tooltip: null};
      ...
      */
      ...
      if (datum.xPct < 50) {
        tooltip.left = datum.xPct + '%';
      } else {
        tooltip.right = (100 - datum.xPct) + '%';
      }

      // TODO also set cursorRevision
