...
export default class ChartBase extends GestureEventListeners(ElementBase) {
  ...
  render() {
    ...
    return html`
      <svg
        ...
        ${(this.yAxis.ticks || []).map(tick => html`
          <text
              x="${this.yAxis.width}"
              y="${tick.yPct}"
              text-anchor="end"
      ...
      <div id="right">
        <div id="brush_handles">
          ${(this.xAxis.brushes || []).map((brush, brushIndex) => html`
            <div class="brush_handle"
                @track="${event => this.onTrackBrushHandle_(event, brushIndex)}"
        ...
        <div id="main"
            @mousemove="${this.onMouseMoveMain_}">
          <svg
              width="100%"
              height="${this.graphHeight}"
      ...
      `)}
            ...
            ${ChartBase.antiBrushes(this.xAxis.brushes).map(antiBrush => html`
              <rect
                  x="${antiBrush.start}"
                  y="0"
          ...
          ${!showTooltip ? '' : html`
            <div id="tooltip"
                style="left: ${this.tooltip.left};
                       right: ${this.tooltip.right};
                       top: ${this.tooltip.top};
                       bottom: ${this.tooltip.bottom};">
              <table style="border-color: ${this.tooltip.color};">
        ...
        <svg
          ...
          ${this.xAxis.ticks.map(tick => html`
            <text
                x="${tick.xPct}"
                y="100%"
  ...
  get isMouseOverMain() {
    const mainStyle = getComputedStyle(this.shadowRoot.querySelector('#main'));
  ...
  async onTrackBrushHandle_(event, brushIndex) {
    const main = this.shadowRoot.querySelector('#main');
    const xPct = ChartBase.computeBrush(
        event.detail.x, await measureElement(main));
    STORE.dispatch(UPDATE(
        `${this.statePath}.xAxis.brushes.${brushIndex}`, {xPct}));
