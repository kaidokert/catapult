...
tr.exportTo('cp', () => {
  ...
  ChartBase.normalizeLinesInPlace = (lines, opt_options) => {
    ...
    if (mode === 'center') {
      for (const line of lines) {
        // Set line.yRange to the per-unit Range, but shifted so that the line's
        // center is in the center of the chart.
        const unitRange = yRangeForUnitName.get(line.unit.unitName);
        const delta = line.yRange.center - unitRange.center;
    ...
    for (const line of lines) {
      line.path = '';
      const yRange = (mode === 'normalizeLine') ? line.yRange :
