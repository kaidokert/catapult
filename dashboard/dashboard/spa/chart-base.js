...
tr.exportTo('cp', () => {
  ...
  ChartBase.normalizeLinesInPlace = (lines, normalize, zeroYAxis) => {
    const xRange = new tr.b.math.Range();
    const yRangeForUnit = new Map();
    let maxLineLength = 0;
    for (const line of lines) {
      ...
      if (!yRangeForUnit.has(line.unit.jsonName)) {
        yRangeForUnit.set(line.unit.jsonName, new tr.b.math.Range());
      }
      yRangeForUnit.get(line.unit.jsonName).addRange(line.yRange);
    }

    // Extend xRange by half the average distance between x values in both
    // directions in order to make it easier to click on the endpoint dots.
    /* TODO keep this from breaking xAxis.ticks
    ...
    */
    ...
    for (const line of lines) {
      ...
      const yRange = normalize ? line.yRange :
        yRangeForUnit.get(line.unit.jsonName);
      for (const datum of line.data) {
        datum.xPct = round(xRange.normalize(getX(datum)));
        // Y coordinates increase downwards.
        datum.yPct = round(1 - line.yRange.normalize(datum.y));
