...
// result first. The loser's promise will be yielded as well, in case the caller
// wants it. After a short period of time, results from the network will be
// written back to IndexedDB.
export async function* race(request) {
  ...
  const network = (async() => {
    ...
    let mark = new Mark('Timeseries', 'Network', requestParams.url);
    const response = await fetch(url, { signal: requestParams.signal });
    mark.end();

    mark = new Mark('Timeseries', 'Network - Parse JSON', requestParams.url);
...
async function openDatabase(key) {
  const mark = new Mark('Timeseries', 'Open', key);
...
async function read(requestParams) {
  ...
  const totalMark = new Mark('Timeseries', 'Read', key);
  ...
  let mark = new Mark('Timeseries', 'Read - Range check', key);
  ...
  // requested.
  mark = new Mark('Timeseries', 'Read - Denormalize', key);
...
async function getMetadata(transaction, key) {
  const mark = new Mark('Timeseries', 'Read - Metadata', key);
...
async function getRanges(transaction, levelOfDetail, key) {
  const mark = new Mark('Timeseries', 'Read - Ranges', key);
...
async function getDataPoints(transaction, minRevision, maxRevision, key) {
  const mark = new Mark('Timeseries', 'Read - Datapoints', key);
...
async function write(requestParams, { data, ...metadata }) {
  ...
  const totalMark = new Mark('Timeseries', 'Write', key);
  ...
  //     [statistic]: 42
  //   }
  let mark = new Mark('Timeseries', 'Write - Normalize', key);
  ...
  // Store timeseries as objects indexed by revision (preferred).
  ...
  // having to merge.
  mark = new Mark('Timeseries', 'Write - Datapoints', key);
  ...
  for (const datapoint of namedDatapoints) {
    ...
    // IndexedDB should be fast enough to "get" for every data point. A notable
    // experiment might be to "getAll" and find by revision. We can then compare
  ...
  mark = new Mark('Timeseries', 'Write - Ranges', key);
  ...
  mark = new Mark('Timeseries', 'Write - Metadata', key);
  ...
  mark = new Mark('Timeseries', 'Write - Queued Tasks', key);
  await transaction.complete;
  mark.end();
  totalMark.end();
}
...
export function parseRequest(request) {
  ...
  const testCase = url.searchParams.get('test_case') || '';
  const buildType = url.searchParams.get('build_type') || '';
  ...
  const levelOfDetail = url.searchParams.get('level_of_detail') || undefined;

  return {
    columns: getColumnsByLevelOfDetail(levelOfDetail, statistic),
    key: `ts/${testSuite}/${measurement}/${bot}/${testCase}/${buildType}`,
    levelOfDetail,
    maxRevision: parseInt(url.searchParams.get('max_revision')) || undefined,
    minRevision: parseInt(url.searchParams.get('min_revision')) || undefined,
