...
export default class TimeseriesCacheRequest extends CacheRequestBase {
  ...
  get raceCacheAndNetwork_() {
    return async function* () {
      ...
      while (networkPromises.length) {
        const {result, index, range, columns} = await Promise.race(
            networkPromises);
        console.log('networkResult', result, index, range, columns);
  ...
  async readNetwork_(range, columns) {
    ...
    return await this.timePromise('Parse JSON', response.json());
  }
