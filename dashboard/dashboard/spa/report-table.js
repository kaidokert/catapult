...
tr.exportTo('cp', () => {
  class ReportTable extends cp.ElementBase {
    ...
    async onSelectSource_(event) {
      if (this.source.selectedOptions.includes(ReportTable.CREATE)) {
        this.$.source.blur();
        for (let i = 0; i < 10; ++i) {
    ...
    isValid_(table) {
      return ReportTable.isValid(table);
    }
  ...
  ReportTable.DEFAULT_NAME = 'Chromium Performance Overview';
  ReportTable.CREATE = '[Create new report]';
  ...
  ReportTable.actions = {
    connected: statePath => async(dispatch, getState) => {
      ...
      if (state.minRevision === undefined ||
        ...
        ReportTable.actions.selectMilestone(
            statePath, state.milestone)(dispatch, getState);
      }
      await ReportTable.actions.loadSources(statePath)(dispatch, getState);
    ...
    authChange: statePath => async(dispatch, getState) => {
      ReportTable.actions.loadSources(statePath)(dispatch, getState);
    },

    selectMilestone: (statePath, milestone) => async(dispatch, getState) => {
      dispatch({
        type: ReportTable.reducers.selectMilestone.name,
        statePath,
        milestone,
      });
    },
    ...
    restoreState: (statePath, options) => async(dispatch, getState) => {
      dispatch({
        type: ReportTable.reducers.restoreState.name,
        statePath,
        options,
      });
    ...
    loadSources: statePath => async(dispatch, getState) => {
      ...
      dispatch({
        type: ReportTable.reducers.receiveSourceOptions.name,
        statePath,
        reportNames,
      });
    },
    ...
    loadReports: statePath => async(dispatch, getState) => {
      ...
      for await (const {results, errors} of new cp.BatchIterator(readers)) {
        ...
        dispatch({
          type: ReportTable.reducers.receiveReports.name,
          statePath,
          reports: results,
          testSuites,
        });
        // ReportTable.actions.renderEditForms(statePath)(dispatch, getState);
        // ReportTable.actions.prefetchCharts(statePath)(dispatch, getState);
      }
    ...
    renderEditForms: statePath => async(dispatch, getState) => {
      ...
      await Promise.all(state.tables.map(async(table, tableIndex) => {
        await cp.idle();
        await ReportTable.actions.renderEditForm(statePath, tableIndex)(
            dispatch, getState);
      }));
    },
    ...
    templateRemoveRow: (statePath, tableIndex, rowIndex) =>
      async(dispatch, getState) => {
        dispatch({
          type: ReportTable.reducers.templateRemoveRow.name,
          statePath,
          tableIndex,
          rowIndex,
        });
    ...
    templateAddRow: (statePath, tableIndex, rowIndex) =>
      async(dispatch, getState) => {
        dispatch({
          type: ReportTable.reducers.templateAddRow.name,
    ...
    templateSave: (statePath, tableIndex) => async(dispatch, getState) => {
      ...
      ReportTable.actions.loadReports(statePath)(dispatch, getState);
    },
  ...
  ReportTable.reducers = {
    ...
    receiveSourceOptions: (state, {reportNames}, rootState) => {
      ...
      if (window.IS_DEBUG || rootState.userEmail) {
        options.push(ReportTable.CREATE);
      }
    ...
    requestReports: (state, action, rootState) => {
      ...
      for (const name of selectedNames) {
        ...
        if (!tableNames.has(name)) {
          if (name === ReportTable.CREATE) {
            tables.push(ReportTable.newTemplate(
    ...
    receiveReports: (state, action, rootState) => {
      ...
      for (const report of action.reports) {
        ...
        const rows = report.report.rows.map(
            row => ReportTable.transformReportRow(
    ...
    templateAddRow: (table, action, rootState) => {
      ...
      const newRow = ReportTable.newTemplateRow({
        testSuite: {
  ...
  ReportTable.newTemplate = (userEmail, testSuites) => {
    return {
      isEditing: true,
      name: '',
      owners: userEmail,
      url: '',
      statistics: [],
      rows: [ReportTable.newTemplateRow({
        testSuite: {
  ...
  ReportTable.newTemplateRow = ({testSuite, bot, testCase}) => {
    return {
      label: '',
      testSuite: {
        ...testSuite,
  ...
  ReportTable.newStateOptionsFromQueryParams = queryParams => {
    const options = {
  ...
  ReportTable.getSessionState = state => {
    return {
  ...
  ReportTable.getRouteParams = state => {
    const routeParams = new URLSearchParams();
    const selectedOptions = state.source.selectedOptions;
    if (state.containsDefaultSection &&
        selectedOptions.length === 1 &&
        selectedOptions[0] === ReportTable.DEFAULT_NAME) {
      return routeParams;
    }
    for (const option of selectedOptions) {
      if (option === ReportTable.CREATE) continue;
  ...
  ReportTable.transformReportRow = (
