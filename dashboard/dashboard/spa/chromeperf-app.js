...
tr.exportTo('cp', () => {
  ...
  ChromeperfApp.actions = {
    ...
    keepDefaultSection: () => async (dispatch, getState) => {
      dispatch(cp.ElementBase.updateObjectAtPath([], {
    ...
    updateRoute: (routeMode, routeQueryParams) =>
      async (dispatch, getState) => {
        ...
        dispatch(cp.ElementBase.updateObjectAtPath([], {
          routeMode, routeModeQueryParams,
        }));
      },
    ...
    closeFabCallout: () => async (dispatch, getState) => {
      dispatch(cp.ElementBase.updateObjectAtPath([], {isExplainingFab: false}));
      // eslint-disable-next-line no-console
  ...
  ChromeperfApp.reducers = {
    focus: (state, action) => {
      ...
      return cp.ElementBase.updateObjectAtPath(
          state, action.statePath, {isFocused: true});
    },
