...
import {afterRender, breakWords, get, simpleGUID, timeout} from './utils.js';
...
export default class ChromeperfApp extends ElementBase {
  ...
  render() {
    ...
    const bugHref = bugPath + '?' + new URLSearchParams({
      description: 'Describe the problem: \n\n' +
        ...
        encodeURIComponent(window.location.origin + '#' + this.route.path),
    ...
    return html`
      ${this.enableNav ? html`
        <chops-header app-title="Chromium Performance">
          ...
          ${window.IS_PRODUCTION ? html`
            <chops-signin></chops-signin>
            <chops-signin-aware @user-update="${this.onUserUpdate_}">
            </chops-signin-aware>
          ` : html``}
        </chops-header>
      ` : html``}
      ...
      <div id="body">
        ...
        <div id="main" enableNav="${this.enableNav}">
            <report-section
                ...
                state-path="${this.statePath}.reportSection"
          ...
          ${(this.alertsSectionIds || []).map(id => html`
            <alerts-section
                state-path="${this.statePath}.alertsSectionsById.${id}"
                linked-state-path="${this.statePath}.linkedChartState"
                @require-sign-in="${this.requireSignIn_}"
                @new-chart="${this.onNewChart_}"
                @close-section="${this.onCloseAlerts_}">
            </alerts-section>
          `)}

          ${(this.chartSectionIds || []).map(id => html`
            <chart-section
                state-path="${this.statePath}.chartSectionsById.${id}"
                linked-state-path="${this.statePath}.linkedChartState"
                @require-sign-in="${this.requireSignIn_}"
                @new-chart="${this.onNewChart_}"
                @close-section="${this.onCloseChart_}">
            </chart-section>
          `)}
      ...
      <cp-toast opened="${this.closedAlertsIds && this.closedAlertsIds.length}">
        <raised-button
            id="reopen_alerts"
  ...
  async requireSignIn_(event) {
    if (this.userEmail || !window.IS_PRODUCTION) return;
