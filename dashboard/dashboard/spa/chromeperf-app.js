...
tr.exportTo('cp', () => {
  ...
  class ChromeperfApp extends Polymer.GestureEventListeners(cp.ElementBase) {
    ...
    newChart_(event) {
      this.dispatch('newSection', this.statePath, cp.ChartSection.is);
    }
      ...
      async (dispatch, getState) => {
        ...
        dispatch(cp.ElementBase.actions.updateObject(statePath, {
          releasingSection: cp.ReleasingSection.newState(
              sessionState.releasingSection),
          showingReleasingSection: sessionState.showingReleasingSection,
          alertsSection: cp.AlertsSection.newState(
              sessionState.alertsSection),
          showingAlertsSection: sessionState.showingAlertsSection,
        }));
        for (const options of sessionState.chartSections) {
          dispatch({
            type: ChromeperfApp.reducers.newSection.typeName,
            statePath,
            sectionType: cp.ChartSection.is,
