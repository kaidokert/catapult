...
tr.exportTo('cp', () => {
  ...
  class TestPathComponent extends cp.Element {
    ...
    onToggleTagGroupExpanded_(e) {
      e.cancelBubble = true;
      this.dispatch(TestPathComponent.toggleTagGroupExpanded(
          this.sectionId, this.componentIndex, e.detail.path));
    }

    static toggleTagGroupExpanded(sectionId, componentIndex, path) {
      return async (dispatch, getState) => {
        dispatch({
  ...
  cp.sectionReducer('test-path-component.toggleTagGroupExpanded',
