...
tr.exportTo('cp', () => {
  ...
  class TestPathComponent extends cp.Element {
    ...
    onSelect_(e) {
      this.dispatch(TestPathComponent.select(
        this.sectionId, this.componentIndex, e.target.selectedOptions));
    }
  ...
  cp.REDUCERS.set('test-path-component.aggregate', (state, action) => {
  ...
  cp.REDUCERS.set('test-path-component.select', (state, action) => {
    ...
    if (action.selectedOptions.length === 1) {
