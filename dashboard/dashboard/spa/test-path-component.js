...
tr.exportTo('cp', () => {
  ...
  class TestPathComponent extends cp.Element {
    ...
    onKeydown_(e) {
      this.dispatch(keydown(this.sectionId, this.componentIndex, e.detail.value));
      this.dispatch({
    ...
    onSelect_(e) {
      this.dispatch(select(this.sectionId, this.componentIndex, e.detail.value));
    }
  ...
  const focus = (sectionId, componentIndex, isFocused) =>
  ...
  const select = (sectionId, componentIndex, value) =>
    async (dispatch, getState) => {
      const currentValue =
        getState().sections[sectionId].testPathComponents[componentIndex].value;
