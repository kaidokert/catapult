...
tr.exportTo('cp', () => {
  ...
  ReportSection.actions = {
    ...
    loadReports: statePath => async(dispatch, getState) => {
      ...
      dispatch({
        type: ReportSection.reducers.finishReceivingReports.typeName,
        statePath,
      });
    },
  ...
  ReportSection.reducers = {
    ...
    finishReceivingReports: (state, action, rootState) => {
      return {
        ...state,
        isLoading: false,
      };
  ...
  ReportSection.transformReportRow = (
    ...
    for (const revision of [minRevision, maxRevision]) {
      for (const statistic of statistics) {
        // The IndexedDB cache can return impartial results if there is no
        // data cached for the requested revision.
    ...
    for (const statistic of statistics) {
      // The IndexedDB cache can return impartial results if there is no data
      // cached for the requested min or max revision.
