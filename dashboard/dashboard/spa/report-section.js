...
tr.exportTo('cp', () => {
    ...
    renderEditForms: statePath => async(dispatch, getState) => {
      ...
      for (let tableIndex = 0; tableIndex < state.tables.length;
        ...
        for (let rowIndex = 0;
          ...
          // TODO not sure how we can get here with invalid rows.
          if (!row) continue;
          if (!row.selectedOptions) continue;
          if (!row.selectedOptions.length) continue;
  ...
  ReportSection.reducers = {
    ...
    templateAddRow: (state, action, rootState) => {
      const tables = [...state.tables];
      const table = tables[action.tableIndex];
      tables[action.tableIndex] = {
        ...table,
