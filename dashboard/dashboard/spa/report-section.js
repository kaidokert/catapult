...
tr.exportTo('cp', () => {
      ...
      const dummyRow = measurement => {
        ...
        for (const revision of this.revisions_) {
          row.data[revision] = {
            ...
            statistics: [10, 0, 0, Math.random() * 1000, 0, 0, Math.random() * 1000],
            revision,
          };
        }
        return row;
  ...
  ReportSection.reducers = {
    selectMilestone: (state, action, rootState) => {
      const maxRevision = (action.milestone === CURRENT_MILESTONE) ? 'latest' : CHROMIUM_MILESTONES[action.milestone + 1];
