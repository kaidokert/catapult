...
tr.exportTo('cp', () => {
  ...
  ReportSection.actions = {
    ...
    authChange: statePath => async(dispatch, getState) => {
      dispatch(ReportSection.actions.loadSources(statePath));
      dispatch({
        type: ReportSection.actions.updateCreate.typeName,
        statePath,
      });
    },
  ...
  ReportSection.reducers = {
    updateCreate: cp.ElementBase.statePathReducer(
        (state, action, rootState) => {
          const options = [...state.source.options];
          if (rootState.authHeaders) {
            options.push(ReportSection.CREATE);
          } else {
            options.splice(options.indexOf(ReportSection.CREATE), 1);
          }
          return {
            ...state,
            source: {...state.source, options},
          };
        }),
