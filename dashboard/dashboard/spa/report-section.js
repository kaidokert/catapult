...
tr.exportTo('cp', () => {
  ...
  ReportSection.actions = {
    ...
    loadReports: statePath => async(dispatch, getState) => {
      ...
      const promises = state.source.selectedOptions.map(name =>
  ...
  ReportSection.reducers = {
    ...
    receiveReports: cp.ElementBase.statePathReducer((state, action) => {
      ...
      for (const report of action.reports) {
        ...
        // {name,url,owners,rows:[{labelParts,scalars}]}
        tables.push({
          ...report,
          rows: report.rows.map(row => {
            ...
            return {labelParts: row.label.split(':'), scalars};
          }),
        });
      }
      return {
  ...
  ReportSection.isValid = template => {
    ...
    for (const row of template.rows) {
      ...
      if (row.statistic.selectedOptions.length === 0) return false;
    }
    return true;
  };
