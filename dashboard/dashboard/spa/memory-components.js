...
export default class MemoryComponents extends ElementBase {
  ...
  render() {
    return this.columns.map((column, columnIndex) => html`
      <div class="column">
        <option-group
            state-path="${this.statePath}.columns.${columnIndex}"
            root-state-path="${this.statePath}.columns.${columnIndex}"
            @option-select="onColumnSelect_">
        </option-group>
      </div>
    `);
  }
