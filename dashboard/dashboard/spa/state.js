...
tr.exportTo('cp', () => {
  ...
  function sectionReducer(actionType, reducer) {
    REDUCERS.set(actionType, (state, action) => {
      ...
      const delta = reducer(state, action, section);
      if (typeof action.sectionId !== 'number') throw `Invalid sectionId ${action.sectionId}`;
      const sectionsById = {...state.sectionsById};
      sectionsById[action.sectionId] = Object.assign({}, section, delta);
