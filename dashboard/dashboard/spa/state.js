...
tr.exportTo('cp', () => {
  ...
  const DEFAULT_STATE = {
    sectionIds: [DEFAULT_SECTION.sectionId],
    sectionsById: {
      [DEFAULT_SECTION.sectionId]: DEFAULT_SECTION,
    },
  ...
  class Element extends ReduxMixin(Polymer.Element) {
    ...
    static register(subclass) {
      if (subclass.reduxProperties) {
        subclass.properties = Element.statePathProperties(
            'statePath', subclass.reduxProperties);
      }
  ...
  function statePathReducer(reducer) {
    return (state, action) => {
      ...
      const branch = Element.getStateAtPath(state, action.statePath);
      const delta = reducer(branch, action);
      return Element.setStateAtPath(state, action.statePath, delta);
    };
  }
