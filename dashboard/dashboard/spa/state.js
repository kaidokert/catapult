...
tr.exportTo('cp', () => {
  ...
  /**
   * This function helps Elements define their properties with statePath.
   *
   * @param {String} prefix
   * @param {!Object} configs
   * @returns {!Object}
   */
  function buildProperties(prefix, configs) {
    const idName = prefix + 'Id';
    const stateContainerName = prefix + 'sById';
    const properties = {};
    ...
    for (const [name, config] of Object.entries(configs)) {
      properties[name] = {
        ...config,
        statePath(state) {
          if (!state[stateContainerName][this[idName]]) return undefined;
          return state[stateContainerName][this[idName]][name];
        },
      };
    }
    return properties;
