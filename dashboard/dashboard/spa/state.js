...
tr.exportTo('cp', () => {
  ...
  /**
   * Action creators must be synchronous, but all async code must be in action
   * creators. This function helps action creators dispatch actions after async
   * processes: exampleAsyncActionCreator() {
   *   return asyncAction({type: 'startLoading'}, async () => {
   *     return {type: 'finishLoading', data: await fetch()};
   *   });
   * }
   *
   * @param {!Object} syncAction
   * @param {!function():Promise.<!Object>} getAsyncAction
   */
  function asyncAction(syncAction, getAsyncAction) {
    getAsyncAction().then(asyncAction => STORE.dispatch(asyncAction));
    return syncAction;
  }
  ...
  return {
    ...
    assignSection,
    asyncAction,
    rootReducerForTesting: rootReducer,
    sectionProperties,
  };
});
