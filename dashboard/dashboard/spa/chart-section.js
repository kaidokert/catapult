...
tr.exportTo('cp', () => {
  class ChartSection extends cp.ElementBase {
    ...
    onMinimapBrush_(event) {
      if (event.detail.sourceEvent.detail.state === 'end') {
        this.dispatch('brushMinimap', this.statePath, event.detail.brushIndex);
      }
    }
  ...
  ChartSection.actions = {
    ...
    brushMinimap: (statePath, brushIndex) => async (dispatch, getState) => {
      dispatch({
  ...
  ChartSection.reducers = {
    brushMinimap: cp.ElementBase.statePathReducer((state, action) => {
      const brushXPct = parseFloat(
          state.minimapLayout.xAxis.brushes[action.brushIndex].xPct);
      ...
      if (action.brushIndex === 0) {
        return {...state, minRevision: datum.x};
      }
      return {...state, maxRevision: datum.x};
    }),
  ...
  ChartSection.isEmpty = state => state.chartLayout.lines.length === 0;
