...
tr.exportTo('cp', () => {
  ...
  ChartSection.actions = {
    ...
    loadTestSuites: statePath => async(dispatch, getState) => {
      const {options, count} = await dispatch(cp.ReadTestSuites());
      dispatch({
    ...
    describeTestSuites: statePath => async(dispatch, getState) => {
      ...
      // Wait to populateMagicOptions until after finalizeParameters, which
      // might change these selectedOptions.
      dispatch(cp.DropdownInput.actions.populateMagicOptions(
  ...
  ChartSection.parameterMatrix = state => {
    ...
    if (testSuiteses.length === 1 &&
        botses.length === 1 &&
        measurements.length === 1 &&
        testCaseses.length === 1 &&
        statistics.length === 1) {
      buildTypes.push('reference');
    }
    return {
      testSuiteses,
      measurements,
