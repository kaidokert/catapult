...
'use strict';
tr.exportTo('cp', () => {
  function dummyHistograms(section, testPathCartesianProduct, colors) {
  ...
  class ChartSection extends cp.Element {
    ...
    onDotClick_(e) {
      this.dispatch('dotClick',
          this.sectionId, e.detail.sequenceIndex, e.detail.datumIndex);
    }
  ...
  ChartSection.actions = {
    ...
    dotClick: (sectionId, sequenceIndex, datumIndex) =>
      async (dispatch, getState) => {
        // eslint-disable-next-line no-console
        console.log('TODO brush around datumIndex');
      },
