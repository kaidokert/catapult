        ...
        if (index === 0) {
          ...
          dispatch(cp.ElementBase.actions.updateObjectAtPath(
              `${statePath}.testPathComponents.1`, {
                options: OptionGroup.groupValues(bots).map(option => {
          ...
          dispatch(cp.ElementBase.actions.updateObjectAtPath(
              `${statePath}.testPathComponents.2`, {
                options: OptionGroup.groupValues(measurements),
          ...
          dispatch(cp.ElementBase.actions.updateObjectAtPath(
              `${statePath}.testPathComponents.3`, {
                options: [{
                  ...
                  isExpanded: true,
                  value: '*',
                  options: OptionGroup.groupValues(stories),
                }],
                placeholder: `Test cases (${stories.length})`,
              }));
          dispatch(cp.ElementBase.actions.updateObjectAtPath(
              `${statePath}.testPathComponents.3.tags`, {
                options: OptionGroup.groupValues(cp.dummyStoryTags(testSuites)),
              }));
        }
          ...
          if (queryParams[param]) {
            ...
            if (!tr.b.setsEqual(new Set(selectedOptions),
              ...
              dispatch(cp.TestPathComponent.actions.select(
                  ...
                  selectedOptions));
              dispatch(ChartSection.actions.selectTestPathComponent(
                  statePath, i));
            }
          }
        }
