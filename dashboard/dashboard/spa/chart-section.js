...
tr.exportTo('cp', () => {
  ...
  class ChartSection extends cp.Element {
    ...
    onDotMouseOver_(e) {
      this.dispatch('dotMouseOver', this.sectionId, e.detail.lineIndex);
    }

    onDotMouseOut_(e) {
      this.dispatch('dotMouseOut', this.sectionId, e.detail.lineIndex);
    }
  ...
  ChartSection.actions = {
    ...
    dotClick: (sectionId, ctrlKey, lineIndex, datumIndex) =>
      async (dispatch, getState) => {
        dispatch({
    ...
    dotMouseOver: (sectionId, lineIndex) => async (dispatch, getState) => {
      const section = getState().sectionsById[sectionId];
    ...
    dotMouseOut: (sectionId, lineIndex) => async (dispatch, getState) => {
      dispatch({
  ...
  ChartSection.reducers = {
    ...
    dotMouseOver: cp.sectionReducer((section, action) => {
      ...
      const lines = cp.assignInArray(
          section.chartLayout.lines, action.lineIndex, {
            strokeWidth: 2,
          });
