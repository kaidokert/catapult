...
tr.exportTo('cp', () => {
  class ChartSection extends cp.Element {
    ...
    static maybeLoadTimeseries(sectionId) {
      return async (dispatch, getState) => {
        ...
        const components = getState().sectionsById[sectionId].testPathComponents;
    ...
    onSelectRelatedTab_(e) {
      this.dispatch(ChartSection.selectRelatedTab(this.sectionId, e.target.selected));
    }
  ...
  cp.sectionReducer('chart-section.selectRelatedTab', (state, action, section) => {
    return {
  ...
  cp.sectionReducer('chart-section.toggleChartOnly', (state, action, section) => {
    return {
      onlyChart: !section.onlyChart,
    };
  });

  cp.sectionReducer('chart-section.clearTimeseries', (state, action, section) => {
    return {
  ...
  cp.sectionReducer('chart-section.layoutRelatedTabs', (state, action, section) => {
