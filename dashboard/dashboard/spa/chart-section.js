...
tr.exportTo('cp', () => {
      ...
      this.dispatchEvent(new CustomEvent('new-chart', {
        ...
        detail: {
          options: {
            ...
            title: this.title,
            parameters: {
              suites: [...this.suite.selectedOptions],
              suitesAggregated: this.suite.isAggregated,
              measurements: [...this.measurement.selectedOptions],
              bots: [...this.bot.selectedOptions],
              botsAggregated: this.bot.isAggregated,
              cases: [...this.case.selectedOptions],
              casesAggregated: this.case.isAggregated,
  ...
  ChartSection.State = {
    ...
    ...cp.ChartPair.State,
    // TODO encapsulate TimeseriesDescriptor state in `this.descriptor`, pass
    // options.parameters to its buildState in order to fix #suite=&etc
    ...cp.TimeseriesDescriptor.State,
    ...
    statistic: options => cp.MenuInput.buildState({
      label: 'Statistics',
      required: true,
  ...
  ChartSection.actions = {
    ...
    maybeLoadTimeseries: statePath => async(dispatch, getState) => {
      ...
      if (state.suite.selectedOptions.length &&
  ...
  ChartSection.reducers = {
    ...
    buildRelatedTabs: (state, action, rootState) => {
      ...
      if (state.suite.selectedOptions.filter(
  ...
  ChartSection.getSessionState = state => {
    return {
      parameters: {
        suites: state.suite.selectedOptions,
        suitesAggregated: state.suite.isAggregated,
        measurements: state.measurement.selectedOptions,
        bots: state.bot.selectedOptions,
        botsAggregated: state.bot.isAggregated,
        cases: state.case.selectedOptions,
        casesAggregated: state.case.isAggregated,
  ...
  ChartSection.getRouteParams = state => {
    ...
    if (state.suite.selectedOptions.length > 2 ||
        state.case.selectedOptions.length > 2 ||
    ...
    for (const suite of state.suite.selectedOptions) {
    ...
    for (const cas of state.case.selectedOptions) {
      routeParams.append('case', cas);
    }
    for (const tag of state.case.tags.selectedOptions) {
  ...
  ChartSection.matchesOptions = (state, options) => {
    ...
    if (options.parameters) {
      if (options.parameters.suites && !tr.b.setsEqual(
          ...
          new Set(state.suite.selectedOptions))) {
        return false;
      }
      ...
      if (options.parameters.cases && !tr.b.setsEqual(
          ...
          new Set(state.case.selectedOptions))) {
        return false;
      }
