...
tr.exportTo('cp', () => {
  ...
  ChartSection.actions = {
    ...
    dotMouseOver: (sectionId, sequenceIndex) => async (dispatch, getState) => {
      const section = getState().sectionsById[sectionId];
      if (section.chartLayout.lines.length < 2) return;
      dispatch({
  ...
  ChartSection.reducers = {
    ...
    dotClick: cp.sectionReducer((section, action) => {
      const sequence = section.chartLayout.lines[action.sequenceIndex];
    ...
    dotMouseOver: cp.sectionReducer((section, action) => {
      ...
      const lines = cp.assignInArray(
          section.chartLayout.lines, action.sequenceIndex, {
            strokeWidth: 2,
          });
      if (action.sequenceIndex !== (lines.length - 1)) {
        // Move action.sequenceIndex to the end.
        [lines[action.sequenceIndex], lines[lines.length - 1]] =
          [lines[lines.length - 1], lines[action.sequenceIndex]];
      }
      return {
        chartLayout: {
          ...section.chartLayout,
