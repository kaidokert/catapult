...
tr.exportTo('cp', () => {
            ...
            [Math.random() * 1e9], {
              diagnostics: new Map([
                ...
                [
                  ...
                  new tr.v.d.GenericSet([sequences[i].color.toString()]),
                ],
              ]),
            });
      }
  ...
  class ChartSection extends cp.Element {
    ...
    onTitleKeyup_(e) {
      this.dispatch('setTitle', this.sectionId, e.target.value);
    }

    toggleOptions_() {
      this.dispatch('showOptions', this.sectionId, !this.isShowingOptions);
    }
    ...
    onChartClick_(e) {
      this.dispatch('chartClick', this.sectionId);
    }
    ...
    onDotClick_(e) {
      this.dispatch('dotClick',
          this.sectionId,
          e.detail.ctrlKey,
          e.detail.sequenceIndex,
          e.detail.datumIndex);
    }

    onDotMouseOver_(e) {
      this.dispatch('dotMouseOver', this.sectionId, e.detail.sequenceIndex);
    }

    onDotMouseOut_(e) {
      this.dispatch('dotMouseOut', this.sectionId, e.detail.sequenceIndex);
    }

    onLegendMouseOver_(e) {
      this.dispatch('legendMouseOver', this.sectionId, e.detail.testPath);
    }

    onLegendMouseOut_(e) {
      this.dispatch('legendMouseOut', this.sectionId, e.detail.testPath);
    }
    ...
    onSelectRelatedTab_(e) {
      this.dispatch('selectRelatedTab', this.sectionId, e.target.selected);
    }
  ...
  ChartSection.actions = {
    setTitle: (sectionId, title) => async (dispatch, getState) => {
      dispatch({
    ...
    showOptions: (sectionId, showOptions) => async (dispatch, getState) => {
      dispatch({
    ...
    brushMinimap: (sectionId, brushIndex, value) =>
    ...
    brushChart: (sectionId, brushIndex, value) =>
      async (dispatch, getState) => {
        dispatch({
    ...
    maybeLoadTimeseries: sectionId => async (dispatch, getState) => {
      ...
      if (components[0].selectedOptions.length &&
        ...
        dispatch(ChartSection.actions.loadTimeseries(sectionId));
      } else {
        dispatch(ChartSection.actions.clearTimeseries(sectionId));
      }
      dispatch(cp.ChromeperfApp.actions.updateLocation());
    },

    clearTimeseries: sectionId => async (dispatch, getState) => {
      dispatch({
    ...
    toggleChartOnly: sectionId => async (dispatch, getState) => {
      dispatch({
    ...
    loadTimeseries: sectionId => async (dispatch, getState) => {
      dispatch({
    ...
    selectRelatedTab: (sectionId, selectedRelatedTabIndex) =>
      async (dispatch, getState) => {
        dispatch({
    ...
    chartClick: sectionId => async (dispatch, getState) => {
      dispatch({
    ...
    dotClick: (sectionId, ctrlKey, sequenceIndex, datumIndex) =>
      async (dispatch, getState) => {
        dispatch({
        ...
        const fetchMark = tr.b.Timing.mark('chart-section', 'fetchHistograms');
        // eslint-disable-next-line no-console
        ...
        dispatch({
          ...
          sectionId,
          histograms: dummyHistograms(getState().sectionsById[sectionId]),
        });
      },

    dotMouseOver: (sectionId, sequenceIndex) => async (dispatch, getState) => {
      const section = getState().sectionsById[sectionId];
      if (section.chartLayout.sequences.length < 2) return;
      dispatch({
    ...
    dotMouseOut: (sectionId, sequenceIndex) => async (dispatch, getState) => {
      dispatch({
        type: 'chart-section.dotMouseOut',
        sectionId,
        sequenceIndex,
      });
    },

    legendMouseOver: (sectionId, testPath) => async (dispatch, getState) => {
      const section = getState().sectionsById[sectionId];
      for (let i = 0; i < section.chartLayout.sequences.length; ++i) {
        if (!ChartSection.testPathEqual(
            section.chartLayout.sequences[i].testPath, testPath)) {
          continue;
        }
        dispatch(ChartSection.actions.dotMouseOver(sectionId, i));
      }
    },

    legendMouseOut: (sectionId, testPath) => async (dispatch, getState) => {
      const section = getState().sectionsById[sectionId];
      for (let i = 0; i < section.chartLayout.sequences.length; ++i) {
        if (!ChartSection.testPathEqual(
            section.chartLayout.sequences[i].testPath, testPath)) {
        ...
        dispatch(ChartSection.actions.dotMouseOut(sectionId, i));
      }
    },
  };

  ChartSection.reducers = {
    receiveHistograms: cp.sectionReducer((section, action) => {
      return {
        isLoading: false,
    ...
    chartClick: cp.sectionReducer((section, action) => {
      return {
        chartLayout: {
          ...section.chartLayout,
          brushes: [],
    ...
    dotClick: cp.sectionReducer((section, action) => {
      const sequence = section.chartLayout.sequences[action.sequenceIndex];
    ...
    dotMouseOver: cp.sectionReducer((section, action) => {
      // Set its strokeWidth:2
      const sequences = cp.assignInArray(
          section.chartLayout.sequences, action.sequenceIndex, {
            strokeWidth: 2,
          });
      if (action.sequenceIndex !== (sequences.length - 1)) {
        // Move action.sequenceIndex to the end.
        [sequences[action.sequenceIndex], sequences[sequences.length - 1]] =
          [sequences[sequences.length - 1], sequences[action.sequenceIndex]];
      }
      return {
        chartLayout: {
          ...section.chartLayout,
    ...
    dotMouseOut: cp.sectionReducer((section, action) => {
      return {
        chartLayout: {
          ...section.chartLayout,
          sequences: cp.assignInArray(
              section.chartLayout.sequences, action.sequenceIndex, {
                strokeWidth: 1,
              }),
        },
    ...
    showOptions: cp.sectionReducer((section, action) => {
      return {isShowingOptions: action.showOptions};
    }),

    setTitle: cp.sectionReducer((section, action) => {
      return {
        title: action.title,
        isTitleCustom: true,
      };
    ...
    selectRelatedTab: cp.sectionReducer((section, action) => {
      return {selectedRelatedTabIndex: action.selectedRelatedTabIndex};
    }),

    layoutChart: cp.sectionReducer((section, action) => {
      return {
        isLoading: false,
        title: action.title,
    ...
    toggleChartOnly: cp.sectionReducer((section, action) => {
      return {onlyChart: !section.onlyChart};
    }),

    clearTimeseries: cp.sectionReducer((section, action) => {
      return {
        chartLayout: false,
        minimapLayout: false,
        histograms: undefined,
    ...
    startLoadingTimeseries: cp.sectionReducer((section, action) => {
      return {isLoading: true};
    }),

    layoutRelatedTabs: cp.sectionReducer((section, action) => {
      return {relatedTabs: action.relatedTabs};
    }),

    brushChart: cp.sectionReducer((section, action) => {
    ...
    brushMinimap: cp.sectionReducer((section, action) => {
  ...
  cp.Element.register(ChartSection);
