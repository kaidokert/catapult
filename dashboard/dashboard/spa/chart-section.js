  ...
  class ChartSection extends cp.ElementBase {
    ...
    onMinimapBrush_(event) {
      this.dispatch('brushMinimap',
          this.statePath, event.detail.brushIndex, event.detail.value);
    }
    ...
    onDotClick_(event) {
      this.dispatch('dotClick', this.statePath, event.detail.ctrlKey,
          event.detail.lineIndex, event.detail.datumIndex);
    }
    ...
    onSelectRelatedTab_(event) {
      this.dispatch('selectRelatedTab', this.statePath, event.target.selected);
    }
    ...
    onSelectTestPathComponent_(event) {
      event.cancelBubble = true;
      this.dispatch('selectTestPathComponent', this.statePath,
          event.model.index);
    }

    onAggregateTestPathComponent_(event) {
  ...
  ChartSection.actions = {
    ...
    loadTimeseries: statePath => async (dispatch, getState) => {
      ...
      for (let i = 1; i < sequenceLength; i += 1) {
        ...
        datum.x = datum.normalized.x;
        datum.y = datum.normalized.y;
      ...
      for (let tpi = 0; tpi < colors.length; ++tpi) {
        ...
        const sequence = {
          color: '' + color,
          testPath,
          data: [{x: 1, y: y0, normalized: {x: 1, y: y0}}],
          strokeWidth: 1,
        };
