...
tr.exportTo('cp', () => {
  ...
  class ChartSection extends cp.Element {
    ...
    onDotMouseOver_(e) {
      this.dispatch('dotMouseOver',
          this.sectionId, e.detail.sequenceIndex, e.detail.datumIndex);
    }

    onDotMouseOut_(e) {
      this.dispatch('dotMouseOut',
          this.sectionId, e.detail.sequenceIndex, e.detail.datumIndex);
    }
  ...
  ChartSection.actions = {
    ...
    dotMouseOver: (sectionId, sequenceIndex, datumIndex) =>
      async (dispatch, getState) => {
        const section = getState().sectionsById[sectionId];
        if (section.chartLayout.sequences.length < 2) return;
        dispatch({
          type: 'chart-section.dotMouseOver',
          sectionId, sequenceIndex, datumIndex,
        });
      },

    dotMouseOut: (sectionId, sequenceIndex, datumIndex) =>
      async (dispatch, getState) => {
        dispatch({
