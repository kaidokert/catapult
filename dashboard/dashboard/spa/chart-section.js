...
tr.exportTo('cp', () => {
  ...
  ChartSection.actions = {
    connected: statePath => async (dispatch, getState) => {
      ...
      if (state.testSuite.selectedOptions.length) {
        dispatch(ChartSection.actions.testSuite(statePath));
      } else {
      ...
      for (const testSuite of testSuites) {
        ...
        (async () => {
          ...
          if (!tr.b.setsEqual(
            ...
            // handled by a separate dispatch of this action creator.
            return;
          }
  ...
  ChartSection.reducers = {
    ...
    describe: cp.ElementBase.statePathReducer((state, action) => {
      ...
      if (bot.selectedOptions.length === 0 ||
        ...
        bot = {...bot, selectedOptions: [...action.bots]};
      }
      ...
      if (action.complete) {
        ...
        bot = {
          ...bot,
          selectedOptions: bot.selectedOptions.filter(bot =>
            action.bots.has(bot)),
        };
        testCase = {
          ...testCase,
  ...
  ChartSection.newStateOptionsFromQueryParams = queryParams => {
    ...
    if (queryParams.maxRev) {
      options.maxRevision = parseInt(queryParams.maxRev);
    }
    if (queryParams.rel) {
      options.relatedTabName = queryParams.rel;
    }
    if (queryParams.norm !== undefined) {
      options.normalize = true;
    }
    if (queryParams.natural !== undefined) {
      options.fixedXAxis = false;
    }
    if (queryParams.zeroY !== undefined) {
      options.zeroYAxis = true;
    }
    if (queryParams.test !== undefined) {
