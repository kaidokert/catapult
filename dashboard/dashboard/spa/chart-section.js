...
tr.exportTo('cp', () => {
  ...
  ChartSection.reducers = {
    receiveHistograms: cp.statePathReducer((section, action) => {
      return {
        isLoading: false,
    ...
    chartClick: cp.statePathReducer((section, action) => {
      return {
        chartLayout: {
          ...section.chartLayout,
          brushes: [],
    ...
    dotClick: cp.statePathReducer((section, action) => {
    ...
    dotMouseOver: cp.statePathReducer((lines, action) => {
    ...
    layoutChart: cp.statePathReducer((section, action) => {
      return {
        isLoading: false,
        title: action.title,
    ...
    toggleChartOnly: cp.statePathReducer((section, action) => {
      return {onlyChart: !section.onlyChart};
    }),

    clearTimeseries: cp.statePathReducer((section, action) => {
      return {
        chartLayout: false,
        minimapLayout: false,
        histograms: undefined,
    ...
    startLoadingTimeseries: cp.statePathReducer((section, action) => {
      return {isLoading: true};
    }),

    layoutRelatedTabs: cp.statePathReducer((section, action) => {
      return {relatedTabs: action.relatedTabs};
    }),

    brushChart: cp.statePathReducer((section, action) => {
    ...
    brushMinimap: cp.statePathReducer((section, action) => {
