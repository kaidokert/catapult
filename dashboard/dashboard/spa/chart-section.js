...
tr.exportTo('cp', () => {
  ...
  ChartSection.reducers = {
    ...
    buildRelatedTabs: cp.ElementBase.statePathReducer((state, action) => {
      const relatedTabs = [];
      if (state.testSuite.selectedOptions.length > 1 &&
          state.testSuite.isAggregated) {
        relatedTabs.push({
          name: 'Test suites',
          sparklines: state.testSuite.selectedOptions.map(testSuite => {
            return {
              name: testSuite,
              lineDescriptors: [],
            };
      ...
      if (state.bot.selectedOptions.length > 1 &&
          state.bot.isAggregated) {
        relatedTabs.push({
          name: 'Bots',
          sparklines: state.bot.selectedOptions.map(bot => {
            return {
              name: bot,
              lineDescriptors: [],
            };
      ...
      if (state.testCase.selectedOptions.length > 1 &&
        ...
        relatedTabs.push({
          name: 'Test cases',
          sparklines: state.testCase.selectedOptions.map(testCase => {
            return {
              name: testCase,
              lineDescriptors: [],
            };
