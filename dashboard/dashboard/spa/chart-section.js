...
tr.exportTo('cp', () => {
        ...
        if (index === 0) {
          ...
          dispatch(cp.ElementBase.actions.updateObjectAtPath(
              `${statePath}.testPathComponents.1`, {
                options: ChartSection.groupNames(bots).map(option => {
          ...
          dispatch(cp.ElementBase.actions.updateObjectAtPath(
              `${statePath}.testPathComponents.2`, {
                options: ChartSection.groupNames(measurements),
  ...
  ChartSection.reducers = {
    ...
    dotClick: cp.ElementBase.statePathReducer((state, action) => {
      ...
      const xBrushes = [0.5 * (datumX + prevX), 0.5 * (datumX + nextX)];
      if (action.ctrlKey) {
    ...
    brushChart: cp.ElementBase.statePathReducer((state, action) => {
      const xBrushes = Array.from(state.chartLayout.xBrushes);
      xBrushes.splice(action.brushIndex, 1, action.value);
      return {...state, chartLayout: {...state.chartLayout, xBrushes}};
    }),

    brushMinimap: cp.ElementBase.statePathReducer((state, action) => {
      const xBrushes = Array.from(state.minimapLayout.xBrushes);
      xBrushes.splice(action.brushIndex, 1, action.value);
      return {...state, minimapLayout: {...state.minimapLayout, xBrushes}};
    }),
  };
