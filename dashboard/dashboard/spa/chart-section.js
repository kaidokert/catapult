...
tr.exportTo('cp', () => {
  ...
  ChartSection.actions = {
    ...
    describeTestSuites: statePath => async(dispatch, getState) => {
      ...
      // Wait to populateColumns until after finalizeParameters, which
      // might change selectedOptions.
  ...
  ChartSection.reducers = {
    ...
    buildRelatedTabs: (state, action, rootState) => {
      ...
      const anyBrowsing = state.testSuite.selectedOptions.filter(
      ...
      if (anyBrowsing && measurements.filter(measurement =>
        !rails.includes(measurement.split('_')[0])).length) {
      ...
      if (anyBrowsing && measurements.filter(measurement =>
        measurement.startsWith('Total:') &&
        ['count', 'duration'].includes(measurement.split(':')[1])).length) {
      ...
      if (anyBrowsing && measurements.filter(measurement =>
        measurement.includes('V8-Only:')).length) {
