...
tr.exportTo('cp', () => {
  ...
  ChartSection.newStateOptionsFromQueryParams = routeParams => {
    ...
    if (routeParams.testSuites) {
      options.parameters.testSuites = routeParams.testSuites.split(',');
    }
    if (routeParams.aggSuites !== undefined) {
      options.parameters.testSuitesAggregated = true;
    }
    if (routeParams.measurements) {
      options.parameters.measurements = routeParams.measurements.split(',');
    }
    if (routeParams.bots) {
      options.parameters.bots = routeParams.bots.split(',');
    }
    if (routeParams.splitBots !== undefined) {
      options.parameters.botsAggregated = false;
    }
    if (routeParams.testCases) {
      options.parameters.testCases = routeParams.testCases.split(',');
    }
    if (routeParams.splitCases !== undefined) {
      options.parameters.testCasesAggregated = false;
    }
    if (routeParams.statistics) {
      options.parameters.statistics = routeParams.statistics.split(',');
    }
    if (routeParams.minRev) {
      options.minRevision = parseInt(routeParams.minRev);
    }
    if (routeParams.maxRev) {
      options.maxRevision = parseInt(routeParams.maxRev);
    }
    if (routeParams.rel) {
      options.relatedTabName = routeParams.rel;
    }
    if (routeParams.norm !== undefined) {
      options.normalize = true;
    }
    if (routeParams.center !== undefined) {
      options.center = true;
    }
    if (routeParams.natural !== undefined) {
      options.fixedXAxis = false;
    }
    if (routeParams.zeroY !== undefined) {
      options.zeroYAxis = true;
    }
    if (routeParams.test !== undefined) {
      options.referenceBuild = false;
    }
    return options;
  };
  ...
  ChartSection.getRouteParams = state => {
    ...
    if (statistics.length > 1 || statistics[0] !== 'avg') {
      for (const statistic of statistics) {
        routeParams.append('statistic', statistic);
      }
    }
