...
tr.exportTo('cp', () => {
  ...
  ChartSection.actions = {
    ...
    dotMouseOver: (statePath, lineIndex) => async (dispatch, getState) => {
      dispatch({
        type: ChartSection.reducers.boldLegendEntry.typeName,
        statePath,
        lineIndex,
      });
    },
  ...
  ChartSection.reducers = {
    boldLegendEntry: cp.ElementBase.statePathReducer((state, action) => {
      if (!state.legend) return state;
      const selected = JSON.stringify(action.lineDescriptor ||
          state.chartLayout.lines[action.lineIndex].descriptor);
      function handleLegendEntry(entry) {
        if (entry.children) {
          return {...entry, children: entry.children.map(handleLegendEntry)};
        }
        let fontWeight = 'normal';
        if (JSON.stringify(entry.lineDescriptor) === selected) {
          fontWeight = 'bold';
        }
        return {...entry, fontWeight};
      }
