...
tr.exportTo('cp', () => {
  ...
  const PrefetchTestSuiteDescriptors = options => async(dispatch, getState) => {
    for await (const _ of dispatch(ReadTestSuiteDescriptors(options))) {
      // We don't actually need these descriptors here, we just want to prefetch
      // them, but ReadTestSuiteDescriptors only actually fetches the data if
      // the async generator is pumped.
    }
  };
