...
tr.exportTo('cp', () => {
  ...
  function computeTicks(range, numTicks = 5) {
    ...
    if (range.min < 0 && range.max > 0) {
      const numNegTicks = Math.round((numTicks - 1) * -range.min / range.range);

      let tick = range.min;
      let delta = range.min / numNegTicks;
      for (let i = 0; i < numNegTicks; ++i) {
        ticks.push(tick);
        tick -= delta;
      }
      ...
      const numPosTicks = numTicks - numNegTicks - 1;
      if (numPosTicks === 1) {
        ticks.push(range.max);
      } else {
        delta = range.max / numPosTicks;
        tick = delta;
        for (let i = 0; i < numPosTicks; ++i) {
          ticks.push(tick);
          tick += delta;
        }
      }
    } else {
      const delta = range.range / (numTicks - 1);
      let tick = range.min;
