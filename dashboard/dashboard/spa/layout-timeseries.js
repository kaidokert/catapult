...
tr.exportTo('cp', () => {
  function layoutTimeseries(state) {
    ...
    // Extend xRange in both directions to reduce clipping xAxis.ticks.
    const xExtension = 0.015;

    // Extend yRange in both directions to prevent clipping yAxis.ticks, which
    // are 15px tall and so can extend 7.5px (3.75% of 200px) above/below
    // extreme points if they happen to be a round number.
    const yExtension = 0.0375;
  ...
  function generateXTicksReducer(state, xRange, rawXs) {
    ...
    const ticks = computeTicks(xTickRange).map(text => {
      let x = text;
      if (rawXs) {
  ...
  function computeTicks(range, numTicks = 5) {
    ...
    range = tr.b.math.Range.fromExplicitRange(
        range.min + Math.abs(range.min % tickPower),
        range.max - Math.abs(range.max % tickPower));

    const lower = Math.max(0, range.min);
    const numPosTicks = Math.ceil(numTicks * (range.max - lower) / range.range);
    const delta = (range.max - lower) / numPosTicks;
    for (let tick = lower; tick < range.max; tick += delta) {
      ticks.push(tick);
    }

    if (range.min < 0) {
      const upper = Math.min(0, range.max);
      const numNegTicks = numTicks - numPosTicks;
      const delta = (upper - range.min) / numNegTicks;
      for (let tick = range.min; tick < upper; tick += delta) {
        if (tr.b.math.approximately(tick, 0, 1e-6)) continue;
        ticks.push(tick);
      }
    }
