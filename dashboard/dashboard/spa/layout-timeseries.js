...
tr.exportTo('cp', () => {
  ...
  const EPOCH_MS = new Date('2010-01-01T00:00:00').getTime();
  ...
  function calendarTick(ms, text, xPct, anchor) {
    const width = CHAR_SIZE_PX.width * text.length;
    let px = ESTIMATED_WIDTH_PX * xPct;
    if (anchor === 'start') {
  ...
  function formatEndDate(range) {
    ...
    if (range.max.getYear() === range.min.getYear()) {
      text = text.slice(5);
  ...
  function generateMonths(
    ...
    for (let month = minMonth; month <= maxMonth; ++month) {
      ...
      const tick = calendarTick(ms, text, xPct);
      if (maybeInsertTick(ticks, tick) && (month < maxMonth)) {
        generateDates(ticks, year, month, 1, daysInMonth(year, month),
            displayRange, rawMs);
      }
    }
  }
  ...
  function calendarTicks(dates, displayRange, rawMs) {
    ...
    if (rawMs.length < 10) {
      // TODO generate ticks for each point
      // return ticks;
    }

    if (dates.max.getYear() !== dates.min.getYear()) {
      generateMonths(ticks, displayYear(dates.min.getYear()),
          dates.min.getMonth() + 1, 11, displayRange, rawMs);
      for (let y = 1 + dates.min.getYear(); y <= dates.max.getYear(); ++y) {
        const text = displayYear(y).toString();
        const date = new Date(text + '-01-01T00:00:00');
        const ms = date.getTime();
        let x = ms;
        if (rawMs) {
          x = tr.b.findLowIndexInSortedArray(rawMs, x => x, x);
        ...
        const xPct = displayRange.normalize(x);
        const tick = calendarTick(ms, text, xPct);
        if (maybeInsertTick(ticks, tick) && (y < dates.max.getYear())) {
          generateMonths(ticks, displayYear(dates.max.getYear()), 2, 11,
              displayRange, rawMs);
        }
      }
      generateMonths(ticks, displayYear(dates.max.getYear()),
      ...
      generateDates(ticks, displayYear(dates.min.getYear()),
          dates.min.getMonth(), dates.min.getDate(), dates.max.getDate(),
          displayRange, rawMs);
    } else if (dates.max.getHour() !== dates.min.getHour()) {
      generateHours(ticks, displayYear(dates.min.getYear()),
      ...
      generateMinutes(ticks, displayYear(dates.min.getYear()),
          dates.min.getMonth(), dates.min.getDate(), dates.min.getHour(),
