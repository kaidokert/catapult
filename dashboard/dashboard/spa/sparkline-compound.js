...
export default class SparklineCompound extends ElementBase {
    ...
    return html`
      <cp-tab-bar selected="${this.selectedRelatedTabName}">
        ${this.relatedTabs.map((tab, tabIndex) => html`
          <cp-tab name="${tab.name}" @click="${this.onRelatedTabClick_}">
            ${tab.name}
          </cp-tab>
        `)}
      </cp-tab-bar>
  ...
  hideTile_(sparkline) {
    return !sparkline.isLoading && (sparkline.layout.lines.length === 0);
  }
