...
tr.exportTo('cp', () => {
  ...
  /**
   * ChartCompound synchronizes revision ranges and axis properties between two
  ...
  ChartCompound.actions = {
    ...
    // Copy lineDescriptors and other configuration to minimapLayout and
    // chartLayout. Only the first non-empty lineDescriptor to minimapLayout.
    // Maybe add a ref build line to chartLayout. Manage revision ranges.
  ...
  ChartCompound.reducers = {
    ...
    brushMinimap: (state, action, rootState) => {
      ...
      return {
        ...
        // unspecified.
        minRevision: (minRevision === state.minimapLayout.xAxis.range.min) ?
          undefined : minRevision,
        maxRevision: (maxRevision === state.minimapLayout.xAxis.range.max) ?
          undefined : maxRevision,
      };
    },
