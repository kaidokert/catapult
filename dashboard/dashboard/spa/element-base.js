...
tr.exportTo('cp', () => {
  ...
  ElementBase.statePathReducer = reducer => {
    const replacement = (state, action) => {
      try {
        return Polymer.Path.setImmutable(
            state, action.statePath, value => reducer(value, action));
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error(replacement.typeName, error, action, state);
        return state;
      }
    };
  ...
  function selfAwarePromise(ignoramus) {
    const socrates = (async () => {
      try {
        return await ignoramus;
      } finally {
  ...
  ElementBase.asyncStream = async function* asyncStream(promises, opt_minMs) {
    const minMs = (opt_minMs === undefined) ? 500 : opt_minMs;
    ...
    while (promises.length) {
      ...
      // Remove the timeout even if it hasn't fired yet.
      promises = promises.filter(p => !p.isResolved && !p.isTimeout);
      ...
      if (result !== NONCE) {
        ...
        if (promises.length > 0) {
          ...
          // It doesn't matter if the timeout is self aware.
          const timeout = (async () => {
            await cp.ElementBase.timeout(minMs);
            return NONCE;
          })();
