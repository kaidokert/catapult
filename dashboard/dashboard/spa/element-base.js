...
tr.exportTo('cp', () => {
  ...
  class ElementBase extends ReduxMixin(Polymer.Element) {
     ...
     * @param {boolean} opt_options.assignObject
     */
    static setStateAtPath(state, path, value, opt_options) {
      ...
      } else if (opt_options && opt_options.assignObject) {
        value = {
          ...existing,
          ...value,
        };
  ...
  REDUCERS.set('element-base.assignObjectAtPath', ElementBase.statePathReducer(
      ((state, action) => action.value), {assignObject: true}));
