...
tr.exportTo('cp', () => {
  ...
  ElementBase.register = subclass => {
    ...
    if (subclass.actions && subclass.actions !== ElementBase.actions) {
      for (const [name, action] of Object.entries(subclass.actions)) {
        const debugName = `${subclass.name}.actions.${name}`;
        const actionReplacement = (...args) => {
          ...
          Object.defineProperty(thunkReplacement, 'name', {value: debugName});
          return thunkReplacement;
        };
        Object.defineProperty(actionReplacement, 'name', {value: debugName});
        subclass.actions[name] = actionReplacement;
      }
    }
