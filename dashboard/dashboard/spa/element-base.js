...
tr.exportTo('cp', () => {
  ...
  class ElementBase extends ReduxMixin(Polymer.Element) {
          ...
          statePath(state) {
            try {
              return ElementBase.getStateAtPath(
                  state, this[statePathPropertyName].concat([name]));
            } catch (error) {
              // eslint-disable-next-line no-console
              console.error({error, this: this, statePathPropertyName,
                statePath: this[statePathPropertyName], state, name,
              });
              return undefined;
            }
          },
    ...
    static statePathReducer(reducer, opt_options) {
      return (state, action) => {
        ...
        try {
          ...
          // eslint-disable-next-line no-console
          console.error({error, state, action, value, opt_options});
          return state;
        }
      };
    }
    ...
    static setStateAtPath(state, path, value, opt_options) {
      ...
      if (path.length === 0) {
        // Recursive base case.
        if (opt_options && opt_options.updateObject) {
          return {
            ...state,
            ...value,
          };
