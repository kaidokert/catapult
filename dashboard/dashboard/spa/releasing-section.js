...
tr.exportTo('cp', () => {
  ...
  ReleasingSection.actions = {
    ...
    restoreFromQueryParams: (statePath, queryParams) =>
      async (dispatch, getState) => {
        ...
        dispatch(ReleasingSection.selectSource(statePath, selectedOptions));
      },
  ...
  ReleasingSection.reducers = {
    focusSource: (state, action) => cp.ElementBase.setStateAtPath(
        cp.ChromeperfApp.clearAllFocused(state),
        action.statePath.concat(['source', 'isFocused']),
        action.isFocused),
    ...
    selectSource: cp.statePathReducer((section, action) => {
      return {
    ...
    toggleEditing: cp.statePathReducer((section, action) => {
