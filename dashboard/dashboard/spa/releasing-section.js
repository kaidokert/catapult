...
tr.exportTo('cp', () => {
  ...
  ReleasingSection.reducers = {
    ...
    requestReports: cp.ElementBase.statePathReducer((state, action) => {
      const source = {...state.source};
      source.inputValue = source.selectedOptions.join(',');
      return {...state, isLoading: true, source};
    }),
  ...
  ReleasingSection.newState = options => {
    ...
    return {
      ...
      isOwner: false,
      source: {
        inputValue: ReleasingSection.DEFAULT_SOURCE,
  ...
  ReleasingSection.getQueryParams = state => {
