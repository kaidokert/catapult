  ...
  AlertsControls.reducers = {
    ...
    removeOrUpdateAlerts: (state, action, rootState) => {
      if (state.showingTriaged || action.bugId === 0) {
        return AlertsControls.reducers.updateBugId(state, action, rootState);
      }
      return AlertsControls.reducers.removeAlerts(state, action, rootState);
    },

    openNewBugDialog: (state, action, rootState) => {
      const alerts = AlertsControls.getSelectedAlerts(state.alertGroups);
    ...
    receiveAlerts: (state, action, rootState) => {
      ...
      alertGroups = alertGroups.map((alerts, groupIndex) => {
        alerts = alerts.map(AlertsControls.transformAlert);
        return {
          isExpanded: false,
          alerts,
          triaged: {
      ...
      alertGroups = AlertsControls.sortGroups(
      ...
      return AlertsControls.reducers.updateColumns({
