...
tr.exportTo('cp', () => {
  ...
  class ReportRequest extends RequestBase {
    constructor(options) {
      super(options);
      this.queryParams_ = new URLSearchParams();
      this.queryParams_.set('name', options.name);
      this.queryParams_.set('minRev', options.minRevision);
      this.queryParams_.set('maxRev', options.maxRevision);
    }
        ...
        tables: [
          {
            ...
            isOwner: true,
            isEditing: false,
            currentVersion: '517411',
            referenceVersion: '508578',
            template: {
              owners: 'benjhayden@chromium.org',
              name: 'ChromiumPerfPublicReport',
              rows: [
                cp.ReportSection.newTemplateRow({
                  options: new TestSuitesRequest({}).localhostResponse_,
                  count: 123,
                }),
              ],
            },
