...
tr.exportTo('cp', () => {
  ...
  class ReportTemplateRequest extends RequestBase {
    constructor(options) {
      ...
      for (const owner of options.owners) this.body_.append('owner', owner);
      if (options.url) this.body_.set('url', options.url);
      for (const row of options.rows) this.body_.append('row', row);
    }
