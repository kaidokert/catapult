...
tr.exportTo('cp', () => {
  ...
  class CacheBase {
    ...
    async fetch_() {
      throw new Error('subclasses must override fetch_()');
    }
    ...
    async read() {
      ...
      const promise = this.fetch_();
      this.onStartRequest_(promise);
      const result = await promise;
      this.onFinishRequest_(result);
      return result;
    }
  }
  ...
  class TestSuitesCache extends CacheBase {
    computeCacheKey_() {
      let internal = '';
  ...
  class TestSuiteDescriptorsCache extends CacheBase {
    buildRequest_() {
      return new DescribeRequest(this.options_);
    }
  }

  const ReadTestSuiteDescriptors = options => async function*(dispatch, getState) {
    for await (const mergedDescriptor of merged) {
