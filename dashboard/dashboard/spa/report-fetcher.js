...
export class ReportFetcher {
  constructor(info, revisions, minMinRevision) {
    this.info_ = info;
    this.revisions_ = revisions;
    this.minMinRevision_ = minMinRevision;
...
// Before the first fetchRevisions_ finishes, other findRange() calls should
// wait for that. After it finishes, if still no revisions, then the first
// findRange() call should return {} but the others should wait for that
// fetchRevisions_. After that finishes, if still no revisions, then the next
// findRange() call should return {} but the others should wait for that
// fetchRevisions_.
...
export class RangeFinder {
  ...
  async findRange(revisions) {
    ...
    // history for their fetchDescriptor, mergeRevisions_, and maybe future
  ...
  isSameSchedule_(revision) {
    const result = isSameSchedule(revision, this.revisions_[0],
