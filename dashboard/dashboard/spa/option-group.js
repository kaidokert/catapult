  ...
  OptionGroup.actions = {
    toggleGroupExpanded: (statePath, optionIndex) =>
      async (dispatch, getState) => {
        const optionStatePath = statePath.concat(['options', optionIndex]);
        const option = Polymer.Path.get(getState(), optionStatePath);
        dispatch(cp.ElementBase.actions.updateObjectAtPath(
            optionStatePath, {isExpanded: !option.isExpanded}));
      },
  };
