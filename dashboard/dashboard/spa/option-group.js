...
tr.exportTo('cp', () => {
  class OptionGroup extends cp.ElementBase {
    ...
    isExpanded_(option, query) {
      ...
      return (query && (query.length >= 2)) || (option && option.isExpanded);
    }

    matches_(option, query) {
      if (!query) return true;
      return OptionGroup.matches(option, query.split(' '));
    }
    ...
    isRoot_() {
      return this.statePath === this.rootStatePath ||
        this.statePath === `${this.rootStatePath}.magic`;
    }

    onSelect_(event) {
      this.dispatch('select', this.rootStatePath, event.target.checked,
          OptionGroup.getValuesFromOption(event.model.option));
  ...
  OptionGroup.matches = (option, queryParts) => {
    ...
    if (option.value) option = option.value;
  ...
  OptionGroup.collect = (options, filter) => {
    const results = [];
    return results;
  };
