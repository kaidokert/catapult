...
tr.exportTo('cp', () => {
  class ChartPair extends cp.ElementBase {
    ...
    onToggleNormalize_(event) {
      ...
      if (this.isLinked) {
        this.dispatch('toggleLinkedNormalize', this.linkedStatePath);
      }
    }
    ...
    onToggleCenter_(event) {
      ...
      if (this.isLinked) {
        this.dispatch('toggleLinkedCenter', this.linkedStatePath);
      }
    }
    ...
    onLinkedCursorRevisionChange_() {
      if (!this.isLinked) return;
      cp.todo('onLinkedCursorRevisionChange_');
    }

    onLinkedMinRevisionChange_() {
      if (!this.isLinked) return;
      cp.todo('onLinkedMinRevisionChange_');
    }

    onLinkedMaxRevisionChange_() {
      if (!this.isLinked) return;
      cp.todo('onLinkedMaxRevisionChange_');
    }

    onLinkedNormalizeChange_() {
      if (!this.isLinked) return;
      if (this.normalize === this.linkedNormalize) return;
      this.dispatch('toggleNormalize', this.statePath);
    }

    onLinkedCenterChange_() {
      if (!this.isLinked) return;
      if (this.center === this.linkedCenter) return;
      this.dispatch('toggleCenter', this.statePath);
    }

    onLinkedZeroYAxisChange_() {
      if (!this.isLinked) return;
      if (this.zeroYAxis === this.linkedZeroYAxis) return;
      this.dispatch('toggleZeroYAxis', this.statePath);
    }

    onLinkedFixedXAxisChange_() {
      if (!this.isLinked) return;
  ...
  ChartPair.properties = {
    ...
    ...cp.ElementBase.statePathProperties('linkedStatePath', {
      linkedCursorRevision: {
        type: Number,
        observer: 'onLinkedCursorRevisionChange_',
      },
      linkedMinRevision: {
        type: Number,
        observer: 'onLinkedMinRevisionChange_',
      },
      linkedMaxRevision: {
        type: Number,
        observer: 'onLinkedMaxRevisionChange_',
      },
      linkedNormalize: {
        type: Boolean,
        observer: 'onLinkedNormalizeChange_',
      },
      linkedCenter: {
        type: Boolean,
        observer: 'onLinkedCenterChange_',
      },
      linkedZeroYAxis: {
        type: Boolean,
        observer: 'onLinkedZeroYAxisChange_',
      },
      linkedFixedXAxis: {
        type: Boolean,
        observer: 'onLinkedFixedXAxisChange_',
      },
    }),
  };
  ...
  ChartPair.actions = {
    toggleLinked: (statePath, linkedStatePath) => async(dispatch, getState) => {
      ...
      dispatch(cp.ElementBase.actions.updateObject(statePath, {
        ...
        normalize: linkedState.linkedNormalize,
        center: linkedState.linkedCenter,
    ...
    toggleLinkedNormalize: linkedStatePath => async(dispatch, getState) => {
      dispatch(cp.ElementBase.actions.toggleBoolean(
          `${linkedStatePath}.linkedNormalize`));
    },
    ...
    toggleLinkedCenter: linkedStatePath => async(dispatch, getState) => {
      dispatch(cp.ElementBase.actions.toggleBoolean(
          `${linkedStatePath}.linkedCenter`));
    },
