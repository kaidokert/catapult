...
tr.exportTo('cp', () => {
  ...
  TimeseriesCache.actions = {
    testSuites: () => async (dispatch, getState) => {
      ...
      if (rootState[path]) return rootState[path];
      const testSuites = await TimeseriesCache.fetchTestSuites(
          rootState.authHeaders);
      const groupedTestSuites = cp.OptionGroup.groupValues(testSuites);
      dispatch(cp.ElementBase.actions.updateObject(
          '', {[path]: groupedTestSuites}));
