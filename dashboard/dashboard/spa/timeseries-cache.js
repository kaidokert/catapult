...
tr.exportTo('cp', () => {
  ...
  TimeseriesCache.fetchDescribe = async (headers, testSuite) => {
    const fetchMark = tr.b.Timing.mark('fetch', 'describe');
    let responseJson;
    if (location.hostname === 'localhost') {
      responseJson = {
        bots: cp.dummyBots([testSuite]),
        measurements: cp.dummyMeasurements([testSuite]),
        test_cases: cp.dummyTestCases([testSuite]),
      };
    } else {
      const url = `/api/describe/${testSuite}`;
      headers = new Headers(headers);
  ...
  TimeseriesCache.actions = {
    ...
    describe: testSuite => async (dispatch, getState) => {
      ...
      let descriptor = await promise;
      descriptor = {
        measurements: descriptor.measurements,
        bots: descriptor.bots,
        testCases: descriptor.test_cases,
      };
      dispatch(cp.ElementBase.actions.updateObject(
