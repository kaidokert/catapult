...
tr.exportTo('cp', () => {
  ...
  class TimeseriesCache extends cp.CacheBase {
    ...
    async* reader() {
      ...
      const listener = new ServiceWorkerListener(
          location.origin + request.url_);
  ...
  class ServiceWorkerListener {
    constructor(url) {
      this.url = url;
      this.supportForSW = ('serviceWorker' in navigator);
    }
    [Symbol.asyncIterator]() {
      return this;
    }
    next() {
      // Do nothing for browsers that do not support Service Workers.
      if (!this.supportForSW) {
      ...
      return new Promise((resolve, reject) => {
        const handler = event => {
          ...
          // Only accept messages for this request, identified by URL.
          ...
          switch (type) {
            ...
            case 'TIMESERIES_FINISHED':
              ...
              return;
            default:
              reject(new Error(`Invalid SW message type: ${type}`));
          }
        };
