...
tr.exportTo('cp', () => {
  ...
  FastHistogram.fromRow = (dict, statistic, conversionFactor) => {
  ...
  class TimeseriesRequest extends cp.RequestBase {
    ...
    constructor(options) {
      ...
      this.queryParams_.set('columns', this.columns_);
    ...
    postProcess_(json) {
      ...
      // TODO merge updates into this.timeseries_
      this.timeseries_ = json.data.map(row => FastHistogram.fromRow(
          csvRow(this.columns_, row), this.statistic_, conversionFactor));
      console.log(json, this.timeseries_);
    }
