...
tr.exportTo('cp', () => {
  ...
  TimeseriesCache.actions = {
    ...
    describeTestSuites: (testSuites, opt_getDelayPromise) =>
      async function* describeTestSuitesActual(dispatch, getState) {
        const promises = testSuites.map(testSuite =>
          dispatch(TimeseriesCache.actions.describe(testSuite)));
        ...
        const batches = cp.RequestBase.batchResponses(
            promises, opt_getDelayPromise);
        for await (const {results} of batches) {
          for (const descriptor of results) {
            if (!descriptor) continue;
            if (descriptor.unparsed) {
              // eslint-disable-next-line no-console
              console.log(descriptor.unparsed);
              continue;
            }
            // Merge the descriptor into mergedDescriptor.
            for (const bot of descriptor.bots) mergedDescriptor.bots.add(bot);
            for (const measurement of descriptor.measurements) {
              mergedDescriptor.measurements.add(measurement);
            }
            for (const testCase of descriptor.testCases) {
              mergedDescriptor.testCases.add(testCase);
            }
          }
    ...
    describe: testSuite => async(dispatch, getState) => {
      ...
      const rootState = getState();
      if (!rootState.testSuiteDescriptors) {
        dispatch(cp.ElementBase.actions.updateObject(
            '', {testSuiteDescriptors: {}}));
      } else if (rootState.testSuiteDescriptors[cacheKey]) {
        return await rootState.testSuiteDescriptors[cacheKey];
      }
      ...
      if (descriptor.measurements.length > 0 &&
          descriptor.bots.length > 0) {
        dispatch(cp.ElementBase.actions.updateObject(
            'testSuiteDescriptors', {[cacheKey]: descriptor}));
      } else {
        // Don't cache invalid descriptors. Allow callers/users to try
        // describing this testSuite again.
        dispatch(cp.ElementBase.actions.updateObject(
            'testSuiteDescriptors', {[cacheKey]: undefined}));
      }
      return descriptor;
    },
