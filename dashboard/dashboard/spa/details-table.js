...
export class DetailsTable extends ElementBase {
  ...
  renderNoteCell_(cell, bodyIndex, cellIndex) {
    ...
    const onSubmit = (event) => this.onNoteSubmit_(event, bodyIndex, cellIndex);
    return html`
  ...
  renderNote_(note, bodyIndex, cellIndex, noteIndex) {
    ...
    const onSubmit = (event) =>
      this.onNoteSubmit_(event, bodyIndex, cellIndex, noteIndex);
    return html`
  ...
  onNoteSubmit_(event, bodyIndex, cellIndex, noteIndex) {
    const cellPath = [
    ...
    if (noteIndex === undefined) {
      // This note was just created.
      const notes = [...cell.notes, event.detail.note];
      STORE.dispatch(UPDATE(cellPath, {notes}));
      return;
    }

    const editPath = [cellPath, 'notes', noteIndex].join('.');
    STORE.dispatch(UPDATE(editPath, event.detail.note));
  }
