...
DetailsTable.buildCell = (
  ...
  for (const alert of alerts) {
    let tooltip = '';
    if (masterWhitelist && !masterWhitelist.includes(alert.master)) {
      tooltip = `Unable to bisect on ${alert.master} bots`;
    }
    if (suiteBlacklist && suiteBlacklist.includes(alert.suite)) {
      tooltip = `Unable to bisect suite "${alert.suite}"`;
    }

    alert.bisect = BisectDialog.buildState({
      ...alert,
      alertKeys: [alert.key],
