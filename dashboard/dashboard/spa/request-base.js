...
tr.exportTo('cp', () => {
  ...
  class CacheBase {
    ...
    // Some caches need to compute cacheKey async, some sync.
    computeCacheKeySync_() {
    }
    async computeCacheKeyAsync_() {
      throw new Error('subclasses must override either ' +
        'computeCacheKeySync_ or computeCacheKeyAsync_');
    }
    ...
    async read() {
      ...
      this.cacheKey_ = this.computeCacheKeySync_();
      if (this.cacheKey_ === undefined) {
        this.cacheKey_ = await this.computeCacheKeyAsync_();
      }
