...
tr.exportTo('cp', () => {
  ...
  class CacheBase {
    ...
    createRequest_() {
      throw new Error(
          'subclasses must override either createRequest_() or fetch_()');
    }

    async fetch_() {
      return await this.createRequest_().response;
    }

    onStartRequest_(promise) {
      // Subclasses may override this to store promise in the redux store.
    }

    onFinishRequest_(result) {
      // Subclasses may override this to store result in the redux store.
    }
    ...
    async read() {
      ...
      this.cacheKey_ = await this.computeCacheKey_();
