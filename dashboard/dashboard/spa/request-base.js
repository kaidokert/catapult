...
tr.exportTo('cp', () => {
  ...
  class CacheBase {
    ...
    async fetch_() {
      ...
      if (!(request instanceof cp.RequestBase)) {
        throw new Error(
            'createRequest()_ must return an instanceof cp.RequestBase'
        );
      }
      ...
      try {
        ...
        if (this.cacheKey_ instanceof Promise) {
          ...
          try {
            ...
            console.error('Error computing cache key', err);
          }
        }
        ...
        if (this.isInCache_) {
          try {
            ...
            console.error('Error while reading from Redux cache', err);
          }
        }
