...
tr.exportTo('cp', () => {
  ...
  async function measureText(textContent, opt_options) {
    ...
    if (measureText.CACHE.has(cacheKey)) {
      return await measureText.CACHE.get(cacheKey);
    }
    ...
    measureText.HOST.appendChild(span);

    const promise = cp.measureElement(span);
    measureText.CACHE.set(cacheKey, promise);
    const rect = await promise;
    measureText.CACHE.set(cacheKey, rect);
    measureText.HOST.removeChild(span);
    return rect;
  }

  measureText.CACHE = new Map();

  measureText.HOST = document.createElement('div');
  measureText.HOST.style.position = 'fixed';
  measureText.HOST.style.visibility = 'hidden';
  measureText.HOST.style.zIndex = -1000;
  window.addEventListener('load', () =>
    document.body.appendChild(measureText.HOST));
