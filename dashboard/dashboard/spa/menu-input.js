  ...
  arrowCursor: (state, {key, statePath}, rootState) => {
    ...
    if (key === 'ArrowUp') {
      if (indices.length) {
        ...
        cursorOption = get(state.options, indices.join('.options.'));
        while (cursorOption && cursorOption.options &&
            cursorOption.options.length && cursorOption.isExpanded) {
          indices.push(cursorOption.options.length - 1);
          cursorOption = cursorOption.options[cursorOption.options.length - 1];
        }
      } else {
        indices.push(state.options.length - 1);
      }
