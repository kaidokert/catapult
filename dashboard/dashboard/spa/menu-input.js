...
MenuInput.reducers = {
  ...
  arrowCursor: (state, {key, statePath}, rootState) => {
    ...
    let cursorRelPath;
    if (state.cursor) {
      cursorRelPath = state.cursor.substr(statePath.length + 1);
      cursorOption = get(state, cursorRelPath);
      indices.push(...cursorRelPath.split('.options.'));
    }
    ...
    if (key === 'ArrowUp') {
      ...
      if (cursorOption && cursorOption.options && cursorOption.options.length &&
        ...
        return setImmutable(state, cursorRelPath, {isExpanded: false});
      }
    } else if (key === 'ArrowRight') {
      // If the option at cursor has children, expand it.
      if (cursorOption && cursorOption.options && cursorOption.options.length &&
          !cursorOption.isExpanded) {
        return setImmutable(state, cursorRelPath, {isExpanded: true});
      }
    } else {
      return state;
    }
    ...
    for (let i = 0; i < indices.length; ++i) {
      ...
      if (!option.isExpanded) {
        indices.splice(i, indices.length - 1);
        break;
      }
