...
export default class TimeseriesCacheRequest extends CacheRequestBase {
  ...
  async getSlices_() {
    ...
    // Sometimes the client will request the same data in multiple different
    // ways: the minimap and the main chart both request XY data for the brushed
    ...
    return missingRanges.map(revisionRange => new TimeseriesSlice({
      columns,
      revisionRange,
      testSuite: this.testSuite_,
      measurement: this.measurement_,
  ...
  get generateResults() {
    return async function* () {
      ...
      const sliceResponses = slices.map(slice => slice.responsePromise);
