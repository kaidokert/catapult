...
def SkiaFileBugHandlerPost():
  ...
  print("===== received json: ", request.is_json)
  if not utils.IsValidSheriffUser():
    return make_response(
      json.dumps({'error': 'You must be logged in to file bugs.'}), 401)
  ...
  if not keys:
    return make_response(
        json.dumps({'error': 'No anomaly keys specified to add bugs to.'}), 400)
