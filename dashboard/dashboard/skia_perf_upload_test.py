...
class SkiaPerfUploadTest(testing_common.TestCase):
  ...
  def testUploadRowExternal(self, mock_gcs):
    ...
    self.assertEqual(len(mock_gcs.mock_calls), 8)
    ...
    init_call = mock_gcs.mock_calls[5]
    write_call = mock_gcs.mock_calls[6]

    mock_args, mock_kwargs = init_call.args, init_call.kwargs

    self.assertTrue(mock_args[0].startswith('/chrome-perf-non-public/'))
    self.assertTrue((
        'ChromiumAndroid/android-cronet-arm-rel/resource_sizes (CronetSample.apk)/'
        'InstallSize/APK size/668729/') in mock_args[0])
    self.assertEqual(mock_args[1], 'w')
  ...
  @mock.patch('dashboard.common.cloud_metric._PublishTSCloudMetric',
              mock.MagicMock())
  @mock.patch.object(skia_perf_upload, 'cloudstorage')
  def testUploadRowExternalWebRTC(self, mock_gcs):
    testing_common.AddTests(['WebRTCPerf'], ['android-cronet-arm-rel'], {
        'resource_sizes (CronetSample.apk)': {
            'InstallSize': {
                'APK size': {}
            }
    ...
    rows = testing_common.AddRows(
        ('WebRTCPerf/android-cronet-arm-rel/'
         'resource_sizes (CronetSample.apk)/InstallSize/APK size'),
        {668729: row_params})

    skia_data_expected = {
        'version': 1,
        'git_hash': 'a7acc4dd8d303310fb1bd2cfafbc032f308b1fbc',
        'key': {
            'master': 'WebRTCPerf',
            'bot': 'android-cronet-arm-rel',
            'benchmark': 'resource_sizes (CronetSample.apk)'
    ...
    self.assertTrue(mock_args[0].startswith('/webrtc-perf-public/'))
    self.assertTrue(
        ('WebRTCPerf/android-cronet-arm-rel/resource_sizes (CronetSample.apk)/'
         'InstallSize/APK size/668729/') in mock_args[0])
    self.assertEqual(mock_args[1], 'w')
    self.assertEqual(mock_kwargs['content_type'], 'application/json')
    ...
    self.assertTrue(mock_args[0].startswith('/webrtc-perf-non-public/'))
    self.assertTrue(
        ('WebRTCPerf/android-cronet-arm-rel/resource_sizes (CronetSample.apk)/'
         'InstallSize/APK size/668729/') in mock_args[0])
    self.assertEqual(mock_args[1], 'w')
