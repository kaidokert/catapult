...
@ndb.tasklet
def _DescribeTestKey(key, measurements, bots, cases):
  desc = yield descriptor.Descriptor.FromTestPathAsync(utils.TestPath(key))
  bots.add(desc.bot)
  if desc.measurement:
...
def _BufferFuture(running_futures, future, buffer_size=1000):
  running_futures.append(future)
  if len(running_futures) < buffer_size:
    return

  running_futures[:] = [f for f in running_futures if _IsRunning(f)]

  while len(running_futures) > (buffer_size / 2):
    done_future = ndb.Future.wait_any(running_futures)
    done_future.check_success()
    running_futures.remove(done_future)
...
def _UpdateDescriptor(test_suite, namespace, start_cursor=None,
  ...
  try:
    for key in query_iter:
      key_count += 1
      _BufferFuture(describe_futures, _DescribeTestKey(
          key, measurements, bots, cases))
  ...
  ndb.Future.wait_all(describe_futures)
  for future in describe_futures:
    future.check_success()

  logging.info('%d keys, %d measurements, %d bots, %d cases',
               key_count, len(measurements), len(bots), len(cases))
  logging.info('per_key:wall_us=%f', round(1e6 * (time.time() - start) / key_count))
