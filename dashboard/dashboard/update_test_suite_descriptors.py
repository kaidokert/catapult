...
# Some test suites are too big to update every day. We can manually update them
# in dev_console when necessary.
BLACKLIST_KEY = 'update_test_suite_descriptors_blacklist'
...
def UpdateTestSuiteDescriptors(namespace):
  ...
  blacklist = stored_object.Get(BLACKLIST_KEY)
  for test_suite in stored_object.Get(key):
    if test_suite not in blacklist:
