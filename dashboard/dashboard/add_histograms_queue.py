...
def _MakeRowDict(revision, test_path, tracing_histogram, stat_name=None):
  ...
  has_story = reserved_infos.STORIES.name in tracing_histogram.diagnostics
  if trace_url_set and has_story:
...
def _AddStdioUris(tracing_histogram, row_dict):
  ...
  if len(log_urls) == 1:
    ...
    for idx, link_list in enumerate(log_urls):
      _AddStdioUri('a_stdio_uri_%d' % idx, link_list, row_dict)


def _AddStdioUri(name, link_list, row_dict):
  _CheckRequest(isinstance(link_list, list),
                'Log URLs must be provided as link tuples')
  row_dict[name] = '[%s](%s)' % tuple(link_list)
