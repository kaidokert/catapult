...
class FetchError(Error):
  ...
  def __init__(self, error):
    super(FetchError,
          self).__init__('Failed fetching project configs: %s' % (error))
    self.error = error
...
class DiscoveryError(Error):
  ...
  def __init__(self, error):
    super(DiscoveryError,
          self).__init__('Service discovery failed: %s' % (error))
    self.error = error
...
class InvalidConfigFieldError(Error):
  ...
  def __init__(self, error, config):
    super(InvalidConfigFieldError, self).__init__(
...
class StorageError(Error):
  ...
  def __init__(self, operation, error):
    super(StorageError,
          self).__init__('Failed to call \'%s\': %s' % (operation, error))
    self.operation = operation
    self.error = error


class NoMatchingConfigs(Error):

  def __init__(self, request):
    super(NoMatchingConfigs,
          self).__init__('No matching configs for request: < %s >' % (request))
...
def StoreConfigs(client, configs):
  ...
  def Transform(config):
    missing_fields = required_fields.difference(config.keys())
    ...
    key = client.key(
        'Subscription',
        '{config_set}:{revision}:{content_hash}'.format(**config),
...
def FindMatchingConfigs(client, request):
  ...
  with client.transaction():
    ...
    config_set_key_ids = subscription_index['config_sets']
    if len(config_set_key_ids) > 1:
      config_set_keys = [
          client.key('Subscription', key, parent=subscription_index_key)
          for key in config_set_key_ids
      ]
      config_sets = client.get_multi(config_set_keys)
    elif len(config_set_key_ids) == 1:
      config_sets = [
          client.get(
              client.key(
                  'Subscription',
                  config_set_key_ids[0],
                  parent=subscription_index_key))
      ]
    else:
      raise NoMatchingConfigs(request)
