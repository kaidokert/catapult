...
def StoreConfigs(client, configs):
  ...
  def Transform(config):
    ...
    key = client.key(
        'Subscription',
        [config['config_set'], config['revision'],
         config['content_hash']].join(':'),
  ...
  entities = [Transform(config) for config in configs]
  with client.transaction():
    ...
    try:
      if len(entities) > 1:
        client.get_multi([key for key, _ in entities], missing=missing_keys)
      else:
        key, _ = entities[0]
        client.get(key)
    ...
    try:
      ...
      def IsMissing(key_entity_pair):
        key, _ = key_entity_pair
        return key in missing_keys

      entities_to_put = [entity for _, entity in filter(entities, IsMissing)]
      if len(entities_to_put) > 1:
        client.put_multi(entities_to_put)
      else:
        _, entity = entities_to_put[0]
        client.put(entity)
    except ValueError as error:
