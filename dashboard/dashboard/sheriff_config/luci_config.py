...
class Matcher(object):
  ...
  def __init__(self, subscription):
    ...
    self.match_subscription = CompileRules(rules)

    if subscription.auto_triage.enable:
      self.match_auto_triage = lambda s: True
    else:
      self.match_auto_triage = CompileRules(subscription.auto_triage.rules)
...
def GetMatcher(revision, subscription):
  ...
  GetMatcher.cache[subscription.name] = (revision, matcher)
  return matcher
...
def FindMatchingConfigs(client, request):
  ...
  for config_set, revision, subscription in ListAllConfigs(client):
    ...
    if matcher.match_subscription(request.path):
      subscription.auto_triage.enable = matcher.match_auto_triage(
          request.path)
