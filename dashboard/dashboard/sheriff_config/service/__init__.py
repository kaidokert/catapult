...
def CreateApp(test_config=None):
  ...
  if test_config is None:
    ...
    domain_parts = {
        ...
        'GAE_SERVICE': os.envion.get('GAE_SERVICE', ''),
    }
    empty_keys = [k for k, v in domain_parts.iteritems() if len(v) == 0]
    if len(empty_keys):
      raise MissingEnvironmentVars(empty_keys)
    domain = '{GAE_SERVICE}-dot-{GAE_APPLICATION}.appspot.com'.format(
        **domain_parts)
  else:
