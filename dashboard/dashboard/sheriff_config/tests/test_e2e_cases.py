...
class LuciContentChangesTest(unittest.TestCase):
  ...
  def _MatchProject1(self, client, code):
    response = client.post(
        '/subscriptions/match',
        json={
            'path': 'Master/Bot/Test/Metric/Something',
  ...
  def _MatchProject2(self, client, code):
    response = client.post(
        '/subscriptions/match',
        json={
            'path': 'project/platform/not-important/memory_peak',
  ...
  def testPollConfigAddsAndRemoves(self):
    ...
    self._MatchProject1(client, 200)
    self._MatchProject2(client, 200)

    # Step 2: Get another configuration, this time with just one config set.
    response = client.get('/configs/update')
    self.assertEqual(response.status_code, 200)

    self._MatchProject1(client, 404)
    self._MatchProject2(client, 200)
