  ...
  for bot, benchmark_dict in jobs_by_bot.iteritems():
    ...
    for benchmark, values in benchmark_dict.iteritems():
      for k, v in values.iteritems():
        h = _CreateHistogram(k, 'count', story=benchmark, summary_options=opts)
        h.AddSample(v)
        hists.append(h)
        summaries[k] += v
    ...
    for k, v in summaries.iteritems():
      ...
      if k != 'total':
        h = _CreateHistogram(k + '.normalized', 'count')
...
@ndb.synctasklet
def _DoPinpointStats(offset=0):
  _ProcessPinpointStats(offset=offset)
