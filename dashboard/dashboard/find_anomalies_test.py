...
class ProcessAlertsTest(testing_common.TestCase):
  ...
  def testProcessTest_BotIdBeforeAndAfter_AreSetIfPresentInRowsEntities(self):
    testing_common.AddTests(['ChromiumGPU'], ['linux-release'], {
        'scrolling_benchmark': {
            'ref': {}
        },
    ...
    new_anomalies = anomaly.Anomaly.query().fetch()
    self.assertEqual(1, len(new_anomalies))
    self.assertEqual('device-id-39', new_anomalies[0].bot_id_before_anomaly)
    self.assertEqual('device-id-41', new_anomalies[0].bot_id_after_anomaly)
