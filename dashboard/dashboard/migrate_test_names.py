...
def MigrateTestNamesPost():
  ...
  if not status:
    try:
      ...
      return request_handler.RequestHandlerReportError('Error: %s' % str(error), status=400)
  elif status:
    ...
    return request_handler.RequestHandlerReportError('Missing required parameters of /migrate_test_names.')
