...
def DumpGraphJsonHandlerGet():
  if request.values.get('sheriff'):
    ...
    num_anomalies = int(request.values.get('num_alerts', _DEFAULT_MAX_ANOMALIES))

    protobuf_json = _DumpAnomalyDataForSheriff(sheriff_name, num_points, num_anomalies)

    return make_response(protobuf_json)
  elif request.values.get('test_path'):
    ...
    request_handler.RequestHandlerReportError('No parameters specified.')
