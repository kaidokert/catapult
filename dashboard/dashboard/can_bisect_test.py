...
class CanBisectTest(testing_common.TestCase):
  ...
  def testDomainNameIsBlacklistedForTriageBisects_NoDomains_ReturnsFalse(self):
    self.assertFalse(
        can_bisect.DomainIsExcludedFromTriageBisects('foo'))

  def testDomainNameIsBlacklistedForTriageBisects_NoMatch_ReturnsFalse(self):
    namespaced_stored_object.Set(
        can_bisect.FILE_BUG_BISECT_DENYLIST_KEY, {'bar': []})
    self.assertFalse(
        can_bisect.DomainIsExcludedFromTriageBisects('foo'))

  def testDomainNameIsBlacklistedForTriageBisects_Match_ReturnsTrue(self):
    namespaced_stored_object.Set(
