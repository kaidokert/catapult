...
def CreateFakeData(index, rts, bot, suite, case=None, measurements=3):
  ...
  for name in range(measurements):
    hs.CreateHistogram('measurement%d' % name, 'unitless_biggerIsBetter',
...
def AddFakeData(revs=100, suites=2, bots=3, cases=3):
  ...
  for suite in range(suites):
    ...
    for d in range(revs):
      ...
      for bot in range(bots):
        bot = 'bot%d' % bot
        hs = CreateFakeData(d, rts, bot, suite)
        for case in range(cases):
          case = 'case%d' % case
          hs.ImportDicts(CreateFakeData(d, rts, bot, suite, case).AsDicts())
