...
def ParseTestPath(parts, suite_name, test_part1_name, unparsed):
  ...
  if suite_name == 'memory.top_10_mobile':
    if len(parts) == 2:
      return parts[0], None
    return parts[0], parts[1] + ':' + parts[2]

  if (Hash(suite_name + ':' + test_part1_name) ==
      '89d460df708abdf78bbc722c755324614c179c8d5c523120ae9e3882d4f4a920'):
    if len(parts) == 2:
  ...
  if (Hash(suite_name + ':' + test_part1_name) in [
      '40f23ec66c9dc358009863f354efb410cb908b6e35b26a05ec7cccbfbf334aac',
      '692b99ee360b23545e5d3b76e0b776c818267654382a34d8c6df890e34f42379',
      'd6d362eecb896c695d945f73036e774d175aed5a5cf758f5d97266182b8a0992',
      'd4686b075579056ac99f3adfae557088241534d2db6a17964ea377d5ec43daa3',
...
def Describe(suite_name, test_part1_name):
  ...
  elif suite_name.startswith('v8.browsing_desktop'):
    # There are too many timeseries, it times out, so use
    # knowledge of structure of the benchmarks to optimize this.
    futures = [
        # Collect all the bots and test cases for a single measurement.
        FetchTestKeys(suite_name, test_part1_name='API:count_avg'),
        # Collect all the measurements.
        FetchTestKeys(suite_name, test_part2_name=''),
    ]
    ndb.Future.wait_all(futures)
