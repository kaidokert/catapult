...
class DescribeHandler(api_request_handler.ApiRequestHandler):
  ...
  def get(self, test_suite):
    ...
    descriptor = self._Describe(suite_name, test_part1_name)
  ...
  def _Describe(suite_name, test_part1_name):
    ...
    test_cases = set()
    unparsed = []
    keys = DescribeHandler._FetchTestKeys(suite_name, test_part1_name)
    ...
    for key in keys:
      ...
      measurements.add(DescribeHandler._StripSuffix(measurement))
      if test_case:
        test_cases.add(test_case)

    if unparsed:
      return {'unparsed': DescribeHandler._FormatUnparsed(unparsed)}
  ...
  @staticmethod
  def _Parse(parts, suite_name, test_part1_name, unparsed):
    if len(parts) == 1:
      return parts[0], None
