...
def FormatUnparsed(unparsed):
  ...
  return '\n'.join(
      ''.join(col.ljust(column_widths[i])
              for i, col in enumerate(row))
      for row in unparsed)
...
def Describe(suite_name, test_part1_name):
  ...
  if suite_name.startswith('system_health'):
    # There are too many timeseries in common_desktop, it times out, so use
    # knowledge of structure of the benchmarks to optimize this.
    futures = [
        # Collect all the bots and test cases for a single measurement.
        FetchTestKeys(suite_name, test_part1_name='benchmark_duration'),
        # Collect all the measurements.
        FetchTestKeys(suite_name, test_part2_name=''),
    ]
  ...
  if unparsed:
    return {'unparsed': FormatUnparsed(unparsed)}
...
class DescribeHandler(api_request_handler.ApiRequestHandler):
  ...
  def get(self, test_suite):
    ...
    descriptor = Describe(suite_name, test_part1_name)
    descriptor_json = json.dumps(descriptor)
    self.response.write(descriptor_json)

    if 'unparsed' not in descriptor and len(descriptor['measurements']):
  ...
  def _PreGet(self):
    ...
    logging.info('internal: %r', utils.IsInternalUser())
