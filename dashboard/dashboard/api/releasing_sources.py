...
class ReleasingSourcesHandler(api_request_handler.ApiRequestHandler):
  ...
  def get(self):
    ...
    sources = table_config.TableConfig.query().fetch()
  ...
  def _CacheKey(is_internal_user):
    return 'api_releasing_sources' + ('_internal' if is_internal_user else '')
