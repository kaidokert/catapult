...
# Map from ChromiumPerf revision to revisions for other bot masters.
REVISIONS_BY_MASTER_HASH = {
    '16a19c8bd2957824d48e8a712f7f623c40fddb50cf5789b084d294dc7666cd05': [
        [416640, 1473196450],
        [423768, 1475824394],
        [433391, 1475841673],
        [433400, 1479536199],
...
def GetRevisionForMaster(revision, master):
  master_hash = describe.Hash(master)
  revisions_for_master = REVISIONS_BY_MASTER_HASH.get(master_hash)
  if revisions_for_master is None:
    return revision

  # translate from ChromiumPerf commit position to the other master's point
  # id scheme. TODO remove this hack after revisionTimestamp
  i = bisect.bisect_left([chromium for (chromium, _) in revisions_for_master],
                         revision)
  if i == 0:
    return revisions_for_master[0][1]
  if i >= len(revisions_for_master):
    return revisions_for_master[-1][1]
  if revisions_for_master[i][0] == revision:
    return revisions_for_master[i][1]

  # Linear interpolation
  lo0 = revisions_for_master[i-1][0]
  lo1 = revisions_for_master[i-1][1]
  hi0 = revisions_for_master[i][0]
  hi1 = revisions_for_master[i][1]
  return lo1 + (hi1 - lo1) * (revision - lo0) / float(hi0 - lo0)
...
def GetStatistics(template_row, statistics, revision):
  ...
  for key in keys:
    ...
    # test_revision = GetRevisionForMaster(revision, avg_test.master_name)

    if (describe.Hash(avg_test.master_name) ==
        '16a19c8bd2957824d48e8a712f7f623c40fddb50cf5789b084d294dc7666cd05'):
      ...
      if len(entities) == 0:
        ...
        i = bisect.bisect(
            [int(entity.r_commit_pos) for entity in entities], revision)
        if i == len(entities):
          i -= 1
        avg_row = entities[i]
    else:
      ...
      if not avg_row:
        ...
        if len(entities) == 0:
          ...
          i = bisect.bisect(
              [entity.revision for entity in entities], revision)
          if i == len(entities):
            i -= 1
          avg_row = entities[i]
    if not avg_row:
