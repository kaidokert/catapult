...
def GetStatistics(template_row, statistics, revision):
  ...
  for key in keys:
    ...
    bot_hash = describe.Hash(avg_test.master_name + ':' + avg_test.bot_name)
    if bot_hash in REVISIONS_BY_BOT_HASH:
      test_revision = REVISIONS_BY_BOT_HASH[bot_hash].get(revision)
