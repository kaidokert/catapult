...
## Timeseries
  ...
  * `/api/report_names`: Return an array of objects `{name, id, modified}`.
  * `/api/report`: Report templates may be created by setting
    `template` to a json object `{rows, statistics}`, `name` to a string, and
    `owners` to a comma-separated list of email addresses; report templates may
    be modified by also setting `id` to the id of an existing report template.
    Reports may be generated by setting `id` to a template id as returned by
    `/api/report_names`, and `revisions` to a comma-separated list of point id
    numbers or `latest`.
