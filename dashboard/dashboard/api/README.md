...
## Timeseries
  ...
  * `/api/report`: Report templates may be created/modified by setting
    `template` to a json object `{rows, statistics}`, `name` to a string, and
    `owners` to a comma-separate list of email addresses. Reports may be
    populated with data by setting `id` to a template id as returns by
