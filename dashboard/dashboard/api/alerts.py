...
from dashboard.common import common_utils
from dashboard.models import anomaly
from dashboard.models import report_template


if common_utils.isRunningFlask():
  ...
  def Post(self, *args, **kwargs):
    ...
    response = {}
    try:
      is_improvement = utils.ParseBool(request.args.get('is_improvement', None))
      recovered = utils.ParseBool(request.args.get('recovered', None))
      ...
      min_timestamp = utils.ParseISO8601(
          request.args.get('min_timestamp', None))
      max_timestamp = utils.ParseISO8601(
          request.args.get('max_timestamp', None))

      test_keys = []
      for template_id in request.args.getlist('report'):
        test_keys.extend(report_template.TestKeysForReportTemplate(template_id))

      try:
        sheriff = request.args.get('sheriff', None)
        alert_list, next_cursor, count = anomaly.Anomaly.QueryAsync(
            bot_name=request.args.get('bot', None),
            bug_id=request.args.get('bug_id', None),
            is_improvement=is_improvement,
            key=request.args.get('key', None),
            limit=int(request.args.get('limit', 100)),
            count_limit=int(request.args.get('count_limit', 0)),
            master_name=request.args.get('master', None),
            max_end_revision=request.args.get('max_end_revision', None),
            max_start_revision=request.args.get('max_start_revision', None),
            max_timestamp=max_timestamp,
            min_end_revision=request.args.get('min_end_revision', None),
            min_start_revision=request.args.get('min_start_revision', None),
            ...
            test_keys=test_keys,
            test_suite_name=request.args.get('test_suite', None)).get_result()
    ...
    response['anomalies'] = alerts.AnomalyDicts(
        alert_list, utils.ParseBool(request.args.get('v2', None)))
    return response
