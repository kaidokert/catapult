...
def QueryAnomalies(
    benchmark_name=None, bot_name=None, bug_id=None, inequality_property=None,
    is_improvement=None, limit=100, master_name=None, max_end_revision=None,
    max_start_revision=None, max_timestamp=None, min_end_revision=None,
    min_start_revision=None, min_timestamp=None, recovered=None, sheriff=None,
    start_cursor=None, test=None):
...
class AlertsHandler(api_request_handler.ApiRequestHandler):
  ...
  def get(self):
    try:
      ...
      # be able to access internal_only data, but they should still be able to
      # access non-internal_only data.
      pass
    self._SetCorsHeadersIfAppropriate()

    try:
      results, next_cursor = QueryAnomalies(
