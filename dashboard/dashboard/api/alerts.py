...
def QueryAnomalies(
  ...
  if not utils.IsInternalUser():
    # _post_get_hook is not called for queries.
    # https://github.com/googlecloudplatform/datastore-ndb-python/issues/211
    # QueryAnomalies could fetch keys_only then call ndb.get_multi(), which
    # would invoke _post_get_hook, but with a performance penalty.
    logging.info('filter:internal_only=False')
