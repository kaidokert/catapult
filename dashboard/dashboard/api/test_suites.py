...
class TestSuitesHandler(api_request_handler.ApiRequestHandler):
  ...
  def get(self):
    ...
    test_suites = set()
    keys = self._FetchRootTestKeys(is_internal_user, test_suites)
    for key in keys:
      test_suite = self._GroupableTestSuite(key)
      if not test_suite:
        continue
      if not self._IsPartialTestSuite(test_suite):
        test_suites.add(test_suite)
        continue
      for subkey in self._FetchSubTestKeys(key):
  ...
  @staticmethod
  def _FetchRootTestKeys(is_internal_user, test_suites):
    ...
    if is_internal_user:
      public = memcache.get(TestSuitesHandler._CacheKey(False))
  ...
  @staticmethod
  def _FetchSubTestKeys(parent_test):
