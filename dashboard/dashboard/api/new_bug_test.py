...
class NewBugTest(testing_common.TestCase):
  ...
  def setUp(self):
    ...
    new_bug.utils.ServiceAccountHttp = mock.Mock(return_value=None)
    self.addCleanup(new_bug.utils.ServiceAccountHttp.stop)
    file_bug.issue_tracker_service = mock.MagicMock()
    file_bug.issue_tracker_service.IssueTrackerService = MockIssueTrackerService
    self.addCleanup(file_bug.issue_tracker_service.stop)
    file_bug.app_identity.get_default_version_hostname = mock.Mock(
        return_value='')
    self.addCleanup(file_bug.app_identity.get_default_version_hostname.stop)
