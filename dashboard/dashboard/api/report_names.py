...
class ReportNamesHandler(api_request_handler.ApiRequestHandler):
  ...
  def get(self):
    ...
    if cached is not None:
      # TODO uncomment _SetCacheControlHeaders when the rate of new reports
      # slows
    ...
    # TODO uncomment _SetCacheControlHeaders when the rate of new reports slows
    # self._SetCacheControlHeader(is_internal_user)
    self.response.write(sources)
    # ReportHandler.AuthorizedPost deletes this key when a new ReportTemplate is
    # put in the database.
    memcache.add(cache_key, sources, time=CACHE_SECONDS)
