...
Optional parameter:
  ...
  output_treces (path for recording the list of trace urls)
...
def Main(argv):
  ...
  parser.add_argument('--output_treces', default='/tmp/allUrls.txt', type=str,
                      help='Path for traceUrl output file')
  parser.add_argument('--output_revs', default='/tmp/revisionMap.csv', type=str,
                      help='Path for revisionMap output file')
  ...
  if response['status'] != '200':
    ...
    for elements in json_data['data']:
      ...
      with open(args.output_revs, 'a+') as revisions_file:
        csv_writer = csv.writer(revisions_file, delimiter=',')
        for url in histogram['diagnostics']['traceUrls']['values']:
          url_tokens = url.split('/')
          file_fullname = url_tokens[len(url_tokens)-1]
          file_name = file_fullname.split('.')[0]
          csv_writer.writerow([file_name, revision['r_commit_pos']])
          url_set.add(url)
    with open(args.output_treces, 'w') as urls_file:
      for url in url_set:
        urls_file.write(url+'\n')
