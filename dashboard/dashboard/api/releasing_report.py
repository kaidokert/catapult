...
def _GetRevisionsFromParams(milestone_param, table_entity,
                            master_bot_pairs):
...
def GetReport(name, milestone):
  table_entity = ndb.Key('TableConfig', name).get()
  master_bot_pairs = [
      bot.parent().string_id() + '/' + bot.string_id()
      for bot in table_entity.bots]
  rev_a, rev_b, milestone_dict = _GetRevisionsFromParams(
      milestone, table_entity, master_bot_pairs)
...
class ReleasingReportHandler(api_request_handler.ApiRequestHandler):
  ...
  def get(self, source, milestone):
    self._PreGet()
    is_internal_user = utils.IsInternalUser()
    cache_key = self._CacheKey(is_internal_user, source, milestone)
    cached = memcache.get(cache_key)
    if cached is not None:
      self.response.write(cached)
      return

    report = GetReport(source, milestone)
  ...
  @staticmethod
  def _CacheKey(is_internal_user, source, milestone):
    key = 'api_releasing_report_' + source + '_' + milestone
    if is_internal_user:
      key += '_internal'
    return key
