...
class DeleteAllEntitiesTest(testing_common.TestCase):
  ...
  def testDeleteAllViaTaskqueue(self):
    ...
    taskqueue_patch = mock.patch.object(delete_all_entities, 'taskqueue')
    taskqueue_patch.add = mock.MagicMock()
    taskqueue_patch.start()
    self.addCleanup(taskqueue_patch.stop)

    delete_all_entities.DeleteAllEntities('PageState')
    self.assertEqual(1, len(ndb.Query(kind='PageState').fetch(keys_only=True)))
    delete_all_entities.taskqueue.add.assert_called_once_with(
        url=delete_all_entities.PATH, params={'kind': 'PageState'})
