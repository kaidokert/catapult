...
_UPDATE_PARENT_DATA = 'update-parent-data'
...
def MigrateTestNamesTasksPost():
  ...
  logging.info('Test Migration Request details:%s', request.get_json())
  ...
  if status == _MIGRATE_TEST_LOOKUP_PATTERNS:
    ...
    logging.info('[Migrate Test Create]: parent_id=%s', parent_id)
    ...
    job_id = request.values.get('job_id')
    parent_id = request.values.get('parent_id')
    logging.info('[Migrate Test Copy Data]: parent_id=%s', parent_id)
