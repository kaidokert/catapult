...
class AddHistogramsQueueTestWithUploadCompletionToken(testing_common.TestCase):
  ...
  def _PrepareHistogramWithMeasurement(self):
    token_id = str(uuid.uuid4())
    test_path = 'Chromium/win7/suite/metric'
    token = upload_completion_token.Token(id=token_id).put().get()
    token.CreateMeasurement(test_path)
    ...
    return token_id, json.dumps(params)

  def _CreateHistogramWithMeasurementAndAdd(self):
    token_id, upload_data = self._PrepareHistogramWithMeasurement()
  ...
  def _PrepareHistogramWithMultipleMeasurements(self):
  ...
  def _CreateHistogramWithMultipleMeasurementAndAdd(self):
    token_id, upload_data = self._PrepareHistogramWithMultipleMeasurements()
    self.testapp.post('/add_histograms_queue', upload_data)
    return upload_completion_token.Token.get_by_id(token_id)
  ...
  def testPostMultipleHistogram_FailToCreateFixture(self):
    ...
    self.assertEqual(token.state, upload_completion_token.State.FAILED)
    self.assertTrue(all(
        substate == upload_completion_token.State.FAILED
  ...
  def testPostMultipleHistogram_MeasrementExpired(self):
    ...
    token_id = str(uuid.uuid4())
    token = upload_completion_token.Token(id=token_id).put().get()
    token.CreateMeasurement(test_path1)
    measurement2 = token.CreateMeasurement(test_path2)
    token.UpdateStateAsync(upload_completion_token.State.COMPLETED).wait()

    measurement2.key.delete()
    measurement2 = upload_completion_token.Measurement.get_by_id(test_path2)
