...
class DescriptorTest(unittest.TestCase):
  ...
  def setUp(self):
    self._orig_partial = descriptor.PARTIAL_TEST_SUITE_HASHES
    descriptor.PARTIAL_TEST_SUITE_HASHES = [
        hashlib.sha256('TEST_PARTIAL_TEST_SUITE').hexdigest()
    ]
    self._orig_composite = descriptor.COMPOSITE_TEST_SUITE_HASHES
    descriptor.COMPOSITE_TEST_SUITE_HASHES = [
        hashlib.sha256('TEST_PARTIAL_TEST_SUITE:COMPOSITE').hexdigest()
    ]
    self._orig_groupable = descriptor.GROUPABLE_TEST_SUITE_PREFIXES
    descriptor.GROUPABLE_TEST_SUITE_PREFIXES = ['TEST_GROUPABLE%']

  def tearDown(self):
    descriptor.PARTIAL_TEST_SUITE_HASHES = self._orig_partial
    descriptor.COMPOSITE_TEST_SUITE_HASHES = self._orig_composite
    descriptor.GROUPABLE_TEST_SUITE_PREFIXES = self._orig_groupable
