...
def publish_pinpoint_job_status_metric(stage,
                                       job_id,
                                       job_type,
                                       job_status,
                                       metric_value=1):
  ...
  publish_ts_cloud_metric("pinpoint", "pinpoint/job/status_change", label_dict,
                          stage, metric_value)


def publish_pinpoint_job_run_time_metric(stage, job_id, job_type, job_status,
                                         metric_value):
  label_dict = {}
  label_dict[JOB_ID] = job_id
  label_dict[JOB_TYPE] = job_type
  label_dict[JOB_STATUS] = job_status
  publish_ts_cloud_metric("pinpoint", "pinpoint/job/run_time", label_dict,
                          stage, metric_value)


def publish_ts_cloud_metric(service_name,
                            metric_type,
                            label_dict,
                            stage=NAMESPACE,
                            metric_value=1):
