...
class Descriptor(object):
  ...
  def ParseTestPath(cls, path):
    ...
    test_suite = path[2]
    # TODO some test_suites include path[4]
    if len(path) == 3:
    ...
    measurement = path[3]
    statistic = None
    # TODO some measurements include path[4]
    ...
    if test_case == REFERENCE_BUILD_TYPE:
      build_type = REFERENCE_BUILD_TYPE
      test_case = None
    # TODO some test_cases include path[5] and/or path[6] and/or path[7]
    # TODO some test_cases should join using colons instead of underscores

    return cls(test_suite=test_suite, bot=bot, measurement=measurement,
               statistic=statistic, test_case=test_case, build_type=build_type)

  def CompileTestPaths(self):
    ...
    # TODO some test_suites need to be modified
    test_path += '/' + self.test_suite
    if not self.measurement:
      return [test_path]

    # TODO some measurements need to be modified
    test_path += '/' + self.measurement
    if self.statistic:
      test_path += '_' + self.statistic
    if self.test_case:
      # TODO some test_cases need to be modified
