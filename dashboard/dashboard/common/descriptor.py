...
class Descriptor(object):
  ...
  def _BotTestPaths(self):
    ...
    aliaseses = yield bot_configurations.AliasesesAsync()
    for aliases in aliaseses:
      if slave in aliases:
        raise ndb.Return([master + '/' + alias for alias in aliases])

    raise ndb.Return([self.bot.replace(':', '/')])
  ...
  def _AppendRef(self, test_paths):
    ...
    for p in test_paths:
      ...
      # way to know which.
      ref_test_paths.append(p + '_ref')
      ref_test_paths.append(p + '/ref')
