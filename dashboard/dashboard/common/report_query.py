...
class ReportQuery(object):
  ...
  def _ResolveCommitPositions(self):
    commit_pos_bots = yield stored_object.GetCachedAsync(
        'bots_with_different_r_commit_pos')
    chromium_commit_pos_bots = yield stored_object.GetCachedAsync(
    ...
    for tri, table_row in enumerate(self._report['rows']):
      for desc in TableRowDescriptors(table_row):
        if (desc.bot, desc.test_suite) in self._revisions_by_bot_suite:
          continue
        prop_name = None
        ...
        if prop_name is None:
          continue
        self._revisions_by_bot_suite[desc.bot, desc.test_suite] = {}
        futures.append(self._BisectCommitPositions(self, desc, prop_name))
    yield futures

  @ndb.tasklet
  def _BisectCommitPositions(self, desc, prop_name):
  ...
  def _GetDataRowForKey(self, test_key, rev, desc):
    ...
    if rev != 'latest' and (
        (desc.bot, desc.test_suite) in self._revisions_by_bot_suite):
