...
class ReportQuery(object):
  ...
  def FetchAsync(self):
    ...
    with timing.CpuTimeLogger('PostProcess'):
      for tri, table_row in enumerate(self._report['rows']):
  ...
  def _IgnoreStaleData(self, tri, table_row):
    ...
    with timing.CpuTimeLogger('IgnoreStaleData'):
  ...
  def _IgnoreIncomparableData(self, table_row):
    ...
    with timing.CpuTimeLogger('IgnoreIncomparableData'):
      for rev, data in table_row['data'].iteritems():
        new_data = []
        for datum in data:
          all_revs = True
  ...
  def _SetRowUnits(self, table_row):
    ...
    with timing.CpuTimeLogger('SetRowUnits'):
      for rev in self._revisions:
  ...
  def _IgnoreDataWithWrongUnits(self, table_row):
    with timing.CpuTimeLogger('IgnoreDataWithWrongUnits'):
      for rev, data in table_row['data'].iteritems():
        new_data = []
        for datum in data:
  ...
  def _MergeData(self, table_row):
    with timing.CpuTimeLogger('MergeData'):
      for rev, data in table_row['data'].iteritems():
