...
#   request.registry['privileged'] = True
  ...
  logging.info('DDEBUG: current - 2 %s' % dir(flask_app))
#   flask_app.registry['privileged'] = True
...
#   request.registry['single_privileged'] = True
  g.single_privileged = True
#   flask_app = flask.current_app
#   flask_app.registry['single_privileged'] = True
...
#   request.registry['single_privileged'] = False
  g.single_privileged = False
#   flask_app = flask.current_app
#   flask_app.registry['single_privileged'] = False
...
#     request = webapp2.get_request()
  ...
  if path.startswith('/_ah/pipeline/'):
    return True
  if 'privileged' in g and g.privilege:
    return True
  if 'single_privileged' in g and g.single_privileged:
    g.pop('single_privileged')
    return True
#   if flask_app.registry.get('privileged', False):
#     return True
#   if flask_app.registry.get('single_privileged', False):
#     flask_app.registry['single_privileged'] = False
#     return True
