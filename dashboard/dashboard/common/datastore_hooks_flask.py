...
def _DatastorePreHook(service, call, ds_request, _):
  ...
  Args:
    ...
    call: String representing function to call. One of 'Put', Get', 'Delete',
        or 'RunQuery'.
    ds_request: Request protobuf.
    _: Response protobuf (not used).
  """
  assert service == 'datastore_v3'
  if call != 'RunQuery':
    return
  if ds_request.kind() not in _INTERNAL_ONLY_KINDS:
    return
  ...
  try:
    external_filter = ds_request.filter_list().add()
    ...
    external_filter = ds_request.add_filter()
