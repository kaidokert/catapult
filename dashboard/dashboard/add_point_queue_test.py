...
class GetOrCreateAncestorsTest(testing_common.TestCase):
  ...
  def testGetOrCreateAncestors_UpdatesAllExpectedEntities(self):
    ...
    t = graph_data.TestMetadata(id='WebRTCPerf/android32-pixel5-android11/webrtc_perf_tests', internal_only=True)
    t.UpdateSheriff()
    t.put()


    t = graph_data.TestMetadata(id='WebRTCPerf/android32-pixel5-android11/webrtc_perf_tests/render_frame_rate_fps', internal_only=True)
    t.UpdateSheriff()
    t.put()

    master_key = graph_data.Master(id='WebRTCPerf', parent=None).put()
    graph_data.Bot(id='android32-pixel5-android11', parent=master_key, internal_only=False).put()
    ...
    parent = add_point_queue.GetOrCreateAncestors(master, bot, full_test_name,
                                                  internal_only=internal_only)
    self.assertEqual('WebRTCPerf/android32-pixel5-android11/webrtc_perf_tests/render_frame_rate_fps/foreman_cif_delay_50_0_plr_5_flexfec', parent.key.id())
    # Check that all the Bot and TestMetadata entities were correctly added.
    created_masters = graph_data.Master.query().fetch()
    created_bots = graph_data.Bot.query().fetch()
    created_tests = graph_data.TestMetadata.query().fetch()
    ...
    self.assertEqual('WebRTCPerf/android32-pixel5-android11/webrtc_perf_tests', created_tests[0].key.id())
    self.assertIsNone(created_tests[0].parent_test)
    ...
    self.assertEqual('WebRTCPerf/android32-pixel5-android11/webrtc_perf_tests/render_frame_rate_fps/foreman_cif_delay_50_0_plr_5_flexfec',
                     created_tests[2].key.id())
    self.assertEqual('WebRTCPerf/android32-pixel5-android11/webrtc_perf_tests/render_frame_rate_fps',
                     created_tests[2].parent_test.id())
    self.assertIsNone(created_tests[2].bot)
