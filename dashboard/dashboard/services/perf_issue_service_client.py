...
def GetCanonicalGroupByIssue(issue_id, project_name):
  url = _SERVICE_URL + _ALERT_GROUP_PREFIX
  url += 'canonical/issue_id/%s/project_name/%s' % (issue_id, project_name)
  try:
    cloud_metric.PublishPerfIssueServiceRequests(
    ...
    logging.warning(
        '[PerfIssueService] Error requesting canonical group by issue. ID: %s. Project: %s. %s',
        issue_id, project_name, str(e))
    return []
