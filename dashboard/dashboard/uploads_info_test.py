...
class UploadInfo(testing_common.TestCase):
  ...
  def setUp(self):
    ...
    app = webapp2.WSGIApplication([
        ('/uploads/(.+?)/?(full_info)?', uploads_info.UploadInfoHandler),
    ])
  ...
  def GetFullInfoRequest(self, token_id, status=200):
    return json.loads(
        self.testapp.get('/uploads/%s/full_info' % token_id,
                         status=status).body)
            ...
            'diagnostics': [
                {
                    ...
                    'value': {
                        'type': 'DiagnosticRef',
                        'original_value': owners_diagnostic.AsDict()
                    }
                },
                {
                    'name': reserved_infos.CHROMIUM_COMMIT_POSITIONS.name,
                    'value': commit_position_diagnostic.AsDict()
                },
            ]
        },]
    }
    response = self.GetFullInfoRequest(token_id)
    expected['measurements'][0]['diagnostics'].sort()
    response['measurements'][0]['diagnostics'].sort()
    self.assertEqual(response, expected)
  ...
  def testGet_NotFound(self, mock_log):
    self.GetFullInfoRequest('inexistent', status=404)
