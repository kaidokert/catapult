...
@ndb.tasklet
def InsertOutOfOrderNamed(new_diagnostic, old_diagnostics, rev):
  for i in xrange(len(old_diagnostics)):
    cur = old_diagnostics[i]

    # clobber diagnostic
    #  - find next revision after
    #   - clone cur diagnostic
    #    clone.start = after, clone.end = cur.end
    #    cur.end = after - 1
    #    cur.data = new data
    if rev == cur.start_revision:
      ...
      next = None
      if i > 0:
        next = old_diagnostics[i-1]

      next_revision = yield _FindNextRev(cur.test, rev)
      ...
      if next and (next.start_revision - 1) != next_revision:
        clone = histogram.SparseDiagnostic(
      ...
      elif not next and next_revision != sys.maxint:
        clone = histogram.SparseDiagnostic(
            data=cur.data, test=cur.test,
            start_revision=next_revision + 1, end_revision=cur.end_revision,
            name=cur.name, internal_only=cur.internal_only)
