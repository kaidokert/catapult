...
class AddHistogramsHandler(api_request_handler.ApiRequestHandler):
  ...
  def AuthorizedPost(self):
    ...
    filename = datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S')
    params = {'gcs_file_path': '/add-histograms-cache/%s.html' % filename}
    ...
    queue.add(
        taskqueue.Task(url='/add_histograms_queue', payload=json.dumps(params)))
