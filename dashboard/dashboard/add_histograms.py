...
def AddHistogramsPost():
  if utils.IsDevAppserver():
    ...
    ProcessHistogramSet(
        _LoadHistogramList(six.StringIO(request.data)), token)
    token.UpdateState(upload_completion_token.State.COMPLETED)
    return make_response(token_info)
  ...
  with timing.WallTimeLogger('decompress'):
    try:
      ...
      zlib.decompressobj().decompress(str.encode(data_str), 100)
      ...
      data_str = zlib.compress(str.encode(data_str))
