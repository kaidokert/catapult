...
class GenerateResults2Test(testing_common.TestCase):
  ...
  def testTypeDispatch_PushBQ(self, mock_swarming, mock_json, mock_render,
    ...
    mock_render.side_effect = TraverseHistograms
    lcp_histogram = histogram_module.Histogram('largestContentfulPaint', 'count')
    lcp_histogram.AddSample(42)
    fcp_histogram = histogram_module.Histogram('firstContentfulPaint', 'count')
    fcp_histogram.AddSample(11)
    expected_histogram_set = histogram_set.HistogramSet([lcp_histogram, fcp_histogram])
    mock_json.return_value = expected_histogram_set.AsDicts()

    expected_rows = [
        {'batch_id': 'fake_batch_id', 'dims': {'device': {'cfg': 'fake_configuration', 'os': 'os'}, 'test_info': {'story': 'fake_story', 'benchmark': 'fake_benchmark'}, 'pairing': {'replica': 0}, 'checkout': {'repo': 'fakeRepo', 'git_hash': 'fakehashB', 'patch_gerrit_revision': 'fake_patch_rev', 'patch_gerrit_change': 'fake_patch_issue'}}, 'measures': {'core_web_vitals': {'first_contentful_paint': 11.0, 'largest_contentful_paint': 42.0}}, 'job_id': 'fake_job_id'},
        {'batch_id': 'fake_batch_id', 'dims': {'device': {'cfg': 'fake_configuration', 'os': 'os'}, 'test_info': {'story': 'fake_story', 'benchmark': 'fake_benchmark'}, 'pairing': {'replica': 0}, 'checkout': {'repo': 'fakerepo', 'git_hash': 'fakehashA'}}, 'measures': {'core_web_vitals': {'first_contentful_paint': 11.0, 'largest_contentful_paint': 42.0}}, 'job_id': 'fake_job_id'}
    ]
