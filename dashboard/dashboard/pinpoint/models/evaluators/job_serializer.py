...
class Serializer(evaluators.DispatchByTaskType):
  ...
  def __call__(self, task, event, accumulator):
    ...
    if 'state' in local_accumulator:
      ...
      if add_execution:
        ...
        if quest_index >= len(executions):
          executions.extend([None] * ((quest_index + 1) - len(executions)))
    ...
    if 'order_changes' in local_accumulator:
      ...
      order_changes = local_accumulator.get('order_changes')
      change_index = {
          change: index for index, change in enumerate(order_changes)
      }
      states = accumulator.get('state')
      logging.debug('changes = %s', order_changes)
      logging.debug('change_index = %s', pprint.pformat(change_index))
      ...
      if states:
        ...
        accumulator['state'] = ordered_states
...
def TaskTransformer(task, _, accumulator):
  """Takes the form:
...
def AnalysisTransformer(task, _, accumulator):
  ...
  """
  task_data = accumulator.get(task.id)
  result = {
      'set_parameters': {
