...
HistogramMetadata = collections.namedtuple(
    'HistogramMetadata', ['attemptNo', "change", "swarming_result"])
      ...
      for execution in attempt.executions:
        ...
        if isinstance(execution, run_test._RunTestExecution):
          ...
          try:
            ...
            logging.error("_FetchHistograms swarming query failed: " +
                          str(e))
          continue
...
RowKey = collections.namedtuple(
    'RowKey', ['change', 'iteration'])
_PROJECT_ID = 'todo'
_DATASET = 'todo'
_TABLE = 'todo'
def _SaveJobToBigQuery(job):
  ...
  for h in _FetchHistograms(job):
    ...
    if len(h.histogram["sampleValues"]) != 1:
      ...
      continue
    rk = RowKey(h.metadata.change, h.metadata.attemptNo)
    if rk not in rows:
      rows[rk] = _PopulateMetadata(h)
    data = rows[rk]

    data["benchmark"] = job.benchmark_arguments["benchmark"]
    data["cfg"] = job.configuration
    data["change_repo"] = h.metadata.change.commits[0].repository
    data["change_hash"] = h.metadata.change.commits[0].git_hash
    ...
    if h.metadata.swarming_result and "bot_dimensions" in h.metadata.swarming_result:
      if "device_type" in h.metadata.swarming_result["bot_dimensions"]:
        data["deviceModel"] = h.metadata.swarming_result["bot_dimensions"][
            "device_type"]
      if "device_os" in h.metadata.swarming_result["bot_dimensions"]:
        data["deviceOS"] = h.metadata.swarming_result["bot_dimensions"][
            "device_os"]

    data["iteration"] = h.metadata.attemptNo

    data["metric"] = h.histogram["name"]
    data["story"] = job.benchmark_arguments["story"]
    data["value"] = h.histogram["sampleValues"][0]
    data["passing"] = True
    _PopulateMetric(data, h.histogram["name"], h.histogram["sampleValues"][0])
    rows[rk] = data #DO_NOT_SUBMIT: Not neccessary if by ref (is it?)

    #rows.append(data)
  logging.error("ROWS:\n\n" + str(rows) + "\n\n") #DO_NOT_SUBMIT: Remove
  _InsertBQRows(_PROJECT_ID, _DATASET, _TABLE, rows)

def _PopulateMetadata(h):
  md = {}
...
def _InsertBQRows(self, project_id, dataset_id, table_id, rows, num_retries=5):
  service = _BQService()
  for i in xrange(0, len(rows), INSERTION_MAX_ROWS):
    rows_chunk = rows[i:i + INSERTION_MAX_ROWS]
    logging.info('Inserting %d rows into %s.%s.', len(rows_chunk), dataset_id,
                 table_id)
    rows_chunk = [{
