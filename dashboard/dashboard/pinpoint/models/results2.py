...
def GetOrScheduleResults2(job):
  if not job:
    raise Results2Error('Error: Unknown job')

  filename = '/chromeperf.appspot.com/results2/%s.html' % job.job_id
  results = cloudstorage.listbucket(filename)

  for _ in results:
    return 'complete', 'https://storage.cloud.google.com' + filename

  if job.task:
    return 'job-incomplete', None

  try:
    ScheduleResults2Generation(job)
