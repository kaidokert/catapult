...
def _SaveJobToBigQuery(job):
  ...
  for hMetadata, h in _FetchHistograms(job):
    ...
    change_repo = hMetadata.change.repository
    change_hash = hMetadata.change.git_hash
    patch_server = None
    patch_repo = None
    patch_number = None
    patch_rev = None
    if hMetadata.change.patch is not None:
      ...
      patch_server = patch_params["patch_gerrit_url"]
      patch_repo = patch_params["project"]
      patch_number = patch_params["patch_issue"]
      patch_rev = patch_params["patch_rev"]
