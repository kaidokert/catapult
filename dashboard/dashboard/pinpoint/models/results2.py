...
def _SaveJobToBigQuery(job):
  ...
  for h in _FetchHistograms(job):
    ...
    rows[rk] = _PopulateMetric(rows[rk], h.histogram["name"], h.histogram["sampleValues"][0])
  logging.info("Saving to BQ: " + str(rows))
  _InsertBQRows(_PROJECT_ID, _DATASET, _TABLE, rows.itervalues())
...
def _PopulateMetadata(job, h):
  ...
  md["dims"]["pairing"] = {}
  md["dims"]["pairing"]["replica"] = h.metadata.attemptNo
...
def _PopulateMetric(data, name, value):
  ...
  if name == "largestContentfulPaint":
    data["measures"]["core_web_vitals"]["largest_contentful_paint"] = float(value)
