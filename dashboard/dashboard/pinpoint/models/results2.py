...
_TABLE_GENERAL = 'rows' # Weird name, but consistent with the name of the Chromeperf table.
...
def _SaveJobToChromeHealthBigQuery(job):
  ...
  for h in _FetchHistograms(job):
    ...
    if rk not in rows:
      rows[rk] = _PopulateChromeHealthMetadata(job, h)
  ...
  if len(rows_with_measures):
    _InsertBQRows(_PROJECT_ID, _DATASET_CHROME_HEALTH, _TABLE_CHROME_HEALTH, rows_with_measures)
