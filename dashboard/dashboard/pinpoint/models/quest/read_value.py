...
class ReadHistogramsJsonValue(quest.Quest):
  ...
  def trace_or_story(self):
    trace_or_story = getattr(self, '_trace_or_story')
    if not trace_or_story:
      setattr(self, '_trace_or_story', getattr(self, '_trace', None))
    return getattr(self, '_trace_or_story')
