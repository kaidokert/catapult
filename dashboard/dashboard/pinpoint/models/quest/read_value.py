...
class _ReadHistogramsJsonValueExecution(execution.Execution):
  ...
  def _Poll(self):
    ...
    for hist in histograms:
      histograms_by_path[histogram_helpers.ComputeTestPath(hist)].append(hist)
    ...
    test_path = self._hist_name
    if self._tir_label:
      test_path += '/' + self._tir_label

    if self._story:
      test_path += '/' + self._story

    matching_histograms = histograms_by_path.get(test_path, [])
    ...
    if not matching_histograms:
      ...
      summary_value = []
      for k, histograms_for_test_path in histograms_by_path.iteritems():
        if k.startswith(test_path):
          for h in histograms_for_test_path:
            summary_value.extend(h.sample_values)
      if summary_value:
