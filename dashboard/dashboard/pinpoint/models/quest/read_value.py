...
class ReadHistogramsJsonValue(quest.Quest):
  ...
  def FromDict(cls, arguments):
    ...
    # TODO(crbug.com/974237): Only read from 'grouping_label' when clients no
    # longer use the legacy name 'tir_label'.
    if 'grouping_label' in arguments:
      grouping_label = arguments.get('grouping_label')
    else:
      grouping_label = arguments.get('tir_label')
