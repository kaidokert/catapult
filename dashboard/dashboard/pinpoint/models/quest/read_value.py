...
class ReadValue(quest.Quest):
  ...
  def __hash__(self):
    return hash(self._results_filename, self.results_path, self._metric,
  ...
  def _Poll(self):
    ...
    if histogram_exception:
      try:
        ...
        logging.debug('Succeess.')
      # TODO(https://crbug.com/1262295): use `faise from` when Python2 trybots retire.
...
def RetrieveCASOutput(cas_root_ref, path, client=None):
  ...
  # TODO(https://crbug.com/1262295): use `decodebytes` when Python2 trybots retire.
...
class ReadHistogramsJsonValue(quest.Quest):
  ...
  def __hash__(self):
    return hash(self._results_filename, self._hist_name, self._grouping_label,
                self.trace_or_story, self._statistic)
