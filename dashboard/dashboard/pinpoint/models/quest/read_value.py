...
class CompleteReadValueAction(
    collections.namedtuple('CompleteReadValueAction', ('job', 'task'))):
  __slots__ = ()

  @task_module.LogStateTransitionFailures
  def __call__(self, _):
    logging.debug('Updating ReadValue status.')
...
class ReadValueEvaluator(
  ...
  def __call__(self, task, _, accumulator):
    ...
    if task.status == 'completed':
      logging.debug('Completed, bailing out.')
      return None
    ...
    return self.HandleGraphJson(data)
  ...
  def HandleHistogramSets(self, task, histogram_dicts):
    ...
    return [CompleteReadValueAction(self.job, task)]

  def HandleGraphJsonData(self, _):
