...
class FindIsolate(quest.Quest):
  ...
  def __init__(self, builder, target, bucket, fallback_target = None, comparison_mode = "performance"):
...
class _FindIsolateExecution(execution.Execution):
  ...
  def _CheckBuildStatusV2(self):
    ...
    if build_status == 'INFRA_FAILURE':
      reason = 'TIMEOUT' if 'timeout' in job_status.get('statusDetails', {}) else 'INFRA_FAILURE'
      if self._IsTryJob():
        raise errors.BuildFailedFatal(reason)
      else:
        raise errors.BuildFailed(reason)
