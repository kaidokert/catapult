...
class UpdateBuildStatusAction(object):
  ...
  def __call__(self, accumulator):
    ...
    try:
      ...
      properties = json.loads(build['result_details_json'])['properties']
      commit_position = properties['got_revision_cp'].replace('@', '(at)')
      suffix = '%s_patch' % ('with' if 'patch_storage' in properties else
                             'without',)
      key = '_'.join(('swarm_hashes', commit_position, suffix))
...
class Evaluator(object):
  ...
  def __call__(self, task, event, accumulator):
    ...
    if event.type == 'initiate':
      return self.HandleInitiate(task, change)
    elif event.type == 'update':
      return self.HandleUpdate(task, event, change)
    else:
      logging.error('Unsupported event type "%s"; event = %s', event.type,
                    event)
      return None

  def HandleInitiate(self, task, change):
  ...
  def HandleUpdate(self, task, event, change):
    # Outline:
