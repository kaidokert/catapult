...
_DEFAULT_IMAGE_PATH = '--system-image-dir=../../third_party/fuchsia-sdk/images-internal/%s/%s'
_IMAGE_MAP = {
  ...
  def _ExtraTestArgs(cls, arguments):
    ...
    for key_value in dimensions:
      if key_value['key'] == 'device_type':
        image_path = _IMAGE_MAP[key_value['value'].lower()]
        break
    extra_test_args += copy.copy(_DEFAULT_EXTRA_ARGS)
    if all(image_path) and len(image_path) == 2:
      extra_test_args.append(_DEFAULT_IMAGE_PATH % image_path)
