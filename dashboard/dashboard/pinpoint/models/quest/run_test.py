...
class RunTest(quest.Quest):
  ...
  def _Start(self, change, index, isolate_server, isolate_hash, extra_args,
    ...
    while len(self._started_executions[change]) <= index:
      self._started_executions[change].append(None)
  ...
  def _StartAllTasks(self):
    ...
    if len(a_list) != self._attempt_count or len(
        b_list) != self._attempt_count or None in a_list or None in b_list:
      logging.debug(
