...
_CLANK_ANDROID_GO_COMMIT = 123456
...
class RunTest(quest.Quest):
  ...
  def _ApplyBrowserTargetChange(self, change, extra_args):
    ...
    commit_dict = change.base_commit.AsDict()
    if commit_dict.get('repository') != 'chromium':
      return

    commit_position = commit_dict.get('commit_position')
    if commit_position < _CLANK_ANDROID_GO_COMMIT:
      try:
        i = extra_args.index('--browser') + 1
        if extra_args[i] == 'android-chrome':
          extra_args[i] = 'android-chromium'
      except ValueError:
        pass
