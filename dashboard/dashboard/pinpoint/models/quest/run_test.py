  ...
  def __call__(self, _):
    ...
    if result.get('failure', False):
      ...
      if not exception_string:
        exception_string = 'No exception found in task standard output.'
      self.task.payload.update({
          'errors': [{
        ...
        evaluators=(
            ...
            evaluators.FilteringEvaluator(
                predicate=evaluators.All(
                    evaluators.TaskTypeFilter('run_test'),
                    evaluators.EventTargetFilter(),
                ),
                delegate=evaluators.DispatchEvaluator({
                    'initiate':
                        evaluators.FilteringEvaluator(
                            predicate=evaluators.Not(
                                evaluators.TaskStatusFilter(
                                    {'ongoing', 'failed', 'completed'})),
                            delegate=InitiateEvaluator(job)),
                    'update':
                        evaluators.FilteringEvaluator(
                            predicate=evaluators.TaskStatusFilter({'ongoing'}),
