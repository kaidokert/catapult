...
_CONFIGURATION_TO_BROWSER = 'bot_browser_map'
...
def _GetBrowser(arguments, used_arguments):
  ...
  if browser:
    ...
    for configuration_name_prefix, browser in configuration_to_dimensions:
      if configuration.startswith(configuration_name_prefix):
        break
    else:
      browser = 'release'
  else:
