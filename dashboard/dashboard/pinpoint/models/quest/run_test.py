...
class RunTest(quest.Quest):
  ...
  def __init__(self, swarming_server, dimensions, extra_args, swarming_tags,
    ...
    self._execution_counts = collections.defaultdict(int) # deprecated
  ...
  def _StartAllTasks(self):
    ...
    if len(self._started_executions.keys) != 2:
      return

    a = self._started_executions[self._started_executions.keys[0]]
    b = self._started_executions[self._started_executions.keys[1]]
    attempt_count = len(a)
    orderings = self._GetABOrderings(attempt_count)
    for i in range(attempt_count):
      if orderings[i]:
        self._started_executions[b][i]._StartTaskReal()
        self._started_executions[a][i]._StartTaskReal()
      else:
        self._started_executions[b][i]._StartTaskReal()
        self._started_executions[a][i]._StartTaskReal()
...
class _RunTestExecution(execution_module.Execution):
  ...
  def _StartTask(self):
    ...
    # Beter idea: Put all of these objects in a list (or map of change -> list of tasks) in owning Quest, call method on Quest to call _StartTaskReal on all. Then don't need to have that datastructure.
    # Also, need to determine which device gets run on earlier on, because we will want to play games with device distribution
    # Need to make sure subclasses work properly (redefine StartTaskReal?)

    # Figure out how many iterations
    # Make data structure that says if A or B go first (50% A, 50% B)
    # Do scheduling, populate self._quest._started_tasks
