...
def Compare(values_a, values_b, attempt_count, mode, magnitude):
  ...
  new_comparison_result = ComparisonResults(UNKNOWN, p_value, new_low_threshold,
                                            new_high_threshold)
  if p_value <= new_low_threshold:
    new_comparison_result.result = DIFFERENT
  elif p_value > new_high_threshold:
    new_comparison_result.result = SAME
  logging.debug('BisectDebug: new_comparison_result: %s', new_comparison_result)

  # If none of the conditions are true, it means the p-value is quite large.
  # We're not suspicious that the two samples
  # might come from different distributions,
  # and we don't care to investigate more.
  ...
  if p_value <= low_threshold:
    ...
    # hypothesis.
    comparison_result.result = DIFFERENT
    ...
    comparison_result.result = UNKNOWN
