...
class DifferenceBugCommentBuilder(object):
  ...
  def BuildComment(self, tags, url):
    ...
    owner, sheriff, cc_list, roll_commit = _ComputePostOwnerSheriffCCList(
        commits_with_deltas)

    # TODO: return a namedtuple
    return (
        _FormatComment(self._differences, self._metric, sheriff, roll_commit,
                       tags, url),
        owner, cc_list)
...
class _Difference(object):
  ...
  def FormatForBug(self, metric):
    ...
    subject = '<b>%s</b> by %s' % (
        commit_info['subject'], commit_info['author'])
