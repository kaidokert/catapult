...
%}Found significant differences after each of {{
differences|length }} commits.{% endif %}</b>
{{ url }}

{% for diff in differences %}<b>{{
diff.commit_info['subject'] }}</b> by {{ diff.commit_info['author'] }}
{{ diff.commit_info['url'] }}
{% if metric %}{{ metric }}: {% endif %}{{ diff.Formatted() }}{%
if not loop.last %}

{% endif %}{% endfor %}{{ notify_why_text }}
