...
def UpdatePostAndMergeDeferred(bug_update_builder,
  ...
  if current_bug_labels and len(current_bug_labels) > 0 and \
     'DoNotNotify' in current_bug_labels:
    logging.info(
        '[DoNotNotify] Removing owner: %s and cc_list: %s for bug_id: %s',
        bug_owner, cc_list, bug_id)
    bug_owner = ''
    cc_list = set()
