  ...
  def __call__(self, task, _, accumulator):
    ...
    if task.status == 'ongoing':
      ...
      def SlidingTriple(iterable):
        ...
        p, c = itertools.tee(itertools.chain([None], iterable))
        next(c, None)
        c, n = itertools.tee(c)
        next(n, None)
        n = itertools.chain(n, [None])
      ...
      if 'comparisons' not in task.payload or task.payload.get(
      ...
      dependent_states = set(changes_by_status)
      can_complete = {'failed', 'completed'
                     } & dependent_states == dependent_states
