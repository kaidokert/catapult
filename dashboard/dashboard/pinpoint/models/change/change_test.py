...
def Change(chromium=None, catapult=None, another_repo=None, patch=False, arm=None):
  commits = []
  ...
  return change.Change(commits, patch=patch_test.Patch() if patch else None, arm=arm)
...
class ChangeTest(test.TestCase):
  ...
  def testChangeWithArm(self):
    base_commit = commit.Commit('chromium', 'aaa7336c821888839f759c6c0a36b56c')
    dep = commit.Commit('catapult', 'e0a2efbb3d1a81aac3c90041eefec24f066d26ba')

    # Also test the deps conversion to tuple.
    c = change.Change([base_commit, dep], patch_test.Patch(), arm=change.ARM_EXP)

    self.assertEqual(c, change.Change((base_commit, dep), patch_test.Patch(), arm=change.ARM_EXP))
    string = 'chromium@aaa7336 catapult@e0a2efb + abc123 (Arm: exp)'
    id_string = ('catapult@e0a2efbb3d1a81aac3c90041eefec24f066d26ba '
                 'chromium@aaa7336c821888839f759c6c0a36b56c + '
                 'https://codereview.com/repo~branch~id/abc123')
    self.assertEqual(str(c), string)
  ...
  def testAsDictWithArm(self):
    c = Change(chromium=123, patch=False, arm=change.ARM_BASE)
  ...
  def testFromDictWithAllFields(self):
    ...
    expected = Change(chromium=123, catapult=456, patch=True, arm=change.ARM_EXP)
