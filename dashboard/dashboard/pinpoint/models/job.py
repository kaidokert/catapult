...
class Job(ndb.Model):
  ...
  def New(cls, quests, changes, arguments=None, bug_id=None,
          comparison_mode=None, gerrit_change_id=None,
          gerrit_server=None, pin=None, tags=None, user=None):
    ...
    job = cls(state=state, arguments=arguments or {},
              bug_id=bug_id, gerrit_change_id=gerrit_change_id,
              gerrit_server=gerrit_server, tags=tags, user=user)
  ...
  def _Complete(self):
    ...
    self._FormatAndPostBugComment()
    self._UpdateGerritIfNeeded()

  def _FormatAndPostBugComment(self):
