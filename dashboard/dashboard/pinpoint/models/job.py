...
def _FormatCommitForBug(commit_info):
  ...
  if 'repository' in commit_info and 'git_hash' in commit_info:
    git_link = commit_info['repository'] + ' @ ' + commit_info['git_hash']
  else:
    git_link = commit_info['url']
  return '\n'.join((subject, git_link))
