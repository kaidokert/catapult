...
class Job(ndb.Model):
  ...
  def _CreateWorkflowExecutionRequest(self, change_a, change_b):
    ...
    return {
        ...
        'start_git_hash':
            change_a.get('commits')[0].get('git_hash'),
        # TODO: verify and throw exception
        'end_git_hash':
            change_b.get('commits')[0].get('git_hash'),
        'project':
            self.project,
    }
  ...
  def _FormatAndPostBugCommentOnComplete(self):
    ...
    print('self.benchmark_arguments.benchmark {}',
          self.benchmark_arguments.benchmark)
    ...
    if self.benchmark_arguments.benchmark in sandwich_allowlist.ALLOWABLE_BENCHMARKS \
      ...
      workflow_group = sandwich_workflow_group.SandwichWorkflowGroup(
          ...
          tags=self.tags,
          url=self.url,
          improvement_dir=_ImprovementDirectionToStr(improvement_dir))
      workflow_group.put()
      ...
      for change_a, change_b in differences:
        ...
        workflow_execution_request = self._CreateWorkflowExecutionRequest(
            change_a, change_b)
