...
class Job(ndb.Model):
  ...
  def _CreateWorkflowExecutionRequest(self, change_a, change_b, improvement_dir):
    start_git_hash = self._GetGitHash(change_a)
    end_git_hash = self._GetGitHash(change_b)
    logging.debug('Pinpoint - creating verification workflow with improvement direction %s', improvement_dir)
