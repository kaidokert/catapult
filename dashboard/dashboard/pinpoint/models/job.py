...
class Job(ndb.Model):
  ...
  def Start(self):
    ...
    self._print_job_metrics("started")
  ...
  def Fail(self, exception=None):
    ...
    if not self.cancelled:
      self._print_job_metrics("failed", True)
  ...
  def Run(self):
    ...
    try:
      if scheduler.IsStopped(self):
        ...
        self._print_job_metrics("cancelled")
      ...
      if self.completed:
        ...
        self._print_job_metrics("completed", True)
  ...
  def _print_job_metrics(self, job_status, with_run_time=False):
    cloud_metric.publish_pinpoint_job_status_metric(app_identity.get_application_id(), self.job_id, self.comparison_mode, job_status)

    if with_run_time:
      job_run_time = self.updated - self.started_time
      cloud_metric.publish_pinpoint_job_run_time_metric(app_identity.get_application_id(), self.job_id, self.comparison_mode, job_status, job_run_time.total_seconds())
