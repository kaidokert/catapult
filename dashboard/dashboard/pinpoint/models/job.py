...
class Job(ndb.Model):
  ...
  def _MaybeScheduleRetry(self):
    ...
    try:
      logging.debug('crbug/1254010 - reschedule - retry count - %s', self.retry_count)
    except:
  ...
  def Run(self):
    ...
    try:
      ...
      if not self._IsTryJob():
        try: 
          logging.debug('crbug/1254010 - job.run() - Explore - num changes - %s', len(self.state._changes))
        except:
      ...
      if work_left:
        try:
          logging.debug('crbug/1254010 - job.run() - schedule - num changes - %s', len(self.state._changes))
        except:
          logging.debug('crbug/1254010 - log messaged failed')
        self._Schedule()
      else:
