...
class Job(ndb.Model):
  ...
  def _CreateWorkflowExecutionRequest(self, change_a, change_b):
    ...
    return {
      ...
      'bot_name': self.configuration,
      'story': self.benchmark_arguments.story,
      'measurement': self.benchmark_arguments.chart,
      ...
      'target': pinpoint_request.GetIsolateTarget(self.configuration, self.benchmark_arguments.benchmark),
      ...
      'start_git_hash': change_a.get('commits')[0].get('git_hash'),
      # TODO: verify and throw exception
      'end_git_hash': change_b.get('commits')[0].get('git_hash'),
  ...
  def _FormatAndPostBugCommentOnComplete(self):
    ...
    if self.benchmark_arguments.benchmark in sandwich_allowlist.ALLOWABLE_BENCHMARKS \
      ...
      for change_a, change_b in differences:
          ...
          workflow_execution_request = self._CreateWorkflowExecutionRequest(change_a, change_b)
          client = workflow_client.SandwichVerificationWorkflow()
          execution_name = client.CreateExecution(workflow_execution_request)
