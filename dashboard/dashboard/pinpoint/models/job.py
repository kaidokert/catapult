...
class Job(ndb.Model):
  ...
  def _Complete(self):
    ...
    for _, change in differences:
      commit_details.append(_FormatCommitForBug(change.last_commit))
    body = '\n\n'.join(commit_details)
    ...
    for _, change in differences:
      ...
      owner = author  # TODO: Assign the largest difference, not just any one.
      cc_list.add(author)
...
def _FormatCommitForBug(commit):
