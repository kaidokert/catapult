...
class FixesTest(unittest.TestCase):
  ...
  def testMatch(self, commit_range):
    ...
    self.assertEqual(fixes.Get(c), change.Change(c.commits, patch))
  ...
  def testNoMatch(self, commit_range):
    ...
    self.assertEqual(fixes.Get(c), c)

  def testAlreadyHasAPatch(self, commit_range):
    commit_range.return_value = [{'commit': 'a'}, {'commit': 'b'}]
    ...
    c = change.Change((change.Commit('chromium', 'b'),), patch)
