...
def _CommitRangeStub(repository_url, first_git_hash, last_git_hash):
  ...
  if last_git_hash == 'mc_100':
    # In this case we add a randomly interleaved sequence of commits between two
    ...
    # The merge algorithm will pick one from two ranges randomly.
    def _RandomInterleave(*branches):
      while True:
        ...
        # Pop the first element from a random branch.
        index = random.randint(0, len(branches) - 1)
        yield branches[index].pop(0)

    commit_tree = [{
        'git_hash': 'mc_100',
        'parents': ['mc_99', 'oc_99']
    }] + [c for c in _RandomInterleave(branch_other, branch_master)]
    return [_InfoStubWithParents(commit) for commit in commit_tree]
