...
def _CreateJob(request):
  ...
  use_execution_engine = (
      ...
      comparison_mode == 'performance' and
      arguments.get('target') == 'performance_test_suite')
  ...
  if use_execution_engine:
    ...
    task_options = performance_bisection.TaskOptions(
        ...
        read_option_template=performance_bisection.ReadOptionTemplate(
            ...
            mode=('histogram_sets' if {'tir_label', 'story', 'statistic'}
                  & arguments else 'graph_json')),
    ...
    task_module.PopulateTaskGraph(
        job, performance_bisection.CreateGraph(task_options))
  return job
