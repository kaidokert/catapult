...
def _ValidateChanges(arguments):
  ...
  if changes:
    # FromRequest() performs input validation.
    return [change.Change.FromRequest(c) for c in json.loads(changes)]
...
def _ValidatePatch(patch_data):
  if patch_data:
    patch_details = change.GerritPatch.FromRequest(patch_data)
...
def _ValidatePin(pin):
  if not pin:
    return None
  return change.Change.FromRequest(pin)
