...
def _SummarizeResults(cloud_workflows_keys, client, bug_update_builder):
  ...
  for wk in cloud_workflows_keys:
    ...
    if response.state == executions.Execution.State.SUCCEEDED:
      ...
      if result_dict['response'] and result_dict['response'][
          'verification'] and result_dict['response']['verification'][
              'decision']:
        decision = result_dict['response']['verification']['decision']
        if decision:
          bug_update_builder.AddDifference(None, w.values_a, w.values_b, w.kind,
                                           w.commit_dict)
          num_verified += 1
    elif response.state in [
        executions.Execution.State.FAILED, executions.Execution.State.CANCELLED,
