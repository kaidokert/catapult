...
def CulpritVerificationResultsUpdateHandler():
  ...
  for group in workflow_groups:
    if _AllExecutionCompleted(group, client):
      ...
      num_succeeded, num_verified = _SummarizeResults(group.cloud_workflows_keys, client, bug_update_builder)
    ...
    if response.state == executions.Execution.State.SUCCEEDED:
      ...
      if result_dict['response'] and result_dict['response']['verification'] and result_dict['response']['verification']['decision']:
        ...
        if decision:
          ...
          num_verified += 1
    elif response.state in [executions.Execution.State.FAILED, executions.Execution.State.CANCELLED, executions.Execution.State.STATE_UNSPECIFIED]:
      bug_update_builder.AddDifference(None, w.values_a, w.values_b, w.kind, w.commit_dict)
