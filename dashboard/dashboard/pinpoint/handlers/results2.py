...
class Results2(webapp2.RequestHandler):
  ...
  def get(self, job_id):
    try:
      ...
      results = gcs.listbucket(filename)

      for r in results:
        self.response.out.write(json.dumps(
            {
                'status': 'complete',
                'url': 'https://storage.cloud.google.com' + r.filename
            }))
        return

      try:
        task_name = 'results2__%s_l' % job_id
        taskqueue.add(
            queue_name='default', url='/api/results2/' + job_id, name=task_name)
  ...
  def post(self, job_id):
    ...
    # If there are any results for that call, there's a file there. Don't do
    # any work.
    for _ in results:
      return
    ...
    gcs_file = _GcsFileStream(filename, gcs.RetryParams(backoff_factor=1.1))
