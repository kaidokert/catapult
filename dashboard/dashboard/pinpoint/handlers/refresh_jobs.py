...
def _FindFrozenJobs():
  ...
  for j in results:
    cloud_metric.PublishFrozenJobMetric(app_identity.get_application_id(), j.job_id, j.comparison_mode, "frozen-job-found")
...
def _ProcessFrozenJob(job):
  ...
  cloud_metric.PublishFrozenJobMetric(app_identity.get_application_id(), job.job_id, job.comparison_mode, "frozen-job-processing")
  ...
  if retries >= _JOB_MAX_RETRIES:
    ...
    cloud_metric.PublishFrozenJobMetric(app_identity.get_application_id(), job.job_id, job.comparison_mode, "frozen-job-failed")
    return
