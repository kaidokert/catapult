...
def revs(cursor=None):
  ...
  for bs in botsuites:
    ...
    nrs = {n: v for n, v in r.to_dict().iteritems() if n.startswith('r_') and isinstance(v, (int,long))}
    if not nrs: continue
    logging.info('%s %d %r', utils.TestMetadataKey(r.parent_test).id(), r.revision, nrs)
