...
def AddPointPost():
  ...
  try:
    ...
    request_handler.RequestHandlerReportError('User unauthorized.', 403)
    return

  data_str = request.values.get('data')
  if not data_str:
    request_handler.RequestHandlerReportError('Missing "data" parameter.', status=400)
    return
...
def AddData(data_str):
  try:
    ...
    request_handler.RequestHandlerReportError('Invalid JSON string.', status=400)
    return
  ...
  try:
    ...
    request_handler.RequestHandlerReportError(str(e), status=400)
