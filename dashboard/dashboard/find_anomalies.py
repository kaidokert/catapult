...
def _ProcessTestStat(config, test, stat, rows, ref_rows):
  ...
  subscription_names = [s.key.string_id() for s in subscriptions or []]
  ...
  for a in anomalies:
    ...
    a.internal_only = (any([s.internal_only for s in subscriptions]) or
                       test.internal_only)
...
def _MakeAnomalyEntity(change_point, test, stat, rows):
  ...
  new_anomaly = anomaly.Anomaly(
      ...
      internal_only=test.internal_only, # legacy, should be removed with sheriff
      units=test.units,
