...
def SkiaAssociateAlertsHandlerPost():
  ...
  if not utils.IsValidSheriffUser():
    ...
    return make_response(
        json.dumps({'error': str(e)}), http.HTTPStatus.UNAUTHORIZED.value)
  ...
  if bug_id:
    ...
    if not project_id:
      return make_response(
          json.dumps({'error': 'No project id specified to add.'}),
          http.HTTPStatus.BAD_REQUEST.value)
...
def _AssociateAlertsWithBugForSkia(bug_id, project_id, keys):
  ...
  return make_response(json.dumps(AssociateAlerts))
