...
def _GetParentTest(row_dict):
  ...
  internal_only = BotInternalOnly(master_name, bot_name)
...
def BotInternalOnly(master_name, bot_name):
  ...
  as internal-only.
  """
  bot = ndb.Key('Master', master_name, 'Bot', bot_name).get()
  if bot is None:
    logging.warning(
        'No bot whitelist available. All data will be internal-only. If this '
        'is not intended, please use change_internal_only in the dev console.')
    return True
  return bot.internal_only
