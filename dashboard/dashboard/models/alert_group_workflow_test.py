...
class AlertGroupWorkflowTest(testing_common.TestCase):
  ...
  def testAutoMerge_DuplicateGroupRun(self):
    ...
    group = self._AddAlertGroup(
        grouped_anomalies[0],
        issue=self._issue_tracker.NewBug(status='Duplicate', state='closed'),
    ...
    self.assertEqual(self._issue_tracker.calls[2], {
        'method': 'AddBugComment',
        'args': [
            # FakeIssueTrackerService.NewBug creates IDs starting from 12345.
            12345,
            _DUPLICATE_NOTIFICATION_TEMPLATE % (group.string_id(), canonical_group.string_id()),
        ],
        'kwargs': {
            'project': 'chromium',
            'send_email': False
