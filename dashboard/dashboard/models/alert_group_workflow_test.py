...
class AlertGroupWorkflowTest(testing_common.TestCase):
  ...
  @staticmethod
  def _AddAnomaly(is_summary=False,
                  test_str='master/bot/test_suite/measurement/test_case',
  ...
  def testSandwich_Allowlist(self):
    ...
    anomalies = []
    for case, improvement in zip(test_cases, test_improvements):
      anomalies.append(
          self._AddAnomaly(test_str=case, is_improvement=improvement))
    group = self._AddAlertGroup(anomalies[0], anomalies=anomalies)
    self._sheriff_config.patterns = {
        '*': [
            subscription.Subscription(name='sheriff', auto_triage_enable=True)
