...
class AlertGroupWorkflowTest(testing_common.TestCase):
  ...
  def testAutoMerge_SucessfulMerge(self):
    ...
    self.assertEqual(self._issue_tracker.calls[3], {
        ...
        'kwargs': {
            ...
            'labels': ['Type-Bug-Regression', 'Chromeperf-Auto-Triaged', 'Restrict-View-Google', 'Pri-2'],
            'cc_list': [],
            'components': ['Foo>Bar'],
            'project': 'chromium',
            'send_email': False
  ...
  def testAutoMerge_AutoMergeNotOptIn(self):
    ...
    self.assertEqual(len(self._issue_tracker.calls[2]['args']), 2)
    self.assertEqual(self._issue_tracker.calls[2]['args'][0], 42)
    self.assertNotIn('was automatically merged into', self._issue_tracker.calls[2]['args'][1])
    self.assertEqual(self._issue_tracker.calls[2]['kwargs'], {
        'summary': '[%s]: %d regressions in %s' % ('sheriff', 3, 'test_suite'),
        'labels': ['Type-Bug-Regression', 'Chromeperf-Auto-Triaged', 'Restrict-View-Google', 'Pri-2'],
        'cc_list': [],
  ...
  def testAutoMerge_NoCanonicalIssue(self):
    ...
    self.assertNotIn('was automatically merged into', self._issue_tracker.calls[1]['args'][1])
    self.assertEqual(self._issue_tracker.calls[1]['kwargs'], {
        'summary': '[%s]: %d regressions in %s' % ('sheriff', 3, 'test_suite'),
        'labels': ['Type-Bug-Regression', 'Chromeperf-Auto-Triaged', 'Restrict-View-Google', 'Pri-2'],
        'cc_list': [],
        'components': ['Foo>Bar'],
  ...
  def testAutoMerge_SucessfulMerge_AutoMergeForOneAnomaly(self):
    ...
    self.assertEqual(self._issue_tracker.calls[3], {
        ...
        'args': (42, None),
        'kwargs': {
            'summary': '[%s]: %d regressions in %s' % ('sheriff', 3, 'regular_suite'),
            'labels': ['Type-Bug-Regression', 'Chromeperf-Auto-Triaged', 'Restrict-View-Google', 'Pri-2'],
            'cc_list': [],
            'components': ['Foo>Bar'],
            'project': 'chromium',
  ...
  def testPrepareGroupUpdate_DuplicateGroupFound(self):
    ...
    duplicate_anomalies = [self._AddAnomaly(groups=[duplicate_group]), self._AddAnomaly(groups=[duplicate_group])]

    canonical_issue = self._issue_tracker.GetIssue(self._issue_tracker.NewBug()['bug_id'])
    canonical_group = self._AddAlertGroup(
        base_anomaly,
        issue=canonical_issue,
        status=alert_group.AlertGroup.Status.triaged,
    )
    canonical_anomalies = [self._AddAnomaly(groups=[canonical_group]), self._AddAnomaly(groups=[canonical_group])]
    ...
    self.assertEqual(update.anomalies, [duplicate_anomalies + canonical_anomalies])
  ...
  def testPrepareGroupUpdate_CanonicalGroupFound(self):
    ...
    anomalies = [self._AddAnomaly(groups=[duplicate_group]), self._AddAnomaly(groups=[duplicate_group])]
    ...
    self._issue_tracker.AddBugComment(duplicate_issue['id'], cc=['someone'])
    # According to Monorail API documentation, mergedInto has string type.
    self._issue_tracker.AddBugComment(duplicate_issue['id'], status='Duplicate', mergedInto=str(canonical_issue['id']))
    ...
    self.assertEqual(update.anomalies, [anomalies])
    self.assertIsNotNone(update.issue)
    self.assertEqual(update.canonical_group, canonical_group)

    w._FindDuplicateGroups.assert_called_once()
    w._FindRelatedAnomalies.assert_called_once_with([duplicate_group.get()])
