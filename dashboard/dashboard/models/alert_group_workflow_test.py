...
class AlertGroupWorkflowTest(testing_common.TestCase):
  ...
  def testPrepareGroupUpdate_DuplicateGroupFound(self):
    ...
    duplicate_issue = self._issue_tracker.GetIssue(
        self._issue_tracker.NewBug(status='Duplicate', state='closed')['bug_id'])
    duplicate_anomalies = [self._AddAnomaly(), self._AddAnomaly()]
    duplicate_group = self._AddAlertGroup(
        duplicate_anomalies[0],
        issue=duplicate_issue,
        anomalies=duplicate_anomalies,
        status=alert_group.AlertGroup.Status.triaged,
    )

    canonical_issue = self._issue_tracker.GetIssue(self._issue_tracker.NewBug()['bug_id'])
    canonical_anomalies = [self._AddAnomaly(), self._AddAnomaly()]
    canonical_group = self._AddAlertGroup(
        canonical_anomalies[0],
        issue=canonical_issue,
    ...
    w._FindDuplicateGroups = mock.MagicMock(return_value=[duplicate_group])
    w._FindRelatedAnomalies = mock.MagicMock(return_value=[duplicate_anomalies + canonical_anomalies])
    ...
    w._FindRelatedAnomalies.assert_called_with([canonical_group.get(), duplicate_group.get()])
