...
class AlertGroupWorkflowTest(testing_common.TestCase):
  ...
  # === Delay Triage ===
  # Delay Triage will be enabled when all the followings are true:
  #  1. auto bisect is enabled
  #  2. the subscription is not opted out.
  # If it is enabled:
  #  the component/cc/labels in subscription will not be added in issue.
  @mock.patch('dashboard.common.utils.ShouldDelayIssueTriage',
              mock.MagicMock(return_value=True))
  def testTriage_GroupUntriaged_DelayTriage_Delayed(self):
    ...
    self.assertIn(
        'Chromium Commit Position: http://test-results.appspot.com/revision_range?start=0&end=100',
        self._issue_tracker.new_bug_kwargs['description'])
    self.assertIn(utils.DELAY_TRIAGE_PLACEHOLDER,
                  self._issue_tracker.new_bug_kwargs['components'])
    self.assertIn(utils.DELAY_TRIAGE_LABEL,
  ...
  @mock.patch('dashboard.common.utils.ShouldDelayIssueTriage',
              mock.MagicMock(return_value=True))
  @mock.patch('dashboard.models.alert_group_workflow._DELAY_TRIAGE_OPTOUT_SUBS',
              ['OptOutSub'])
  def testTriage_GroupUntriaged_DelayTriage_NotDelayed_OptedOut(self):
    test_subscription = 'OptOutSub'
    enable_auto_bisect = True
    anomalies = [self._AddAnomaly(), self._AddAnomaly()]
    group = self._AddAlertGroup(
        anomalies[0],
    ...
    self.assertIn(
        'Chromium Commit Position: http://test-results.appspot.com/revision_range?start=0&end=100',
        self._issue_tracker.new_bug_kwargs['description'])
    self.assertNotIn(utils.DELAY_TRIAGE_PLACEHOLDER,
                     self._issue_tracker.new_bug_kwargs['components'])
    self.assertNotIn(utils.DELAY_TRIAGE_LABEL,
                     self._issue_tracker.new_bug_kwargs['labels'])
  ...
  @mock.patch('dashboard.common.utils.ShouldDelayIssueTriage',
              mock.MagicMock(return_value=True))
  def testTriage_GroupUntriaged_DelayTriage_NotDelayed_NoBisect(self):
    test_subscription = 'AnySub'
