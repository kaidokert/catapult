...
class AlertGroupWorkflowTest(testing_common.TestCase):
  ...
  def testPrepareGroupUpdate_AlertGroupBugBackwardsCompatiblity(self):
    ...
    print('---- Bug %r Backwards bug %r' % (self._group.bug, self._group.bug_backwards_compatible))
    print('---- _ug %s Backwards bug %s' % (str(self._group.bug), str(self._group.bug_backwards_compatible)))

    group_entity = group.get()
    group_entity.bug_backwards_compatible = alert_group.BugInfo(
        bug_id=self._issue_tracker.issue.get('id'),
        project=self._issue_tracker.issue.get('projectId', 'chromium'),
    )
    group_entity.put()

    print('2--- Bug %r Backwards bug %r' % (self._group.bug, self._group.bug_backwards_compatible))
    print('2--- _ug %s Backwards bug %s' % (str(self._group.bug), str(self._group.bug_backwards_compatible)))
