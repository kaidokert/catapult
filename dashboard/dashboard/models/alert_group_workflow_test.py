...
class AlertGroupWorkflowTest(testing_common.TestCase):
  ...
  def testSandwich_TryVerifyRegression_sandwiched_SUCCEEDED_skipBisect(self):
    ...
    self.assertEqual(
        self._issue_tracker.calls[1]['kwargs'], {
            'comment':
                '🥪 Regression verification execution-id-0 job '
                "pinpoint-job-id-12345 for test: Key('TestMetadata', "
                "'master/android-pixel2-perf/speedometer2/dummy/metric/parts')\n"
                'did NOT reproduce the regression with statistic: .\n'
                'Issue closed.',
            'status': 'WontFix',
            'labels':
    ...
    self.assertEqual(
        self._issue_tracker.calls[2]['kwargs'], {
            ...
            'comment':
                ...
                '  '
                'https://chromeperf.appspot.com/group_report?group_id=%s\n'
                '\n'
                'Top 1 affected measurements in bot:\n'
                '  - test_suite/measurement/test_case\n'
                '       18.18%%: 1.1 -> 1.3 \n' % group.id(),
            'send_email':
                False
        })
