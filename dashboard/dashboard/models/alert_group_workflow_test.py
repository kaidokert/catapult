...
class AlertGroupWorkflowTest(testing_common.TestCase):
  ...
  def testPrepareGroupUpdate_DuplicateGroupFound(self):
    ...
    self.assertEqual(update.anomalies, [a.get() for a in canonical_anomalies + duplicate_anomalies])
  ...
  def testPrepareGroupUpdate_CanonicalGroupLoop(self):
    ...
    looped_issue = self._issue_tracker.GetIssue(self._issue_tracker.NewBug()['bug_id'])
    ...
    self._issue_tracker.issue_comments.update({
        ('chromium', duplicate_issue['id']): [{
            ...
            'updates': {
                'status': 'Duplicate',
                # According to Monorail API documentation, mergedInto has string type.
                'mergedInto': str(looped_issue['id'])
            },
        }]
    })
    ...
    self.assertEqual(update.canonical_group, None)
