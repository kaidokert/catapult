...
class AlertGroupWorkflowTest(testing_common.TestCase):
  ...
  def testAddAnomalies_GroupUntriaged_handlePartialDeviceChangeData(self):
    anomalies = [self._AddAnomaly(), self._AddAnomaly()]
    added = [
        self._AddAnomaly(
            False,
            # Only set the bot_id_after_anomaly value, simulating
            # how this would behave on sketchy or incomplete prod data.
            bot_id_after_anomaly='device-b456',
        ),
        self._AddAnomaly(),
    ]
    ...
    # Should count all anomalies as valid:
    self.assertEqual(len(group.get().anomalies), 4)
