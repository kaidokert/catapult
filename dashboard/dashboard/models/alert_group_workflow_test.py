...
class AlertGroupWorkflowTest(testing_common.TestCase):
  ...
  def testSandwich_TryVerifyRegression_sandwiched_FAILED(self):
    ...
    # Post-conditions:
    # - The anomaly's AlertGroup state is now 'bisected' (if sandwich fails it should fail safe into the default behavior)
    ...
    # - A pinpoint bisection job has been started for the alert group
    # - The issue tracker has been called to update the bug with label Chromeperf-Auto-Bisected
    feature_flags.SANDWICH_VERIFICATION = True
  ...
  def testSandwich_TryVerifyRegression_sandwiched_SUCCEEDED_skipBisect(self):
    ...
    # - The issue tracker has beeb called to update associated issue as closed / WontFix
    feature_flags.SANDWICH_VERIFICATION = True
