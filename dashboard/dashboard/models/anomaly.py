...
class Anomaly(internal_only_model.InternalOnlyModel):
  ...
  def QueryAsync(
    ...
    while not results and time.time() < deadline:
      ...
      if test:
        query = query.filter(cls.test == utils.TestMetadataKey(test))
      if master_name:
      ...
      start = time.time()
      results, start_cursor, more = yield query.fetch_page_async(
      ...
      if not start_cursor:
        break
    raise ndb.Return((results, start_cursor))
