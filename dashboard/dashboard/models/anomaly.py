...
class Anomaly(internal_only_model.InternalOnlyModel):
  ...
  # This is not the "bot_name" as in "the id of the physical hardware device in
  # the perf lab but rather the "bot_name" as in "the name of device
  # configuration" e.g. "win-10-perf"
  ...
  def absolute_delta(self):
    ...
    try:
      ...
      msg = "Anomaly key: %s failed to calculate absolute delta." % self.key
      logging.error(msg)
      raise e
