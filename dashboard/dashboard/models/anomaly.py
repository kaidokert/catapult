...
class Anomaly(internal_only_model.InternalOnlyModel):
  ...
  def GetAll(cls,
    ...
    query = cls._InequalityFilters(
        query, inequality_property_name, min_start_revision, max_start_revision,
        min_end_revision, max_end_revision, min_timestamp, max_timestamp)

    # All properties must be involved in the query in order to hit the single
    # index. There can only be one inequality filter, but arbitrarily many sort
    # orders.
    query = query.order(cls.internal_only)
    query = query.order(cls.bug_id)
    query = query.order(cls.sheriff)
    query = query.order(cls.is_improvement)
    query = query.order(cls.recovered)
    query = query.order(cls.test)
    query = query.order(cls.master_name)
    query = query.order(cls.bot_name)
    query = query.order(cls.benchmark_name)

    results = yield query.fetch_async(limit=limit)
    results = cls._PostFilter(
        results, inequality_property_name, min_start_revision,
        max_start_revision, min_end_revision, max_end_revision, min_timestamp,
        max_timestamp)
    raise ndb.Return(results)

  @classmethod
  def _InequalityFilters(cls, query, inequality_property_name,
  ...
  @staticmethod
  def _PostFilter(
      results, inequality_property_name, min_start_revision, max_start_revision,
      min_end_revision, max_end_revision, min_timestamp, max_timestamp):
    postfilter = False
    if inequality_property_name == 'start_revision':
      postfilter = (
      ...
      postfilter = (
          min_start_revision is not None or
