...
class SparseDiagnosticTest(testing_common.TestCase):
  ...
  def _CheckExpectations(self, diagnostic, guid_mapping, expected):
    ...
    for s in sparse:
      print '%s  %s %s   -- %s' % (s.key.id(), s.start_revision, s.end_revision, s.data)
    print
  ...
  def testFindOrInsertDiagnostics_OutOfOrder_Splits_CurDiff_NextDiff(self):
    ...
    # No expected modifications since there are no revisions after the split
    # point.
    self.assertEqual(
        [],
        modified_ranges)
  ...
  def testFindOrInsertDiagnostics_OutOfOrder_Clobber_Next_NoRevs_Diff(self):
    ...
    # Overwrites and thus no expected changes.
    self.assertEqual(
        [],
        modified_ranges)
    self._CheckExpectations(
