...
class AlertGroup(ndb.Model):
  ...
  def Update(self):
    ...
    if self.status == self.Status.triaged:
      sheriff_config = sheriff_config_client.GetSheriffConfigClient()
      updated = [a for a in anomalies if a.key not in self.anomalies]
      for a in updated:
        subscriptions, _ = sheriff_config.Match(a.test.string_id(), check=True)
        if not a.bug_id and any(s.auto_triage_enable for s in subscriptions):
