...
class AlertGroup(ndb.Model):
  ...
  def Update(self, now, active_window, triage_delay):
    ...
    if self.updated + active_window < now and (
      ...
      print(self.updated + active_window, now)
      self._Archive()
  ...
  def _NewPinpointBisect(self):
    ...
    regression = max(bisect_enabled, key=lambda r: r.relative_delta)
    request = _NewPinpointRequest(regression)
...
def _NewPinpointRequest(alert):
  if not hasattr(_NewPinpointRequest, 'user'):
    _NewPinpointRequest.user = utils.ServiceAccountEmail()
  user = _NewPinpointRequest.user
