...
class ReportQuery(object):
  ...
  def FetchAsync(self):
    ...
    for tri, table_row in enumerate(self._report['rows']):
      for rev, data in table_row['data'].iteritems():
        table_row['data'][rev] = self._MergeCell(tri, data)
    raise ndb.Return(self._report)

  def _MergeCell(self, tri, data):
    descriptors = []
    statistics = RunningStatistics()
    for datum in data:
      if not self._MergeDatum(tri, datum):
        continue
    ...
    return {'statistics': statistics, 'descriptors': descriptors}

  def _MergeDatum(self, tri, datum):
    ...
    for data in self._report['rows'][tri].itervalues():
  ...
  def _GetUnsuffixedCell(self, table_row, desc, test, test_path, rev):
    ...
    mrk = (desc.test_suite, desc.bot, self._report['rows'].index(table_row))
