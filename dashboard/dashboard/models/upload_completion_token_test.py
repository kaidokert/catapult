...
class UploadCompletionTokenTest(testing_common.TestCase):
  ...
  def testPutAndUpdate(self):
    ...
    # Set 5 seconds to be sure that there won't be any false negative results.
    max_delta = datetime.timedelta(seconds=5)
    ...
    # Sleep for max_delta second, so update_time change is visible.
    time.sleep(max_delta.total_seconds())
