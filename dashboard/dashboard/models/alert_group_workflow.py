...
class AlertGroupWorkflow(object):
  ...
  def _TryBisect(self, now, anomalies):
    try:
      job_id, bisected = self._NewPinpointBisect(anomalies)
      if not job_id:
        return
  ...
  def _SelectAutoBisectRegression(regressions):
    ...
    for _, rs in itertools.groupby(
      ...
      for r in rs:
        count += 1
        group_max = MaxRregression(group_max, r)
      if count >= max_count:
        max_count = count
        max_regression = MaxRregression(max_regression, group_max)
  ...
  def _UpdateWithBisectError(self, now, error):
    ...
    self._issue_tracker.AddBugComment(
        self._group.bug.bug_id,
        'Auto-Bisection failed with %s. Not retrying.' % (error,),
