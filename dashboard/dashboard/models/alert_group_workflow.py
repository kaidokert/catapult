...
class AlertGroupWorkflow:
  ...
  def Process(self, update=None):
    ...
    elif len(self._CheckSandwichAllowlist(update.anomalies)) > 0: # and not group.verified:
      logging.info('attempting sandwich verification for AlertGroup: %s', self._group.key.string_id())
  ...
  def _TryVerifyRegression(self, update):
    ...
    # TODO: try/catch, as soon as we can figure out what errors or exceptions this
    # client may raise. The docs are useless for determining this, unfortunately.
    sandwich_execution_id = self._cloud_workflows.CreateExecution(
      {
        ...
        'target':
            pinpoint_request.GetIsolateTarget(
                regression.bot_name, regression.benchmark_name),
