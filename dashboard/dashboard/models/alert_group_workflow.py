...
class AlertGroupWorkflow:
  ...
  def Process(self, update=None):
    ...
    for a in update.anomalies:
      ...
      non_sandwich_subs = [s for s in matching_subs if s.name not in sandwich_allowlist.ALLOWABLE_SUBSCRIPTIONS]
  ...
  def _UpdateRegressionVerification(self, execution, regression, update):
    ...
    if execution['state']  == workflow_service.EXECUTION_STATE_SUCCEEDED:
      ...
      if decision:
        ...
        components = list(
          ...
          # Intentionally ignoring _GetComponentsFromRegressions in this case for sandwiched regressions.
          # See https://bugs.chromium.org/p/chromium/issues/detail?id=1459035
        )
      else:
        comment = ('Regression verification %s job %s for test: %s\n'
    ...
    logging.info('would have set issue components to %s, but components are disabled for sandwiched alert group issues',
      components)
    components = []
    perf_issue_service_client.PostIssueComment(
        self._group.bug.bug_id,
  ...
  def _GetComponentsFromSubscriptions(self, subscriptions):
    #subscriptions = [s for s in subscriptions if s.name not in sandwich_allowlist.ALLOWABLE_SUBSCRIPTIONS]
