...
class AlertGroupWorkflow:
  ...
  def _FilterOutSwarmingBotChangeAnomalies(self, anomalies):
    ret = []
    for a in anomalies:
      ...
      if (a.bot_id_before_anomaly is not None
        ...
        logging.info(
            'ignoring and deleting anomaly %s, reason: device changed (before: %s, after: %s)',
  ...
  def Process(self, update=None):
    ...
    filtered_anomalies = self._FilterOutSwarmingBotChangeAnomalies(
        update.anomalies)
