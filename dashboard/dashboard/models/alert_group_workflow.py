...
"""The workflow to manipulate AlertGroup.
We want to seperate the workflow from data model it self. So workflow
is used to implement all AlertGroup's state transition.

A typical workflow including these steps:
- Update AlertGroup it self (etc. anomalies link to it)
- Upaate related issues
- Trigger auto-triage if possible
- Trigger auto-bisection if possible
- Manage AlertGroup's lifecycle

`AlertGroupWorkflow(group).Process()` is enough for most of use cases.
But it provided the ability to mock any input and any service, which makes
...
class AlertGroupWorkflow(object):
  ...
  def Process(self, update=None):
    """Process workflow on group.
    The workflow promise only depending on update and services as input.
    So the result is reproducable if all inputs are same. And the group is
    always putted when Process returned."""
    update = update or self._GetUpdate()
