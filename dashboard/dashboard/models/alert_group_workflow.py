...
class AlertGroupWorkflow(object):
  ...
  def Process(self, update=None):
    ...
    if update.issue:
      canonical_group_updated = self._UpdateCanonicalGroup(
          update.anomalies, update.canonical_group)
  ...
  def _UpdateCanonicalGroup(self, anomalies, canonical_group):
    ...
    if ((canonical_group is None and self._group.canonical_group is not None)
        or (self._group.canonical_group != canonical_group.key
