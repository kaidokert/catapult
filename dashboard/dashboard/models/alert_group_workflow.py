  ...
  def _TryBisect(self, update):
    ...
    try:
      ...
      if self._group.status == self._group.Status.sandwiched:
        ...
        if sandwich_state in {
            workflow_service.EXECUTION_STATE_FAILED,
            workflow_service.EXECUTION_STATE_CANCELLED
        }:
          # Don't return here, but let control flow fall through this conditional
          # block and pick back up at the non-sandwich _TryBisect logic after logging
          # the error from the cloud workflow execution.
          sandwich_error = sandwich_exec['error']
          logging.error(
              'sandwich verification workflow resulted in error: %s',
              sandwich_error)
          ...
          if sandwich_decision:
            logging.info('verified anomaly %s, proceeding with auto-bisection for %s',
                sandwiched_anomaly, sandwiched_statistic)
          else:
            self._group.updated = update.now
            self._group.status = self._group.Status.closed
            self._CommitGroup()
