...
class AlertGroupWorkflow:
  ...
  def _UpdateRegressionVerification(self, execution, regression, update):
    ...
    if execution['state']  == workflow_service.EXECUTION_STATE_SUCCEEDED:
      ...
      if (decision and 'control_median' in results_dict
          and 'treatment_median' in results_dict):
        ...
        if relative_magnitude < _RELATIVE_MAGNITUDE_THRESHOLD:
          ...
          logging.warning(
              ...
              'regression magnitude: %s is much less than comparison '
              'magnitude %s.', execution['name'], regression_magnitude,
              regression.comparison_magnitude)
      logging.info(
