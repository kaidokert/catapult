    ...
    for name, diag in hist.diagnostics.items():
      if name in histogram_helpers.SUITE_LEVEL_SPARSE_DIAGNOSTIC_NAMES:
        ...
        if existing_entity is None:
          #print("would have created a new SparseDiagnostic entity for ", name)
          dictionary = dict(
              key=diag.guid,
              id=diag.guid,
              data=diag.AsDict(),
...
def _MakeRowDict(revision, test_path, tracing_histogram, stat_name=None):
  ...
  for diag_name, annotation in DIAGNOSTIC_NAMES_TO_ANNOTATION_NAMES.items():
    ...
    if diag_name == reserved_infos.REVISION_TIMESTAMPS.name:
      ...
      #print(diag_name, revision_info.guid, 'should be revision info?')
...
def FindOrInsertDiagnostics(new_entities, test, rev, last_rev):
  print("find or insert diagnostics: %s" % new_entities)
  ret = {}
  for new_entity in new_entities:
    print("setting %s to %s" % (new_entity.id, new_entity))
    ret[new_entity.id] = new_entity
...
def ProcessDiagnostics(diagnostic_data, revision, test_key, internal_only):
  ...
  print ("returning %s" % new_guids_to_existing_diagnostics)
...
def _AddHistogramFromData(params, revision, test_key, internal_only):
  ...
  print ("got %s" % new_guids_to_existing_diagnostics)
  for new_guid, existing_diagnostic in new_guids_to_existing_diagnostics.items():
    print("ed: %s" % new_guid)
    print("ed:", new_guids_to_existing_diagnostics[new_guid])
  ...
  dictionary = dict(
      ...
      revision=revision,
      internal_only=internal_only)
  return namedtuple("FakeHistogramEntityClass", dictionary.keys())(*dictionary.values())
...
def _ProcessRowAndHistogram(params):
  """
  TODO(sean): in here we should be able to separate the actual metric,
  as well as a few dimensions like story (name, tags etc), build
  and run (master, bot etc).
  """
  ...
  print('num_values: %s' % hist.num_values)
  ...
  for stat_name, scalar in statistics_scalars.items():
    ...
    print('\tmetric: %s' % histogram_name)
    print('\taggregate function: %s' % stat_name)
    print('\trest: %s' % rest)
    ...
    rows = _AddRowsFromData(params, revision, parent_test, legacy_parent_tests)
    print("added %d rows" % len(rows))

    for h in _AddHistogramFromData(params, revision, test_key, internal_only):
      print("added histogram: %s" % h)
...
if __name__ == '__main__':
  ...
  #print(json.dumps(histogram_dicts))
