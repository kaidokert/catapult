...
class ReadRowsForAnomalies(beam.PTransform):
  ...
  def _FilterForQueryRowsForAnomaly(k_anomaly):
    ...
    if test_key.kind == 'Test':
      #row_parent_test = beam_ds_types.Key.from_client_key(test_key)
      ...
      for part in path_parts[2:]:
        key_parts += ['Test', part]
      #row_parent_test = beam_ds_types.Key(key_parts)
    ...
    ##return ds_query.Query(
    ##    client=None,  # this is set later by _QueryFn.process
    ##    namespace='[default]',
    ##    project='chromeperf',
    ##    kind='Row',
    ##    filters=[
    ##        ('parent_test', '==', row_parent_test),
    ##        ('revision', '>=', anomaly['start_revision']),
    ##        ('revision', '>=', anomaly['end_revision']),
    ##    ],
    ##)
