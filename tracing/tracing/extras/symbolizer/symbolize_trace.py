...
# Matches paths from Android mapped file names and the library name and
# extension can be obtained from groups 'name' and 'extn' respectively.
ANDROID_PATH_MATCHER = re.compile(r'.*/(?P<name>.*)\.(?P<extn>(so|apk))')
...
def RemapAndroidFiles(symfiles, output_path, chrome_soname):
  for symfile in symfiles:
    ...
    if match:
      ...
      extn = match.group('extn')
      if 'so' in extn:
        symfile.symbolizable_path = os.path.join(
            output_path, ANDROID_UNSTRIPPED_SUBPATH, name + '.' + extn)
      elif extn == 'apk' and chrome_soname:
        ...
        print ('Problem with ', symfile.path, ': Cannot be mapped to any '
               'library file.')
    else:
