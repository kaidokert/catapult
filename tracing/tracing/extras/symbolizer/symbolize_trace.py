...
def ReplaceTypeNameWithPC(process, allocation):
  ...
  if types:
    ...
    GetFrameParent = lambda frame_id : frame_id\
        if process._stack_frame_map._frame_by_id[frame_id].parent_id is None\
        else process._stack_frame_map._frame_by_id[frame_id].parent_id

    for i in range(type_size):
      # Skip two frames to avoid hook functions
      parent = nodes[i]
      parent = GetFrameParent(GetFrameParent(parent))
      type_id = process._type_name_map.AddType(
          ...
          if maps and allocators:
            ...
            if self._frame_as_object_type:
              for alloc in allocators.values():
                ReplaceTypeNameWithPC(process, alloc)
...
def main(args):
  ...
  parser.add_argument(
      ...
      help="Indicate that the object types should in frame name type.")
