...
def GetFrameParent(process, frame_id):
  if frame_id is None:
    parent = frame_id
  else:
    parent = process._stack_frame_map._frame_by_id[frame_id].parent_id
  return parent
...
def ReInsertTypeWithPC(process, allocation):
  ...
  if types:
    ...
    for i in range(type_size):
      ...
      parent = nodes[i]
      parent = GetFrameParent(process, parent)
      parent = GetFrameParent(process, parent)
          ...
          if maps and allocators:
            ...
            if self._frame_as_object_type:
              for alloc_type in alloc_types:
                types = allocators.get(alloc_type)
                if types:
                  ReInsertTypeWithPC(process, types)
