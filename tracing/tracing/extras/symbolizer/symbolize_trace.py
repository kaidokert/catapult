...
class TypeNameMap(NodeWrapper):
  ...
  """
  class Type(object):
    """Holds ids, names and PCs if the type names are in the form of PCs.

    We get PCs from names ,resolve the corresponding mmap region and symbolize
    PCs with corresponding function names.
    """
  ...
  def ReplaceTypeNameWithPC(self, heap_dump_version, type_name_node,
    ...
    """Get the corresponding PCs of type from frame names.
    ...
    for count in range(type_size):
      # Skip two frames to avoid hook functions if the OS version is linux and
      # get the frame which called new function
...
def ResolveSymbolizableFilesByNodes(
  ...
  (stack frame or object type) to the corresponding mmap region. Nodes that
  failed to resolve are symbolized with '<unresolved>'.
  """
  for node in nodes:
    if node.pc is None:
      continue
...
def ResolveSymbolizableFiles(processes, trace_from_win, frame_as_object_type):
  ...
  object-type" is enabled, the type name will be PCs, which need to be resolved
  and symbolized.
  """
  symfile_by_path = {}
...
def main(args):
  ...
  parser.add_argument(
      ...
      help="Indicate that output of object types should use frame name.")
