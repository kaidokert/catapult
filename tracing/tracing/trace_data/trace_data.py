...
class TraceDataBuilder(object):
  ...
  def AddTraceFor(self, part, trace, unstructured=False):
    if self._raw_data is None:
    ...
    if isinstance(trace, StringTypes):
      if not unstructured:
        raise ValueError('must pass unstructured=True for text data')
    elif not isinstance(trace, (dict, TraceFileHandle)):
...
def CreateTestTrace(number=1):
  ...
  responsible for cleaning up trace data as usual.

  Clients are meant to treat these test traces as opaque. No guarantees are
  mande about their contents, which they shouldn't try to read.
  """
...
def CreateFromRawChromeEvents(events):
  ...
  representation, so there is no need for cleaning up any trace files.
