...
class _TraceData(object):
  ...
  may require large amounts of memory if the traces to process are very big.
  ...
  pass the serialized output to a trace processing script.
  """
  ...
  def HasTracesFor(self, part):
    ...
    if part.raw_field_name not in self._raw_data:
      return False
    return len(self._raw_data[part.raw_field_name]) > 0
  ...
  def AsData(self):
    # TODO(crbug.com/928278): Temporary method needed while we transition the
    # output of tracing_controller.StopStracing from _TraceData to a frozen
    # TraceDataWriter.
    return self
...
class TraceDataWriter(object):
  """TraceDataWriter helps writing a trace from multiple trace agents.

  Note that, to save memory, the trace writer collects trace data into disk.
  Clients are thus responsible of calling CleanUpTraceData() to free up the
  resources held by the writer when they are done with it. Alternatively,
  legacy clients can call AsData() to load the trace data into memory and also
  free up the disk resources.
  """
  def __init__(self):
    self._raw_data = {}
  ...
  def CleanUpTraceData(self):
    """Free up the resources held by this trace writer."""
    data = self.AsData()
    data.CleanUpTraceData()
