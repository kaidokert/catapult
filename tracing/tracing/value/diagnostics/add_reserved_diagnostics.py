...
def AddReservedDiagnostics(histogram_dicts, names_to_values):
  ...
  if len(hs_with_stories) != 0:
    first_hist = hs_with_stories.GetFirstHistogram()
    had_failures = first_hist.diagnostics.get(reserved_infos.HAD_FAILURES.name)
    if had_failures and len(had_failures) == 1:
      had_failures = had_failures.GetOnlyElement()
    else:
      had_failures = False
  else:
    had_failures = False
