...
def _AddDiagnostic(dicts, name, diag):
  hs = histogram_set.HistogramSet()
  hs.ImportDicts(dicts)
  for h in hs:
    h.diagnostics[name] = diag
  return hs.AsDicts()
...
def ComputeTestPath(hist):
  ...
  is_summary = list(
      hist.diagnostics.get(reserved_infos.IS_SUMMARY.name, []))
...
def AddReservedDiagnostics(histogram_dicts, names_to_values):
  ...
  for h in hs:
    h.diagnostics[reserved_infos.IS_SUMMARY.name] = (
