...
class TagMapUnittest(unittest.TestCase):
  ...
  def testRoundtrip(self):
    ...
    self.assertEqual(histogram_unittest.ToJSON(_SortStories(d)),
                     histogram_unittest.ToJSON(_SortStories(
                         clone.AsDict(None))))
    self.assertSetEqual(
  ...
  def testMerge(self):
    ...
    m0 = diagnostic.FromDict('TagMap', t0.AsDict(None), None)
    ...
    m1 = diagnostic.FromDict('TagMap', t1.AsDict(None), None)
    m1.AddDiagnostic(t0)

    self.assertDictEqual(_SortStories(m0.AsDict(None)),
                         _SortStories(m1.AsDict(None)))

    m2 = diagnostic.FromDict('TagMap', t1.AsDict(None), None)

    self.assertNotEqual(_SortStories(m2.AsDict(None)),
                        _SortStories(m0.AsDict(None)))

    # Test round-tripping of merged diagnostic
    clone = diagnostic.FromDict('TagMap', m0.AsDict(None), None)
