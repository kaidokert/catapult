...
class HistogramGroupingUnittest(unittest.TestCase):
  ...
  def testReservedGenericSetGroupings(self):
    self.assertIsInstance(
        histogram_grouping.BY_KEY[reserved_infos.ARCHITECTURES.name],
        histogram_grouping.GenericSetGrouping)
    self.assertIsInstance(histogram_grouping.BY_KEY[
    ...
    self.assertIsInstance(
        histogram_grouping.BY_KEY[reserved_infos.MEMORY_AMOUNTS.name],
        histogram_grouping.GenericSetGrouping)
    self.assertIsInstance(histogram_grouping.BY_KEY[
    ...
    self.assertIsInstance(
        histogram_grouping.BY_KEY[reserved_infos.PRODUCT_VERSIONS.name],
        histogram_grouping.GenericSetGrouping)
    self.assertIsInstance(histogram_grouping.BY_KEY[
        reserved_infos.STORIES.name], histogram_grouping.GenericSetGrouping)
    self.assertIsInstance(
        histogram_grouping.BY_KEY[reserved_infos.STORYSET_REPEATS.name],
        histogram_grouping.GenericSetGrouping)
  ...
  def testDateRange(self):
    ...
    self.assertEqual(
        grouping.callback(hist), hist.diagnostics['foo'].ToString())

  def testReservedDateRangeGroupings(self):
    self.assertIsInstance(
        histogram_grouping.BY_KEY[reserved_infos.BENCHMARK_START.name],
