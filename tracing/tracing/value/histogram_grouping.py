...
class HistogramGrouping(object):
  ...
  def __init__(self, key, callback):
    ...
    HistogramGrouping.BY_KEY[key] = self
  ...
  def BuildFromTags(tags, diagnostic_name):
    ...
    groupings = [
        HistogramGrouping._BuildBooleanTagGrouping(tag, diagnostic_name)
        for tag in boolean_tags]
    groupings += [
        HistogramGrouping._BuildKeyValueTagGrouping(key, diagnostic_name)
        for key in key_value_tags]
    return groupings
  ...
  def _BuildBooleanTagGrouping(tag, diagnostic_name):
    def Closure(hist):
      ...
      if not tags or tag not in tags:
        return '~' + tag
      return tag
    return HistogramGrouping(tag, Closure)
  ...
  def _BuildKeyValueTagGrouping(key, diagnostic_name):
    ...
    return HistogramGrouping(key, Closure)
...
class GenericSetGrouping(HistogramGrouping):
  ...
  def __init__(self, name):
    super(GenericSetGrouping, self).__init__(
        name, lambda hist: self._Compute(hist))
...
class DateRangeGrouping(HistogramGrouping):
  ...
  def __init__(self, name):
    super(DateRangeGrouping, self).__init__(
        name, lambda hist: self._Compute(hist))
