  ...
  def Deserialize(data, deserializer):
    ...
    if statistics_names:
      for stat_name in statistics_names:
        ...
        elif stat_name.startswith('ci_'):
          percent = None
          if stat_name[-6:] in ['_lower', '_upper']:
            percent = PercentFromString(stat_name[3:-6])
          else:
            percent = PercentFromString(stat_name[3:])
          if percent and percent not in hist._summary_options.get('ci'):
            hist._summary_options.get('ci').append(percent)
