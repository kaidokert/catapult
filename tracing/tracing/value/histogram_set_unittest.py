...
def _AddHist(hist_set, name=None, unit=None):
  ...
  hist.name = name or '_'
  hist.unit.unit = unit or histogram_pb2.MS
  return hist
...
class HistogramSetUnittest(unittest.TestCase):
  ...
  def testBasicImportFromProto(self):
    ...
    hist = hist_set.histograms.add()
    hist.name = 'metric1'
    hist.unit.unit = histogram_pb2.TS_MS

    hist = hist_set.histograms.add()
    hist.name = 'metric2'
    hist.unit.unit = histogram_pb2.SIGMA
    hist.unit.improvement_direction = histogram_pb2.BIGGER_IS_BETTER
  ...
  def testComplexBinBounds(self):
    ...
    spec2.bin_spec.boundary_type = (
        histogram_pb2.BinBoundaryDetailedSpec.EXPONENTIAL)
    spec2.bin_spec.maximum_bin_boundary = 19
    spec2.bin_spec.num_bin_boundaries = 20
    spec3 = hist.bin_boundaries.bin_specs.add()
    spec3.bin_spec.boundary_type = histogram_pb2.BinBoundaryDetailedSpec.LINEAR
