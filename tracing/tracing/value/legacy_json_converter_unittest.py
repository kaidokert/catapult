...
class LegacyJsonConverterUnittest(unittest.TestCase):
  ...
  def testConvertBasic(self):
    ...
    self.assertAlmostEqual(h.average, 42.0)
    self.assertAlmostEqual(h.standard_deviation, 0)

  def testConvertWithStdDev(self):
    dicts = [{
        'master': 'Master',
        'bot': 'Bot',
        'test': 'Suite/Metric',
    ...
    self.assertEqual(h.num_values, 1)
    self.assertAlmostEqual(h.average, 42)
    self.assertAlmostEqual(h.standard_deviation, 20)
