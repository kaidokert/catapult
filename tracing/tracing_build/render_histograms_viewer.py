...
VULCANIZED_HISTOGRAMS_VIEWER_PATH = os.path.join(
...
def RenderHistogramsViewer(histogram_dicts, output_stream, reset_results=False,
                           vulcanized_html=None,
                           vulcanized_path=VULCANIZED_HISTOGRAMS_VIEWER_PATH):
  ...
  Args:
    ...
    vulcanized_path: path to vulcanized histograms viewer if vulcanized_html is
    None.
  """
  ...
  if not vulcanized_html:
    with file(vulcanized_path, 'rb') as vulcanized_file:
      vulcanized_html = vulcanized_file.read()
