...
def LoadTraces(chrome_trace_filename):
  ...
  several traces. TODO(chiniforooshan): create a module for reading/writing
  different trace file formats and make every other tool use that module.
  """
  traces = []
...
def _ConcatTraces(traces):
  ...
  # a metadata field, we can trivially concat them. TODO(chiniforooshan): to
  # support writing several traces with several metadata fields in a JSON file,
  # we can write a simple trace_list_importer.html that treats each of them as a
  # subtrace.
  #
