...
class _DeleteHelper(object):
  """Context manager that deletes files on exit."""

  def __init__(self, return_paths, delete_paths=None):
    """Creates a _DeleteHelper.

    args:
      return_paths: Paths returned on enter.
      delete_paths: Paths deleted on exit. If None |return_paths| will be
        deleted on exit.
    """
    ...
    if delete_paths is not None:
      ...
      self._delete_paths = return_paths

  def __enter__(self):
    return self._return_paths

  def __exit__(self, type_, value, traceback):
    paths = self._delete_paths if isinstance(self._delete_paths,
                                             list) else [self._delete_paths]
    for path in paths:
  ...
  def Release(self):
    """Releases paths, i.e. on exit no paths will be deleted."""
    self._return_paths = []
    self._delete_paths = []
...
class BaseApkHelper(object):
  ...
  def GetApkPaths(self, device, modules=None, allow_cached_props=False):
    """Returns contetx manager providing list of split APK paths for |device|.

    The paths may be deleted when the context manager exists. Must be
...
class ApkHelper(BaseApkHelper):
  ...
  def _GetBaseApkPath(self):
    return _DeleteHelper(self._apk_path, [])
...
class SplitApkHelper(BaseApkHelper):
  ...
  def _GetBaseApkPath(self):
    return _DeleteHelper(self._base_apk_path, [])
  ...
  def GetApkPaths(self, device, modules=None, allow_cached_props=False):
    ...
    return _DeleteHelper([self._base_apk_path] + splits, [])
...
class BaseBundleHelper(BaseApkHelper):
  ...
  def _GetBaseApkPath(self):
    base_apk_path_helper = _DeleteHelper(tempfile.mkdtemp())
    with base_apk_path_helper as base_apk_path:
      with self._GetApksPath() as apks_path:
        with zipfile.ZipFile(apks_path) as z:
          ...
          base_apk_path_helper.Release()
          return _DeleteHelper(
              os.path.join(base_apk_path, base_apks[0]), base_apk_path)

  def GetApkPaths(self, device, modules=None, allow_cached_props=False):
    with self._GetApksPath() as apks_path:
      split_dir_helper = _DeleteHelper(tempfile.mkdtemp())
      with split_dir_helper as split_dir:
...
class ApksHelper(BaseBundleHelper):
  ...
  def _GetApksPath(self):
    return _DeleteHelper(self._apks_path, [])
...
class BundleScriptHelper(BaseBundleHelper):
  ...
  def _GetApksPath(self):
    apks_path_helper = _DeleteHelper(tempfile.mkstemp())
    with apks_path_helper as apks_path:
      cmd = [
      ...
      apks_path_helper.Release()
      return _DeleteHelper(apks_path)
