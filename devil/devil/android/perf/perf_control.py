...
class PerfControl(object):
      ...
      if not isinstance(cpu_max_freq, dict):
        ...
        for key, max_frequency in cpu_max_freq.iteritems():
          if '-' not in key:
            ...
            # Convert from 'X-Y' to 'cpuX cpu<X+1> .. cpuY'.
            range_min, range_max = map(int, key.split('-'))
  ...
  def _ForEachCpuFromList(self, cmd, cpu_list):
    """Runs a command on the device for each of the CPUs from the list.
