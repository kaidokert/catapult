...
class PerfControl(object):
  ...
  _CPU_CORE_RANGE_PATTERN = re.compile(
      r'^(?P<range_min>[0-9]+)[.][.](?P<range_max>[0-9]+)$')
    ...
    if cpu_max_freq:
      if not isinstance(cpu_max_freq, dict):
        ...
        for key, max_frequency in cpu_max_freq.iteritems():
          ...
          match = re.match(self._CPU_CORE_RANGE_PATTERN, key)
          if not match:
            range_min = range_max = int(key)
          else:
            range_min = int(match.group('range_min'))
            range_max = int(match.group('range_max'))
