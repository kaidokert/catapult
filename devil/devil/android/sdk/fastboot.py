...
class Fastboot(object):
  ...
  _fastboot_path = lazy.WeakConstant(lambda: devil_env.config.FetchPath(
      'fastboot'))
  _mke2fs_path = lazy.WeakConstant(lambda: devil_env.config.FetchPath('mke2fs'))
  ...
  def FlashAll(self,
    ...
    cmd = []
    if wipe:
      # mke2fs could be called during wiping.
      assert self._mke2fs_path.read()
      cmd.append('-w')
    if force:
      cmd.append('--force')
    if skip_reboot:
  ...
  @decorators.WithTimeoutAndRetriesFromInstance()
  def Wipe(self, timeout=None, retries=None):
    """Wipe userdata."""
    # mke2fs could be called during wiping.
    assert self._mke2fs_path.read()
    self._RunDeviceFastbootCommand(['-w'])
