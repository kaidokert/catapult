...
class AdbWrapper(object):
  ...
  def Push(self, local, remote, sync=False,
    ...
    Raises:
      AdbCommandFailedError if sync=True with versions of adb prior to 1.0.39.
    """
    ...
    push_cmd = ['push', local, remote]

    if sync:
      push_cmd += ['--sync']
      if (du_version.LooseVersion(self.Version()) <
          du_version.LooseVersion('1.0.39')):
        # The --sync flag for `adb push` is a relatively recent addition, but
        # it exists at least as far back as 1.0.39.
        raise device_errors.AdbCommandFailedError(
            push_cmd, '--sync not supported prior to adb 1.0.39')
