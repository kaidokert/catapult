...
class AdbWrapper(object):
  ...
  @decorators.WithTimeoutAndConditionalRetries(
      _ShouldRetryAdbCmd, pass_values=True)
  def _RunAdbCmd(cls, args, timeout=None, retries=None, device_serial=None,
                 check_error=True, cpu_affinity=None, force_adb_timeout=False):
    if not force_adb_timeout:
  ...
  def _RunDeviceAdbCmd(
      self, args, timeout, retries, check_error=True, force_adb_timeout=False):
    ...
    return self._RunAdbCmd(args, timeout=timeout, retries=retries,
                           ...
                           force_adb_timeout=force_adb_timeout)
  ...
  def Shell(self, command, expect_status=0, timeout=DEFAULT_TIMEOUT,
            retries=DEFAULT_RETRIES, force_adb_timeout=False):
    ...
    Args:
      ...
      timeout: (optional) Timeout per try in seconds.
      retries: (optional) Number of retries to attempt.
      force_adb_timeout: If True, passes the specified timeout into the internal
        adb command even if the function runs in a timeout thread group already.
