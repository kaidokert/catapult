...
class AdbWrapper(object):
  ...
  class PersistentShell(object):
    ...
    def IterRunCommand(self, command, include_status, close=False):
        ...
        with self._lock:
          ...
          while True:
            if self._process.poll() is not None:
              logging.warning("-AaA-: Shell crashed %r")
    ...
    def EnsureStarted(self, force_restart=False):
      ...
      self.Start()
      self.WaitForReady()
      logging.warning("-AaA-: Persistent Shell started")
  ...
  def Shell(self, command, expect_status=0, timeout=DEFAULT_TIMEOUT,
    ...
    if self._persistent_shell:
      output, status = self._persistent_shell.RunCommand(command, keepends=True)
