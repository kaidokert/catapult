...
class AdbWrapper(object):
  ...
  class PersistentShell(object):
    ...
    def IterRunCommand(self,
      ...
      Args:
        ...
        expect_crash: If True, ignore adb process terminating check.
      ...
      if close:
        ...
        print(self._process)
        print("process poll is %s" % self._process.poll())
        print("SENDING COMMAND: %s" % send_cmd)
      ...
      if not self._terminating and self._registry:
    ...
    def HardStop(self):
      ...
      logger.warning("want to kill process: process %s, process poll %s" % (self._process, self._process.poll()))
      if self._process is not None and self._process.poll() is None:
        logger.warning("DECIDING TO KILL PROCESS")
        self._process.kill()
        time.sleep(5)
  ...
  def __init__(self, device_serial, persistent_shell=False):
    ...
    if persistent_shell:
      ...
      for _ in range(0, 2):
        shell = self.PersistentShell(self._device_serial, self)
        shell.Start()
  ...
  def KillPersistentAdbs(self):
    ...
    self._all_persistent_shells = []
    print("KILLING PERSISTENT ADBS print")
    logging.error("KILLING PERSISTENT ADBS, %d", len(aps))
    # TODO(awong): There is still a race here on adding a new shell.
    for x in aps:
      logging.warning("X is %s", x)
      x.HardStop()
  ...
  def _RunAdbCmd(cls,
    ...
    print("Returning output from here:%s" % args)
    return output
  ...
  def _Shell(self, command, timeout, retries):
    ...
    print("output end is:%s:" % output_end)
    print("finished")
  ...
  def KillServer(cls, timeout=DEFAULT_TIMEOUT, retries=DEFAULT_RETRIES):
    print("IN KILL SERFVER ADB WRAPPER")
    logging.error("IN KILL SERVER ADB WRAPPER")
    print(dir(cls))
  ...
  def Shell(self,
    ...
    """
    if self._persistent_shells:
      output, status = self._GetAShell().RunCommand(command, keepends=True)
      output = ''.join(output).rstrip()
    else:
      print("NOT RUNNING A PERSISTENT SHELL")
      output, status = self._Shell(command, timeout, retries)

    print("EXPECT STATUS IS: %s, STATUS IS %s" % (expect_status, status))
    if expect_status is not None and status != expect_status:
      print("IN EXPECT STATUS CHECK")
      raise device_errors.AdbShellCommandFailedError(
  ...
  def IterShell(self, command, timeout):
    ...
    """
    if self._persistent_shells:
      return self._GetAShell().IterRunCommand(command, False)
    else:
      args = ['shell', command]
  ...
  def Reboot(self,
    ...
    logging.error("REBOOTING DEVICE")
    if to_bootloader:
  ...
  def Root(self, timeout=DEFAULT_TIMEOUT, retries=DEFAULT_RETRIES):
    ...
    try:
      ...
      logging.warning("WE ARE ROOTING")
      self.KillPersistentAdbs()
