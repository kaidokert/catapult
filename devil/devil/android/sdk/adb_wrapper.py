...
class AdbWrapper(object):
      ...
      if close:
        ...
        while True:
          try:
            ...
            output_lines.append(six.ensure_str(self._outq.get(timeout=0.2)))
          except Empty:
            self._process = None
            break
  ...
  def __init__(self, device_serial, persistent_shell=False):
    ...
    if persistent_shell:
      ...
      # comes in while the first shell is being used. Use lock to prevent
      # shells from being killed while we're adding.
      self._lock.acquire()
      for _ in range(0, 2):
