...
class AdbWrapper(object):
      ...
      if close:
        ...
        while True:
          ...
          if self._process.poll() is not None:
            expect_crash = True
            if expect_crash:
              # Need to read exit code to prevent defunct processes.
              print("READ THE PROCESS CODE")
              self._process.wait()
              print("DONE WITH THE PROCESS CODE")
              self._process = None
              time.sleep(2)
              self.Start()
              print("TRYING TO SEND COMMAND: %s" % send_cmd)
              #self._process.stdin.write(send_cmd.encode('utf-8'))
              #self._process.stdin.flush()  # Ensure underlying stdio flushes.
            else:
              raise device_errors.AdbShellCommandFailedError(
  ...
  def Shell(self,
    ...
    if self._persistent_shells:
      ...
      print("IN PERSISTENT SHELL")
      print("STATUS for cmd %s IS: %s" % (command, status))
