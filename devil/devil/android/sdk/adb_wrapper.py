...
class AdbWrapper(object):
  ...
  class PersistentShell(object):
    ...
    def __init__(self, serial, registry=None):
      ...
      Args:
        ...
        registry: An identifier for the adb instance.
      """
      ...
      if close:
        ...
        while True:
          ...
          if not self._start_found:
            if output_line[:len(_SHELL_OUTPUT_START_MARKER
                                )] == _SHELL_OUTPUT_START_MARKER:
              self._start_found = True
            continue

          if output_line[:len(_SHELL_OUTPUT_END_MARKER
                              )] == _SHELL_OUTPUT_END_MARKER:
            self._start_found = False
            if include_status:
              yield int(output_line[len(_SHELL_OUTPUT_END_MARKER):])
            break
          yield output_line

      print("WE ARE HERE IN ITER RUN COMMAND")
      if not self._terminating and self._registry:
        print("self_registry: %s" % self._registry)
        self._registry.ReAddAdb(self)

    def RunCommand(self, command, close=False, keepends=False):
      """Runs an ADB command and returns the output line by line.
  ...
  def __init__(self, device_serial, persistent_shell=False):
    ...
    if persistent_shell:
      ...
      # Add multiple shells for speed improvement. This makes sure that
  ...
  def KillPersistentAdbs(self):
    ...
    # TODO(1349176): There is still a race here if a new shell is added while
    # removing the shells. Put in a state check or lock.
    for x in aps:
      x.HardStop()
