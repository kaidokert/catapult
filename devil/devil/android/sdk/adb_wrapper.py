...
class AdbWrapper(object):
  ...
  class PersistentShell(object):
    ...
    def IterRunCommand(
      ...
      """
      if close:
        closed_timeout = DEFAULT_TIMEOUT
        ...
        while True:
          try:
            ...
            line = six.ensure_str(self._outq.get(timeout=closed_timeout))
            if not _IsExtraneousLine(line, send_cmd):
              output_lines.append(line)
              # If we detect a closed status, there may not be any more output.
              # We don't want to wait 30 seconds for output.
              if line.strip() == '0':
                closed_timeout = 2
          except Empty:
            self._terminating = True
  ...
  def __init__(self, device_serial, persistent_shell=False):
    ...
    # The self.is_ready call doesn't work on some builders as it requires adb
    # calls which some builders don't have the infra for.
