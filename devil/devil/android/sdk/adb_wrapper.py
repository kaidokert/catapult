...
class AdbWrapper(object):
  ...
  class PersistentShell(object):
    ...
    def WaitForReady(self):
      ...
      """
      # Disable -v and -x using "set -". Some bots have input echo turned on.
      self._process.stdin.write('set -; echo\n')
    ...
    def IterRunCommand(self, command, include_status, close=False, timeme=False):
      ...
      Args:
        ...
        include_status: When False, the status code is not included in output.
            When True, the status code, as an int, is the last output.
      ...
      if close:
        ...
        logging.warning(">> %s IterRunCommand start: %r" % (datetime.now(), send_cmd))
        with self._lock:
          ...
          while True:
            ...
            if wlist:
              ...
              logging.warning(">> %s writing: %r" % (datetime.now(), part_to_write))
            ...
            if rlist:
              logging.warning(">> %s start read" % (datetime.now()))
              output_line = self._process.stdout.readline()
              logging.warning(">> %s read: %r" % (datetime.now(), output_line))
              if output_line[:5] == 'DoNe:':
                if include_status:
                  yield int(output_line[5:])
                break
              yield output_line
    ...
    def RunCommand(self, command, close=False, keepends=False, timeme=False):
      ...
      """
      logging.warning("AA: sending command %s" % command)
      output_iter = self.IterRunCommand(command, True, close, timeme=timeme)
      logging.warning("AA: sent and iterator returned")
      result = [line if keepends else line.rstrip() for line in output_iter]
      logging.warning("AA: result gotten. exit code: %d" % result[-1])
    ...
    def EnsureStarted(self):
      ...
      logging.warning("AA: Starting persistnet shell")
      self.Start()
      logging.warning("AA: start sent")
      self.WaitForReady()
      logging.warning("AA: persistent shell is ready")
