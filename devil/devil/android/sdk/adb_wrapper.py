    ...
    def IterRunCommand(
      ...
      if self._registry:
        if self._terminating:
          self._registry.KillPersistentAdb(self)
        else:
  ...
  def __init__(self, device_serial, persistent_shell=False):
    ...
    Args:
      ...
      persistent_shell: If True, uses a long-running "adb shell" for executing
          commands
    """
    print("PERSISTENT SHELL IS THIS: %s\n\n\n" % persistent_shell)
    ...
    for _ in range(5):
      if not self.is_ready:
        ...
        # persistent_shell does not work with a device that isn't ready.
    ...
    if persistent_shell and not self.is_emulator:
      print("STARTING PERSISTENT SHELL")
      if not self.is_ready:
