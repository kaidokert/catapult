...
class AdbWrapper(object):
  ...
  class PersistentShell(object):
    ...
    def Start(self):
      ...
      logging.warning("-AaA-: Persistent Shell starting")
      # pylint: disable=protected-access
      ...
      logging.error("Creating queue")
      self._outq = Queue()
      self._reader_thread = threading.Thread(target=output_reader, args=(self._process, self._outq))
    ...
    def WaitForReady(self):
      ...
      """
      self._process.stdin.write('echo ready\n')
      self._process.stdin.flush()
      output_line = self._outq.get()
      while output_line.rstrip() != 'ready':
        output_line = self._outq.get()

    def IterRunCommand(self, command, include_status, close=False, queue=None):
      ...
      if close:
        ...
        logging.warning("-AaA-: Starting run command %r" % send_cmd)

        with self._lock:
          if self._start_found:
            logging.warning("-AaA-: Unknown Adb shell state. Hard termianting")
            ...
            logging.warning("-AaA-: Restarted %r")

          logging.warning("-AaA-: sending to shell %r" % send_cmd)
          self._process.stdin.write(send_cmd)
          self._process.stdin.flush()  # Ensure underlying stdio flushes.
          timeout_if_no_start_by = datetime.now() + timedelta(milliseconds=500) # TODO(awong): This is long
          logging.warning("-AaA-: command sent")
          ...
          while True:
            if not self._start_found and datetime.now() > timeout_if_no_start_by:
              ...
              self._process = None
              self.Start()
              logging.warning("-AaA-: resending to shell %r" % send_cmd)
              self._process.stdin.write(send_cmd)
              self._process.stdin.flush()  # Ensure underlying stdio flushes.
              timeout_if_no_start_by = datetime.now() + timedelta(milliseconds=500) # TODO(awong): This is long
              logging.warning("-AaA-: command resent")

            if self._process.poll() is not None:
              logging.warning("-AaA-: Shell crashed")
            ...
            try:
              output_line = self._outq.get(timeout=0.001)
            except Empty:
              continue
            ...
            if not self._start_found:
              logging.warning("-AaA-: skipping %r" % output_line)
              if output_line[:5] == _SHELL_OUTPUT_START_MARKER:
                logging.warning("-AaA-: StArT found")
                self._start_found = True
              continue

            logging.warning("-AaA-: Line: %r" % output_line)
            if output_line[:4] == _SHELL_OUTPUT_END_MARKER:
              logging.warning("-AaA-: DoNe found")
              self._start_found = False
              if include_status:
      ...
      if not self._terminating and queue:
        queue.ReaddAdb(self)

    def RunCommand(self, command, close=False, keepends=False, queue=None):
      """Runs an ADB command and yields the output line by line.
      ...
      """
      logging.warning("-AaA-: RunCommand %s %s %s" % (command, close, keepends))
      output_iter = self.IterRunCommand(command, True, close, queue)
    ...
    def EnsureStarted(self, force_restart=False):
      ...
      if self._process is not None:
        if force_restart:
          logging.warning("-AaA-: Unknown Adb shell state sending kill")
          self._process.kill()
          retcode = None
        else:
  ...
  def __init__(self, device_serial, persistent_shell=False):
    ...
    self._persistent_shells = None
    self._all_persistent_shells = []
    if persistent_shell:
      self._persistent_shells = Queue()
      for i in xrange(0, 2):
        shell = self.PersistentShell(self._device_serial)
        shell.Start()
        self._persistent_shells.put(shell)
        self._all_persistent_shells.append(shell)

  def ReaddAdb(self, adb):
    if self._persistent_shells:
      self._persistent_shells.put(adb)

  def KillPersistentAdbs(self):
    aps = self._all_persistent_shells
    self._all_persistent_shells = []
    # TODO(awong): There is still a race here on adding a new shell.
    for x in aps:
      aps.HardStop()
    ...
    if self._persistent_shells:
      try:
        while True:
          self._persistent_shells.get(block=False)
      except Empty:
        pass
  ...
  def Shell(self, command, expect_status=0, timeout=DEFAULT_TIMEOUT,
    ...
    """
    if self._persistent_shells:
      output, status = self._GetAShell().RunCommand(command, keepends=True, queue=self)
  ...
  def IterShell(self, command, timeout):
    ...
    """
    if self._persistent_shells:
      return self._GetAShell().IterRunCommand(command, False, queue=self)
    else:
      args = ['shell', command]
      return cmd_helper.IterCmdOutputLines(
  ...
  def _GetAShell(self):
    try:
      return self._persistent_shells.get(block=False)
    except Empty:
      shell = self.PersistentShell(self._device_serial)
      shell.Start()
      return shell
