      ...
      if close:
        ...
        with self._lock:
          if self._start_found:
            logging.warning("-AaA-: Unknown Adb shell state. Hard termianting %r")
            # A lock has been busted midway through a command to the other
            # process. Hard restart the shell.
            self._process.terminate()
            logging.warning("-AaA-: Unknown Adb terminated  %r")
            time.sleep(0.01)
            if self._process.poll() is not None:
              logging.warning("-AaA-: Unknown Adb shell sending kill %r")
              self._process.kill()
            logging.warning("-AaA-: Unknown Adb shell resetarting %r")
            self._process = None
            self.EnsureStarted()
          ...
          while True:
            ...
            if xlist:
              logging.warning("-AaA-: exception found")
              raise device_errors.AdbShellCommandFailedError(
                  command, "ADB select exception", status=None,
                  device_serial=self._device_serial)
            ...
            if wlist:
              ...
              logging.warning("-AaA-: sending to shell %r" % part_to_write)
