...
class AdbWrapper(object):
    ...
    def RunCommand(self, command, close=False, keepends=False):
      ...
      if close:
        def run_cmd(cmd):
          ...
          self._process = None
          for x in output.splitlines(keepends=keepends):
            yield x
            ...
            while looking_for_done:
              if self._process.poll() is not None:
                raise device_errors.AdbShellCommandFailedError(
                    cmd, "ADB shell crashed", status=None,
      ...
      result = [line if keepends else line.rstrip() for line in run_cmd(command)]
  ...
  def IterShell(self, command, timeout):
    ...
    """
    if self._persistent_shell:
      return iter(self._persistent_shell.RunCommand(command))
    else:
      args = ['shell', command]
      return cmd_helper.IterCmdOutputLines(
