...
class NamedDeviceTemporaryDirectory(object):
  ...
  def close(self):
    ...
    def delete_temporary_dir():
      try:
        if (self._device_utils and
            self._device_utils.target_user is not None and
            self.name.startswith('/sdcard')):
          self._device_utils.RunShellCommand(
              ['content', 'delete', '--user', '11', '--uri',
               f'content://android.tradefed.contentprovider{self.name}'],
              check_return=False,
              retries=0,
          )
          return
        elif self._device_utils:
          device_path = self.name
          if self._device_utils.target_user is not None:
            device_path = self._device_utils.GetShellWritablePath(device_path)
          self._device_utils.RunShellCommand(
