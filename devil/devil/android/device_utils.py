...
class DeviceUtils(object):
  ...
  def RunShellCommand(self, cmd, shell=False, check_return=False, cwd=None,
    ...
    Args:
      ...
      timeout: timeout in seconds
      retries: number of retries
      keep_logs_on_timeout: If True, will use a slightly smaller timeout for the
          internal adb command, which allows to retrive logs on timeout. Note
          that that logs are not guaranteed to be produced with this option as
          adb command may still hang and fail to respect the reduced timeout.
    ...
    def run(cmd):
      if keep_logs_on_timeout:
        return self.adb.Shell(cmd, reduced_adb_timeout=True)
      else:
