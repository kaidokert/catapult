...
class DeviceUtils(object):
  ...
  def PushChangedFiles(self, host_device_tuples, timeout=None,
    ...
    if delete_device_stale and all_stale_files:
      self.RunShellCommand(['rm', '-rf'] + all_stale_files, check_return=True)
  ...
  def _GetStaleDirectories(self, host_path, device_path):
    ...
    Returns:
      ...
      considered stale.
    """
    def abs_to_relative(full_path, top_level_dir):
      return full_path.replace(top_level_dir, '', 1).lstrip('/')
    ...
    def get_device_dirs_recursive(path, top_level_dir):
      ...
      for file_or_dir in stats:
        if stat.S_ISDIR(file_or_dir['st_mode']):
          ...
          # host.
          directories.add(abs_to_relative(full_path, top_level_dir))
          directories.update(
    ...
    def get_host_dirs(path):
      ...
      for root, _, _ in os.walk(path):
        if root != path:
          ...
          # host.
          directories.add(abs_to_relative(root, path))
      return directories
    ...
    for directory in device_dirs:
      if directory not in host_dirs:
        stale_dirs.append(posixpath.join(device_path, directory))
    return stale_dirs
