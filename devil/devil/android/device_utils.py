...
# Lists all files for the specified directories, and creates missing directories
...
class DeviceUtils(object):
  ...
  def _GetChangedFiles(self, host_device_tuples, delete_stale=False):
    ...
    try:
      ...
      file_tuples = []
      # All folders we're pushing files to.
      device_paths = set()
      # All files and folders we expect to have on the device after pushing
      # files.
      expected_device_nodes = set()

      # Keep list of host md5 paths un-expanded, as expanding them leads to a
      # really really long command line if we have to checksum all files
      # anyways.
      host_md5_paths = set()
      for h, d in host_device_tuples:
        ...
        if os.path.isdir(host_path):
          ...
          for root, _, filenames in os.walk(host_path):
            ...
            posix_path = posixpath.join(device_path, relative_dir).rstrip('/')
            expected_device_nodes.add(posix_path)
            device_paths.add(posix_path)
            files = (
          ...
          device_paths.add(posixpath.dirname(device_path))
          file_tuples.append((host_path, device_path))

      if not file_tuples:
        return (host_device_tuples, lambda: 0)

      current_device_nodes = self._GetDeviceNodes(device_paths)
