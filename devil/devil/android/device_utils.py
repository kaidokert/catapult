...
class DeviceUtils(object):
  ...
  @decorators.WithTimeoutAndRetriesFromInstance()
  def IsModuleInstalled(self, package, version_code, timeout=None,
                        retries=None):
    """
  ...
  def InstallApex(self, apex, timeout=None, retries=None):
    ...
    with self._apex_lock:
      with apex.GetApkPaths(self) as apex_file_paths:
        ...
        logger.info('Installing apex %s using APKs %s', package_name,
                    apex_file_path)
    ...
    if not self.IsModuleInstalled(package_name, version):
      raise device_errors.CommandFailedError(
