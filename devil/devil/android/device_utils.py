...
class DeviceUtils(object):
  ...
  def _GetChangedFiles(self, host_device_tuples, delete_stale=False):
    ...
    def calculate_host_checksums():
      ...
      if self._enable_device_files_cache:
        return device_utils_helper.CalculateHostHashes(
            [t[0] for t in file_tuples])
      else:
        return device_utils_helper.CalculateHostHashes(
    ...
    def calculate_device_checksums():
      ...
      sums.update(
          dict(device_utils_helper.CalculateDeviceHashes(list(paths), self)))
      if self._enable_device_files_cache:
  ...
  def _ComputeDeviceChecksumsForApks(self, package_name):
    ...
    if ret is None:
      ...
      file_to_checksums = device_utils_helper.CalculateDeviceHashes(
          device_paths, self)
  ...
  def _ComputeStaleApks(self, package_name, host_apk_paths):
    def calculate_host_checksums():
      return device_utils_helper.CalculateHostHashes(host_apk_paths)
