...
class DeviceUtils(object):
  ...
  def Uninstall(self, package_name, keep_data=False, timeout=None,
    ...
    if not installed:
      return
    # cached package paths are in-determinate due to system apps taking over
  ...
  @decorators.WithTimeoutAndRetriesFromInstance()
  def HasGoogleFeatures(self, timeout=None, retries=None):
    """Determines if the device has any google features.

    Returns: True  - has google features
             False - does not have google features
    Args:
      timeout: timeout in seconds
      retries: number of retries
    ...
    """
    output = self.RunShellCommand(
        ['dumpsys', 'package', 'features'], check_return=True)
    for line in output:
      if re.search(_GOOGLE_FEATURES_RE, line):
        return True
    return False
  ...
  def GetWebViewUpdateServiceDump(self, timeout=None, retries=None):
    ...
    Returns a dictionary with these possible entries:
