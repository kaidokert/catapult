  ...
  def _GetStaleDirectories(self, host_path, device_path):
    ...
    def get_device_dirs(path):
      ...
      for line in self.RunShellCommand(
          ['find', device_path, '-type', 'd', '-print']):
        if line != path:
          directories.add(posixpath.relpath(line, path))
      return directories
    ...
    logging.error("Got host dirs %s", str(host_dirs))
    logging.error("Got device dirs %s", str(device_dirs))
