...
_DEVICE_DIRECTORY_LIST_SCRIPT_FILE = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__), 'device_scripts', 'list_device_dirs.sh'))
_DEVICE_DIRECTORY_LIST_SCRIPT_LOCATION = '/data/local/tmp/list_device_dirs.sh'
...
class DeviceUtils(object):
  ...
  def __init__(self, device, enable_device_files_cache=False,
    ...
    self._device_directory_list_script = None
  ...
  def _GetStaleDirectories(self, host_path, device_path):
    ...
    def get_device_dirs(path):
      if not self._device_directory_list_script:
        self.adb.Push(
            _DEVICE_DIRECTORY_LIST_SCRIPT_FILE,
            _DEVICE_DIRECTORY_LIST_SCRIPT_LOCATION)
        self._device_directory_list_script = (
            _DEVICE_DIRECTORY_LIST_SCRIPT_LOCATION)

      directories = set()
      for line in self.RunShellCommand(
          ['sh', self._device_directory_list_script, path]):
