...
class DeviceUtils(object):
  ...
  def _GetStaleDirectories(self, host_path, device_path):
    ...
    def get_device_dirs(path):
      directories = set()
      command = _RECURSIVE_DIRECTORY_LIST_SCRIPT % path
      # We need to make sure not to single quote the command, as otherwise sh
      # complains. So, use shell=True to prevent RunShellCommand from
      # automatically single quoting what we give it.
