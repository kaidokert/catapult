...
class DeviceUtils(object):
  ...
  def InstallSplitApk(self, base_apk, split_apks, allow_downgrade=False,
    ...
    """
    self._InstallInternal(
        apk_helper.ToHelper(base_apk, split_apks),
        reinstall=reinstall,
  ...
  def _InstallInternal(self,
                       ...
                       permissions=None,
                       modules=None):
    split_apks = apk.GetSplits(self, modules, allow_cached_props)
    if not split_apks:
    ...
    missing_apks = [a for a in split_apks if not os.path.exists(a)]
    if missing_apks:
      raise device_errors.CommandFailedError(
    ...
    if not device_apk_paths:
      apks_to_install = split_apks
    elif len(device_apk_paths) > 1 and len(split_apks) == 1:
      logger.warning(
      ...
      try:
        apks_to_install, host_checksums = (
            self._ComputeStaleApks(package_name, split_apks))
      except EnvironmentError as e:
        logger.warning('Error calculating md5: %s', e)
        apks_to_install, host_checksums = split_apks, None
    ...
    if apks_to_install:
      ...
      partial = package_name if len(apks_to_install) < len(split_apks) else None
  ...
  def GetFeatures(self, timeout=None, retries=None):
    ...
    if not all([l.startswith('feature:') for l in lines]):
      raise Exception(
          'Some features don\'t have feature: prefix: %s' % ','.join(lines))
    # Remove 'feature:' prefix.
    return [f[8:] for f in lines]
