...
class DeviceUtils(object):
  ...
  def GetUserWritablePath(self, device_path, target_user):
    ...
    sdcard via the path "/sdcard". However, it can, with the root permission and
    use the path "/mnt/user/10/emulated/10".
  ...
  def _IsApplicationInstalledDumpsys(self,
                                     ...
                                     timeout=None,
                                     retries=None):
    package_matcher = re.compile(_DUMPSYS_PACKAGE_RE_STR % package)
    package_user_matcher = None
    if self._force_current_user:
      package_user_matcher = re.compile(
          _DUMPSYS_PACKAGE_USER_RE_STR.format(self.GetCurrentUser(cache=True)))
    ...
    for line in dumpsys_output:
      ...
      if package_match:
        installed_version = package_match.groupdict().get('library_version')
  ...
  def BroadcastIntent(self, intent_obj, timeout=None, retries=None):
    ...
    So this param won't be added even when "_force_current_user" is True.
