...
class DeviceUtils(object):
  ...
  def Install(self, apk, allow_downgrade=False, reinstall=False,
    ...
    all_modules = self._GetAllModules(modules, fake_modules)
    with apk.GetApkPaths(self, modules=all_modules) as apk_paths:
      ...
      self._PerformFakeInstall(fake_apk_paths, fake_modules)
  ...
  @staticmethod
  def _GetAllModules(modules, fake_modules):
    modules_set = set(modules or [])
    fake_modules_set = set(fake_modules or [])
    overlap = modules_set.intersection(fake_modules_set)
    assert not overlap, 'These modules appear in both {}'.format(overlap)
    return modules_set.union(fake_modules_set)
    ...
    def IsFakeModulePath(path):
      if fake_modules:
        ...
        for fake_module in fake_modules:
          if name.startswith(fake_module):
            return True
      return False

    return set(path for path in apk_paths if IsFakeModulePath(path))

  def _PerformFakeInstall(self, fake_apk_paths, fake_modules):
      ...
      for path in fake_apk_paths:
        ...
        if 'master' in suffix:
          assert module_name in still_need_master, (
              'Duplicate master apk file for {}'.format(module_name))
          still_need_master.remove(module_name)
          new_filename = '{}.apk'.format(module_name)
        else:
          # |suffix| includes .apk extension.
          new_filename = '{}.config.{}'.format(module_name, suffix)
        new_path = os.path.join(modules_dir, new_filename)
        os.rename(path, new_path)

      assert not still_need_master, (
          'Missing master apk file for {}'.format(still_need_master))
      self.PushChangedFiles([(modules_dir, self.MODULES_SRC_DIRECTORY_PATH)],
                            delete_device_stale=True)
