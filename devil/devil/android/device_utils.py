...
def _CreateAdbWrapper(device, persistent_shell=False):
  if isinstance(device, adb_wrapper.AdbWrapper):
    return device
  else:
    return adb_wrapper.AdbWrapper(device, persistent_shell=persistent_shell)
  ...
  def HealthyDevices(cls, blacklist=None, device_arg='default', retries=1,
    ...
    Args:
      ...
      persistent_shell: Makes AdbWrapper pipe commands through a single
          "adb shell" instead of spawning a new shell for each. Can save a lot of
          time if there are multiple "adb shell" invocations.
