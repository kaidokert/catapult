...
class DeviceUtilsIsApplicationInstalledTest(DeviceUtilsTest):
  ...
  def testIsApplicationInstalled_installed_currentUser(self):
    with self.patch_call(self.call.device._force_current_user,
                         return_value=True):
      with self.assertCalls(
          (self.call.device.GetCurrentUser(cache=True), 11),
...
class DeviceUtilsStartServiceTest(DeviceUtilsTest):
  ...
  def testStartService_success_currentUser(self):
    ...
    with self.patch_call(self.call.device.build_version_sdk,
                         return_value=version_codes.NOUGAT), \
         self.patch_call(self.call.device._force_current_user,
                         return_value=True):
      with self.assertCalls(
          (self.call.device.GetCurrentUser(cache=True), 11),
  ...
  def testStartInstrumentation_currentUser(self):
    with self.patch_call(self.call.device._force_current_user,
      ...
      with self.assertCalls((
          ...
          'p=test.package;am instrument --user 11 "$p"/.TestInstrumentation',
          shell=True,
          check_return=True,
          large_output=True))):
    ...
    with PatchLogger() as logger:
      with self.patch_call(self.call.device.build_version_sdk,
        ...
        with self.assertCalls(
            (self.call.device.GetCurrentUser(cache=True), 11),
            (self.call.device.RunShellCommand(
                AnyStringWith('appops set --user 11 '
