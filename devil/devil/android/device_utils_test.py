...
class IterPushableComponentsTest(unittest.TestCase):
  ...
  def sampleLayout(cls):
    Layout = collections.namedtuple(
        'Layout',
        ['root', 'basic_file', 'symlink', 'dir_with_symlinks',
    ...
    with tempfile_ext.NamedTemporaryDirectory() as layout_root:
      ...
      yield Layout(layout_root, basic_file, symlink, dir1, dir2)
  ...
  def testSymlink(self):
    with self.sampleLayout() as layout:
      ...
      expected = [(os.path.realpath(layout.symlink), device_path, False)]
      actual = list(
          device_utils._IterPushableComponents(
              layout.symlink, device_path))
      self.assertItemsEqual(expected, actual)
    ...
    with self.sampleLayout() as layout:
      ...
      expected = [
          (layout.basic_file,
           ...
           True),
          (os.path.realpath(layout.symlink),
           posixpath.join(device_path, os.path.basename(layout.symlink)),
           False),
      ]
      actual = list(
          device_utils._IterPushableComponents(
    ...
    with self.sampleLayout() as layout:
      ...
      expected = [
          ...
          (os.path.realpath(layout.symlink),
           posixpath.join(
               device_path,
               *os.path.split(os.path.relpath(layout.symlink, layout.root))),
           False),
      ]
      actual = list(
          device_utils._IterPushableComponents(
