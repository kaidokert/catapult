...
def BuildTargetsForCpu(targets, cpu, output_dir):
  ...
  ec = cmd_helper.RunCmd(['ninja', '-C', output_dir, '-j', '1000', '-l', '15'] +
                         targets)
  if ec:
...
def UpdateDependency(dependency_name, dependency_info, local_path, platform):
  ...
  if ec:
    raise base_error.BaseError(
        'Failed to determine SHA1 for %s' % sha1sum_output)
  ...
  cmd_helper.RunCmd([
      'gsutil', 'cp', local_path,
      'gs://%s/%s/%s' % (bucket, folder, remote_path)
  ])
...
def main(raw_args):
  ...
  chromium_parser.add_argument(
      '--chromium-src-dir', type=os.path.realpath, default=os.getcwd())
  ...
  dependency_parser.add_argument('--name')
  dependency_parser.add_argument('--path', type=os.path.abspath)
  dependency_parser.add_argument('--platform')
