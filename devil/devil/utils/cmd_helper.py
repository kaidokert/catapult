...
def StartCmd(args, cwd=None, shell=False, env=None):
  ...
  The returned process handle should be passed to FinishCmd.
  ...
  Returns:
    A process handled that should ultimately be passed to FinishCmd.
  """
  _ValidateAndLogCommand(args, cwd, shell)
  return Popen(args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
               shell=shell, cwd=cwd, env=env)


def FinishCmd(handle):
  """Waits for a process handle from StartCmd to complete.

  Args:
    handle: A process handle previously returned by a call to StartCmd.
