...
class TestChromeVersion(TestCase):
  ...
  def testChromeVersionWithString(self):
  ...
  def testChromeVersionWithFourInts(self):
    chrome_version = apk_finder.ChromeVersion(68, 0, 3440, 64)
    self.assertEqual(68, chrome_version.milestone)
    self.assertEqual(0, chrome_version.minor)
    self.assertEqual(3440, chrome_version.build)
...
class TestGetLatestAvailableVersionURI(TestCase):
  ...
  def testGetLatestAvailableVersionURI_foundImmediately(
      self, subprocess_call_function):
    subprocess_call_function.return_value = 0
    # use monochrome
    self.assertEqual(
        'gs://chrome-signed/android-*/61.0.0.0/arm/MonochromeStable.apk',
        apk_finder.GetLatestAvailableVersionURI('61.0.0.0'))
    # use chrome
    self.assertEqual(
        'gs://chrome-signed/android-*/51.0.0.0/arm/ChromeStable.apk',
        apk_finder.GetLatestAvailableVersionURI('51.0.0.0'))

  @mock.patch('subprocess.call')
  def testGetLatestAvailableVersionURI_foundThreeRetry(
      self, subprocess_call_function):
    subprocess_call_function.side_effect = [1, 1, 1, 0, 1, 1, 1, 0]
    # use monochrome
    self.assertEqual(
        'gs://chrome-signed/android-*/61.0.0.0/arm/MonochromeStable.apk',
        apk_finder.GetLatestAvailableVersionURI('61.0.0.3'))
    # use chrome
    self.assertEqual(
        'gs://chrome-signed/android-*/51.0.0.0/arm/ChromeStable.apk',
        apk_finder.GetLatestAvailableVersionURI('51.0.0.3'))

  @mock.patch('subprocess.call')
  def testGetLatestAvailableVersionURI_failedToFound(
      self, subprocess_call_function):
...
class TestDownloadAPKFromURI(TestCase):
  ...
  @mock.patch('subprocess.call')
  def testDownloadAPKFromURI_success(
      self, subprocess_call_function):
    subprocess_call_function.return_value = 0
  ...
  @mock.patch('subprocess.call')
  def testDownloadAPKFromURI_failed(
      self, subprocess_call_function):
    subprocess_call_function.return_value = 1
    with self.assertRaises(utils.CloudDownloadFailed):
...
class TestGetAPK(TestCase):
  ...
  def testGetAPK_localApkNotExists(
      self, milestone_info, get_latest_available_version_uri_function,
      download_apk_from_uri_function,
      get_local_apk_function):
    milestone_info.GetVersionNumberFromMilestone.return_value = None
    get_latest_available_version_uri_function.return_value = None
    download_apk_from_uri_function.result_value = None
    get_local_apk_function.return_value = 'path_to_apk'
