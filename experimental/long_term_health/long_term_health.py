...
class MilestoneInfo(object):
  ...
  def __init__(self):
    path_to_full_info_table = os.path.join(APP_DIR, 'full_milestone_info.csv')
    if os.path.exists(path_to_full_info_table) and (
        os.path.getsize(path_to_full_info_table) > 0):
      self._table = []
      with open(
          os.path.join(APP_DIR, 'full_milestone_info.csv')) as full_info_table:
        reader = csv.DictReader(full_info_table)
        for row in reader:
          self._table.append({
      ...
      raise CloudDownloadFailed('Full Info Table not found or is empty.')
...
def ProcessArguments(args, milestone_info):
  ...
  Args:
    ...
    milestone_info(FullInfoTable class): the object that stores all the table
        content
  """
  if args.from_date:
...
def main(args):
  ...
  milestone_info_table = MilestoneInfo()
  ProcessArguments(args, milestone_info_table)
  ...
  try:
    for milestone in range(args.from_milestone, args.to_milestone + 1):
      uri = milestone_info_table.GetLatestVersionURI(milestone)
