...
class MilestoneInfo(object):
  ...
  def __init__(self, path_to_full_info_table):
    if os.path.exists(path_to_full_info_table) and (
      ...
      self._table = self._LoadCsvTable(path_to_full_info_table)
    else:
  ...
  def _LoadCsvTable(self, path_to_full_info_table):
    if os.path.exists(path_to_full_info_table) and (
        os.path.getsize(path_to_full_info_table) > 0):
      with open(path_to_full_info_table) as full_info_table:
        ...
        if len(list(reader)) > 0:
          return [{
              ...
              'release_date': ParseIsoFormatDate(row['release_date'])}
                  for row in reader]
    raise CloudDownloadFailed(
        'Full Info Table is not found, empty, or have no rows.')
