...
from utils import CloudDownloadFailed
from utils import LONG_TERM_HEALTH_ROOT
from utils import ParseIsoFormatDate
...
def GenerateFullInfoCSV():
  ...
  with open(os.path.join(
      LONG_TERM_HEALTH_ROOT, 'full_milestone_info.csv'), 'w') as new_table:
    writer = csv.writer(new_table)
    writer.writerow(['milestone', 'branch', 'version_number', 'release_date'])
    with open(os.path.join(
        LONG_TERM_HEALTH_ROOT, 'milestone_build_mapping_table.csv'
