...
class PerfDashboardCommunicator(object):
  ...
  def MakeApiRequest(self, request, retry=3, delay=3):
    ...
    if resp['status'] != '200':
      ...
      if retry:
        print ('Retrying command after %s seconds. %s retries left...'
               % (delay, retry - 1))
        time.sleep(delay)
        return self.MakeApiRequest(request, retry=retry-1, delay=delay*2)
    return (resp, content)

  def ListTimeseries(self, benchmark, sheriff=False):
    r = 'list_timeseries/%s' % benchmark
    if sheriff:
      r += '?sheriff=%s' % sheriff
    _, content = self.MakeApiRequest(r)
