...
def GenerateNoiseData(timeseries_path, alerts_path, save_path):
  ...
  # TODO(rnephew): Figure out how to name Juans noise stuff.
  def juan_noise_transform(ts):
    s = pd.Series(ts['value'].values, index=ts['timestamp'])
    return s.resample('2D').std().mean()
  juan_noise_data = df.groupby(['bot', 'metric', 'story']).apply(juan_noise_transform).reset_index()
  ...
  for bot in bot_list:
    ...
    for metric in metric_list:
      ...
      # TODO(rnephew): Figure out how to name Juans noise stuff.
      noise_juan = FilterMetric(
          juan_noise_data[juan_noise_data['bot']==bot], metric)
      noise_juan = noise_juan[0].values[0] if noise_juan[0].values else None
      ...
      d = [
          bot, metric, noise_juan, c, ref_c, std, ref_std, num_alerts,
  ...
  headers = ['bot', 'metric', 'juan_noise', 'cv', 'ref_cv', 'std', 'ref_std',
             'Alerts', 'Ignored', 'Invalid', 'NoBug', 'OpenBug', 'UntriagedBug',
