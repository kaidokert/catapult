...
class BarPlotter {
  ...
  plot(graph, chart, legend, chartDimensions) {
    ...
    dataInAverages.forEach(({ data, color, key }, index) => {
      chart.selectAll(`.bar-${getClassNameSuffix(key)}`)
          ...
          .enter()
          .call(
              this.appendBar.bind(this),
              graph,
              color,
              key,
              chartDimensions,
              getClassNameSuffix)
          .call(this.appendErrorLine.bind(this), key)
          .call(this.appendErrorLineCaps.bind(this), key);
      const boxSize = 10;
  ...
  appendBar(selection, graph, color, key, chartDimensions, getClassNameSuffix) {
  ...
  appendErrorLineCaps(selection, key) {
    const quarter = 0.25;
