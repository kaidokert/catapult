...
Vue.component('data-table', {
  ...
  data() {
    ...
    return {
      ...
      plots_examples: ['Cumulative frequency plot', 'Dot plot'],
      chosen_plot: ''
    };
  },
  computed: {
    ...
    diagnostics_options() {
      ...
      this.diagnostic_ !== null) {
        ...
        const result = sampleArr
            .filter(value => value.name === this.metric_.metric &&
                  guidValue
        ...
        for (const val of result) {
          const diagnosticItem = guidValue.get(
              val.diagnostics[this.diagnostic_]);
  ...
  methods: {
    ...
    toggleMetric(entry) {
      ...
      for (const e of sampleArr) {
        const nameOfStory = guidValue.get(e.diagnostics.stories)[0];
  ...
  watch: {
    ...
    selected_diagnostics() {
      if (this.selected_diagnostics !== null &&
          this.selected_diagnostics.length !== 0 &&
          this.diagnostic_ !== null) {
        app.plotDiagnostics(this.metric_.metric,
            this.story_.story, this.diagnostic_,
            this.selected_diagnostics);
      }
    },
