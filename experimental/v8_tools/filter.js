...
const selectComponent = new Vue({
  ...
  computed: {
    ...
    //  compute all diagnostic elemenst; the final result will actually
        ...
        const result = this.sampleArr
            .filter(value => value.name === this.selected_metric &&
                    this.guidValue
                        ...
                        this.selected_story);
        return Object.keys(result[0].diagnostics);
      }
    },
      ...
      if (this.selected_story !== null &&
        ...
        for (const val of result) {
          ...
          if (content.has(storyItem)) {
            ...
            aux.push(val.sampleValues);
            content.set(storyItem, aux);
          } else {
            const localSampleVal = [];
            localSampleVal.push(val.sampleValues);
            content.set(storyItem, localSampleVal);
          }
        }
