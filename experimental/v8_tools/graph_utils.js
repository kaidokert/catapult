...
class GraphUtils {
   ...
   * @param {boolean} zoomableDimensions.x
   * Set to true if the x axis should be rescaled.
   * @param {boolean} zoomableDimensions.y
   ...
   * @param {Object} axes Provides the axes drawings, scales and generators
   * in the format:
   * {
   *   x: {
   *     generator: Object,
   ...
   *   },
   * }
   * Information about the axis must be provided for any axis set to true
   * in zoomableDimensions, otherwise it may be omitted.
   */
  static createZoom(zoomableDimensions, chart, chartDimensions, redraw, axes) {
    const transformLinePlot = (xAxisScale, yAxisScale, redraw, axes) => {
      if (xAxisScale) {
    ...
    const setUpZoom = (zoom) => {
      const onZoom = () => {
        ...
        const transformedScaleForYAxis = zoomableDimensions.y ?
          transform.rescaleY(axes.y.scale) : undefined;
        const transformedScaleForXAxis = zoomableDimensions.x ?
          transform.rescaleX(axes.x.scale) : undefined;
        transformLinePlot(
            transformedScaleForXAxis, transformedScaleForYAxis, redraw, axes);
      };
    ...
    const setUpZoomReset = (zoom) => {
      ...
      resetButton
          ...
          .on('click', () => {
            ...
            const originalXAxisScale = zoomableDimensions.x ?
              axes.x.scale : undefined;
            const originalYAxisScale = zoomableDimensions.y ?
              axes.y.scale : undefined;
            transformLinePlot(
                originalXAxisScale, originalYAxisScale, redraw, axes);
          });
    };
    const zoom = d3.zoom();
    setUpZoom(zoom);
