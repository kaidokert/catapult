...
class GraphData {
   ...
   */
  __max(projection) {
    const maxFromEachDataSource = [];
    this.dataSources.forEach((dataSource) =>
      maxFromEachDataSource.push(
          Math.max(...dataSource.data.map(point => projection(point)))
      )
    );
    return Math.max(...maxFromEachDataSource);
  }
   ...
   */
  xAxisMax() {
    return this.__max((point) => point.x);
  }
   ...
   */
  yAxisMax() {
    return this.__max((point) => point.y);
  }
